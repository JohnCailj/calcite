...
class ArrayTable extends AbstractQueryableTable implements ScannableTable {
    ...
    @SuppressWarnings("unchecked") private static <T> Pair<Object, T> toPair(Object dataSet) {
    ...
    public static class Column {
        ...
        public Column permute(int[] sources) {
            return new Column(representation, representation.permute(dataSet, sources), cardinality);
        }

        @Override public String toString() {
            return "Column(representation=" + representation + ", value=" + representation.toString(dataSet) + ")";
        }
    ...
    public static class PrimitiveArray implements Representation {
        ...
        public String toString() {
            return "PrimitiveArray(ordinal=" + ordinal + ", primitive=" + primitive + ", p=" + p + ")";
        }
        ...
        public Object freeze(ColumnLoader.ValueSet valueSet, int[] sources) {
            ...
            return primitive.toArray2(permuteList((List) valueSet.values, sources));
        }
    ...
    public static class ObjectDictionary implements Representation {
        ...
        public String toString() {
            return "ObjectDictionary(ordinal=" + ordinal + ", representation=" + representation + ")";
        }
    ...
    public static class BitSlicedPrimitiveArray implements Representation {
        ...
        @Override public String toString() {
            return "BitSlicedPrimitiveArray(ordinal=" + ordinal + ", bitCount=" + bitCount + ", primitive=" + primitive
                   + ", signed=" + signed + ")";
        }
        ...
        public Object freeze(ColumnLoader.ValueSet valueSet, int[] sources) {
            ...
            if (valueCount > 0 && valueList.get(0) instanceof Boolean) {
                @SuppressWarnings("unchecked") final List<Boolean> booleans = (List) valueList;
                for (i = 0; i < n; i++) {
                    long v = 0;
                ...
                @SuppressWarnings("unchecked") final List<Number> numbers = (List) valueList;
                for (i = 0; i < n; i++) {
                    long v = 0;
                    for (int j = 0; j < chunksPerWord; j++) {
        ...
        public Object permute(Object dataSet, int[] sources) {
            ...
            for (int i = 0; i < n; i++) {
                orLong(bitCount, longs, i, getLong(bitCount, longs0, sources[i]));
            }
            return longs;
        }
        ...
        public static long getLong(int bitCount, long[] values, int ordinal) {
            return getLong(bitCount, 64 / bitCount, (1L << bitCount) - 1L, values, ordinal);
        }
    ...
    public static class Content {
        ...
        Content(List<? extends Column> columns, int size, int sortField) {
            this(columns, size,
                 sortField >= 0 ? RelCollations.createSingleton(sortField) : ImmutableList.<RelCollation>of());
        }

        @SuppressWarnings("unchecked") public <T> Enumerator<T> enumerator() {
            if (columns.size() == 1) {
