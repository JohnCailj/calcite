...
class ColumnLoader<T> {
    ...
    static final long[] LONG_B = { 0x2, 0xC, 0xF0, 0xFF00, 0xFFFF0000, 0xFFFFFFFF00000000L };
    static final int[]  LONG_S = { 1, 2, 4, 8, 16, 32 };

    private static final Function<Timestamp, Long> TIMESTAMP_TO_LONG = new Function<Timestamp, Long>() {
    ...
    private static final Function<Time, Integer> TIME_TO_INT = new Function<Time, Integer>() {

        public Integer apply(Time a0) {
            return a0 == null ? null : (int) (a0.getTime() % DateTimeUtils.MILLIS_PER_DAY);
        }
    };

    private static final Function<Date, Integer> DATE_TO_INT = new Function<Date, Integer>() {
     ...
     */
    ColumnLoader(JavaTypeFactory typeFactory, Enumerable<T> sourceTable, RelProtoDataType protoRowType,
    ...
    private void load(final RelDataType elementType, List<ColumnMetaData.Rep> repList, int[] sort) {
        ...
        for (final Ord<Type> pair : Ord.zip(types)) {
            @SuppressWarnings("unchecked") final List<?> sliceList =
                    types.size() == 1 ? list : new AbstractList<Object>() {
            ...
            final List<?> list2 = wrap(repList.get(pair.i), sliceList,
            ...
            if (sort != null && sort[0] < 0 && valueSet.map.keySet().size() == list.size()) {
                ...
                sort[0] = pair.i;
                final Comparable[] values = valueSet.values.toArray(new Comparable[list.size()]);
                ...
                if (isIdentity(sources)) {
                    ...
                    for (int i = 0; i < pair.i; i++) {
                        representationValues.set(i, representationValues.get(i).permute(sources));
                    }
                }
            }
