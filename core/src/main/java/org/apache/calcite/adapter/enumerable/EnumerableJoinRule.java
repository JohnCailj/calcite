...
class EnumerableJoinRule extends ConverterRule {
    ...
    @Override public RelNode convert(RelNode rel) {
        ...
        if (!info.isEqui() && join.getJoinType() != JoinRelType.INNER) {
            ...
            try {
                return new EnumerableThetaJoin(cluster, traitSet, left, right, join.getCondition(),
        ...
        RelNode newRel;
        try {
            newRel = new EnumerableJoin(cluster, join.getTraitSet().replace(EnumerableConvention.INSTANCE), left, right,
                                        info.getEquiCondition(left, right, cluster.getRexBuilder()), info.leftKeys,
        ...
        if (!info.isEqui()) {
            newRel = new EnumerableFilter(cluster, newRel.getTraitSet(), newRel,
