...
public class RexImpTable {
    ...
    RexImpTable() {
        ...
        defineImplementor(RAND, NullPolicy.STRICT, new NotNullImplementor() {

            final NotNullImplementor[] implementors = { new ReflectiveCallNotNullImplementor(BuiltInMethod.RAND.method),
        ...
        defineImplementor(RAND_INTEGER, NullPolicy.STRICT, new NotNullImplementor() {

            final NotNullImplementor[] implementors = { null, new ReflectiveCallNotNullImplementor(
                    BuiltInMethod.RAND_INTEGER.method), new ReflectiveCallNotNullImplementor(
        ...
        // datetime
        defineImplementor(DATETIME_PLUS, NullPolicy.STRICT, new DatetimeArithmeticImplementor(), false);
        defineImplementor(MINUS_DATE, NullPolicy.STRICT, new DatetimeArithmeticImplementor(), false);
        defineMethod(EXTRACT_DATE, BuiltInMethod.UNIX_DATE_EXTRACT.method, NullPolicy.STRICT);
        defineImplementor(FLOOR, NullPolicy.STRICT, new FloorImplementor(BuiltInMethod.FLOOR.method.getName(),
                                                                         BuiltInMethod.UNIX_TIMESTAMP_FLOOR.method,
                                                                         BuiltInMethod.UNIX_DATE_FLOOR.method), false);
        defineImplementor(CEIL, NullPolicy.STRICT, new FloorImplementor(BuiltInMethod.CEIL.method.getName(),
        ...
        defineImplementor(NOT_LIKE, NullPolicy.STRICT, NotImplementor.of(likeImplementor), false);
        ...
        defineImplementor(NOT_SIMILAR_TO, NullPolicy.STRICT, NotImplementor.of(similarImplementor), false);
        ...
        defineImplementor(REINTERPRET, NullPolicy.STRICT, new ReinterpretImplementor(), false);
        ...
        map.put(DEFAULT, new CallImplementor() {
        ...
        defineImplementor(CURRENT_VALUE, NullPolicy.STRICT,
                          new SequenceImplementor(BuiltInMethod.SEQUENCE_CURRENT_VALUE.method), false);
        defineImplementor(NEXT_VALUE, NullPolicy.STRICT,
                          new SequenceImplementor(BuiltInMethod.SEQUENCE_NEXT_VALUE.method), false);
    ...
    private void defineImplementor(SqlOperator operator, NullPolicy nullPolicy, NotNullImplementor implementor,
                                   boolean harmonize) {
    ...
    public static CallImplementor createImplementor(final NotNullImplementor implementor, final NullPolicy nullPolicy,
...
*/
                    ...
                    public Expression implement(RexToLixTranslator translator, RexCall call, NullAs nullAs) {
                        ...
                        switch (nullAs) {
                            ...
                            case FALSE:
                                ...
                                final List<Expression> expressions = translator.translateList(call2.getOperands(),
                                                                                              nullAs);
                                return Expressions.foldAnd(expressions);
                            case NULL:
                            case IS_NULL:
                            case IS_NOT_NULL:
                                final List<Expression> nullAsTrue = translator.translateList(call2.getOperands(),
                                                                                             NullAs.TRUE);
                                final List<Expression> nullAsIsNull = translator.translateList(call2.getOperands(),
                                                                                               NullAs.IS_NULL);
                                Expression hasFalse = Expressions.not(Expressions.foldAnd(nullAsTrue));
                                Expression hasNull = Expressions.foldOr(nullAsIsNull);
                                Expression result = nullAs.handle(Expressions.condition(hasFalse, BOXED_FALSE_EXPR,
                                                                                        Expressions.condition(hasNull,
                                                                                                              NULL_EXPR,
                                                                                                              BOXED_TRUE_EXPR)));
                                return result;
                            default:
                ...
                return new CallImplementor() {
                    ...
                    public Expression implement(RexToLixTranslator translator, RexCall call, final NullAs nullAs) {
                        ...
                        switch (nullAs) {
                            ...
                            case IS_NULL:
                            case IS_NOT_NULL:
                                final List<Expression> nullAsFalse = translator.translateList(call2.getOperands(),
                                                                                              NullAs.FALSE);
                                final List<Expression> nullAsIsNull = translator.translateList(call2.getOperands(),
                                                                                               NullAs.IS_NULL);
                                Expression hasTrue = Expressions.foldOr(nullAsFalse);
                                Expression hasNull = Expressions.foldOr(nullAsIsNull);
                                Expression result = nullAs.handle(Expressions.condition(hasTrue, BOXED_TRUE_EXPR,
                                                                                        Expressions.condition(hasNull,
                                                                                                              NULL_EXPR,
                                                                                                              BOXED_FALSE_EXPR)));
                                return result;
    ...
    private void defineBinary(SqlOperator operator, ExpressionType expressionType, NullPolicy nullPolicy,
    ...
    private static Expression implementNullSemantics0(RexToLixTranslator translator, RexCall call, NullAs nullAs,
    ...
    private static Expression implementNullSemantics(RexToLixTranslator translator, RexCall call, NullAs nullAs,
        ...
        switch (nullAs) {
            case NULL:
                ...
                return optimize(
                        Expressions.condition(Expressions.foldOr(list), Types.castIfNecessary(box.getType(), NULL_EXPR),
                                              box));
            case FALSE:
    ...
    private static Expression implementCall(RexToLixTranslator translator, RexCall call, NotNullImplementor implementor,
                                            NullAs nullAs) {
        final List<Expression> translatedOperands = translator.translateList(call.getOperands());
    ...
    static class CountWinImplementor extends StrictWinAggImplementor {
        ...
        @Override public void implementNotNullAdd(WinAggContext info, WinAggAddContext add) {
        ...
        @Override protected Expression implementNotNullResult(WinAggContext info, WinAggResultContext result) {
            if (justFrameRowCount) {
    ...
    static class SumImplementor extends StrictAggImplementor {
        ...
        @Override protected void implementNotNullReset(AggContext info, AggResetContext reset) {
            Expression start = info.returnType() == BigDecimal.class ? Expressions.constant(
                    BigDecimal.ZERO) : Expressions.constant(0);
        ...
        @Override public Expression implementNotNullResult(AggContext info, AggResultContext result) {
            return super.implementNotNullResult(info, result);
        }
    }
    ...
    static class MinMaxImplementor extends StrictAggImplementor {
        ...
        @Override protected void implementNotNullReset(AggContext info, AggResetContext reset) {
            ...
            reset.currentBlock().add(
                    Expressions.statement(Expressions.assign(acc, Expressions.constant(inf, acc.getType()))));
        }
        ...
        @Override public void implementNotNullAdd(AggContext info, AggAddContext add) {
            ...
            final Method method = (aggregation == MIN ? BuiltInMethod.LESSER : BuiltInMethod.GREATER).method;
            Expression next = Expressions.call(method.getDeclaringClass(), method.getName(), acc,
    ...
    static class SingleValueImplementor implements AggImplementor {
        ...
        public void implementReset(AggContext info, AggResetContext reset) {
            ...
            reset.currentBlock().add(
                    Expressions.statement(Expressions.assign(acc.get(0), Expressions.constant(false))));
            reset.currentBlock().add(
                    Expressions.statement(Expressions.assign(acc.get(1), getDefaultValue(acc.get(1).getType()))));
        }

        public void implementAdd(AggContext info, AggAddContext add) {
            List<Expression> acc = add.accumulator();
            Expression flag = acc.get(0);
            add.currentBlock().add(Expressions.ifThen(flag, Expressions.throw_(
    ...
    static class CollectImplementor extends StrictAggImplementor {
        ...
        @Override protected void implementNotNullReset(AggContext info, AggResetContext reset) {
            // acc[0] = new ArrayList();
            reset.currentBlock().add(Expressions.statement(
                    Expressions.assign(reset.accumulator().get(0), Expressions.new_(ArrayList.class))));
        }

        @Override public void implementNotNullAdd(AggContext info, AggAddContext add) {
            // acc[0].add(arg);
            add.currentBlock().add(Expressions.statement(
                    Expressions.call(add.accumulator().get(0), BuiltInMethod.COLLECTION_ADD.method,
                                     add.arguments().get(0))));
        }
    }
        ...
        public Expression implementResult(AggContext info, AggResultContext result) {
            ...
            if (info.groupSets().size() > 1) {
                ...
                for (int k : keys) {
                    ...
                    assert i >= 0;
                    final Expression e2 = Expressions.condition(result.keyField(keyOrdinals.size() + i),
                                                                Expressions.constant(x), Expressions.constant(0L));
                    if (e == null) {
                        e = e2;
                    } else {
    ...
    public static class UserDefinedAggReflectiveImplementor extends StrictAggImplementor {
        ...
        @Override protected void implementNotNullReset(AggContext info, AggResetContext reset) {
            ...
            if (!afi.isStatic) {
                reset.currentBlock().add(
                        Expressions.statement(Expressions.assign(acc.get(1), Expressions.new_(afi.declaringClass))));
            }
            reset.currentBlock().add(Expressions.statement(Expressions.assign(acc.get(0), Expressions.call(
        ...
        @Override protected void implementNotNullAdd(AggContext info, AggAddContext add) {
            ...
            args.add(acc.get(0));
            args.addAll(aggArgs);
            add.currentBlock().add(Expressions.statement(Expressions.assign(acc.get(0), Expressions.call(
                    afi.isStatic ? null : acc.get(1), afi.addMethod, args))));
        }

        @Override protected Expression implementNotNullResult(AggContext info, AggResultContext result) {
    ...
    static class RankImplementor extends StrictWinAggImplementor {
        ...
        @Override protected void implementNotNullAdd(WinAggContext info, WinAggAddContext add) {
            ...
            add.currentBlock().add(Expressions.ifThen(Expressions.lessThan(
                    add.compareRows(Expressions.subtract(add.currentPosition(), Expressions.constant(1)),
                                    add.currentPosition()), Expressions.constant(0)), Expressions.statement(
                    Expressions.assign(acc, computeNewRank(acc, add)))));
            add.exitBlock();
            add.currentBlock().add(Expressions.ifThen(Expressions.greaterThan(add.currentPosition(), add.startIndex()),
                                                      builder.toBlock()));
        }
    ...
    static class DenseRankImplementor extends RankImplementor {
        ...
        @Override protected Expression computeNewRank(Expression acc, WinAggAddContext add) {
    ...
    static class FirstLastValueImplementor implements WinAggImplementor {
        ...
        public Expression implementResult(AggContext info, AggResultContext result) {
            ...
            return Expressions.condition(winResult.hasRows(), winResult.rowTranslator(
                    winResult.computeIndex(Expressions.constant(0), seekType)).translate(
                    winResult.rexArguments().get(0), info.returnType()), getDefaultValue(info.returnType()));
        }
    }
    ...
    static class LeadLagImplementor implements WinAggImplementor {
        ...
        public Expression implementResult(AggContext info, AggResultContext result) {
            ...
            Expression offset;
            RexToLixTranslator currentRowTranslator = winResult.rowTranslator(
            ...
            Expression defaultValue =
                    rexArgs.size() == 3 ? currentRowTranslator.translate(rexArgs.get(2), res.type) : getDefaultValue(
                            res.type);

            result.currentBlock().add(Expressions.declare(0, res, null));
            result.currentBlock().add(Expressions.ifThenElse(rowInRange, thenBranch, Expressions.statement(
                    Expressions.assign(res, defaultValue))));
            return res;
        }
    }
    ...
    static class NtileImplementor implements WinAggImplementor {
        ...
        public Expression implementResult(AggContext info, AggResultContext result) {
            ...
            Expression ntile = Expressions.add(Expressions.constant(1), Expressions.divide(
                    Expressions.multiply(tiles, Expressions.subtract(winResult.index(), winResult.startIndex())),
    ...
    private static class TrimImplementor implements NotNullImplementor {
        ...
        public Expression implement(RexToLixTranslator translator, RexCall call, List<Expression> translatedOperands) {
            ...
            return Expressions.call(BuiltInMethod.TRIM.method, Expressions.constant(
                    flag == SqlTrimFunction.Flag.BOTH || flag == SqlTrimFunction.Flag.LEADING), Expressions.constant(
                    flag == SqlTrimFunction.Flag.BOTH || flag == SqlTrimFunction.Flag.TRAILING),
                                    translatedOperands.get(1), translatedOperands.get(2));
        }
    }
    ...
    private static class FloorImplementor extends MethodNameImplementor {
                ...
                case 2:
                    ...
                    switch (call.getType().getSqlTypeName()) {
                        case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
                            operand = Expressions.call(BuiltInMethod.TIMESTAMP_WITH_LOCAL_TIME_ZONE_TO_TIMESTAMP.method,
                                                       operand, Expressions.call(BuiltInMethod.TIME_ZONE.method,
                                                                                 translator.getRoot()));
                            // fall through
                        case TIMESTAMP:
                            type = long.class;
        ...
        private Expression call(Expression operand, Type type, TimeUnit timeUnit) {
            return Expressions.call(SqlFunctions.class, methodName, Types.castIfNecessary(type, operand),
    ...
    private static class SequenceImplementor extends MethodImplementor {
        ...
        public Expression implement(RexToLixTranslator translator, RexCall call, List<Expression> translatedOperands) {
            ...
            final Prepare.CatalogReader catalogReader = Prepare.CatalogReader.THREAD_LOCAL.get();
    ...
    private static class BinaryImplementor implements NotNullImplementor {
         ...
         * {@code <}.
         */
        private static final List<Primitive> COMP_OP_TYPES = ImmutableList.of(Primitive.BYTE, Primitive.CHAR,
                                                                              Primitive.SHORT, Primitive.INT,
                                                                              Primitive.LONG, Primitive.FLOAT,
                                                                              Primitive.DOUBLE);

        private static final List<SqlBinaryOperator> COMPARISON_OPERATORS        = ImmutableList.of(
                SqlStdOperatorTable.LESS_THAN, SqlStdOperatorTable.LESS_THAN_OR_EQUAL, SqlStdOperatorTable.GREATER_THAN,
        ...
        public Expression implement(RexToLixTranslator translator, RexCall call, List<Expression> expressions) {
            ...
            final Type returnType = translator.typeFactory.getJavaClass(call.getType());
            return Types.castIfNecessary(returnType, Expressions.makeBinary(expressionType, expressions.get(0),
                                                                            expressions.get(1)));
        }
    ...
    private static class UnaryImplementor implements NotNullImplementor {
        ...
        public Expression implement(RexToLixTranslator translator, RexCall call, List<Expression> translatedOperands) {
            return Expressions.makeUnary(expressionType, translatedOperands.get(0));
        }
    }
            ...
            if (i == operands.size() - 1) {
                ...
                try {
                    ifTrue = translator.translate(
                            translator.builder.ensureType(call.getType(), operands.get(i + 1), false), nullAs);
                } catch (RexToLixTranslator.AlwaysNull e) {
                    ifTrue = null;
                }
                ...
                return ifTrue == null || ifFalse == null ? Util.first(ifTrue, ifFalse) : Expressions.condition(test,
                                                                                                               ifTrue,
                                                                                                               ifFalse);
            }
        }
    ...
    private static class CastOptimizedImplementor implements CallImplementor {
        ...
        public Expression implement(RexToLixTranslator translator, RexCall call, NullAs nullAs) {
            ...
            if (SqlTypeUtil.equalSansNullability(translator.typeFactory, call.getType(), arg.getType())
                && nullAs == NullAs.NULL && translator.deref(arg) instanceof RexLiteral) {
                return RexToLixTranslator.translateLiteral((RexLiteral) translator.deref(arg), call.getType(),
    ...
    private static class CastImplementor implements NotNullImplementor {
        ...
        public Expression implement(RexToLixTranslator translator, RexCall call, List<Expression> translatedOperands) {
            ...
            final boolean nullable = translator.isNullable(call) && sourceType.isNullable() && !Primitive.is(
                    translatedOperands.get(0).getType());
    ...
    private static class SystemFunctionImplementor implements CallImplementor {
        ...
        public Expression implement(RexToLixTranslator translator, RexCall call, NullAs nullAs) {
            ...
            if (op == CURRENT_USER || op == SESSION_USER || op == USER) {
    ...
    private static class IsXxxImplementor implements CallImplementor {
        ...
        public Expression implement(RexToLixTranslator translator, RexCall call, NullAs nullAs) {
            ...
            if (seek == null) {
                return translator.translate(operands.get(0), negate ? NullAs.IS_NOT_NULL : NullAs.IS_NULL);
            } else {
                return maybeNegate(negate == seek,
                                   translator.translate(operands.get(0), seek ? NullAs.FALSE : NullAs.TRUE));
            }
        }
    }
        ...
        public Expression implement(RexToLixTranslator translator, RexCall call, List<Expression> translatedOperands) {
                        ...
                        default:
                            switch (typeName1) {
                                ...
                                case INTERVAL_SECOND:
                                    trop1 = Expressions.convert_(Expressions.divide(trop1, Expressions.constant(
                                            DateTimeUtils.MILLIS_PER_DAY)), int.class);
                            }
                    }
                    break;
                case TIME:
            ...
            switch (typeName1) {
                ...
                case INTERVAL_MONTH:
                    ...
                    final BuiltInMethod method = operand0.getType().getSqlTypeName()
                                                 == SqlTypeName.TIMESTAMP ? BuiltInMethod.ADD_MONTHS : BuiltInMethod.ADD_MONTHS_INT;
