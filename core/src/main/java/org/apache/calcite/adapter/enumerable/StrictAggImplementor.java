...
public abstract class StrictAggImplementor implements AggImplementor {
    ...
    protected final void accAdvance(AggAddContext add, Expression acc, Expression next) {
        add.currentBlock().add(Expressions.statement(Expressions.assign(acc, Types.castIfNecessary(acc.type, next))));
    }
    ...
    public final void implementAdd(AggContext info, final AggAddContext add) {
        ...
        if (trackNullsPerRow) {
            ...
            thenBlock.add(
                    Expressions.statement(Expressions.assign(acc.get(acc.size() - 1), Expressions.constant(true))));
        }
        if (argsNotNull) {
    ...
    public final Expression implementResult(AggContext info, final AggResultContext result) {
        ...
        Expression seenNotNullRows = trackNullsPerRow ? acc.get(
                acc.size() - 1) : ((WinAggResultContext) result).hasRows();
        ...
        result.currentBlock().add(Expressions.ifThenElse(seenNotNullRows, thenBranch, Expressions.statement(
                Expressions.assign(res, RexImpTable.getDefaultValue(res.getType())))));
        return res;
    }
