...
public class Bindables {
     ...
     */
    public static final ImmutableList<RelOptRule> RULES = ImmutableList.of(NoneToBindableConverterRule.INSTANCE,
                                                                           BINDABLE_TABLE_SCAN_RULE,
                                                                           BINDABLE_FILTER_RULE, BINDABLE_PROJECT_RULE,
                                                                           BINDABLE_SORT_RULE, BINDABLE_JOIN_RULE,
                                                                           BINDABLE_UNION_RULE, BINDABLE_VALUES_RULE,
    ...
    public static class BindableTableScan extends TableScan implements BindableRel {
         ...
         */
        BindableTableScan(RelOptCluster cluster, RelTraitSet traitSet, RelOptTable table,
        ...
        public static BindableTableScan create(RelOptCluster cluster, RelOptTable relOptTable) {
            return create(cluster, relOptTable, ImmutableList.<RexNode>of(), identity(relOptTable));
        }

        /**
         * Creates a BindableTableScan.
         */
        public static BindableTableScan create(RelOptCluster cluster, RelOptTable relOptTable, List<RexNode> filters,
                                               List<Integer> projects) {
            final Table table = relOptTable.unwrap(Table.class);
            final RelTraitSet traitSet = cluster.traitSetOf(BindableConvention.INSTANCE).replaceIfs(
            ...
            return new BindableTableScan(cluster, traitSet, relOptTable, ImmutableList.copyOf(filters),
        ...
        @Override public RelWriter explainTerms(RelWriter pw) {
            return super.explainTerms(pw).itemIf("filters", filters, !filters.isEmpty()).itemIf("projects", projects,
                                                                                                !projects.equals(
                                                                                                        identity()));
        }
    ...
    private static class BindableFilterRule extends ConverterRule {
        ...
        private BindableFilterRule() {
            super(LogicalFilter.class, RelOptUtil.FILTER_PREDICATE, Convention.NONE, BindableConvention.INSTANCE,
                  "BindableFilterRule");
        }
        ...
        public RelNode convert(RelNode rel) {
            ...
            return BindableFilter.create(
                    convert(filter.getInput(), filter.getInput().getTraitSet().replace(BindableConvention.INSTANCE)),
                    filter.getCondition());
        }
    }
    ...
    private static class BindableProjectRule extends ConverterRule {
        ...
        private BindableProjectRule() {
            super(LogicalProject.class, RelOptUtil.PROJECT_PREDICATE, Convention.NONE, BindableConvention.INSTANCE,
                  "BindableProjectRule");
        }

        public RelNode convert(RelNode rel) {
            final LogicalProject project = (LogicalProject) rel;
            return new BindableProject(rel.getCluster(), rel.getTraitSet().replace(BindableConvention.INSTANCE),
                                       convert(project.getInput(),
                                               project.getInput().getTraitSet().replace(BindableConvention.INSTANCE)),
    ...
    public static class BindableProject extends Project implements BindableRel {
        ...
        public BindableProject(RelOptCluster cluster, RelTraitSet traitSet, RelNode input,
    ...
    private static class BindableSortRule extends ConverterRule {
        ...
        public RelNode convert(RelNode rel) {
            ...
            return new BindableSort(rel.getCluster(), traitSet,
                                    convert(input, input.getTraitSet().replace(BindableConvention.INSTANCE)),
    ...
    public static class BindableSort extends Sort implements BindableRel {
        ...
        public BindableSort(RelOptCluster cluster, RelTraitSet traitSet, RelNode input, RelCollation collation,
                            RexNode offset, RexNode fetch) {
            super(cluster, traitSet, input, collation, offset, fetch);
            assert getConvention() instanceof BindableConvention;
        }

        @Override public BindableSort copy(RelTraitSet traitSet, RelNode newInput, RelCollation newCollation,
                                           RexNode offset, RexNode fetch) {
    ...
    private static class BindableJoinRule extends ConverterRule {
        ...
        public RelNode convert(RelNode rel) {
            ...
            return new BindableJoin(rel.getCluster(), traitSet, convert(join.getLeft(),
                                                                        join.getLeft().getTraitSet().replace(
                                                                                BindableConvention.INSTANCE)),
                                    convert(join.getRight(),
                                            join.getRight().getTraitSet().replace(BindableConvention.INSTANCE)),
    ...
    public static class BindableJoin extends Join implements BindableRel {
         ...
         */
        protected BindableJoin(RelOptCluster cluster, RelTraitSet traitSet, RelNode left, RelNode right,
        ...
        public BindableJoin copy(RelTraitSet traitSet, RexNode conditionExpr, RelNode left, RelNode right,
    ...
    public static class BindableValues extends Values implements BindableRel {
        ...
        BindableValues(RelOptCluster cluster, RelDataType rowType, ImmutableList<ImmutableList<RexLiteral>> tuples,
    ...
    private static class BindableValuesRule extends ConverterRule {
        ...
        @Override public RelNode convert(RelNode rel) {
            ...
            return new BindableValues(values.getCluster(), values.getRowType(), values.getTuples(),
    ...
    public static class BindableAggregate extends Aggregate implements BindableRel {
        ...
        public BindableAggregate(RelOptCluster cluster, RelTraitSet traitSet, RelNode input, boolean indicator,
        ...
        @Override public BindableAggregate copy(RelTraitSet traitSet, RelNode input, boolean indicator,
                                                ImmutableBitSet groupSet, List<ImmutableBitSet> groupSets,
    ...
    private static class BindableAggregateRule extends ConverterRule {
        ...
        public RelNode convert(RelNode rel) {
            ...
            try {
                return new BindableAggregate(rel.getCluster(), traitSet, convert(agg.getInput(), traitSet),
    ...
    public static class BindableWindow extends Window implements BindableRel {
         ...
         */
        BindableWindow(RelOptCluster cluster, RelTraitSet traitSet, RelNode input, List<RexLiteral> constants,
    ...
    private static class BindableWindowRule extends ConverterRule {
        ...
        public RelNode convert(RelNode rel) {
            ...
            return new BindableWindow(rel.getCluster(), traitSet, convertedInput, winAgg.getConstants(),
