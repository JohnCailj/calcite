...
public class Interpreter extends AbstractEnumerable<Object[]> implements AutoCloseable {
    ...
    private RelNode optimize(RelNode rootRel) {
        final HepProgram hepProgram = new HepProgramBuilder().addRuleInstance(CalcSplitRule.INSTANCE).addRuleInstance(
                FilterTableScanRule.INSTANCE).addRuleInstance(FilterTableScanRule.INTERPRETER).addRuleInstance(
                ProjectTableScanRule.INSTANCE).addRuleInstance(ProjectTableScanRule.INTERPRETER).build();
    ...
    RelDataType combinedRowType(List<RelNode> inputs) {
        final RelDataTypeFactory.Builder builder = dataContext.getTypeFactory().builder();
    ...
    private static class ListSink implements Sink {
        ...
        @SuppressWarnings("deprecation") @Override public void setSourceEnumerable(Enumerable<Row> enumerable)
    ...
    public static class Compiler extends RelVisitor implements ReflectiveVisitor {
        ...
        private final ReflectiveVisitDispatcher<Compiler, RelNode> dispatcher = ReflectUtil.createDispatcher(
                Compiler.class, RelNode.class);
            ...
            if (!found) {
                if (p instanceof InterpretableRel) {
                    ...
                    node = interpretableRel.implement(
                            new InterpretableRel.InterpreterImplementor(interpreter, null, null));
                } else {
