...
public class SortNode extends AbstractSingleNode<Sort> {
    ...
    public void run() throws InterruptedException {
        final int offset = rel.offset == null ? 0 : ((RexLiteral) rel.offset).getValueAs(Integer.class);
        final int fetch = rel.fetch == null ? -1 : ((RexLiteral) rel.fetch).getValueAs(Integer.class);
        ...
        if (rel.getCollation().getFieldCollations().isEmpty()) {
            ...
            final int end = fetch < 0 || offset + fetch > list.size() ? list.size() : offset + fetch;
    ...
    private Comparator<Row> comparator() {
        ...
        return Ordering.compound(Iterables.transform(rel.getCollation().getFieldCollations(),
