...
public class TableScanNode implements Node {
     ...
     */
    static TableScanNode create(Interpreter interpreter, TableScan rel, ImmutableList<RexNode> filters,
        ...
        if (pfTable != null) {
            return createProjectableFilterable(interpreter, rel, filters, projects, pfTable);
        }
        final FilterableTable filterableTable = relOptTable.unwrap(FilterableTable.class);
        if (filterableTable != null) {
            return createFilterable(interpreter, rel, filters, projects, filterableTable);
        }
        final ScannableTable scannableTable = relOptTable.unwrap(ScannableTable.class);
        if (scannableTable != null) {
            return createScannable(interpreter, rel, filters, projects, scannableTable);
        }
        //noinspection unchecked
        final Enumerable<Row> enumerable = relOptTable.unwrap(Enumerable.class);
        if (enumerable != null) {
            return createEnumerable(interpreter, rel, enumerable, null, filters, projects);
        }
        final QueryableTable queryableTable = relOptTable.unwrap(QueryableTable.class);
        if (queryableTable != null) {
            return createQueryable(interpreter, rel, filters, projects, queryableTable);
        }
        throw new AssertionError("cannot convert table " + relOptTable + " to enumerable");
    }

    private static TableScanNode createScannable(Interpreter interpreter, TableScan rel, ImmutableList<RexNode> filters,
                                                 ImmutableIntList projects, ScannableTable scannableTable) {
        final Enumerable<Row> rowEnumerable = Enumerables.toRow(scannableTable.scan(interpreter.getDataContext()));
        return createEnumerable(interpreter, rel, rowEnumerable, null, filters, projects);
    }

    private static TableScanNode createQueryable(Interpreter interpreter, TableScan rel, ImmutableList<RexNode> filters,
                                                 ImmutableIntList projects, QueryableTable queryableTable) {
        ...
        if (elementType instanceof Class) {
            //noinspection unchecked
            final Queryable<Object> queryable = Schemas.queryable(root, (Class) elementType,
    ...
    private static TableScanNode createEnumerable(Interpreter interpreter, TableScan rel, Enumerable<Row> enumerable,
                                                  final ImmutableIntList acceptedProjects,
