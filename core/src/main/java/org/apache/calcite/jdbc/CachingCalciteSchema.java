...
class CachingCalciteSchema extends CalciteSchema {
    ...
    private CachingCalciteSchema(CalciteSchema parent, Schema schema, String name, NameMap<CalciteSchema> subSchemaMap,
        ...
        super(parent, schema, name, subSchemaMap, tableMap, latticeMap, functionMap, functionNames, nullaryFunctionMap,
              path);
    ...
    protected CalciteSchema snapshot(CalciteSchema parent, SchemaVersion version) {
        CalciteSchema snapshot = new CachingCalciteSchema(parent, schema.snapshot(version), name, null, tableMap,
                                                          latticeMap, functionMap, functionNames, nullaryFunctionMap,
                                                          getPath());
    ...
    private static class SubSchemaCache {
        ...
        private SubSchemaCache(final CalciteSchema calciteSchema, Set<String> names) {
            ...
            this.cache = CacheBuilder.newBuilder().build(new CacheLoader<String, CalciteSchema>() {
                ...
                @SuppressWarnings("NullableProblems") @Override public CalciteSchema load(String schemaName) {
