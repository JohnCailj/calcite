 ...
 */
abstract class CalciteConnectionImpl extends AvaticaConnection implements CalciteConnection, QueryProvider {
     ...
     */
    protected CalciteConnectionImpl(Driver driver, AvaticaFactory factory, String url, Properties info,
        ...
        if (iface == RelRunner.class) {
            return iface.cast(new RelRunner() {
                ...
                public PreparedStatement prepare(RelNode rel) {
                    try {
                        return prepareStatement_(CalcitePrepare.Query.of(rel), ResultSet.TYPE_FORWARD_ONLY,
    ...
    @Override public CalciteStatement createStatement(int resultSetType, int resultSetConcurrency,
                                                      int resultSetHoldability) throws SQLException {
        return (CalciteStatement) super.createStatement(resultSetType, resultSetConcurrency, resultSetHoldability);
    }

    @Override public CalcitePreparedStatement prepareStatement(String sql, int resultSetType, int resultSetConcurrency,
                                                               int resultSetHoldability) throws SQLException {
    ...
    private CalcitePreparedStatement prepareStatement_(CalcitePrepare.Query<?> query, int resultSetType,
                                                       int resultSetConcurrency, int resultSetHoldability)
            throws SQLException {
        try {
            final Meta.Signature signature = parseQuery(query, createPrepareContext(), -1);
            final CalcitePreparedStatement calcitePreparedStatement = (CalcitePreparedStatement) factory.newPreparedStatement(
                    this, null, signature, resultSetType, resultSetConcurrency, resultSetHoldability);
    ...
    <T> CalcitePrepare.CalciteSignature<T> parseQuery(CalcitePrepare.Query<T> query,
    ...
    @Override public AtomicBoolean getCancelFlag(Meta.StatementHandle handle) throws NoSuchStatementException {
    ...
    public <T> Enumerable<T> enumerable(Meta.StatementHandle handle, CalcitePrepare.CalciteSignature<T> signature)
            throws SQLException {
    ...
    private static class CalciteServerImpl implements CalciteServer {
        ...
        public void addStatement(CalciteConnection connection, Meta.StatementHandle h) {
            ...
            final CalciteServerStatement previous = statementMap.put(h.id, new CalciteServerStatementImpl(c));
            if (previous != null) {
                throw new AssertionError();
            }
        }

        public CalciteServerStatement getStatement(Meta.StatementHandle h) throws NoSuchStatementException {
    ...
    static class DataContextImpl implements DataContext {
        ...
        DataContextImpl(CalciteConnectionImpl connection, Map<String, Object> parameters, CalciteSchema rootSchema) {
            ...
            builder.put(Variable.UTC_TIMESTAMP.camelName, time).put(Variable.CURRENT_TIMESTAMP.camelName,
                                                                    time + currentOffset).put(
                    Variable.LOCAL_TIMESTAMP.camelName, time + localOffset).put(Variable.TIME_ZONE.camelName,
                                                                                timeZone).put(Variable.STDIN.camelName,
                                                                                              streamHolder.get()[0]).put(
                    Variable.STDOUT.camelName, streamHolder.get()[1]).put(Variable.STDERR.camelName,
                                                                          streamHolder.get()[2]);
        ...
        private SqlAdvisor getSqlAdvisor() {
            ...
            final List<String> schemaPath =
                    schemaName == null ? ImmutableList.<String>of() : ImmutableList.of(schemaName);
            final SqlValidatorWithHints validator = new SqlAdvisorValidator(SqlStdOperatorTable.instance(),
    ...
    static class ContextImpl implements CalcitePrepare.Context {
        ...
        public DataContext getDataContext() {
            return connection.createDataContext(ImmutableMap.<String, Object>of(), rootSchema);
        }
