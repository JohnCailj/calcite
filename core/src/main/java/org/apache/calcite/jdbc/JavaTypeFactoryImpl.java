...
public class JavaTypeFactoryImpl extends SqlTypeFactoryImpl implements JavaTypeFactory {
    ...
    private Type fieldType(Field field) {
        ...
        final org.apache.calcite.adapter.java.Map map = field.getAnnotation(org.apache.calcite.adapter.java.Map.class);
        if (map != null) {
    ...
    public static RelDataType toSql(final RelDataTypeFactory typeFactory, RelDataType type) {
        if (type instanceof RelRecordType) {
            return typeFactory.createStructType(
                    Lists.transform(type.getFieldList(), new Function<RelDataTypeField, RelDataType>() {
        ...
        if (type instanceof JavaType) {
            return typeFactory.createTypeWithNullability(typeFactory.createSqlType(type.getSqlTypeName()),
                                                         type.isNullable());
        }
        return type;
    }
    ...
    public Type createSyntheticType(List<Type> types) {
        ...
        for (final Ord<Type> ord : Ord.zip(types)) {
            syntheticType.fields.add(
                    new RecordFieldImpl(syntheticType, "f" + ord.i, ord.e, !Primitive.is(ord.e), Modifier.PUBLIC));
        }
