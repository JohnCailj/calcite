...
public class Lattice {
    ...
    private Lattice(CalciteSchema rootSchema, ImmutableList<Node> nodes, boolean auto, boolean algorithm,
        ...
        uniqueColumnNames = ImmutableList.copyOf(SqlValidatorUtil.uniquify(Lists.transform(columns, GET_ALIAS), true));
    ...
    public static class Measure implements Comparable<Measure> {
        ...
        @Override public boolean equals(Object obj) {
            return obj == this || obj instanceof Measure && this.agg.equals(((Measure) obj).agg) && this.args.equals(
                    ((Measure) obj).args);
        }
    ...
    public static class Builder {
        ...
        public Builder(CalciteSchema schema, String sql) {
            ...
            for (RelNode relNode : TopologicalOrderIterator.of(graph)) {
                ...
                if (previous == null) {
                    ...
                    node = new Node((TableScan) relNode, null, null, previousColumn, column, aliases.get(nodes.size()));
                } else {
                    if (edges.size() != 1) {
                        throw new RuntimeException("child node must have precisely one parent: " + relNode);
                    }
                    final Edge edge = edges.get(0);
                    node = new Node((TableScan) relNode, map.get(edge.getSource()), edge.pairs, previousColumn, column,
        ...
        public Lattice build() {
            LatticeStatisticProvider statisticProvider =
                    this.statisticProvider != null ? AvaticaUtils.instantiatePlugin(LatticeStatisticProvider.class,
                                                                                    this.statisticProvider) : Lattices.CACHED_SQL;
            Preconditions.checkArgument(rootSchema.isRoot(), "must be root schema");
