...
public class TileSuggester {
    ...
    private static final Function<Attribute, Lattice.Column> TO_COLUMN = new Function<Attribute, Lattice.Column>() {
    ...
    public Iterable<? extends Lattice.Tile> tiles() {
        ...
        final StatisticsProvider statisticsProvider = new StatisticsProviderImpl(lattice);
        ...
        if (lattice.algorithmMaxMillis >= 0) {
            map.put(Algorithm.ParameterEnum.timeLimitSeconds, Math.max(1, (int) (lattice.algorithmMaxMillis / 1000L)));
        }
    ...
    private static class SchemaImpl implements Schema {
        ...
        SchemaImpl(Lattice lattice, StatisticsProvider statisticsProvider) {
            ...
            final ImmutableList.Builder<AttributeImpl> attributeBuilder = ImmutableList.builder();
