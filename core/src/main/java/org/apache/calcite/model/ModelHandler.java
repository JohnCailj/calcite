...
public class ModelHandler {
     ...
     */
    @Deprecated public static void create(SchemaPlus schema, String functionName, List<String> path, String className,
                                          String methodName) {
     ...
     */
    public static void addFunctions(SchemaPlus schema, String functionName, List<String> path, String className,
                                    String methodName, boolean upCase) {
        final Class<?> clazz;
        try {
        ...
        throw new RuntimeException(
                "Not a valid function class: " + clazz + ". Scalar functions and table macros have an 'eval' method; "
    ...
    private void checkRequiredAttributes(Object json, String... attributeNames) {
        for (String attributeName : attributeNames) {
            try {
                ...
                if (o == null) {
                    throw new RuntimeException("Field '" + attributeName + "' is required in " + c.getSimpleName());
                }
            } catch (NoSuchFieldException | IllegalAccessException e) {
                throw new RuntimeException("while accessing field " + attributeName, e);
            }
        }
    }
    ...
    public void visit(JsonRoot jsonRoot) {
        ...
        final Pair<String, SchemaPlus> pair = Pair.of(null, connection.getRootSchema());
    ...
    private static ImmutableList<ImmutableList<String>> stringListList(List path) {
        final ImmutableList.Builder<ImmutableList<String>> builder = ImmutableList.builder();
        for (Object s : path) {
    ...
    public void visit(JsonCustomSchema jsonSchema) {
        try {
            ...
            final Schema schema = schemaFactory.create(parentSchema, jsonSchema.name,
                                                       operandMap(jsonSchema, jsonSchema.operand));
    ...
    public void visit(JsonJdbcSchema jsonSchema) {
        ...
        final DataSource dataSource = JdbcSchema.dataSource(jsonSchema.jdbcUrl, jsonSchema.jdbcDriver,
                                                            jsonSchema.jdbcUser, jsonSchema.jdbcPassword);
        JdbcSchema schema = JdbcSchema.create(parentSchema, jsonSchema.name, dataSource, jsonSchema.jdbcCatalog,
    ...
    public void visit(JsonMaterialization jsonMaterialization) {
        try {
            ...
            if (!schema.isMutable()) {
                throw new RuntimeException("Cannot define materialization; parent schema '" + currentSchemaName()
            ...
            schema.add(viewName, MaterializedViewTable.create(calciteSchema, jsonMaterialization.getSql(),
                                                              jsonMaterialization.viewSchemaPath, viewPath,
    ...
    public void visit(JsonLattice jsonLattice) {
        try {
            ...
            final SchemaPlus schema = currentSchema();
            if (!schema.isMutable()) {
                throw new RuntimeException("Cannot define lattice; parent schema '" + currentSchemaName()
                                           + "' is not a SemiMutableSchema");
            }
            CalciteSchema calciteSchema = CalciteSchema.from(schema);
            Lattice.Builder latticeBuilder = Lattice.builder(calciteSchema, jsonLattice.getSql()).auto(
                    jsonLattice.auto).algorithm(jsonLattice.algorithm);
    ...
    public void visit(JsonView jsonView) {
        try {
            ...
            final List<String> viewPath = ImmutableList.<String>builder().addAll(path).add(jsonView.name).build();
            schema.add(jsonView.name,
                       ViewTable.viewMacro(schema, jsonView.getSql(), path, viewPath, jsonView.modifiable));
        } catch (Exception e) {
    ...
    private SchemaPlus currentMutableSchema(String elementType) {
        ...
        if (!schema.isMutable()) {
            throw new RuntimeException(
                    "Cannot define " + elementType + "; parent schema '" + schema.getName() + "' is not mutable");
        }
        return schema;
    }
    ...
    public void visit(JsonFunction jsonFunction) {
        ...
        try {
            ...
            final List<String> path = Util.first(jsonFunction.path, currentSchemaPath());
            create(schema, jsonFunction.name, path, jsonFunction.className, jsonFunction.methodName);
        } catch (Exception e) {
    ...
    public void visit(JsonMeasure jsonMeasure) {
        ...
        final Lattice.Measure measure = latticeBuilder.resolveMeasure(jsonMeasure.agg, jsonMeasure.args);
        if (tileBuilder != null) {
