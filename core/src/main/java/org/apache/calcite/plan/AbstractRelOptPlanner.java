...
public abstract class AbstractRelOptPlanner implements RelOptPlanner {
    ...
    @SuppressWarnings("deprecation") public void setCancelFlag(CancelFlag cancelFlag) {
        // ignored
    }
    ...
    protected void mapRuleDescription(RelOptRule rule) {
        ...
        assert !INTEGER_PATTERN.matcher(description).matches() :
                "Rule's description should not be an integer: " + rule.getClass().getName() + ", " + description;
        ...
        if (existingRule != null) {
            if (existingRule == rule) {
                ...
                throw new AssertionError(
                        "Rule's description should be unique; " + "existing rule=" + existingRule + "; new rule="
                        + rule);
            }
        }
    }
    ...
    @SuppressWarnings("deprecation") public RelOptCost getCost(RelNode rel) {
    ...
    public Iterable<Class<? extends RelNode>> subClasses(final Class<? extends RelNode> clazz) {
        return Iterables.filter(classes, new PredicateImpl<Class<? extends RelNode>>() {
