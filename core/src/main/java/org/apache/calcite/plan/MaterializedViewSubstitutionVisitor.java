...
public class MaterializedViewSubstitutionVisitor extends SubstitutionVisitor {
    ...
    private static final ImmutableList<UnifyRule> EXTENDED_RULES = ImmutableList.<UnifyRule>builder().addAll(
            DEFAULT_RULES).add(ProjectToProjectUnifyRule1.INSTANCE).add(FilterToFilterUnifyRule1.INSTANCE).add(
            FilterToProjectUnifyRule1.INSTANCE).build();
    ...
    private static class ProjectToProjectUnifyRule1 extends AbstractUnifyRule {
        ...
        @Override protected UnifyResult apply(UnifyRuleCall call) {
            ...
            final List<RelDataTypeField> oldFieldList = query.getInput().rowType.getFieldList();
            final List<RelDataTypeField> newFieldList = call.target.rowType.getFieldList();
            ...
            final MutableProject newProject = MutableProject.of(query.rowType, call.target, newProjects);

            final MutableRel newProject2 = MutableRels.strip(newProject);
            return call.result(newProject2);
        }

        @Override protected UnifyRuleCall match(SubstitutionVisitor visitor, MutableRel query, MutableRel target) {
            ...
            if (queryOperand.matches(visitor, query)) {
                if (targetOperand.matches(visitor, target)) {
                    ...
                    if (queryProject.getInput() instanceof MutableFilter) {
                        ...
                        return visitor.new UnifyRuleCall(this, query, newFilter, copy(visitor.slots, slotCount));
                    }
                }
            }
            return null;
    ...
    private static class FilterToProjectUnifyRule1 extends AbstractUnifyRule {
        ...
        private FilterToProjectUnifyRule1() {
            super(operand(MutableFilter.class, query(0)),
                  operand(MutableProject.class, operand(MutableFilter.class, target(0))), 1);
        }

        public UnifyResult apply(UnifyRuleCall call) {
            final MutableRel query = call.query;

            final List<RelDataTypeField> oldFieldList = query.rowType.getFieldList();
            final List<RelDataTypeField> newFieldList = call.target.rowType.getFieldList();
            List<RexNode> newProjects;
            try {
                newProjects = transformRex(
                        (List<RexNode>) call.getCluster().getRexBuilder().identityProjects(query.rowType), oldFieldList,
                        newFieldList);
            } catch (MatchFailed e) {
                return null;
            }
            ...
            if (queryOperand.matches(visitor, query)) {
                if (targetOperand.matches(visitor, target)) {
                    if (visitor.isWeaker(query, ((MutableProject) target).getInput())) {
                        ...
                        RexNode newCondition;
                        try {
                            newCondition = transformRex(filter.condition, filter.getInput().rowType.getFieldList(),
                                                        target.rowType.getFieldList());
                        } catch (MatchFailed e) {
                            return null;
                        }
    ...
    private static RexNode transformRex(RexNode node, final List<RelDataTypeField> oldFields,
