...
class RelCompositeTrait<T extends RelMultipleTrait> implements RelTrait {
     ...
     */
    @SuppressWarnings("unchecked") static <T extends RelMultipleTrait> RelCompositeTrait<T> of(RelTraitDef def,
                                                                                               List<T> traitList) {
        final RelCompositeTrait<T> compositeTrait;
        if (traitList.isEmpty()) {
            compositeTrait = new EmptyCompositeTrait<T>(def);
        } else {
            final RelMultipleTrait[] traits = traitList.toArray(new RelMultipleTrait[traitList.size()]);
    ...
    @Override public boolean equals(Object obj) {
        return this == obj || obj instanceof RelCompositeTrait && Arrays.equals(traits,
                                                                                ((RelCompositeTrait) obj).traits);
    }
     ...
     * @param <T> trait type
     */
    private static class EmptyCompositeTrait<T extends RelMultipleTrait> extends RelCompositeTrait<T> {
