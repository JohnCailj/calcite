...
public class RelOptMaterialization {
    ...
    public static RelNode tryUseStar(RelNode rel, final RelOptTable starRelOptTable) {
        ...
        RelNode rel2 = rel.accept(new RelShuttleImpl() {
            ...
            @Override public RelNode visit(TableScan scan) {
                ...
                if (table.equals(starTable.tables.get(0))) {
                    Mappings.TargetMapping mapping = Mappings.createShiftMapping(
                            starRelOptTable.getRowType().getFieldCount(), 0, 0,
            ...
            private void match(ProjectFilterTable left, ProjectFilterTable right, RelOptCluster cluster) {
                ...
                if (leftTable instanceof StarTable && ((StarTable) leftTable).tables.contains(rightTable)) {
                    ...
                    Mappings.TargetMapping mapping = Mappings.merge(leftMapping, Mappings.offsetTarget(
                            Mappings.offsetSource(rightMapping, offset), leftMapping.getTargetCount()));
                    final RelNode project = RelOptUtil.createProject(LogicalTableScan.create(cluster, leftRelOptTable),
                ...
                if (rightTable instanceof StarTable && ((StarTable) rightTable).tables.contains(leftTable)) {
                    ...
                    Mappings.TargetMapping mapping = Mappings.merge(Mappings.offsetSource(leftMapping, offset),
        ...
        final Program program = Programs.hep(
                ImmutableList.of(ProjectFilterTransposeRule.INSTANCE, AggregateProjectMergeRule.INSTANCE,
                                 AggregateFilterTransposeRule.INSTANCE), false, DefaultRelMetadataProvider.INSTANCE);
        return program.run(null, rel2, null, ImmutableList.<RelOptMaterialization>of(),
    ...
    private static class ProjectFilterTable {
        ...
        public Mappings.TargetMapping mapping() {
            return mapping != null ? mapping : Mappings.createIdentity(scan.getRowType().getFieldCount());
        }
    ...
    public static RelNode toLeafJoinForm(RelNode rel) {
        final Program program = Programs.hep(
                ImmutableList.of(JoinProjectTransposeRule.RIGHT_PROJECT, JoinProjectTransposeRule.LEFT_PROJECT,
                                 FilterJoinRule.FilterIntoJoinRule.FILTER_ON_JOIN, ProjectRemoveRule.INSTANCE,
                                 ProjectMergeRule.INSTANCE), false, DefaultRelMetadataProvider.INSTANCE);
        if (CalcitePrepareImpl.DEBUG) {
            System.out.println(
                    RelOptUtil.dumpPlan("before", rel, SqlExplainFormat.TEXT, SqlExplainLevel.DIGEST_ATTRIBUTES));
        }
        final RelNode rel2 = program.run(null, rel, null, ImmutableList.<RelOptMaterialization>of(),
                                         ImmutableList.<RelOptLattice>of());
        if (CalcitePrepareImpl.DEBUG) {
            System.out.println(
                    RelOptUtil.dumpPlan("after", rel2, SqlExplainFormat.TEXT, SqlExplainLevel.DIGEST_ATTRIBUTES));
        }
        return rel2;
    }
}
