...
public abstract class RelOptRule {
    ...
    public static <R extends RelNode> RelOptRuleOperand operand(Class<R> clazz, RelOptRuleOperandChildren operandList) {
        return new RelOptRuleOperand(clazz, null, Predicates.<R>alwaysTrue(), operandList.policy, operandList.operands);
    }
     ...
     * children
     */
    public static <R extends RelNode> RelOptRuleOperand operand(Class<R> clazz, RelTrait trait,
                                                                RelOptRuleOperandChildren operandList) {
        return new RelOptRuleOperand(clazz, trait, Predicates.<R>alwaysTrue(), operandList.policy,
     ...
     */
    public static <R extends RelNode> RelOptRuleOperand operand(Class<R> clazz, RelTrait trait,
                                                                Predicate<? super R> predicate, RelOptRuleOperand first,
     ...
     * list of children
     */
    public static <R extends RelNode> RelOptRuleOperand operand(Class<R> clazz, RelOptRuleOperand first,
                                                                RelOptRuleOperand... rest) {
     ...
     */
    protected static <R extends RelNode> ConverterRelOptRuleOperand convertOperand(Class<R> clazz,
                                                                                   Predicate<? super R> predicate,
                                                                                   RelTrait trait) {
    ...
    protected boolean equals(RelOptRule that) {
        ...
        return this.description.equals(that.description) && (this.getClass() == that.getClass()) && this.operand.equals(
                that.operand);
    }
     ...
     */
    protected static List<RelNode> convertList(List<RelNode> rels, final RelTrait trait) {
        return Lists.transform(rels, new Function<RelNode, RelNode>() {
    ...
    static String guessDescription(String className) {
        ...
        int punc = Math.max(className.lastIndexOf('.'), className.lastIndexOf('$'));
        if (punc >= 0) {
            description = className.substring(punc + 1);
        }
        if (description.matches("[0-9]+")) {
            throw new RuntimeException("Derived description of rule class " + className + " is an integer, not valid. "
    ...
    private static class ConverterRelOptRuleOperand extends RelOptRuleOperand {
        ...
        <R extends RelNode> ConverterRelOptRuleOperand(Class<R> clazz, RelTrait in, Predicate<? super R> predicate) {
            super(clazz, in, predicate, RelOptRuleOperandChildPolicy.ANY, ImmutableList.<RelOptRuleOperand>of());
        }
        ...
        public boolean matches(RelNode rel) {
            ...
            if (rel instanceof Converter) {
                if (((ConverterRule) getRule()).getTraitDef() == ((Converter) rel).getTraitDef()) {
                    return false;
                }
            }
