...
public abstract class RelOptUtil {
    ...
    public static final Predicate<Filter> FILTER_PREDICATE = new PredicateImpl<Filter>() {
        ...
        public boolean test(Filter filter) {
            return !(B && RexMultisetUtil.containsMultiset(filter.getCondition(), true) || RexOver.containsOver(
                    filter.getCondition()));
        }
    };
    ...
    public static final Predicate<Project> PROJECT_PREDICATE = new PredicateImpl<Project>() {
        ...
        public boolean test(Project project) {
            return !(B && RexMultisetUtil.containsMultiset(project.getProjects(), true) || RexOver.containsOver(
                    project.getProjects(), null));
        }
    };
    ...
    public static List<String> findAllTableQualifiedNames(RelNode rel) {
        return Lists.transform(findAllTables(rel), new Function<RelOptTable, String>() {
    ...
    public static RelNode createExistsPlan(RelOptCluster cluster, RelNode seekRel, List<RexNode> conditions,
        ...
        if (extraExpr != null) {
            ...
            final AggregateCall aggCall = AggregateCall.create(SqlStdOperatorTable.MIN, false, ImmutableList.of(0), -1,
                                                               0, ret, null, extraName);
    ...
    public static Exists createExistsPlan(RelNode seekRel, SubQueryType subQueryType, Logic logic, boolean notIn) {
        ...
        final AggregateCall aggCall = AggregateCall.create(SqlStdOperatorTable.MIN, false,
                                                           ImmutableList.of(projectedKeyCount), -1, projectedKeyCount,
                                                           ret, null, null);
    ...
    public static RelNode createRenameRel(RelDataType outputType, RelNode rel) {
        ...
        for (Pair<RelDataTypeField, RelDataTypeField> pair : Pair.zip(inputFields, outputFields)) {
            ...
            renames.add(Pair.<RexNode, String>of(rexBuilder.makeInputRef(inputField.getType(), inputField.getIndex()),
                                                 outputField.getName()));
        }
    ...
    public static RelNode createSingleValueAggRel(RelOptCluster cluster, RelNode rel) {
        ...
        for (int i = 0; i < aggCallCnt; i++) {
            aggCalls.add(
                    AggregateCall.create(SqlStdOperatorTable.SINGLE_VALUE, false, ImmutableList.of(i), -1, 0, rel, null,
                                         null));
        }
    ...
    public static RexNode splitJoinCondition(List<RelDataTypeField> sysFieldList, RelNode leftRel, RelNode rightRel,
                                             RexNode condition, List<RexNode> leftJoinKeys, List<RexNode> rightJoinKeys,
                                             List<Integer> filterNulls, List<SqlOperator> rangeOp) {
    ...
    public static RexNode splitJoinCondition(List<RelDataTypeField> sysFieldList, List<RelNode> inputs,
                                             RexNode condition, List<List<RexNode>> joinKeys, List<Integer> filterNulls,
    ...
    private static void splitJoinCondition(List<RelDataTypeField> sysFieldList, List<RelNode> inputs, RexNode condition,
                                           List<List<RexNode>> joinKeys, List<Integer> filterNulls,
                                           List<SqlOperator> rangeOp, List<RexNode> nonEquiList) {
        ...
        if (condition instanceof RexCall) {
            ...
            if ((kind == SqlKind.EQUALS) || (filterNulls != null && kind == SqlKind.IS_NOT_DISTINCT_FROM) || (
                    rangeOp != null && rangeOp.isEmpty() && (kind == SqlKind.GREATER_THAN
                ...
                for (int i = 0; i < inputs.size() && !foundBothInputs; i++) {
                    if (projRefs0.intersects(inputsRange[i]) && projRefs0.union(inputsRange[i]).equals(
                            inputsRange[i])) {
                        if (leftKey == null) {
                            leftKey = op0;
                            leftInput = i;
                    ...
                    } else if (projRefs1.intersects(inputsRange[i]) && projRefs1.union(inputsRange[i]).equals(
                            inputsRange[i])) {
                        if (leftKey == null) {
                            leftKey = op1;
                            leftInput = i;
                ...
                if ((leftKey != null) && (rightKey != null)) {
                    ...
                    if (leftKeyType != rightKeyType) {
                        // perform casting
                        RelDataType targetKeyType = typeFactory.leastRestrictive(
    ...
    private static void splitCorrelatedFilterCondition(LogicalFilter filter, RexNode condition,
                                                       List<RexInputRef> joinKeys, List<RexNode> correlatedJoinKeys,
    ...
    private static void splitCorrelatedFilterCondition(LogicalFilter filter, RexNode condition, List<RexNode> joinKeys,
                                                       List<RexNode> correlatedJoinKeys, List<RexNode> nonEquiList,
                                                       boolean extractCorrelatedFieldAccess) {
        if (condition instanceof RexCall) {
            RexCall call = (RexCall) condition;
            if (call.getOperator().getKind() == SqlKind.AND) {
    ...
    public static void projectJoinInputs(RelNode[] inputRels, List<RexNode> leftJoinKeys, List<RexNode> rightJoinKeys,
                                         int systemColCount, List<Integer> leftKeys, List<Integer> rightKeys,
        ...
        if (newLeftKeyCount > 0) {
            leftRel = createProject(leftRel, newLeftFields,
                                    SqlValidatorUtil.uniquify(newLeftFieldNames, typeSystem.isSchemaCaseSensitive()));
        }

        if (newRightKeyCount > 0) {
            rightRel = createProject(rightRel, newRightFields,
                                     SqlValidatorUtil.uniquify(newRightFieldNames, typeSystem.isSchemaCaseSensitive()));
        }
    ...
    public static RelNode createProjectJoinRel(List<Integer> outputProj, RelNode joinRel) {
        ...
        if ((newProjectOutputSize > 0) && (newProjectOutputSize < joinOutputFields.size())) {
            ...
            for (int fieldIndex : outputProj) {
                ...
                newProjects.add(Pair.<RexNode, String>of(rexBuilder.makeInputRef(field.getType(), fieldIndex),
                                                         field.getName()));
            }

            // Create a project rel on the output of the join.
            return createProject(joinRel, Pair.left(newProjects), Pair.right(newProjects));
        }
    ...
    public static RelDataType createDmlRowType(SqlKind kind, RelDataTypeFactory typeFactory) {
        switch (kind) {
            ...
            case DELETE:
            case UPDATE:
                return typeFactory.createStructType(ImmutableList.of(Pair.of(AvaticaConnection.ROWCOUNT_COLUMN_NAME,
                                                                             typeFactory.createSqlType(
                                                                                     SqlTypeName.BIGINT))));
            case EXPLAIN:
                return typeFactory.createStructType(ImmutableList.of(Pair.of(AvaticaConnection.PLAN_COLUMN_NAME,
                                                                             typeFactory.createSqlType(
                                                                                     SqlTypeName.VARCHAR,
    ...
    public static boolean classifyFilters(RelNode joinRel, List<RexNode> filters, JoinRelType joinType,
                                          boolean pushInto, boolean pushLeft, boolean pushRight,
                                          List<RexNode> joinFilters, List<RexNode> leftFilters,
            ...
            if (pushLeft && leftBitmap.contains(inputBits)) {
                ...
                if (!filter.isAlwaysTrue()) {
                    ...
                    final RexNode shiftedFilter = shiftFilter(nSysFields + nFieldsLeft, nTotalFields,
                                                              -(nSysFields + nFieldsLeft), rexBuilder, joinFields,
                                                              nTotalFields, rightFields, filter);
    ...
    private static RexNode shiftFilter(int start, int end, int offset, RexBuilder rexBuilder,
                                       List<RelDataTypeField> joinFields, int nTotalFields,
                                       List<RelDataTypeField> rightFields, RexNode filter) {
    ...
    public static List<RexNode> createSwappedJoinExprs(RelNode newJoin, Join origJoin, boolean origOrder) {
        ...
        final int nFields = origOrder ? origJoin.getRight().getRowType().getFieldCount() : origJoin.getLeft().getRowType().getFieldCount();
    ...
    public static MultiJoin projectMultiJoin(MultiJoin multiJoin, LogicalProject project) {
        ...
        return new MultiJoin(multiJoin.getCluster(), multiJoin.getInputs(), multiJoin.getJoinFilter(),
                             ...
                             multiJoin.getJoinTypes(), Lists.transform(newProjFields, ImmutableBitSet.FROM_BIT_SET),
    ...
    public static <T extends RelNode> T addTrait(T rel, RelTrait trait) {
        ...
        return (T) rel.copy(rel.getTraitSet().replace(trait), (List) rel.getInputs());
    }
    ...
    public static RelNode createProject(RelNode child, List<Pair<RexNode, String>> projectList, boolean optimize) {
        return createProject(child, Pair.left(projectList), Pair.right(projectList), optimize,
    ...
    public static RelNode createProject(RelNode child, List<? extends RexNode> exprs, List<String> fieldNames,
        ...
        final RelOptCluster cluster = child.getCluster();
        final RelDataType rowType = RexUtil.createStructType(cluster.getTypeFactory(), exprs, fieldNames,
        ...
        if (optimize && RexUtil.isIdentity(exprs, child.getRowType())) {
            if (child instanceof Project && fieldNames != null) {
                ...
                child = childProject.copy(childProject.getTraitSet(), childProject.getInput(),
    ...
    public static RelNode permute(RelNode rel, Permutation permutation, List<String> fieldNames) {
        ...
        for (int i = 0; i < permutation.getTargetCount(); i++) {
            ...
            outputNameList.add(((fieldNames == null) || (fieldNames.size() <= i) || (fieldNames.get(i)
                                                                                     == null)) ? targetField.getName() : fieldNames.get(
                    i));
    ...
    public static RelNode projectMapping(RelNode rel, Mapping mapping, List<String> fieldNames,
        ...
        for (int i = 0; i < mapping.getTargetCount(); i++) {
            ...
            outputNameList.add(((fieldNames == null) || (fieldNames.size() <= i) || (fieldNames.get(i)
                                                                                     == null)) ? sourceField.getName() : fieldNames.get(
                    i));
    ...
    public static class RexInputConverter extends RexShuttle {
        ...
        public RexInputConverter(RexBuilder rexBuilder, List<RelDataTypeField> srcFields,
                                 List<RelDataTypeField> leftDestFields, List<RelDataTypeField> rightDestFields,
                                 int[] adjustments) {
