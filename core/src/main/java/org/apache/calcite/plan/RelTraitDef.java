...
public abstract class RelTraitDef<T extends RelTrait> {
    ...
    private final LoadingCache<T, T> canonicalMap = CacheBuilder.newBuilder().softValues().build(
            new CacheLoader<T, T>() {
     ...
     */
    private final LoadingCache<Object, RelCompositeTrait> canonicalCompositeMap = CacheBuilder.newBuilder().softValues().build(
            new CacheLoader<Object, RelCompositeTrait>() {
                ...
                @Override public RelCompositeTrait load(@Nonnull Object key) {
                    ...
                    @SuppressWarnings("unchecked") final List<RelMultipleTrait> list = (List<RelMultipleTrait>) key;
    ...
    public final T canonize(T trait) {
        ...
        assert getTraitClass().isInstance(trait) :
                getClass().getName() + " cannot canonize a " + trait.getClass().getName();
