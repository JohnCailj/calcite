...
public final class RelTraitSet extends AbstractList<RelTrait> {
    ...
    public RelTraitSet replace(int index, RelTrait trait) {
        assert traits[index].getTraitDef()
               == trait.getTraitDef() : "RelTrait has different RelTraitDef than replacement";
    ...
    @Override public boolean equals(Object obj) {
        return this == obj || obj instanceof RelTraitSet && Arrays.equals(traits, ((RelTraitSet) obj).traits);
    }
    ...
    public RelTraitSet plus(RelTrait trait) {
        ...
        switch (traits.length) {
            ...
            case 2:
                ...
                break;
            default:
                newTraits = ImmutableList.<RelTrait>builder().add(traits).add(canonizedTrait).build();
        }
    ...
    public RelTraitSet simplify() {
        ...
        for (int i = 0; i < traits.length; i++) {
            final RelTrait trait = traits[i];
            if (trait instanceof RelCompositeTrait) {
                x = x.replace(i, ((RelCompositeTrait) trait).size() == 1 ? ((RelCompositeTrait) trait).trait(
                        0) : trait.getTraitDef().getDefault());
            }
        }
        return x;
    }
    ...
    private static class Cache {
        ...
        RelTraitSet getOrAdd(List<RelTrait> traits) {
            ...
            final RelTraitSet traitSet = new RelTraitSet(this, traits.toArray(new RelTrait[traits.size()]));
