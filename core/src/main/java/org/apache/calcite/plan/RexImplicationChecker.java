...
public class RexImplicationChecker {
    ...
    private boolean checkSupport(InputUsageFinder firstUsageFinder, InputUsageFinder secondUsageFinder) {
        ...
        for (Map.Entry<RexInputRef, InputRefUsage<SqlOperator, RexNode>> entry : secondUsageMap.entrySet()) {
            ...
            if (firstUsage == null || firstUsage.usageList.size() != firstUsage.usageCount
            ...
            if (firstLen == 2 && secondLen == 2 && !(isEquivalentOp(fKind, sKind) && isEquivalentOp(fKind2, sKind2))
                && !(isEquivalentOp(fKind, sKind2) && isEquivalentOp(fKind2, sKind))) {
                return false;
            } else if (firstLen == 1 && secondLen == 1 && fKind != SqlKind.EQUALS && !isSupportedUnaryOperators(sKind)
                ...
                // x > 30 and x > 40 implies x < 70
                if (!isOppositeOp(fKind, fKind2) && !isSupportedUnaryOperators(sKind) && !(isEquivalentOp(fKind, fKind2)
                                                                                           && isEquivalentOp(fKind,
                                                                                                             sKind))) {
                    return false;
                }
            }
