...
import org.apache.calcite.rel.mutable.*;
...
public class SubstitutionVisitor {
    ...
    protected static final ImmutableList<UnifyRule> DEFAULT_RULES = ImmutableList.<UnifyRule>of(TrivialRule.INSTANCE,
     ...
     * problem.</p>
     */
    @VisibleForTesting public static RexNode splitFilter(final RexSimplify simplify, RexNode condition,
                                                         RexNode target) {
        ...
        if (mayBeSatisfiable(x)) {
            RexNode x2 = RexUtil.composeConjunction(simplify.rexBuilder, ImmutableList.of(condition, target), false);
    ...
    private List<List<Replacement>> go(MutableRel replacement) {
        ...
        for (; ; ) {
            ...
            while (queryDescendant != null) {
                ...
                final MutableRel childOrNext = queryDescendant.getInputs().isEmpty() ? next : queryDescendant.getInputs().get(
                        0);
                for (MutableRel targetDescendant : targetDescendants) {
                    for (UnifyRule rule : applicableRules(queryDescendant, targetDescendant)) {
                        ...
                        if (call != null) {
                            ...
                            if (result != null) {
                                ...
                                assert result.result.rowType.equals(result.call.query.rowType) : Pair.of(result.result,
                                                                                                         result.call.query);
    ...
    private static boolean mightMatch(UnifyRule rule, Class queryClass, Class targetClass) {
        return rule.queryOperand.clazz.isAssignableFrom(queryClass) && rule.targetOperand.clazz.isAssignableFrom(
                targetClass);
    }
    ...
    protected static class MatchFailed extends ControlFlowException {
        ...
        @SuppressWarnings("ThrowableInstanceNeverThrown") public static final MatchFailed INSTANCE = new MatchFailed();
    }
    ...
    private static List<AggregateCall> apply(final Mapping mapping, List<AggregateCall> aggCallList) {
        return Lists.transform(aggCallList, new Function<AggregateCall, AggregateCall>() {
            ...
            public AggregateCall apply(AggregateCall call) {
                return call.copy(Mappings.apply2(mapping, call.getArgList()), Mappings.apply(mapping, call.filterArg));
            }
        });
    }
    ...
    public static MutableRel unifyAggregates(MutableAggregate query, MutableAggregate target) {
        ...
        if (query.groupSet.equals(target.groupSet)) {
            ...
            for (AggregateCall aggregateCall : query.aggCalls) {
                ...
                aggregateCalls.add(
                        AggregateCall.create(getRollup(aggregateCall.getAggregation()), aggregateCall.isDistinct(),
    ...
    public static class FilterOnProjectRule extends RelOptRule {
        ...
        public void onMatch(RelOptRuleCall call) {
            ...
            RelDataType newRowType = cluster.getTypeFactory().builder().addAll(project.getRowType().getFieldList()).add(
                    "condition", Util.last(newProjects).getType()).build();
            final RelNode newProject = project.copy(project.getTraitSet(), project.getInput(), newProjects, newRowType);
