...
public class HepPlanner extends AbstractRelOptPlanner {
    ...
    public HepPlanner(HepProgram program, Context context, boolean noDAG, Function2<RelNode, RelNode, Void> onCopyHook,
        ...
        this.onCopyHook = Util.first(onCopyHook, Functions.<RelNode, RelNode, Void>ignore2());
...
*/
        ...
        Iterable<HepRelVertex> iter = TopologicalOrderIterator.of(graph);
    ...
    private HepRelVertex applyRule(RelOptRule rule, HepRelVertex vertex, boolean forceConversions) {
        ...
        HepRuleCall call = new HepRuleCall(this, rule.getOperand(), bindings.toArray(new RelNode[bindings.size()]),
    ...
    private void dumpGraph() {
        ...
        for (HepRelVertex vertex : BreadthFirstIterator.of(graph, root)) {
            ...
            RelNode rel = vertex.getCurrentRel();
            sb.append(rel).append(", rowcount=").append(mq.getRowCount(rel)).append(", cumulative cost=").append(
                    getCost(rel, mq)).append('\n');
        }
        sb.append("}");
