...
public class RelSubset extends AbstractRelNode {
    ...
    @Override public void explain(RelWriter pw) {
        ...
        pw.item("subset", s);
        final AbstractRelNode input = (AbstractRelNode) Util.first(getBest(), getOriginal());
        if (input == null) {
            return;
        }
    ...
    void add(RelNode rel) {
        ...
        // row type.
        if (set.rel != null) {
            RelOptUtil.equal("rowtype of new rel", rel.getRowType(), "rowtype of set", getRowType(), Litmus.THROW);
        }
        set.addInternal(rel);
    ...
    void propagateCostImprovements0(VolcanoPlanner planner, RelMetadataQuery mq, RelNode rel,
    ...
    public Iterable<RelNode> getRels() {
        return new Iterable<RelNode>() {
            ...
            public Iterator<RelNode> iterator() {
                return Linq4j.asEnumerable(set.rels).where(new Predicate1<RelNode>() {
