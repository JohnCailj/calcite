...
public class VolcanoPlanner extends AbstractRelOptPlanner {
     ...
     * null VARCHAR(10).
     */
    private final Map<Pair<String, RelDataType>, RelNode> mapDigestToRel = new HashMap<>();
    ...
    private void registerMaterializations() {
        ...
        final List<Pair<RelNode, List<RelOptMaterialization>>> materializationUses = RelOptMaterializations.useMaterializedViews(
        ...
        final Set<RelOptMaterialization> applicableMaterializations = new HashSet<>(
                RelOptMaterializations.getApplicableMaterializations(originalRoot, materializations));
        for (Pair<RelNode, List<RelOptMaterialization>> use : materializationUses) {
            applicableMaterializations.removeAll(use.right);
        }
        for (RelOptMaterialization materialization : applicableMaterializations) {
            RelSubset subset = registerImpl(materialization.queryRel, null);
            RelNode tableRel2 = RelOptUtil.createCastRel(materialization.tableRel,
                                                         materialization.queryRel.getRowType(), true);
            registerImpl(tableRel2, subset.set);
        }

        // Register rels using lattices.
        final List<Pair<RelNode, RelOptLattice>> latticeUses = RelOptMaterializations.useLattices(originalRoot,
                                                                                                  ImmutableList.copyOf(
                                                                                                          latticeByName.values()));
    ...
    protected void validate() {
        for (RelSet set : allSets) {
            if (set.equivalentSet != null) {
                throw new AssertionError("set [" + set + "] has been merged: it should not be in the list");
            }
            for (RelSubset subset : set.subsets) {
                if (subset.set != set) {
                    throw new AssertionError("subset [" + subset.getDescription() + "] is in wrong set [" + set + "]");
                }
                for (RelNode rel : subset.getRels()) {
                    RelOptCost relCost = getCost(rel, rel.getCluster().getMetadataQuery());
                    if (relCost.isLt(subset.bestCost)) {
                        throw new AssertionError(
                                "rel [" + rel.getDescription() + "] has lower cost " + relCost + " than best cost "
                                + subset.bestCost + " of subset [" + subset.getDescription() + "]");
                    }
                }
            }
        }
    ...
    public void dump(PrintWriter pw) {
        ...
        pw.println("Sets:");
        Ordering<RelSet> ordering = Ordering.from(new Comparator<RelSet>() {
        ...
        for (RelSet set : ordering.immutableSortedCopy(allSets)) {
            ...
            for (RelSubset subset : set.subsets) {
                ++j;
                pw.println("\t" + subset.getDescription() + ", best=" + ((subset.best == null) ? "null" : ("rel#"
                                                                                                           + subset.best.getId()))
    ...
    void rename(RelNode rel) {
        ...
        if (fixUpInputs(rel)) {
            final Pair<String, RelDataType> oldKey = Pair.of(oldDigest, rel.getRowType());
    ...
    private RelSubset registerImpl(RelNode rel, RelSet set) {
        ...
        if (traits.size() != traitDefs.size()) {
            throw new AssertionError(
                    "Relational expression " + rel + " does not have the correct number of traits: " + traits.size()
                    + " != " + traitDefs.size());
        }
