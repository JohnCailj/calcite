...
class CalciteMaterializer extends CalcitePrepareImpl.CalcitePreparingStmt {
    ...
    CalciteMaterializer(CalcitePrepareImpl prepare, CalcitePrepare.Context context, CatalogReader catalogReader,
                        CalciteSchema schema, RelOptPlanner planner, SqlRexConvertletTable convertletTable) {
        super(prepare, context, catalogReader, catalogReader.getTypeFactory(), schema, EnumerableRel.Prefer.ANY,
              planner, BindableConvention.INSTANCE, convertletTable);
    }
    ...
    private void useStar(CalciteSchema schema, Materialization materialization) {
        for (Callback x : useStar(schema, materialization.queryRel)) {
            ...
            if (CalcitePrepareImpl.DEBUG) {
                System.out.println(
                        "Materialization " + materialization.materializedTable + " matched star table " + x.starTable
                        + "; query after re-write: " + RelOptUtil.toString(materialization.queryRel));
            }
        }
    }
    ...
    private Iterable<Callback> useStar(CalciteSchema schema, RelNode queryRel) {
        ...
        for (CalciteSchema.TableEntry starTable : starTables) {
            ...
            RelOptTableImpl starRelOptTable = RelOptTableImpl.create(catalogReader, table.getRowType(typeFactory),
                                                                     starTable, null);
