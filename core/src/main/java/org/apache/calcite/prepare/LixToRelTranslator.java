...
class LixToRelTranslator implements RelOptTable.ToRelContext {
        ...
        if (expression instanceof MethodCallExpression) {
            ...
            switch (method) {
                case SELECT:
                    ...
                    return LogicalProject.create(input, toRex(input, (FunctionExpression) call.expressions.get(0)),
                                                 (List<String>) null);
                ...
                case AS_QUERYABLE:
                    return LogicalTableScan.create(cluster, RelOptTableImpl.create(null, typeFactory.createJavaType(
                            Types.toClass(Types.getElementType(call.targetExpression.getType()))),
                                                                                   ImmutableList.<String>of(),
                                                                                   call.targetExpression));

                case SCHEMA_GET_TABLE:
                    return LogicalTableScan.create(cluster, RelOptTableImpl.create(null, typeFactory.createJavaType(
                            (Class) ((ConstantExpression) call.expressions.get(1)).value), ImmutableList.<String>of(),
                                                                                   call.targetExpression));
    ...
    private List<RexNode> toRex(RelNode child, FunctionExpression expression) {
        ...
        CalcitePrepareImpl.ScalarTranslator translator = CalcitePrepareImpl.EmptyScalarTranslator.empty(
                rexBuilder).bind(expression.parameterList, list);
    ...
    List<RexNode> toRexList(FunctionExpression expression, RelNode... inputs) {
        ...
        return CalcitePrepareImpl.EmptyScalarTranslator.empty(rexBuilder).bind(expression.parameterList,
                                                                               list).toRexList(expression.body);
    }
    ...
    RexNode toRex(FunctionExpression expression, RelNode... inputs) {
        ...
        return CalcitePrepareImpl.EmptyScalarTranslator.empty(rexBuilder).bind(expression.parameterList, list).toRex(
                expression.body);
    }
}
