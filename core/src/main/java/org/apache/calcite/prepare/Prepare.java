...
public abstract class Prepare {
    ...
    protected abstract PreparedResult createPreparedExplanation(RelDataType resultType, RelDataType parameterRowType,
                                                                RelRoot root, SqlExplainFormat format,
     ...
     */
    protected RelRoot optimize(RelRoot root, final List<Materialization> materializations,
        ...
        for (Materialization materialization : materializations) {
            ...
            materializationList.add(new RelOptMaterialization(materialization.tableRel, materialization.queryRel,
                                                              materialization.starRelOptTable, qualifiedTableName));
        }
        ...
        for (CalciteSchema.LatticeEntry lattice : lattices) {
            ...
            final RelOptTableImpl starRelOptTable = RelOptTableImpl.create(catalogReader,
    ...
    protected RelTraitSet getDesiredRootTraitSet(RelRoot root) {
        ...
        return root.rel.getTraitSet().replace(resultConvention).replace(root.collation).simplify();
    }
    ...
    public PreparedResult prepareSql(SqlNode sqlQuery, SqlNode sqlNodeOriginal, Class runtimeContextClass,
                                     SqlValidator validator, boolean needsValidation) {
        queryString = sqlQuery.toString();

        init(runtimeContextClass);

        final SqlToRelConverter.ConfigBuilder builder = SqlToRelConverter.configBuilder().withTrimUnusedFields(
                true).withExpand(THREAD_EXPAND.get()).withExplain(sqlQuery.getKind() == SqlKind.EXPLAIN);
        ...
        if (sqlExplain != null) {
            switch (sqlExplain.getDepth()) {
                ...
                default:
                    ...
                    return createPreparedExplanation(null, parameterRowType, root, sqlExplain.getFormat(),
     ...
     * SqlToRelConverter.
     */
    protected abstract SqlToRelConverter getSqlToRelConverter(SqlValidator validator, CatalogReader catalogReader,
    ...
    protected RelRoot trimUnusedFields(RelRoot root) {
        final SqlToRelConverter.Config config = SqlToRelConverter.configBuilder().withTrimUnusedFields(
                shouldTrim(root.rel)).withExpand(THREAD_EXPAND.get()).build();
    ...
    public abstract static class AbstractPreparingTable implements PreparingTable {
        ...
        public boolean columnHasDefaultValue(RelDataType rowType, int ordinal, InitializerContext initializerContext) {
            ...
            if (table != null && table instanceof Wrapper) {
                final InitializerExpressionFactory initializerExpressionFactory = ((Wrapper) table).unwrap(
                        InitializerExpressionFactory.class);
                if (initializerExpressionFactory != null) {
                    return initializerExpressionFactory.newColumnDefaultValue(this, ordinal,
                                                                              initializerContext).getType().getSqlTypeName()
                           != SqlTypeName.NULL;
                }
            }
    ...
    public abstract static class PreparedExplain implements PreparedResult {
        ...
        public PreparedExplain(RelDataType rowType, RelDataType parameterRowType, RelRoot root, SqlExplainFormat format,
                               SqlExplainLevel detailLevel) {
    ...
    public abstract static class PreparedResultImpl implements PreparedResult, Typed {
        ...
        public PreparedResultImpl(RelDataType rowType, RelDataType parameterRowType, List<List<String>> fieldOrigins,
                                  List<RelCollation> collations, RelNode rootRel,
                                  LogicalTableModify.Operation tableModOp, boolean isDml) {
