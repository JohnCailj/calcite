...
public class RelOptTableImpl extends Prepare.AbstractPreparingTable {
    ...
    public static RelOptTableImpl create(RelOptSchema schema, RelDataType rowType, Table table, Path path) {
        ...
        Function<Class, Expression> expressionFunction = getClassExpressionFunction(schemaPlus, Util.last(path).left,
                                                                                    table);
        return new RelOptTableImpl(schema, rowType, Pair.left(path), table, expressionFunction,
    ...
    public RelOptTableImpl copy(RelDataType newRowType) {
        return new RelOptTableImpl(this.schema, newRowType, this.names, this.table, this.expressionFunction,
                                   this.rowCount);
    }

    private static Function<Class, Expression> getClassExpressionFunction(CalciteSchema.TableEntry tableEntry,
                                                                          Table table) {
        return getClassExpressionFunction(tableEntry.schema.plus(), tableEntry.name, table);
    }

    private static Function<Class, Expression> getClassExpressionFunction(final SchemaPlus schema,
    ...
    @Override protected RelOptTable extend(Table extendedTable) {
        ...
        return new RelOptTableImpl(getRelOptSchema(), extendedRowType, getQualifiedName(), extendedTable,
                                   expressionFunction, getRowCount());
    }
    ...
    private static class MySchemaPlus implements SchemaPlus {
        ...
        @Override public void add(String name, org.apache.calcite.schema.Function function) {
            throw new UnsupportedOperationException();
        }
        ...
        @Override public Collection<org.apache.calcite.schema.Function> getFunctions(String name) {
