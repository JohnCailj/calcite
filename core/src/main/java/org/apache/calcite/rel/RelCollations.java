...
public class RelCollations {
     ...
     * columns.
     */
    public static final RelCollation EMPTY = RelCollationTraitDef.INSTANCE.canonize(
    ...
    public static final RelCollation PRESERVE = RelCollationTraitDef.INSTANCE.canonize(
    ...
    public static boolean contains(RelCollation collation, Iterable<Integer> keys) {
        ...
        for (int i = 0; i < n; i++) {
            final RelFieldCollation fieldCollation = collation.getFieldCollations().get(i);
            if (!iterator.hasNext()) {
                return true;
            }
