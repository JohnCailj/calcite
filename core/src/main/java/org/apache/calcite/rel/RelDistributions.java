...
public class RelDistributions {
     ...
     */
    public static final RelDistribution RANDOM_DISTRIBUTED = new RelDistributionImpl(
            RelDistribution.Type.RANDOM_DISTRIBUTED, EMPTY);

    /**
     * The singleton round-robin distribution.
     */
    public static final RelDistribution ROUND_ROBIN_DISTRIBUTED = new RelDistributionImpl(
            RelDistribution.Type.ROUND_ROBIN_DISTRIBUTED, EMPTY);

    /**
     * The singleton broadcast distribution.
     */
    public static final RelDistribution BROADCAST_DISTRIBUTED = new RelDistributionImpl(
    ...
    private static class RelDistributionImpl implements RelDistribution {
        ...
        private RelDistributionImpl(Type type, ImmutableIntList keys) {
            ...
            assert type != Type.HASH_DISTRIBUTED || keys.size() < 2 || Ordering.natural().isOrdered(
                    keys) : "key columns of hash distribution must be in order";
        ...
        @Override public boolean equals(Object obj) {
            return this == obj
                   || obj instanceof RelDistributionImpl && type == ((RelDistributionImpl) obj).type && keys.equals(
                    ((RelDistributionImpl) obj).keys);
        }
        ...
        public RelDistribution apply(Mappings.TargetMapping mapping) {
            ...
            return getTraitDef().canonize(
                    new RelDistributionImpl(type, ImmutableIntList.copyOf(Mappings.apply((Mapping) mapping, keys))));
        }
        ...
        public boolean satisfies(RelTrait trait) {
            ...
            if (trait == RANDOM_DISTRIBUTED) {
                ...
                return type == Type.HASH_DISTRIBUTED || type == Type.ROUND_ROBIN_DISTRIBUTED
