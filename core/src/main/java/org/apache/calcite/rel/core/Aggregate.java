...
public abstract class Aggregate extends SingleRel {
     ...
     */
    public static final Predicate<Aggregate> IS_SIMPLE = new PredicateImpl<Aggregate>() {
    ...
    public static final Predicate<Aggregate> NO_INDICATOR = new PredicateImpl<Aggregate>() {
    ...
    public static final Predicate<Aggregate> IS_NOT_GRAND_TOTAL = new PredicateImpl<Aggregate>() {
     ...
     */
    protected Aggregate(RelOptCluster cluster, RelTraitSet traits, RelNode child, boolean indicator,
        ...
        for (AggregateCall aggCall : aggCalls) {
            ...
            Preconditions.checkArgument(aggCall.filterArg < 0 || isPredicate(child, aggCall.filterArg),
    ...
    protected Aggregate(RelInput input) {
        this(input.getCluster(), input.getTraitSet(), input.getInput(), input.getBoolean("indicator", false),
     ...
     */
    public abstract Aggregate copy(RelTraitSet traitSet, RelNode input, boolean indicator, ImmutableBitSet groupSet,
    ...
    public RelWriter explainTerms(RelWriter pw) {
        ...
        super.explainTerms(pw).item("group", groupSet).itemIf("groups", groupSets,
                                                              getGroupType() != Group.SIMPLE).itemIf("indicator",
                                                                                                     indicator,
                                                                                                     indicator).itemIf(
                "aggs", aggCalls, pw.nest());
        if (!pw.nest()) {
    ...
    protected RelDataType deriveRowType() {
        return deriveRowType(getCluster().getTypeFactory(), getInput().getRowType(), indicator, groupSet, groupSets,
                             aggCalls);
    }
     ...
     */
    public static RelDataType deriveRowType(RelDataTypeFactory typeFactory, final RelDataType inputRowType,
                                            boolean indicator, ImmutableBitSet groupSet,
                                            List<ImmutableBitSet> groupSets, final List<AggregateCall> aggCalls) {
    ...
    public boolean isValid(Litmus litmus, Context context) {
        return super.isValid(litmus, context) && litmus.check(Util.isDistinct(getRowType().getFieldNames()),
    ...
    public static class AggCallBinding extends SqlOperatorBinding {
         ...
         */
        public AggCallBinding(RelDataTypeFactory typeFactory, SqlAggFunction aggFunction, List<RelDataType> operands,
                              int groupCount, boolean filter) {
            ...
            assert groupCount >= 0 :
                    "number of group by columns should be greater than zero in " + "aggregate call. Got " + groupCount;
        }
