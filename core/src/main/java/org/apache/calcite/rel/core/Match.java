...
public abstract class Match extends SingleRel {
    ...
    @Override public RelWriter explainTerms(RelWriter pw) {
        return super.explainTerms(pw).item("partition", getPartitionKeys()).item("order", getOrderKeys()).item(
                "outputFields", getRowType().getFieldNames()).item("allRows", isAllRows()).item("after",
                                                                                                getAfter()).item(
                "pattern", getPattern()).item("isStrictStarts", isStrictStart()).item("isStrictEnds",
                                                                                      isStrictEnd()).itemIf("interval",
                                                                                                            getInterval(),
                                                                                                            getInterval()
                                                                                                            != null).item(
                "subsets", getSubsets().values().asList()).item("patternDefinitions",
                                                                getPatternDefinitions().values().asList()).item(
                "inputFields", getInput().getRowType().getFieldNames());
    }
    ...
    private static class AggregateFinder extends RexVisitorImpl {
        ...
        @Override public Object visitCall(RexCall call) {
            ...
            if (aggFunction != null) {
                RexMRAggCall aggCall = new RexMRAggCall(aggFunction, call.getType(), call.getOperands(),
                                                        aggregateCalls.size());
