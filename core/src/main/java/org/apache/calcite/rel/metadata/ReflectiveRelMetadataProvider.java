...
public class ReflectiveRelMetadataProvider implements RelMetadataProvider, ReflectiveVisitor {
     ...
     */
    protected ReflectiveRelMetadataProvider(ConcurrentMap<Class<RelNode>, UnboundMetadata> map,
                                                             ...
                                                             new InvocationHandler() {
                                                                 ...
                                                                 public Object invoke(Object proxy, Method method,
                                                                     ...
                                                                     if (method.equals(
                                                                         ...
                                                                         return space.metadataClass0.getSimpleName()
                                                                                + "(" + rel + ")";
                                                                     }
                                                                     int i = methods.indexOf(method);
                                                                     if (i < 0) {
                                                                         throw new AssertionError(
                                                                                 "not handled: " + method + " for "
                                                                                 + rel);
                                                                     }
                                                                     ...
                                                                     if (args == null) {
                                                                         ...
                                                                         args1[0] = rel;
                                                                         args1[1] = mq;
                                                                         System.arraycopy(args, 0, args1, 2,
                                                                                          args.length);
                                                                     ...
                                                                     if (mq.map.put(key, NullSentinel.INSTANCE)
                                                                         != null) {
    ...
    private static boolean couldImplement(Method handlerMethod, Method method) {
        if (!handlerMethod.getName().equals(method.getName()) || (handlerMethod.getModifiers() & Modifier.STATIC) != 0
            || (handlerMethod.getModifiers() & Modifier.PUBLIC) == 0) {
            return false;
        }
        final Class<?>[] parameterTypes1 = handlerMethod.getParameterTypes();
        final Class<?>[] parameterTypes = method.getParameterTypes();
        return parameterTypes1.length == parameterTypes.length + 2 && RelNode.class.isAssignableFrom(parameterTypes1[0])
               && RelMetadataQuery.class == parameterTypes1[1] && Arrays.asList(parameterTypes).equals(
                Util.skip(Arrays.asList(parameterTypes1), 2));
    }
    ...
    @SuppressWarnings({ "unchecked", "SuspiciousMethodCalls" }) public <M extends Metadata> UnboundMetadata<M> apply(
    ...
    static class Space {
            ...
            for (Map.Entry<Method, MetadataHandler> entry : providerMap.entries()) {
                ...
                for (final Method handlerMethod : provider.getClass().getMethods()) {
                    if (couldImplement(handlerMethod, method)) {
                        @SuppressWarnings("unchecked") final Class<RelNode> relNodeClass = (Class<RelNode>) handlerMethod.getParameterTypes()[0];
         ...
         * {@code map}.
         */
        @SuppressWarnings({ "unchecked", "SuspiciousMethodCalls" }) Method find(
                final Class<? extends RelNode> relNodeClass, Method method) {
            ...
            for (Class r = relNodeClass; ; ) {
                ...
                if (r == null || !RelNode.class.isAssignableFrom(r)) {
                    throw new IllegalArgumentException(
                            "No handler for method [" + method + "] applied to argument of type [" + relNodeClass
