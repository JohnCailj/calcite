...
public class RelMdAllPredicates implements MetadataHandler<BuiltInMetadata.AllPredicates> {
    ...
    public static final RelMetadataProvider SOURCE = ReflectiveRelMetadataProvider.reflectiveSource(
            BuiltInMethod.ALL_PREDICATES.method, new RelMdAllPredicates());
    ...
    public RelOptPredicateList getAllPredicates(Filter filter, RelMetadataQuery mq) {
        ...
        return predsBelow.union(rexBuilder, RelOptPredicateList.of(rexBuilder,
                                                                   RelMdExpressionLineage.createAllPossibleExpressions(
    ...
    public RelOptPredicateList getAllPredicates(Join join, RelMetadataQuery mq) {
        ...
        for (RelNode input : join.getInputs()) {
            ...
            if (input == join.getLeft()) {
                ...
                final List<RexNode> updatedPreds = Lists.newArrayList(
                        Iterables.transform(inputPreds.pulledUpPredicates, new Function<RexNode, RexNode>() {
        ...
        // Replace with new expressions and return union of predicates
        return newPreds.union(rexBuilder, RelOptPredicateList.of(rexBuilder,
                                                                 RelMdExpressionLineage.createAllPossibleExpressions(
                                                                         rexBuilder, pred, mapping)));
    }
