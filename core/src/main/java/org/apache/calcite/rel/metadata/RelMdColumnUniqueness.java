...
public class RelMdColumnUniqueness implements MetadataHandler<BuiltInMetadata.ColumnUniqueness> {
     ...
     */
    private static final Predicate<RelNode> SAFE_REL = new PredicateImpl<RelNode>() {
    ...
    public Boolean areColumnsUnique(Correlate rel, RelMetadataQuery mq, ImmutableBitSet columns, boolean ignoreNulls) {
        switch (rel.getJoinType()) {
            ...
            case LEFT:
            case INNER:
                final Pair<ImmutableBitSet, ImmutableBitSet> leftAndRightColumns = splitLeftAndRightColumns(
                        rel.getLeft().getRowType().getFieldCount(), columns);
            ...
            default:
                throw new IllegalStateException(
                        "Unknown join type " + rel.getJoinType() + " for correlate relation " + rel);
        }
    }
    ...
    public Boolean areColumnsUnique(HepRelVertex rel, RelMetadataQuery mq, ImmutableBitSet columns,
                                    boolean ignoreNulls) {
    ...
    public Boolean areColumnsUnique(RelSubset rel, RelMetadataQuery mq, ImmutableBitSet columns, boolean ignoreNulls) {
        ...
        for (RelNode rel2 : rel.getRels()) {
            if (rel2 instanceof Aggregate || rel2 instanceof Filter || rel2 instanceof Values
                || rel2 instanceof TableScan || simplyProjects(rel2, columns)) {
                try {
     ...
     */
    private static Pair<ImmutableBitSet, ImmutableBitSet> splitLeftAndRightColumns(int leftCount,
