...
public class RelMdExpressionLineage implements MetadataHandler<BuiltInMetadata.ExpressionLineage> {
        ...
        for (int idx = 0; idx < rel.getRowType().getFieldList().size(); idx++) {
            if (idx < nLeftColumns) {
                ...
                final Set<RexNode> updatedExprs = Sets.newHashSet(
                        Iterables.transform(originalExprs, new Function<RexNode, RexNode>() {
     ...
     */
    protected static Set<RexNode> createAllPossibleExpressions(RexBuilder rexBuilder, RexNode expr,
    ...
    private static Set<RexNode> createAllPossibleExpressions(RexBuilder rexBuilder, RexNode expr,
    ...
    private static void createExpressions(RexBuilder rexBuilder, RexNode expr, ImmutableBitSet predFieldsUsed,
                                          Map<RexInputRef, Set<RexNode>> mapping,
