...
public class RelMdPredicates implements MetadataHandler<BuiltInMetadata.Predicates> {
    ...
    public RelOptPredicateList getPredicates(Project project, RelMetadataQuery mq) {
        ...
        Mapping m = Mappings.create(MappingType.PARTIAL_FUNCTION, input.getRowType().getFieldCount(),
        ...
        for (Ord<RexNode> expr : Ord.zip(project.getProjects())) {
            if (RexLiteral.isNullLiteral(expr.e)) {
                projectPullUpPredicates.add(
                        rexBuilder.makeCall(SqlStdOperatorTable.IS_NULL, rexBuilder.makeInputRef(project, expr.i)));
            } else if (RexUtil.isConstant(expr.e)) {
                final List<RexNode> args = ImmutableList.of(rexBuilder.makeInputRef(project, expr.i), expr.e);
                final SqlOperator op = args.get(0).getType().isNullable() || args.get(
                        1).getType().isNullable() ? SqlStdOperatorTable.IS_NOT_DISTINCT_FROM : SqlStdOperatorTable.EQUALS;
     ...
     */
    private RexNode projectPredicate(final RexBuilder rexBuilder, RelNode input, RexNode r,
        ...
        if (columnsMapped.intersects(rCols)) {
            ...
            for (int c : columnsMapped.intersect(rCols)) {
                if (input.getRowType().getFieldList().get(c).getType().isNullable() && Strong.isNull(r,
                                                                                                     ImmutableBitSet.of(
                                                                                                             c))) {
    ...
    public RelOptPredicateList getPredicates(Filter filter, RelMetadataQuery mq) {
        ...
        return Util.first(inputInfo, RelOptPredicateList.EMPTY).union(rexBuilder, RelOptPredicateList.of(rexBuilder,
    ...
    public RelOptPredicateList getPredicates(SemiJoin semiJoin, RelMetadataQuery mq) {
        ...
        JoinConditionBasedPredicateInference jI = new JoinConditionBasedPredicateInference(semiJoin,
                                                                                           RexUtil.composeConjunction(
                                                                                                   rB,
                                                                                                   leftInfo.pulledUpPredicates,
                                                                                                   false),
                                                                                           RexUtil.composeConjunction(
                                                                                                   rB,
                                                                                                   rightInfo.pulledUpPredicates,
                                                                                                   false));
    ...
    static class JoinConditionBasedPredicateInference {
        ...
        private JoinConditionBasedPredicateInference(Join joinRel, boolean isSemiJoin, RexNode lPreds, RexNode rPreds) {
            ...
            if (lPreds == null) {
                ...
                Mappings.TargetMapping leftMapping = Mappings.createShiftMapping(nSysFields + nFieldsLeft, nSysFields,
                                                                                 0, nFieldsLeft);
            ...
            List<RexNode> exprs = RelOptUtil.conjunctions(
        ...
        public RelOptPredicateList inferPredicates(boolean includeEqualityInference) {
            ...
            switch (joinType) {
                case INNER:
                case LEFT:
                    infer(leftChildPredicates, allExprsDigests, inferredPredicates, includeEqualityInference,
            ...
            switch (joinType) {
                case INNER:
                case RIGHT:
                    infer(rightChildPredicates, allExprsDigests, inferredPredicates, includeEqualityInference,
                          joinType == JoinRelType.RIGHT ? leftFieldsBitSet : allFieldsBitSet);
                    break;
            }

            Mappings.TargetMapping rightMapping = Mappings.createShiftMapping(nSysFields + nFieldsLeft + nFieldsRight,
                                                                              0, nSysFields + nFieldsLeft,
                                                                              nFieldsRight);
            final RexPermuteInputsShuttle rightPermute = new RexPermuteInputsShuttle(rightMapping, joinRel);
            Mappings.TargetMapping leftMapping = Mappings.createShiftMapping(nSysFields + nFieldsLeft, 0, nSysFields,
                                                                             nFieldsLeft);
            ...
            switch (joinType) {
                case INNER:
                    ...
                    if (isSemiJoin) {
                        pulledUpPredicates = Iterables.concat(RelOptUtil.conjunctions(leftChildPredicates),
                    ...
                    return RelOptPredicateList.of(rexBuilder, pulledUpPredicates, leftInferredPredicates,
                                                  rightInferredPredicates);
                case LEFT:
                    return RelOptPredicateList.of(rexBuilder, RelOptUtil.conjunctions(leftChildPredicates),
                                                  leftInferredPredicates, rightInferredPredicates);
                case RIGHT:
                    return RelOptPredicateList.of(rexBuilder, RelOptUtil.conjunctions(rightChildPredicates),
        ...
        private void infer(RexNode predicates, Set<String> allExprsDigests, List<RexNode> inferedPredicates,
            ...
            for (RexNode r : RelOptUtil.conjunctions(predicates)) {
                ...
                for (Mapping m : mappings(r)) {
                    ...
                    if (inferringFields.contains(RelOptUtil.InputFinder.bits(tr)) && !allExprsDigests.contains(
                            tr.toString()) && !isAlwaysTrue(tr)) {
        ...
        class ExprsItr implements Iterator<Mapping> {
            ...
            private void initializeMapping() {
                nextMapping = Mappings.create(MappingType.PARTIAL_FUNCTION, nSysFields + nFieldsLeft + nFieldsRight,
                                              nSysFields + nFieldsLeft + nFieldsRight);
