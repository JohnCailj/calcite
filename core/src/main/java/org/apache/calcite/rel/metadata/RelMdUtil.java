...
public class RelMdUtil {
    ...
    public static final SqlFunction ARTIFICIAL_SELECTIVITY_FUNC = new SqlFunction("ARTIFICIAL_SELECTIVITY",
                                                                                  SqlKind.OTHER_FUNCTION,
                                                                                  ReturnTypes.BOOLEAN,
                                                                                  // returns boolean since we'll AND it
                                                                                  null, OperandTypes.NUMERIC,
                                                                                  // takes a numeric param
    ...
    public static double computeSemiJoinSelectivity(RelMetadataQuery mq, SemiJoin rel) {
        return computeSemiJoinSelectivity(mq, rel.getLeft(), rel.getRight(), rel.getLeftKeys(), rel.getRightKeys());
    }
     ...
     * @return calculated selectivity
     */
    public static double computeSemiJoinSelectivity(RelMetadataQuery mq, RelNode factRel, RelNode dimRel,
                                                    SemiJoin rel) {
        return computeSemiJoinSelectivity(mq, factRel, dimRel, rel.getLeftKeys(), rel.getRightKeys());
    }
     ...
     */
    public static double computeSemiJoinSelectivity(RelMetadataQuery mq, RelNode factRel, RelNode dimRel,
                                                    List<Integer> factKeyList, List<Integer> dimKeyList) {
     ...
     * if no metadata is available)
     */
    public static boolean areColumnsDefinitelyUniqueWhenNullsFiltered(RelMetadataQuery mq, RelNode rel,
    ...
    public static Boolean areColumnsUniqueWhenNullsFiltered(RelMetadataQuery mq, RelNode rel,
     ...
     */
    public static void setLeftRightBitmaps(ImmutableBitSet groupKey, ImmutableBitSet.Builder leftMask,
                                           ImmutableBitSet.Builder rightMask, int nFieldsOnLeft) {
    ...
    public static double guessSelectivity(RexNode predicate, boolean artificialOnly) {
        ...
        for (RexNode pred : RelOptUtil.conjunctions(predicate)) {
            if (pred.getKind() == SqlKind.IS_NOT_NULL) {
                sel *= .9;
            } else if ((pred instanceof RexCall) && (((RexCall) pred).getOperator()
     ...
     */
    public static void splitCols(List<RexNode> projExprs, ImmutableBitSet groupKey, ImmutableBitSet.Builder baseCols,
    ...
    public static Double getJoinPopulationSize(RelMetadataQuery mq, RelNode joinRel, ImmutableBitSet groupKey) {
        ...
        Double population = NumberUtil.multiply(mq.getPopulationSize(left, leftMask.build()),
     ...
     */
    public static Double getJoinDistinctRowCount(RelMetadataQuery mq, RelNode joinRel, JoinRelType joinType,
                                                 ImmutableBitSet groupKey, RexNode predicate, boolean useMaxNdv) {
        Double distRowCount;
        ImmutableBitSet.Builder leftMask = ImmutableBitSet.builder();
        ImmutableBitSet.Builder rightMask = ImmutableBitSet.builder();
        RelNode left = joinRel.getInputs().get(0);
        ...
        if (useMaxNdv) {
            distRowCount = Math.max(mq.getDistinctRowCount(left, leftMask.build(), leftPred),
                                    mq.getDistinctRowCount(right, rightMask.build(), rightPred));
        } else {
            distRowCount = NumberUtil.multiply(mq.getDistinctRowCount(left, leftMask.build(), leftPred),
                                               mq.getDistinctRowCount(right, rightMask.build(), rightPred));
        }
     ...
     * {@link SemiJoin}.
     */
    public static Double getSemiJoinRowCount(RelMetadataQuery mq, RelNode left, RelNode right, JoinRelType joinType,
                                             RexNode condition) {
        // TODO:  correlation factor
    ...
    private static class CardOfProjExpr extends RexVisitorImpl<Double> {
        ...
        public Double visitInputRef(RexInputRef var) {
            ...
            Double distinctRowCount = mq.getDistinctRowCount(rel.getInput(), col, null);
        ...
        public Double visitCall(RexCall call) {
            ...
            if (call.isA(SqlKind.MINUS_PREFIX)) {
                ...
                distinctRowCount = NumberUtil.multiply(cardOfProjExpr(mq, rel, call.getOperands().get(0)),
     ...
     */
    public static boolean checkInputForCollationAndLimit(RelMetadataQuery mq, RelNode input, RelCollation collation,
