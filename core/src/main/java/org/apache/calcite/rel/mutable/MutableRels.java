...
public abstract class MutableRels {
    ...
    public static MutableRel createProject(final MutableRel child, final List<Integer> posList) {
        ...
        return MutableProject.of(
                RelOptUtil.permute(child.cluster.getTypeFactory(), rowType, Mappings.bijection(posList)), child,
    ...
    public static MutableRel createCastRel(MutableRel rel, RelDataType castRowType, boolean rename) {
        ...
        final List<String> fieldNames = rename ? castRowType.getFieldNames() : rowType.getFieldNames();
    ...
    public static RelNode fromMutable(MutableRel node, RelBuilder relBuilder) {
        switch (node.type) {
            ...
            case WINDOW: {
                ...
                return LogicalWindow.create(child.getTraitSet(), child, window.constants, window.rowType,
                                            window.groups);
            }
            case TABLE_MODIFY:
                ...
                return LogicalTableModify.create(modify.table, modify.catalogReader,
                                                 fromMutable(modify.getInput(), relBuilder), modify.operation,
                                                 modify.updateColumnList, modify.sourceExpressionList,
                                                 modify.flattened);
            case SAMPLE:
            ...
            case TABLE_FUNCTION_SCAN:
                ...
                return LogicalTableFunctionScan.create(tableFunctionScan.cluster,
                                                       ...
                                                       tableFunctionScan.rexCall, tableFunctionScan.elementType,
                                                       tableFunctionScan.rowType, tableFunctionScan.columnMappings);
            case JOIN:
    ...
    public static MutableRel toMutable(RelNode rel) {
        ...
        if (rel instanceof Aggregate) {
            ...
            return MutableAggregate.of(input, aggregate.getGroupSet(), aggregate.getGroupSets(),
        ...
        if (rel instanceof TableModify) {
            ...
            return MutableTableModify.of(modify.getRowType(), input, modify.getTable(), modify.getCatalogReader(),
        ...
        if (rel instanceof TableFunctionScan) {
            ...
            return MutableTableFunctionScan.of(tableFunctionScan.getCluster(), tableFunctionScan.getRowType(), inputs,
                                               tableFunctionScan.getCall(), tableFunctionScan.getElementType(),
        ...
        if (rel instanceof SemiJoin) {
            ...
            return MutableSemiJoin.of(semiJoin.getRowType(), left, right, semiJoin.getCondition(),
        ...
        if (rel instanceof Join) {
            ...
            return MutableJoin.of(join.getRowType(), left, right, join.getCondition(), join.getJoinType(),
        ...
        if (rel instanceof Correlate) {
            ...
            return MutableCorrelate.of(correlate.getRowType(), left, right, correlate.getCorrelationId(),
