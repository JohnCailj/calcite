...
public class RelToSqlConverter extends SqlImplementor implements ReflectiveVisitor {
    ...
    public Result visit(Aggregate e) {
        ...
        for (AggregateCall aggCall : e.getAggCallList()) {
            ...
            if (aggCall.getAggregation() instanceof SqlSingleValueAggFunction) {
                aggCallSqlNode = rewriteSingleValueExpr(aggCallSqlNode, dialect);
            }
    ...
    public Result visit(Union e) {
        return setOpToSql(e.all ? SqlStdOperatorTable.UNION_ALL : SqlStdOperatorTable.UNION, e);
    }
    ...
    public Result visit(Intersect e) {
        return setOpToSql(e.all ? SqlStdOperatorTable.INTERSECT_ALL : SqlStdOperatorTable.INTERSECT, e);
    }
    ...
    public Result visit(Minus e) {
        return setOpToSql(e.all ? SqlStdOperatorTable.EXCEPT_ALL : SqlStdOperatorTable.EXCEPT, e);
    }
        ...
        switch (modify.getOperation()) {
            ...
            case UPDATE: {
                ...
                final SqlUpdate sqlUpdate = new SqlUpdate(POS, sqlTargetTable,
                                                          ...
                                                          ((SqlSelect) input.node).getWhere(), input.asSelect(), null);
            ...
            case DELETE: {
                ...
                final SqlDelete sqlDelete = new SqlDelete(POS, sqlTargetTable, input.asSelect().getWhere(),
                                                          input.asSelect(), null);
    ...
    private SqlNodeList exprList(final Context context, List<? extends RexNode> exprs) {
        return new SqlNodeList(Lists.transform(exprs, new Function<RexNode, SqlNode>() {
    ...
    private SqlNodeList identifierList(List<String> names) {
        return new SqlNodeList(Lists.transform(names, new Function<String, SqlNode>() {
    ...
    public Result visit(Match e) {
        ...
        if (e.getOrderKeys() != null) {
            for (RelFieldCollation fc : e.getOrderKeys().getFieldCollations()) {
                if (fc.nullDirection != RelFieldCollation.NullDirection.UNSPECIFIED
                    ...
                    orderBySqlList.add(ISNULL_FUNCTION.createCall(POS, context.field(fc.getFieldIndex())));
        ...
        final SqlLiteral rowsPerMatch = e.isAllRows() ? SqlMatchRecognize.RowsPerMatchOption.ALL_ROWS.symbol(
                POS) : SqlMatchRecognize.RowsPerMatchOption.ONE_ROW.symbol(POS);
        ...
        final SqlNode matchRecognize = new SqlMatchRecognize(POS, tableRef, pattern, strictStart, strictEnd,
                                                             patternDefList, measureList, after, subsetList,
                                                             rowsPerMatch, partitionList, orderByList, interval);
    ...
    @Override public void addSelect(List<SqlNode> selectList, SqlNode node, RelDataType rowType) {
