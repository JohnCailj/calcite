...
public abstract class SqlImplementor {
    ...
    public static SqlNode rewriteSingleValueExpr(SqlNode aggCall, SqlDialect sqlDialect) {
        ...
        final SqlNode countCall = SqlStdOperatorTable.COUNT.createCall(POS, operand);
        ...
        switch (sqlDialect.getDatabaseProduct()) {
            ...
            case HSQLDB:
                ...
                switch (sqlDialect.getDatabaseProduct()) {
                    case MYSQL:
                        ...
                        unionOperand = new SqlSelect(POS, SqlNodeList.EMPTY, selectList, null, null, null, null,
            ...
            default:
                LOGGER.debug("SINGLE_VALUE rewrite not supported for {}", sqlDialect.getDatabaseProduct());
                return aggCall;
        }
        ...
        SqlNode caseExpr = new SqlCase(POS, caseOperand, whenList, thenList, elseExpr);
    ...
    public static SqlNode convertConditionToSqlNode(RexNode node, Context leftContext, Context rightContext,
        ...
        switch (node.getKind()) {
            ...
            case LESS_THAN_OR_EQUAL:
                ...
                operands = ((RexCall) node).getOperands();
                op = ((RexCall) node).getOperator();
                if (operands.size() == 2 && operands.get(0) instanceof RexInputRef && operands.get(
    ...
    public abstract class Context {
            ...
            switch (rex.getKind()) {
                ...
                case NOT:
                    ...
                    switch (operand.getKind()) {
                        ...
                        case SIMILAR:
                            return SqlStdOperatorTable.NOT_SIMILAR_TO.createCall(POS,
        ...
        private SqlCall toSql(RexProgram program, RexOver rexOver) {
            ...
            final SqlNode lowerBound = createSqlWindowBound(rexWindow.getLowerBound());
            final SqlNode upperBound = createSqlWindowBound(rexWindow.getUpperBound());
            ...
            final SqlCall aggFunctionCall = rexOver.getAggOperator().createCall(POS, nodeList);

            return SqlStdOperatorTable.OVER.createCall(POS, aggFunctionCall, sqlWindow);
        }
                ...
                case MYSQL:
                    switch (type.getSqlTypeName()) {
                        case VARCHAR:
                            ...
                            return new SqlDataTypeSpec(new SqlIdentifier("CHAR", POS), type.getPrecision(), -1, null,
                                                       null, POS);
                        case INTEGER:
                            return new SqlDataTypeSpec(new SqlIdentifier("_UNSIGNED", POS), type.getPrecision(), -1,
                                                       null, null, POS);
                    }
                    break;
            }
            if (type instanceof BasicSqlType) {
                return new SqlDataTypeSpec(new SqlIdentifier(type.getSqlTypeName().name(), POS), type.getPrecision(),
                                           type.getScale(), type.getCharset() != null
                                                            && dialect.supportsCharSet() ? type.getCharset().name() : null,
                                           null, POS);
            }
        ...
        public SqlNode toSql(AggregateCall aggCall) {
            ...
            return op.createCall(aggCall.isDistinct() ? SqlSelectKeyword.DISTINCT.symbol(POS) : null, POS,
            ...
            for (Map.Entry<String, RelDataType> alias : aliases.entrySet()) {
                ...
                if (ordinal < fields.size()) {
                    ...
                    return new SqlIdentifier(
                            !qualified ? ImmutableList.of(field.getName()) : ImmutableList.of(alias.getKey(),
                                                                                              field.getName()), POS);
                }
                ordinal -= fields.size();
            }
    ...
    public class Builder {
        ...
        public void addOrderItem(List<SqlNode> orderByList, RelFieldCollation field) {
            if (field.nullDirection != RelFieldCollation.NullDirection.UNSPECIFIED
                ...
                orderByList.add(ISNULL_FUNCTION.createCall(POS, context.field(field.getFieldIndex())));
