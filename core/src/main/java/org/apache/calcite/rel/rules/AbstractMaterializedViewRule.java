...
public abstract class AbstractMaterializedViewRule extends RelOptRule {
    ...
    private static final CalciteLogger LOGGER = new CalciteLogger(
            LoggerFactory.getLogger(AbstractMaterializedViewRule.class));

    public static final MaterializedViewProjectFilterRule INSTANCE_PROJECT_FILTER = new MaterializedViewProjectFilterRule(
            RelFactories.LOGICAL_BUILDER, true);

    public static final MaterializedViewOnlyFilterRule INSTANCE_FILTER = new MaterializedViewOnlyFilterRule(
            RelFactories.LOGICAL_BUILDER, true);

    public static final MaterializedViewProjectJoinRule INSTANCE_PROJECT_JOIN = new MaterializedViewProjectJoinRule(
            RelFactories.LOGICAL_BUILDER, true);

    public static final MaterializedViewOnlyJoinRule INSTANCE_JOIN = new MaterializedViewOnlyJoinRule(
            RelFactories.LOGICAL_BUILDER, true);

    public static final MaterializedViewProjectAggregateRule INSTANCE_PROJECT_AGGREGATE = new MaterializedViewProjectAggregateRule(
            RelFactories.LOGICAL_BUILDER, true);

    public static final MaterializedViewOnlyAggregateRule INSTANCE_AGGREGATE = new MaterializedViewOnlyAggregateRule(
            RelFactories.LOGICAL_BUILDER, true);
    ...
    protected AbstractMaterializedViewRule(RelOptRuleOperand operand, RelBuilderFactory relBuilderFactory,
                                           String description, boolean generateUnionRewriting) {
    ...
    protected void perform(RelOptRuleCall call, Project topProject, RelNode node) {
        ...
        final RexSimplify simplify = new RexSimplify(rexBuilder, true, planner.getExecutor()
                                                                       != null ? planner.getExecutor() : RexUtil.EXECUTOR);
        ...
        if (!materializations.isEmpty()) {
            ...
            final List<RelOptMaterialization> applicableMaterializations = RelOptMaterializations.getApplicableMaterializations(
                ...
                for (RelOptMaterialization materialization : applicableMaterializations) {
                    ...
                    if (!queryTableRefs.equals(viewTableRefs)) {
                        ...
                        if (viewTableRefs.containsAll(queryTableRefs)) {
                            ...
                            ViewPartialRewriting partialRewritingResult = compensateViewPartial(call.builder(),
                                                                                                rexBuilder, mq, view,
                                                                                                topProject, node,
                                                                                                queryTableRefs, qEC,
                                                                                                topViewProject,
                    ...
                    for (BiMap<RelTableRef, RelTableRef> queryToViewTableMapping : flatListMappings) {
                        ...
                        final RexNode viewColumnsEquiPred = RexUtil.swapTableReferences(rexBuilder, viewPreds.getLeft(),
                        ...
                        Triple<RexNode, RexNode, RexNode> compensationPreds = computeCompensationPredicates(rexBuilder,
                                                                                                            simplify,
                                                                                                            currQEC,
                                                                                                            queryPreds,
                                                                                                            queryBasedVEC,
                        ...
                        if (compensationPreds == null && generateUnionRewriting) {
                            ...
                            compensationPreds = computeCompensationPredicates(rexBuilder, simplify, queryBasedVEC,
                                                                              viewPreds, currQEC, queryPreds,
                                                                              queryToViewTableMapping.inverse());
                            ...
                            RexNode otherCompensationPred = RexUtil.composeConjunction(rexBuilder, ImmutableList.of(
                                    compensationPreds.getMiddle(), compensationPreds.getRight()), false);
                            assert !compensationColumnsEquiPred.isAlwaysTrue() || !otherCompensationPred.isAlwaysTrue();

                            // b. Generate union branch (query).
                            final RelNode unionInputQuery = rewriteQuery(call.builder(), rexBuilder, simplify, mq,
                                                                         compensationColumnsEquiPred,
                                                                         otherCompensationPred, topProject, node,
                                                                         queryToViewTableMapping, queryBasedVEC,
                                                                         currQEC);
                            ...
                            final RelNode unionInputView = rewriteView(call.builder(), rexBuilder, mq, matchModality,
                                                                       true, view, topProject, node, topViewProject,
                            ...
                            final RelNode result = createUnion(call.builder(), rexBuilder, topProject, unionInputQuery,
                                                               unionInputView);
                            if (result == null) {
                                // Skip it
                                continue;
                            ...
                            if (!compensationColumnsEquiPred.isAlwaysTrue() || !otherCompensationPred.isAlwaysTrue()) {
                                ...
                                List<RexNode> viewExprs = topViewProject == null ? extractReferences(rexBuilder,
                                                                                                     view) : topViewProject.getChildExps();
                                // For compensationColumnsEquiPred, we use the view equivalence classes,
                                // since we want to enforce the rest
                                if (!compensationColumnsEquiPred.isAlwaysTrue()) {
                                    compensationColumnsEquiPred = rewriteExpression(rexBuilder, mq, viewNode, viewExprs,
                                                                                    queryToViewTableMapping.inverse(),
                                                                                    queryBasedVEC, false,
                                                                                    compensationColumnsEquiPred);
                                ...
                                if (!otherCompensationPred.isAlwaysTrue()) {
                                    otherCompensationPred = rewriteExpression(rexBuilder, mq, viewNode, viewExprs,
                                                                              queryToViewTableMapping.inverse(),
                                                                              currQEC, true, otherCompensationPred);
                            ...
                            final RexNode viewCompensationPred = RexUtil.composeConjunction(rexBuilder,
                                                                                            ImmutableList.of(
                                                                                                    compensationColumnsEquiPred,
                            ...
                            final RelNode result = rewriteView(builder, rexBuilder, mq, matchModality, false,
                                                               builder.build(), topProject, node, topViewProject,
                                                               viewNode, queryToViewTableMapping, currQEC);
                            if (result == null) {
                                // Skip it
                                continue;
     ...
     */
    protected abstract ViewPartialRewriting compensateViewPartial(RelBuilder relBuilder, RexBuilder rexBuilder,
                                                                  RelMetadataQuery mq, RelNode input,
                                                                  Project topProject, RelNode node,
                                                                  Set<RelTableRef> queryTableRefs,
                                                                  EquivalenceClasses queryEC, Project topViewProject,
                                                                  RelNode viewNode, Set<RelTableRef> viewTableRefs);
     ...
     */
    protected abstract RelNode rewriteQuery(RelBuilder relBuilder, RexBuilder rexBuilder, RexSimplify simplify,
                                            RelMetadataQuery mq, RexNode compensationColumnsEquiPred,
                                            RexNode otherCompensationPred, Project topProject, RelNode node,
     ...
     * operator.
     */
    protected abstract RelNode createUnion(RelBuilder relBuilder, RexBuilder rexBuilder, RelNode topProject,
     ...
     * be produced, we will return null.
     */
    protected abstract RelNode rewriteView(RelBuilder relBuilder, RexBuilder rexBuilder, RelMetadataQuery mq,
                                           MatchModality matchModality, boolean unionRewriting, RelNode input,
                                           Project topProject, RelNode node, Project topViewProject, RelNode viewNode,
     ...
     */
    private abstract static class MaterializedViewJoinRule extends AbstractMaterializedViewRule {
        ...
        @Override protected boolean isValidPlan(Project topProject, RelNode node, RelMetadataQuery mq) {
            return isValidRelNodePlan(node, mq);
        }

        @Override protected ViewPartialRewriting compensateViewPartial(RelBuilder relBuilder, RexBuilder rexBuilder,
                                                                       RelMetadataQuery mq, RelNode input,
                                                                       Project topProject, RelNode node,
                                                                       Set<RelTableRef> queryTableRefs,
                                                                       EquivalenceClasses queryEC,
        ...
        @Override protected RelNode rewriteQuery(RelBuilder relBuilder, RexBuilder rexBuilder, RexSimplify simplify,
                                                 RelMetadataQuery mq, RexNode compensationColumnsEquiPred,
                                                 RexNode otherCompensationPred, Project topProject, RelNode node,
                                                 BiMap<RelTableRef, RelTableRef> viewToQueryTableMapping,
                                                 EquivalenceClasses viewEC, EquivalenceClasses queryEC) {
            ...
            if (!compensationColumnsEquiPred.isAlwaysTrue()) {
                compensationColumnsEquiPred = rewriteExpression(rexBuilder, mq, node, queryExprs,
            ...
            // For the rest, we use the query equivalence classes
            if (!otherCompensationPred.isAlwaysTrue()) {
                otherCompensationPred = rewriteExpression(rexBuilder, mq, node, queryExprs,
            ...
            final RexNode queryCompensationPred = RexUtil.not(RexUtil.composeConjunction(rexBuilder, ImmutableList.of(
                    compensationColumnsEquiPred, otherCompensationPred), false));
        ...
        @Override protected RelNode createUnion(RelBuilder relBuilder, RexBuilder rexBuilder, RelNode topProject,
                                                RelNode unionInputQuery, RelNode unionInputView) {
        ...
        @Override protected RelNode rewriteView(RelBuilder relBuilder, RexBuilder rexBuilder, RelMetadataQuery mq,
                                                MatchModality matchModality, boolean unionRewriting, RelNode input,
                                                Project topProject, RelNode node, Project topViewProject,
                                                RelNode viewNode,
                                                BiMap<RelTableRef, RelTableRef> queryToViewTableMapping,
                                                EquivalenceClasses queryEC) {
            List<RexNode> exprs = topProject == null ? extractReferences(rexBuilder, node) : topProject.getChildExps();
            ...
            List<RexNode> viewExprs =
                    topViewProject == null ? extractReferences(rexBuilder, viewNode) : topViewProject.getChildExps();
            List<RexNode> rewrittenExprs = rewriteExpressions(rexBuilder, mq, viewNode, viewExprs,
                                                              queryToViewTableMapping.inverse(), queryEC, true,
                                                              exprsLineage);
    ...
    public static class MaterializedViewProjectJoinRule extends MaterializedViewJoinRule {
        ...
        public MaterializedViewProjectJoinRule(RelBuilderFactory relBuilderFactory, boolean generateUnionRewriting) {
            super(operand(Project.class, operand(Join.class, any())), relBuilderFactory,
                  "MaterializedViewJoinRule(Project-Join)", generateUnionRewriting);
        }
    ...
    public static class MaterializedViewProjectFilterRule extends MaterializedViewJoinRule {
        ...
        public MaterializedViewProjectFilterRule(RelBuilderFactory relBuilderFactory, boolean generateUnionRewriting) {
            super(operand(Project.class, operand(Filter.class, any())), relBuilderFactory,
                  "MaterializedViewJoinRule(Project-Filter)", generateUnionRewriting);
        }
    ...
    public static class MaterializedViewOnlyJoinRule extends MaterializedViewJoinRule {
        ...
        public MaterializedViewOnlyJoinRule(RelBuilderFactory relBuilderFactory, boolean generateUnionRewriting) {
            super(operand(Join.class, any()), relBuilderFactory, "MaterializedViewJoinRule(Join)",
    ...
    public static class MaterializedViewOnlyFilterRule extends MaterializedViewJoinRule {
        ...
        public MaterializedViewOnlyFilterRule(RelBuilderFactory relBuilderFactory, boolean generateUnionRewriting) {
            super(operand(Filter.class, any()), relBuilderFactory, "MaterializedViewJoinRule(Filter)",
                  generateUnionRewriting);
        }
     ...
     */
    private abstract static class MaterializedViewAggregateRule extends AbstractMaterializedViewRule {
        ...
        @Override protected ViewPartialRewriting compensateViewPartial(RelBuilder relBuilder, RexBuilder rexBuilder,
            ...
            groupSet.addAll(ImmutableBitSet.range(aggregateViewNode.getInput().getRowType().getFieldCount(),
                                                  aggregateViewNode.getInput().getRowType().getFieldCount() + offset));
            final RelNode newViewNode = aggregateViewNode.copy(aggregateViewNode.getTraitSet(), relBuilder.build(),
                                                               aggregateViewNode.indicator, groupSet.build(), null,
        ...
        @Override protected RelNode rewriteQuery(RelBuilder relBuilder, RexBuilder rexBuilder, RexSimplify simplify,
            ...
            if (!compensationColumnsEquiPred.isAlwaysTrue()) {
                compensationColumnsEquiPred = rewriteExpression(rexBuilder, mq, aggregateInput, queryExprs,
            ...
            // For the rest, we use the query equivalence classes
            if (!otherCompensationPred.isAlwaysTrue()) {
                otherCompensationPred = rewriteExpression(rexBuilder, mq, aggregateInput, queryExprs,
        ...
        @Override protected RelNode createUnion(RelBuilder relBuilder, RexBuilder rexBuilder, RelNode topProject,
            ...
            for (int i = 0; i < aggregate.getAggCallList().size(); i++) {
                ...
                aggregateCalls.add(relBuilder.aggregateCall(SubstitutionVisitor.getRollup(aggCall.getAggregation()),
                                                            aggCall.isDistinct(), null, aggCall.name, ImmutableList.of(
                                rexBuilder.makeInputRef(relBuilder.peek(), aggregate.getGroupCount() + i))));
            }
            RelNode result = relBuilder.aggregate(relBuilder.groupKey(groupSet, null), aggregateCalls).build();
            if (topProject != null) {
                // Top project
        ...
        @Override protected RelNode rewriteView(RelBuilder relBuilder, RexBuilder rexBuilder, RelMetadataQuery mq,
            ...
            if (topProject != null && !unionRewriting) {
                ...
                final RelOptUtil.InputFinder inputFinder = new RelOptUtil.InputFinder(
                        new LinkedHashSet<RelDataTypeField>());
            ...
            Multimap<Integer, Integer> m = generateMapping(rexBuilder, mq, queryAggregate.getInput(),
                                                           viewAggregate.getInput(), indexes.build(),
                                                           queryToViewTableMapping, queryEC);
            if (m == null) {
                // Bail out
                return null;
            }
            ...
            for (int idx = 0; idx < queryAggregate.getAggCallList().size(); idx++) {
                ...
                for (int j = 0; j < viewAggregate.getAggCallList().size(); j++) {
                    ...
                    if (queryAggCall.getAggregation() != viewAggCall.getAggregation()
                        ...
                        || queryAggCall.getType() != viewAggCall.getType() || viewAggCall.filterArg >= 0) {
                        // Continue
                        continue;
                    }
            ...
            if (queryAggregate.getGroupCount() != viewAggregate.getGroupCount()
                ...
                rewritingMapping = Mappings.create(MappingType.FUNCTION, topViewProject
                                                                         != null ? topViewProject.getRowType().getFieldCount() : viewAggregate.getRowType().getFieldCount(),
                                                   queryAggregate.getRowType().getFieldCount());
                ...
                for (int i = 0; i < queryAggregate.getAggCallList().size(); i++) {
                    ...
                    if (topViewProject != null) {
                        ...
                        for (int k = 0; k < topViewProject.getChildExps().size(); k++) {
                            ...
                            int ref = ((RexInputRef) n).getIndex();
                            if (ref == targetIdx) {
                                aggregateCalls.add(relBuilder.aggregateCall(
                                        SubstitutionVisitor.getRollup(queryAggCall.getAggregation()),
                                        queryAggCall.isDistinct(), null, queryAggCall.name,
                        ...
                        aggregateCalls.add(
                                relBuilder.aggregateCall(SubstitutionVisitor.getRollup(queryAggCall.getAggregation()),
                                                         queryAggCall.isDistinct(), null, queryAggCall.name,
                                                         ImmutableList.of(rexBuilder.makeInputRef(input, targetIdx))));
                        rewritingMapping.set(targetIdx, sourceIdx);
                    }
                }
                result = relBuilder.push(result).aggregate(relBuilder.groupKey(groupSet, null), aggregateCalls).build();
                // We introduce a project on top, as group by columns order is lost
                List<RexNode> projects = new ArrayList<>();
                Mapping inverseMapping = rewritingMapping.inverse();
                for (int i = 0; i < queryAggregate.getGroupCount(); i++) {
                    projects.add(rexBuilder.makeInputRef(result, groupSet.indexOf(inverseMapping.getTarget(i))));
                }
                for (int i = 0; i < queryAggregate.getAggCallList().size(); i++) {
    ...
    public static class MaterializedViewProjectAggregateRule extends MaterializedViewAggregateRule {
        ...
        public MaterializedViewProjectAggregateRule(RelBuilderFactory relBuilderFactory,
            ...
            super(operand(Project.class, operand(Aggregate.class, any())), relBuilderFactory,
                  "MaterializedViewAggregateRule(Project-Aggregate)", generateUnionRewriting);
        }
    ...
    public static class MaterializedViewOnlyAggregateRule extends MaterializedViewAggregateRule {
        ...
        public MaterializedViewOnlyAggregateRule(RelBuilderFactory relBuilderFactory, boolean generateUnionRewriting) {
            super(operand(Aggregate.class, any()), relBuilderFactory, "MaterializedViewAggregateRule(Aggregate)",
                  generateUnionRewriting);
        }
    ...
    private static List<BiMap<RelTableRef, RelTableRef>> generateTableMappings(
        ...
        List<BiMap<RelTableRef, RelTableRef>> result = ImmutableList.<BiMap<RelTableRef, RelTableRef>>of(
    ...
    private static boolean isValidRelNodePlan(RelNode node, RelMetadataQuery mq) {
        ...
        for (Class<? extends RelNode> c : m.keySet()) {
            if (!TableScan.class.isAssignableFrom(c) && !Project.class.isAssignableFrom(c)
                && !Filter.class.isAssignableFrom(c) && !Join.class.isAssignableFrom(c)) {
                // Skip it
                return false;
            }
        }
        ...
        for (RexNode e : RelOptUtil.conjunctions(pred)) {
            switch (e.getKind()) {
                case EQUALS:
                    ...
                    if (RexUtil.isReferenceOrAccess(eqCall.getOperands().get(0), false) && RexUtil.isReferenceOrAccess(
                            eqCall.getOperands().get(1), false)) {
                        equiColumnsPreds.add(e);
                    } else if ((RexUtil.isReferenceOrAccess(eqCall.getOperands().get(0), false) && RexUtil.isConstant(
                            eqCall.getOperands().get(1))) || (
                ...
                case NOT_EQUALS:
                    ...
                    if ((RexUtil.isReferenceOrAccess(rangeCall.getOperands().get(0), false) && RexUtil.isConstant(
                            rangeCall.getOperands().get(1))) || (
     ...
     */
    private static boolean compensatePartial(Set<RelTableRef> sourceTableRefs, EquivalenceClasses sourceEC,
        ...
        for (RelTableRef tRef : graph.vertexSet()) {
            ...
            for (RelReferentialConstraint constraint : constraints) {
                Collection<RelTableRef> parentTableRefs = tableVNameToTableRefs.get(
                        constraint.getTargetQualifiedName());
                for (RelTableRef parentTRef : parentTableRefs) {
                    ...
                    for (int pos = 0; pos < constraint.getNumColumns(); pos++) {
                        ...
                        RelDataType foreignKeyColumnType = tRef.getTable().getRowType().getFieldList().get(
                                foreignKeyPos).getType();
                        ...
                        RexTableInputRef uniqueKeyColumnRef = RexTableInputRef.of(parentTRef, uniqueKeyPos,
                                                                                  parentTRef.getTable().getRowType().getFieldList().get(
                                                                                          uniqueKeyPos).getType());
                        if (!foreignKeyColumnType.isNullable() && sourceEC.getEquivalenceClassesMap().containsKey(
                                uniqueKeyColumnRef) && sourceEC.getEquivalenceClassesMap().get(
                                uniqueKeyColumnRef).contains(foreignKeyColumnRef)) {
    ...
    private static Triple<RexNode, RexNode, RexNode> computeCompensationPredicates(RexBuilder rexBuilder,
        ...
        return ImmutableTriple.<RexNode, RexNode, RexNode>of(compensationColumnsEquiPred, compensationRangePred,
     ...
     */
    private static RexNode generateEquivalenceClasses(RexBuilder rexBuilder, EquivalenceClasses sourceEC,
                                                      EquivalenceClasses targetEC) {
        ...
        final Multimap<Integer, Integer> mapping = extractPossibleMapping(sourceEquivalenceClasses,
            ...
            if (!mapping.containsKey(i)) {
                ...
                for (int j : mapping.get(i)) {
                    ...
                    for (RexTableInputRef e : difference) {
                        RexNode equals = rexBuilder.makeCall(SqlStdOperatorTable.EQUALS, e,
                                                             targetEquivalenceClasses.get(j).iterator().next());
                        compensationPredicate = rexBuilder.makeCall(SqlStdOperatorTable.AND, compensationPredicate,
                                                                    equals);
                    }
                }
            }
     ...
     */
    private static RexNode rewriteExpression(RexBuilder rexBuilder, RelMetadataQuery mq, RelNode node,
                                             List<RexNode> nodeExprs, BiMap<RelTableRef, RelTableRef> tableMapping,
                                             EquivalenceClasses ec, boolean swapTableColumn, RexNode exprToRewrite) {
        List<RexNode> rewrittenExprs = rewriteExpressions(rexBuilder, mq, node, nodeExprs, tableMapping, ec,
                                                          swapTableColumn, ImmutableList.of(exprToRewrite));
        if (rewrittenExprs == null) {
            return null;
        }
     ...
     */
    private static List<RexNode> rewriteExpressions(RexBuilder rexBuilder, RelMetadataQuery mq, RelNode node,
                                                    List<RexNode> nodeExprs,
                                                    BiMap<RelTableRef, RelTableRef> tableMapping, EquivalenceClasses ec,
        ...
        for (RexNode exprToRewrite : exprsToRewrite) {
            RexNode rewrittenExpr = replaceWithOriginalReferences(rexBuilder, nodeExprs, nodeLineage, exprToRewrite);
    ...
    private static NodeLineage generateSwapColumnTableReferencesLineage(RexBuilder rexBuilder, RelMetadataQuery mq,
        ...
        for (int i = 0; i < nodeExprs.size(); i++) {
            ...
            final RexNode e = RexUtil.swapColumnTableReferences(rexBuilder, s.iterator().next(),
     ...
     */
    private static Multimap<Integer, Integer> generateMapping(RexBuilder rexBuilder, RelMetadataQuery mq, RelNode node,
                                                              RelNode target, ImmutableBitSet positions,
                                                              BiMap<RelTableRef, RelTableRef> tableMapping,
                                                              EquivalenceClasses sourceEC) {
        ...
        for (int i = 0; i < target.getRowType().getFieldCount(); i++) {
            ...
            exprsLineage.put(RexUtil.swapTableColumnReferences(rexBuilder, s.iterator().next(), tableMapping.inverse(),
                                                               equivalenceClassesMap).toString(), i);
        }
        ...
        for (int i : positions) {
            ...
            Collection<Integer> c = exprsLineage.get(
                    RexUtil.swapColumnReferences(rexBuilder, s.iterator().next(), equivalenceClassesMap).toString());
            if (c == null) {
                // Bail out
                return null;
            }
     ...
     * point to.
     */
    private static RexNode replaceWithOriginalReferences(final RexBuilder rexBuilder, final List<RexNode> nodeExprs,
                                                         final NodeLineage nodeLineage, final RexNode exprToRewrite) {
        ...
        RexShuttle visitor = new RexShuttle() {
            ...
            private RexNode replace(RexNode e) {
                ...
                if (pos != null) {
                    // Found it
                    return rexBuilder.makeCast(e.getType(), rexBuilder.makeInputRef(nodeExprs.get(pos).getType(), pos));
                }
                return null;
            }
        };
    ...
    private static class NodeLineage {
        ...
        protected static NodeLineage of(Map<String, Integer> exprsLineage,
    ...
    private static class Edge extends DefaultEdge {
        ...
        public static final DirectedGraph.EdgeFactory<RelTableRef, Edge> FACTORY = new DirectedGraph.EdgeFactory<RelTableRef, Edge>() {
