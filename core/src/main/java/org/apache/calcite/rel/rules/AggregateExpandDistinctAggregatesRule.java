...
public final class AggregateExpandDistinctAggregatesRule extends RelOptRule {
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        if (nonDistinctAggCallCount == 0 && argLists.size() == 1 && aggregate.getGroupSets().size() == 1) {
        ...
        for (AggregateCall aggCall : aggregate.getAggCallList()) {
            ...
            refs.add(new RexInputRef(groupAndIndicatorCount + newAggCallList.size(),
    ...
    private RelBuilder convertSingletonDistinct(RelBuilder relBuilder, Aggregate aggregate,
        ...
        relBuilder.push(
                aggregate.copy(aggregate.getTraitSet(), relBuilder.build(), false, ImmutableBitSet.of(bottomGroupSet),
                               null, bottomAggregateCalls));
        ...
        for (AggregateCall aggCall : originalAggCalls) {
            ...
            if (aggCall.isDistinct()) {
                ...
                newCall = AggregateCall.create(aggCall.getAggregation(), false, newArgList, -1,
                                               originalGroupSet.cardinality(), relBuilder.peek(), aggCall.getType(),
                                               aggCall.name);
            } else {
                // If aggregate B had a COUNT aggregate call the corresponding aggregate at
                // aggregate A must be SUM. For other aggregates, it remains the same.
                final List<Integer> newArgs = Lists.newArrayList(
                        bottomGroupSet.size() + nonDistinctAggCallProcessedSoFar);
                if (aggCall.getAggregation().getKind() == SqlKind.COUNT) {
                    newCall = AggregateCall.create(new SqlSumEmptyIsZeroAggFunction(), false, newArgs, -1,
                                                   originalGroupSet.cardinality(), relBuilder.peek(), aggCall.getType(),
                                                   aggCall.getName());
                } else {
                    newCall = AggregateCall.create(aggCall.getAggregation(), false, newArgs, -1,
                                                   originalGroupSet.cardinality(), relBuilder.peek(), aggCall.getType(),
                                                   aggCall.name);
                }
    ...
    private void rewriteUsingGroupingSets(RelOptRuleCall call, Aggregate aggregate) {
        ...
        for (AggregateCall aggCall : aggregate.getAggCallList()) {
            if (!aggCall.isDistinct()) {
                ...
                groupSetTreeSet.add(
                        ImmutableBitSet.of(aggCall.getArgList()).setIf(aggCall.filterArg, aggCall.filterArg >= 0).union(
        ...
        distinctAggCalls.add(
                AggregateCall.create(SqlStdOperatorTable.GROUPING, false, ImmutableIntList.copyOf(fullGroupSet), -1,
        ...
        for (AggregateCall aggCall : aggregate.getAggCallList()) {
            ...
            if (!aggCall.isDistinct()) {
                ...
                newFilterArg = filters.get(
                        ImmutableBitSet.of(aggCall.getArgList()).setIf(aggCall.filterArg, aggCall.filterArg >= 0).union(
                                aggregate.getGroupSet()));
            }
            final AggregateCall newCall = AggregateCall.create(aggregation, false, newArgList, newFilterArg,
    ...
    private void doRewrite(RelBuilder relBuilder, Aggregate aggregate, int n, List<Integer> argList, int filterArg,
        ...
        for (AggregateCall aggCall : aggCalls) {
            ...
            final AggregateCall newAggCall = AggregateCall.create(aggCall.getAggregation(), false, newArgs,
            ...
            if (n == 0) {
                ...
                refs.set(i, new RexInputRef(leftFields.size() + groupAndIndicatorCount + aggCallList.size(),
                                            newAggCall.getType()));
            }
    ...
    private static void rewriteAggCalls(List<AggregateCall> newAggCalls, List<Integer> argList,
        ...
        for (int i = 0; i < newAggCalls.size(); i++) {
            ...
            final AggregateCall newAggCall = AggregateCall.create(aggCall.getAggregation(), false, newArgs, -1,
