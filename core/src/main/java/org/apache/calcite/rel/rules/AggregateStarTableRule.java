...
public class AggregateStarTableRule extends RelOptRule {
    ...
    public static final AggregateStarTableRule INSTANCE = new AggregateStarTableRule(
            operand(Aggregate.class, null, Aggregate.IS_SIMPLE, some(operand(StarTable.StarTableScan.class, none()))),
    ...
    protected void apply(RelOptRuleCall call, Project postProject, final Aggregate aggregate,
        ...
        if (tileKey == null) {
            ...
            if (CalcitePrepareImpl.DEBUG) {
                System.out.println("Using materialization " + aggregateRelOptTable.getQualifiedName() + ", rolling up "
                                   + tileKey.dimensions + " to " + aggregate.getGroupSet());
            }
            ...
            relBuilder.push(aggregate.copy(aggregate.getTraitSet(), relBuilder.build(), false, groupSet.build(), null,
                                           aggCalls));
            ...
            relBuilder.project(relBuilder.fields(
                    new AbstractSourceMapping(tileKey.dimensions.cardinality() + tileKey.measures.size(),
    ...
    private static AggregateCall rollUp(int groupCount, RelBuilder relBuilder, AggregateCall aggregateCall,
        ...
        if (i >= 0) {
            ...
            return AggregateCall.create(roll, false, ImmutableList.of(offset + i), -1, groupCount, relBuilder.peek(),
                                        null, aggregateCall.name);
        }
