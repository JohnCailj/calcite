...
public abstract class CalcRelSplitter {
    ...
    RelNode execute() {
        ...
        for (int level = 0; level < levelCount; level++) {
            ...
            RexProgram program1 = createProgramForLevel(level, levelCount, rel.getRowType(), exprs, exprLevels,
                                                        inputExprOrdinals, projectExprOrdinals, conditionExprOrdinal,
                                                        outputRowType);
                ...
                if (level >= levelCount) {
                    ...
                    for (int relTypeOrdinal = 0; relTypeOrdinal < relTypes.length; relTypeOrdinal++) {
                        ...
                        if (relTypes[relTypeOrdinal].canImplement(expr, condition)) {
                            ...
                            assert (level == 0) || (levelTypeOrdinals[level - 1]
    ...
    private RexProgram createProgramForLevel(int level, int levelCount, RelDataType inputRowType, RexNode[] allExprs,
                                             int[] exprLevels, int[] inputExprOrdinals, final int[] projectExprOrdinals,
        ...
        for (int i = 0; i < allExprs.length; i++) {
            if (exprLevels[i] == level || exprLevels[i] == -1 && level == (levelCount - 1)
    ...
    private static class CannotImplement extends RuntimeException {
        ...
        @SuppressWarnings("ThrowableInstanceNeverThrown") static final CannotImplement INSTANCE = new CannotImplement();
    }
