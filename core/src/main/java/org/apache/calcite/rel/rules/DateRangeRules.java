...
public abstract class DateRangeRules {
    ...
    private static final Map<TimeUnitRange, Integer> TIME_UNIT_CODES = ImmutableMap.<TimeUnitRange, Integer>builder().put(
            TimeUnitRange.YEAR, Calendar.YEAR).put(TimeUnitRange.MONTH, Calendar.MONTH).put(TimeUnitRange.DAY,
                                                                                            Calendar.DAY_OF_MONTH).put(
            TimeUnitRange.HOUR, Calendar.HOUR).put(TimeUnitRange.MINUTE, Calendar.MINUTE).put(TimeUnitRange.SECOND,
                                                                                              Calendar.SECOND).put(
            TimeUnitRange.MILLISECOND, Calendar.MILLISECOND).build();

    private static final Map<TimeUnitRange, TimeUnitRange> TIME_UNIT_PARENTS = ImmutableMap.<TimeUnitRange, TimeUnitRange>builder().put(
            TimeUnitRange.MONTH, TimeUnitRange.YEAR).put(TimeUnitRange.DAY, TimeUnitRange.MONTH).put(TimeUnitRange.HOUR,
                                                                                                     TimeUnitRange.DAY).put(
            TimeUnitRange.MINUTE, TimeUnitRange.HOUR).put(TimeUnitRange.SECOND, TimeUnitRange.MINUTE).put(
            TimeUnitRange.MILLISECOND, TimeUnitRange.SECOND).put(TimeUnitRange.MICROSECOND,
     ...
     */
    @SuppressWarnings("WeakerAccess") public static class FilterDateRangeRule extends RelOptRule {

        public FilterDateRangeRule(RelBuilderFactory relBuilderFactory) {
            super(operand(Filter.class, null, FILTER_PREDICATE, any()), relBuilderFactory, "FilterDateRangeRule");
        }
    ...
    public static class ExtractShuttle extends RexShuttle {
                    ...
                    switch (op0.getKind()) {
                        case LITERAL:
                            if (isExtractCall(op1)) {
                                return foo(call.getKind().reverse(), ((RexCall) op1).getOperands().get(1),
                                           (RexLiteral) op0);
                            }
                    }
                    switch (op1.getKind()) {
                        case LITERAL:
                            if (isExtractCall(op0)) {
                                return foo(call.getKind(), ((RexCall) op0).getOperands().get(1), (RexLiteral) op1);
                            }
                    }
                    // fall through
                default:
        ...
        @Override protected List<RexNode> visitList(List<? extends RexNode> exprs, boolean[] update) {
            if (exprs.isEmpty()) {
            ...
            switch (calls.peek().getKind()) {
                ...
                default:
                    final Map<String, RangeSet<Calendar>> save = ImmutableMap.copyOf(operandRanges);
        ...
        private RexNode toRex(RexNode operand, Range<Calendar> r) {
            final List<RexNode> nodes = new ArrayList<>();
            if (r.hasLowerBound()) {
                final SqlBinaryOperator op = r.lowerBoundType()
                                             == BoundType.CLOSED ? SqlStdOperatorTable.GREATER_THAN_OR_EQUAL : SqlStdOperatorTable.GREATER_THAN;
                nodes.add(rexBuilder.makeCall(op, operand, rexBuilder.makeDateLiteral(
                        DateString.fromCalendarFields(r.lowerEndpoint()))));
            }
            if (r.hasUpperBound()) {
                final SqlBinaryOperator op = r.upperBoundType()
                                             == BoundType.CLOSED ? SqlStdOperatorTable.LESS_THAN_OR_EQUAL : SqlStdOperatorTable.LESS_THAN;
                nodes.add(rexBuilder.makeCall(op, operand, rexBuilder.makeDateLiteral(
