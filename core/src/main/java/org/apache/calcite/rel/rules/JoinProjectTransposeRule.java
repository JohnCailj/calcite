...
public class JoinProjectTransposeRule extends RelOptRule {
    ...
    public static final JoinProjectTransposeRule BOTH_PROJECT = new JoinProjectTransposeRule(
            operand(LogicalJoin.class, operand(LogicalProject.class, any()), operand(LogicalProject.class, any())),
    ...
    public static final JoinProjectTransposeRule RIGHT_PROJECT = new JoinProjectTransposeRule(
            operand(LogicalJoin.class, operand(RelNode.class, any()), operand(LogicalProject.class, any())),
            "JoinProjectTransposeRule(Other-Project)");

    public static final JoinProjectTransposeRule BOTH_PROJECT_INCLUDE_OUTER = new JoinProjectTransposeRule(
            operand(LogicalJoin.class, operand(LogicalProject.class, any()), operand(LogicalProject.class, any())),
            "Join(IncludingOuter)ProjectTransposeRule(Project-Project)", true, RelFactories.LOGICAL_BUILDER);
     ...
     */
    public JoinProjectTransposeRule(RelOptRuleOperand operand, String description, boolean includeOuter,
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        RelDataType joinChildrenRowType = SqlValidatorUtil.deriveJoinRowType(leftJoinChild.getRowType(),
        ...
        createProjectExprs(rightProj, rightJoinChild, nFieldsLeft, rexBuilder, joinChildrenRowType.getFieldList(),
                           projects);
        ...
        RexProgram bottomProgram = RexProgram.create(joinChildrenRowType, Pair.left(projects), null, projRowType,
                                                     rexBuilder);
        ...
        Join newJoinRel = joinRel.copy(joinRel.getTraitSet(), newCondition, leftJoinChild, rightJoinChild,
                                       joinRel.getJoinType(), joinRel.isSemiJoinDone());
        ...
        for (int i = 0; i < nProjExprs; i++) {
            ...
            if (joinType != JoinRelType.INNER) {
                newExpr = newExpr.accept(
                        new RelOptUtil.RexInputConverter(rexBuilder, joinChildrenRowType.getFieldList(), newJoinFields,
                                                         adjustments));
            }
     ...
     */
    protected void createProjectExprs(Project projRel, RelNode joinChild, int adjustmentAmount, RexBuilder rexBuilder,
                                      List<RelDataTypeField> joinChildrenFields, List<Pair<RexNode, String>> projects) {
        ...
        if (projRel != null) {
            ...
            for (Pair<RexNode, String> pair : namedProjects) {
                ...
                if (adjustmentAmount != 0) {
                    ...
                    e = e.accept(
                            new RelOptUtil.RexInputConverter(rexBuilder, childFields, joinChildrenFields, adjustments));
                }
            ...
            for (int i = 0; i < childFields.size(); i++) {
                ...
                projects.add(Pair.of((RexNode) rexBuilder.makeInputRef(field.getType(), i + adjustmentAmount),
                                     field.getName()));
            }
        }
    }
