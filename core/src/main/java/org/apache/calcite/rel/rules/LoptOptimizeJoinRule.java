...
public class LoptOptimizeJoinRule extends RelOptRule {
    ...
    public static final LoptOptimizeJoinRule INSTANCE = new LoptOptimizeJoinRule(RelFactories.LOGICAL_BUILDER);
    ...
    public LoptOptimizeJoinRule(RelFactories.JoinFactory joinFactory, RelFactories.ProjectFactory projectFactory,
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        final LoptSemiJoinOptimizer semiJoinOpt = new LoptSemiJoinOptimizer(call.getMetadataQuery(), multiJoin,
                                                                            rexBuilder);
        ...
        while (!removalCandidates.isEmpty()) {
            ...
            for (int factIdx : removalCandidates) {
                ...
                for (RexNode filter : ojFilters) {
                    ...
                    if ((filterCall.getOperator() != SqlStdOperatorTable.EQUALS) || !(filterCall.getOperands().get(
                            0) instanceof RexInputRef) || !(filterCall.getOperands().get(1) instanceof RexInputRef)) {
                        continue;
                    }
                    ...
                    setJoinKey(joinKeyBuilder, otherJoinKeyBuilder, leftRef, rightRef, firstFieldNum, lastFieldNum,
                               true);
                }
                ...
                // keys.
                if (RelMdUtil.areColumnsDefinitelyUniqueWhenNullsFiltered(mq, multiJoin.getJoinFactor(factIdx),
                                                                          joinKeys)) {
     ...
     *                      one
     */
    private void setJoinKey(ImmutableBitSet.Builder joinKeys, ImmutableBitSet.Builder otherJoinKeys, int ref1, int ref2,
                            int firstFieldNum, int lastFieldNum, boolean swap) {
    ...
    private void findRemovableSelfJoins(RelMetadataQuery mq, LoptMultiJoin multiJoin) {
        ...
        for (Integer factor1 : selfJoinPairs.keySet()) {
            ...
            if ((selfJoinFilters.size() > 0) && isSelfJoinFilterUnique(mq, multiJoin, factor1, factor2,
                                                                       selfJoinFilters)) {
     ...
     */
    private boolean isSelfJoinFilterUnique(RelMetadataQuery mq, LoptMultiJoin multiJoin, int leftFactor,
                                           int rightFactor, List<RexNode> joinFilterList) {
        ...
        joinFilters = joinFilters.accept(new RelOptUtil.RexInputConverter(rexBuilder, multiJoin.getMultiJoinFields(),
     ...
     */
    private void findBestOrderings(RelMetadataQuery mq, RelBuilder relBuilder, LoptMultiJoin multiJoin,
                                   LoptSemiJoinOptimizer semiJoinOpt, RelOptRuleCall call) {
     ...
     */
    private RelNode createTopProject(RelBuilder relBuilder, LoptMultiJoin multiJoin, LoptJoinTree joinTree,
     ...
     */
    private Double computeJoinCardinality(RelMetadataQuery mq, LoptMultiJoin multiJoin,
                                          LoptSemiJoinOptimizer semiJoinOpt, LoptJoinTree joinTree,
                                          List<RexNode> filters, int factor) {
        final ImmutableBitSet childFactors = ImmutableBitSet.builder().addAll(joinTree.getTreeOrder()).set(
                factor).build();
        ...
        setFactorJoinKeys(multiJoin, RelOptUtil.conjunctions(multiJoin.getOuterJoinCond(factor)), childFactors,
                          factorStart, nFields, joinKeys);
     ...
     */
    private void setFactorJoinKeys(LoptMultiJoin multiJoin, List<RexNode> filters, ImmutableBitSet joinFactors,
                                   int factorStart, int nFields, ImmutableBitSet.Builder joinKeys) {
        for (RexNode joinFilter : filters) {
            ...
            if (joinFactors.contains(filterFactors)) {
                ...
                for (int field = joinFields.nextSetBit(factorStart);
                     (field >= 0) && (field < (factorStart + nFields)); field = joinFields.nextSetBit(field + 1)) {
     ...
     */
    private LoptJoinTree createOrdering(RelMetadataQuery mq, RelBuilder relBuilder, LoptMultiJoin multiJoin,
                                        LoptSemiJoinOptimizer semiJoinOpt, int firstFactor) {
        ...
        while (factorsToAdd.cardinality() > 0) {
            ...
            if (factorsAdded.cardinality() == 0) {
                ...
                if ((selfJoinFactor != null) && !factorsAdded.get(selfJoinFactor)) {
                    ...
                    nextFactor = getBestNextFactor(mq, multiJoin, factorsToAdd, factorsAdded, semiJoinOpt, joinTree,
                                                   filtersToAdd);
                }
            }
            ...
            joinTree = addFactorToTree(mq, relBuilder, multiJoin, semiJoinOpt, joinTree, nextFactor, factorsNeeded,
                                       filtersToAdd, selfJoin);
            if (joinTree == null) {
                return null;
            }
     ...
     */
    private int getBestNextFactor(RelMetadataQuery mq, LoptMultiJoin multiJoin, BitSet factorsToAdd,
                                  BitSet factorsAdded, LoptSemiJoinOptimizer semiJoinOpt, LoptJoinTree joinTree,
        ...
        for (int factor : BitSets.toIter(factorsToAdd)) {
            ...
            if ((dimWeight > bestWeight) || ((dimWeight == bestWeight) && ((bestCardinality == null) || (
     ...
     */
    private LoptJoinTree addFactorToTree(RelMetadataQuery mq, RelBuilder relBuilder, LoptMultiJoin multiJoin,
                                         LoptSemiJoinOptimizer semiJoinOpt, LoptJoinTree joinTree, int factorToAdd,
                                         BitSet factorsNeeded, List<RexNode> filtersToAdd, boolean selfJoin) {
        ...
        if (multiJoin.isRemovableOuterJoinFactor(factorToAdd)) {
            return createReplacementJoin(relBuilder, multiJoin, semiJoinOpt, joinTree, -1, factorToAdd,
                                         ImmutableIntList.of(), null, filtersToAdd);
        }
        ...
        LoptJoinTree topTree = addToTop(mq, relBuilder, multiJoin, semiJoinOpt, joinTree, factorToAdd, filtersToAdd,
                                        selfJoin);
        LoptJoinTree pushDownTree = pushDownFactor(mq, relBuilder, multiJoin, semiJoinOpt, joinTree, factorToAdd,
                                                   factorsNeeded, (topTree == null) ? filtersToAdd : tmpFilters,
                                                   selfJoin);
     ...
     * returned
     */
    private LoptJoinTree pushDownFactor(RelMetadataQuery mq, RelBuilder relBuilder, LoptMultiJoin multiJoin,
                                        LoptSemiJoinOptimizer semiJoinOpt, LoptJoinTree joinTree, int factorToAdd,
                                        BitSet factorsNeeded, List<RexNode> filtersToAdd, boolean selfJoin) {
        ...
        subTree = addFactorToTree(mq, relBuilder, multiJoin, semiJoinOpt, subTree, factorToAdd, factorsNeeded,
                                  filtersToAdd, selfJoin);
        ...
        newCondition = adjustFilter(multiJoin, left, right, newCondition, factorToAdd, origJoinOrder,
        ...
        return createJoinSubtree(mq, relBuilder, multiJoin, left, right, newCondition, joinType, filtersToAdd, false,
                                 false);
    }
     ...
     */
    private LoptJoinTree addToTop(RelMetadataQuery mq, RelBuilder relBuilder, LoptMultiJoin multiJoin,
                                  LoptSemiJoinOptimizer semiJoinOpt, LoptJoinTree joinTree, int factorToAdd,
                                  List<RexNode> filtersToAdd, boolean selfJoin) {
        ...
        return createJoinSubtree(mq, relBuilder, multiJoin, joinTree, rightTree, condition, joinType, filtersToAdd,
                                 true, selfJoin);
    }
     ...
     * current join tree
     */
    private RexNode addFilters(LoptMultiJoin multiJoin, LoptJoinTree leftTree, int leftIdx, LoptJoinTree rightTree,
        ...
        if (adjust && (condition != null)) {
            ...
            if (needsAdjustment(multiJoin, adjustments, leftTree, rightTree, false)) {
                condition = condition.accept(
                        new RelOptUtil.RexInputConverter(rexBuilder, multiJoin.getMultiJoinFields(),
     ...
     */
    private RexNode adjustFilter(LoptMultiJoin multiJoin, LoptJoinTree left, LoptJoinTree right, RexNode condition,
                                 int factorAdded, List<Integer> origJoinOrder, List<RelDataTypeField> origFields) {
        final List<Integer> newJoinOrder = new ArrayList<>();
        left.getTreeOrder(newJoinOrder);
        right.getTreeOrder(newJoinOrder);

        int totalFields =
                left.getJoinTree().getRowType().getFieldCount() + right.getJoinTree().getRowType().getFieldCount()
        ...
        for (int newPos = 0; newPos < newJoinOrder.size(); newPos++) {
            ...
            if (factor != factorAdded) {
                ...
                if (remapJoinReferences(multiJoin, factor, newJoinOrder, newPos, adjustments, nFieldsOld, nFieldsNew,
                                        false)) {
                    needAdjust = true;
                }
            }
        ...
        if (needAdjust) {
            RexBuilder rexBuilder = multiJoin.getMultiJoinRel().getCluster().getRexBuilder();
            condition = condition.accept(new RelOptUtil.RexInputConverter(rexBuilder, origFields,
     ...
     */
    private boolean remapJoinReferences(LoptMultiJoin multiJoin, int factor, List<Integer> newJoinOrder, int newPos,
                                        int[] adjustments, int offset, int newOffset, boolean alwaysUseDefault) {
        boolean needAdjust = false;
        int defaultAdjustment = -offset + newOffset;
        if (!alwaysUseDefault && multiJoin.isRightFactorInRemovableSelfJoin(factor) && (newPos != 0)
     ...
     * been joined in yet
     */
    private LoptJoinTree createReplacementSemiJoin(RelBuilder relBuilder, LoptMultiJoin multiJoin,
                                                   LoptSemiJoinOptimizer semiJoinOpt, LoptJoinTree factTree, int dimIdx,
                                                   List<RexNode> filtersToAdd) {
        ...
        return createReplacementJoin(relBuilder, multiJoin, semiJoinOpt, factTree, factIdx, dimIdx, dimKeys,
                                     replacementKeys, filtersToAdd);
    }
     ...
     * that can be removed
     */
    private LoptJoinTree createReplacementJoin(RelBuilder relBuilder, LoptMultiJoin multiJoin,
                                               LoptSemiJoinOptimizer semiJoinOpt, LoptJoinTree currJoinTree,
                                               int leftIdx, int factorToAdd, ImmutableIntList newKeys,
                                               Integer[] replacementKeys, List<RexNode> filtersToAdd) {
        ...
        for (int i = 0; i < nCurrFields; i++) {
            projects.add(Pair.of((RexNode) rexBuilder.makeInputRef(currFields.get(i).getType(), i),
     ...
     */
    private LoptJoinTree createJoinSubtree(RelMetadataQuery mq, RelBuilder relBuilder, LoptMultiJoin multiJoin,
                                           LoptJoinTree left, LoptJoinTree right, RexNode condition,
                                           JoinRelType joinType, List<RexNode> filtersToAdd, boolean fullAdjust,
                                           boolean selfJoin) {
        RexBuilder rexBuilder = multiJoin.getMultiJoinRel().getCluster().getRexBuilder();
        ...
        if (fullAdjust) {
            ...
            if (needsAdjustment(multiJoin, adjustments, left, right, selfJoin)) {
                condition = condition.accept(
                        new RelOptUtil.RexInputConverter(rexBuilder, multiJoin.getMultiJoinFields(),
                                                         left.getJoinTree().getRowType().getFieldList(),
                                                         right.getJoinTree().getRowType().getFieldList(), adjustments));
            }
        }
     ...
     */
    private void addAdditionalFilters(RelBuilder relBuilder, LoptMultiJoin multiJoin, LoptJoinTree left,
                                      LoptJoinTree right, List<RexNode> filtersToAdd) {
     ...
     */
    private boolean swapInputs(RelMetadataQuery mq, LoptMultiJoin multiJoin, LoptJoinTree left, LoptJoinTree right,
                               boolean selfJoin) {
        boolean swap = false;
        ...
        if ((leftRowCount != null) && (rightRowCount != null) && ((leftRowCount < rightRowCount) || (
                (Math.abs(leftRowCount - rightRowCount) < RelOptUtil.EPSILON) && (rowWidthCost(left.getJoinTree())
                                                                                  < rowWidthCost(
                        right.getJoinTree()))))) {
            swap = true;
        }
     ...
     */
    private RexNode swapFilter(RexBuilder rexBuilder, LoptMultiJoin multiJoin, LoptJoinTree origLeft,
                               LoptJoinTree origRight, RexNode condition) {
        ...
        condition = condition.accept(
                new RelOptUtil.RexInputConverter(rexBuilder, multiJoin.getJoinFields(origLeft, origRight),
                                                 multiJoin.getJoinFields(origRight, origLeft), adjustments));
     ...
     */
    private boolean needsAdjustment(LoptMultiJoin multiJoin, int[] adjustments, LoptJoinTree joinTree,
                                    LoptJoinTree otherTree, boolean selfJoin) {
        ...
        for (int newPos = 0; newPos < joinOrder.size(); newPos++) {
            ...
            // self-join.
            if (remapJoinReferences(multiJoin, origPos, joinOrder, newPos, adjustments, joinStart, nFields, selfJoin)) {
                needAdjustment = true;
            }
     ...
     */
    private static boolean areSelfJoinKeysUnique(RelMetadataQuery mq, RelNode leftRel, RelNode rightRel,
                                                 RexNode joinFilters) {
