...
public class LoptSemiJoinOptimizer {
    ...
    public void makePossibleSemiJoins(LoptMultiJoin multiJoin) {
        ...
        for (int factIdx = 0; factIdx < nJoinFactors; factIdx++) {
            ...
            for (Integer dimIdx : dimIdxes) {
                ...
                if (joinFilters != null) {
                    SemiJoin semiJoin = findSemiJoinIndexByCost(multiJoin, joinFilters, factIdx, dimIdx);
     ...
     */
    private SemiJoin findSemiJoinIndexByCost(LoptMultiJoin multiJoin, List<RexNode> joinFilters, int factIdx,
                                             int dimIdx) {
        ...
        FemLocalIndex bestIndex = indexOptimizer.findSemiJoinIndexByCost(dimRel, actualLeftKeys, rightKeys,
                                                                         bestKeyOrder);
        ...
        if (actualLeftKeys.size() == bestKeyOrder.size()) {
            ...
            semiJoinCondition = removeExtraFilters(truncatedLeftKeys, multiJoin.getNumFieldsInJoinFactor(factIdx),
                                                   semiJoinCondition);
        }
        return SemiJoin.create(factRel, dimRel, semiJoinCondition, ImmutableIntList.copyOf(truncatedLeftKeys),
     ...
     */
    private RexNode adjustSemiJoinCondition(LoptMultiJoin multiJoin, int leftAdjustment, RexNode semiJoinCondition,
        ...
        if ((leftAdjustment != 0) || (rightAdjustment != 0)) {
            ...
            if (leftAdjustment != 0) {
                for (int i = -leftAdjustment; i < (-leftAdjustment + numFieldsLeftIdx); i++) {
                    adjustments[i] = leftAdjustment;
                }
            }
            if (rightAdjustment != 0) {
                for (int i = rightStart; i < (rightStart + numFieldsRightIdx); i++) {
                    adjustments[i] = rightAdjustment;
                }
            }
            return semiJoinCondition.accept(
                    new RelOptUtil.RexInputConverter(rexBuilder, multiJoin.getMultiJoinFields(), adjustments));
        }
     ...
     * otherwise null
     */
    private LcsTable validateKeys(RelNode factRel, List<Integer> leftKeys, List<Integer> rightKeys,
    ...
    public boolean chooseBestSemiJoin(LoptMultiJoin multiJoin) {
        ...
        int nJoinFactors = multiJoin.getNumJoinFactors();
        List<Integer> sortedFactors = factorCostOrdering.immutableSortedCopy(Util.range(nJoinFactors));
        ...
        for (int i = 0; i < nJoinFactors; i++) {
            ...
            Map<Integer, SemiJoin> possibleDimensions = possibleSemiJoins.get(factIdx);
            ...
            for (Integer dimIdx : dimIdxes) {
                ...
                double score = computeScore(factRel, chosenSemiJoins[dimIdx], semiJoin);
            ...
            if (bestDimIdx != -1) {
                ...
                SemiJoin chosenSemiJoin = SemiJoin.create(factRel, chosenSemiJoins[bestDimIdx], semiJoin.getCondition(),
                ...
                removePossibleSemiJoin(possibleDimensions, factIdx, bestDimIdx);
                ...
                removePossibleSemiJoin(possibleSemiJoins.get(bestDimIdx), bestDimIdx, factIdx);
                return true;
            }
    ...
    private static class LcsIndexOptimizer {
        ...
        public FemLocalIndex findSemiJoinIndexByCost(RelNode dimRel, List<Integer> actualLeftKeys,
                                                     List<Integer> rightKeys, List<Integer> bestKeyOrder) {
            return null;
        }
    }
