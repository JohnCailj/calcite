...
public class MaterializedViewFilterScanRule extends RelOptRule {
    ...
    private final HepProgram program = new HepProgramBuilder().addRuleInstance(
            FilterProjectTransposeRule.INSTANCE).addRuleInstance(ProjectMergeRule.INSTANCE).build();
    ...
    protected MaterializedViewFilterScanRule(RelBuilderFactory relBuilderFactory) {
        super(operand(Filter.class, operand(TableScan.class, null, none())), relBuilderFactory,
    ...
    protected void apply(RelOptRuleCall call, Filter filter, TableScan scan) {
        ...
        if (!materializations.isEmpty()) {
            ...
            List<RelOptMaterialization> applicableMaterializations = RelOptMaterializations.getApplicableMaterializations(
            ...
            for (RelOptMaterialization materialization : applicableMaterializations) {
                if (RelOptUtil.areRowTypesEqual(scan.getRowType(), materialization.queryRel.getRowType(), false)) {
                    ...
                    List<RelNode> subs = new MaterializedViewSubstitutionVisitor(target, root).go(
