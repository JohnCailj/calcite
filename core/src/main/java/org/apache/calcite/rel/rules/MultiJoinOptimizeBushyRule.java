...
public class MultiJoinOptimizeBushyRule extends RelOptRule {
    ...
    public static final MultiJoinOptimizeBushyRule INSTANCE = new MultiJoinOptimizeBushyRule(
    ...
    @Override public void onMatch(RelOptRuleCall call) {
        ...
        for (; ; ) {
            ...
            double cost = majorVertex.cost * minorVertex.cost * RelMdUtil.guessSelectivity(
                    RexUtil.composeConjunction(rexBuilder, conditions, false));
            final Vertex newVertex = new JoinVertex(v, majorFactor, minorFactor, newFactors, cost,
            ...
            for (int i = 0; i < unusedEdges.size(); i++) {
                ...
                if (edge.factors.intersects(merged)) {
                    ...
                    final LoptMultiJoin.Edge newEdge = new LoptMultiJoin.Edge(edge.condition, newEdgeFactors,
                                                                              edge.columns);
        ...
        for (Vertex vertex : vertexes) {
            if (vertex instanceof LeafVertex) {
                ...
                final Mappings.TargetMapping mapping = Mappings.offsetSource(
                        Mappings.createIdentity(leafVertex.rel.getRowType().getFieldCount()), leafVertex.fieldOffset,
                ...
                final Mappings.TargetMapping mapping = Mappings.merge(leftMapping, Mappings.offsetTarget(rightMapping,
                ...
                final RelNode join = relBuilder.push(left).push(right).join(JoinRelType.INNER,
                                                                            condition.accept(shuttle)).build();
    ...
    private void trace(List<Vertex> vertexes, List<LoptMultiJoin.Edge> unusedEdges, List<LoptMultiJoin.Edge> usedEdges,
    ...
    static class LeafVertex extends Vertex {
        ...
        @Override public String toString() {
            return "LeafVertex(id: " + id + ", cost: " + Util.human(cost) + ", factors: " + factors + ", fieldOffset: "
                   + fieldOffset + ")";
        }
    }
    ...
    static class JoinVertex extends Vertex {
        ...
        JoinVertex(int id, int leftFactor, int rightFactor, ImmutableBitSet factors, double cost,
        ...
        @Override public String toString() {
            return "JoinVertex(id: " + id + ", cost: " + Util.human(cost) + ", factors: " + factors + ", leftFactor: "
                   + leftFactor + ", rightFactor: " + rightFactor + ")";
        }
    }
}
