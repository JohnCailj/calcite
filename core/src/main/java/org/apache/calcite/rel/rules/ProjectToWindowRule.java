...
public abstract class ProjectToWindowRule extends RelOptRule {
     ...
     */
    public static final ProjectToWindowRule INSTANCE = new ProjectToWindowRule(
            operand(Calc.class, null, PREDICATE, any()), "ProjectToWindowRule") {
     ...
     */
    public static final ProjectToWindowRule PROJECT = new ProjectToWindowRule(
            operand(Project.class, null, PREDICATE2, any()), "ProjectToWindowRule:project") {
            ...
            final CalcRelSplitter transform = new WindowedAggRelSplitter(calc, call.builder()) {
                ...
                @Override protected RelNode handle(RelNode rel) {
                    ...
                    if (!program.projectsOnlyIdentity()) {
                        relBuilder.project(
                                Lists.transform(program.getProjectList(), new Function<RexLocalRef, RexNode>() {
    ...
    static class WindowedAggRelSplitter extends CalcRelSplitter {
        ...
        private static final RelType[] REL_TYPES = { new RelType("CalcRelType") {
        ...
        private boolean isDependent(final DirectedGraph<Integer, DefaultEdge> graph, final List<Integer> rank,
                                    final int ordinal1, final int ordinal2) {
        ...
        private DirectedGraph<Integer, DefaultEdge> createGraphFromExpression(final List<RexNode> exprs) {
            ...
            for (final Ord<RexNode> expr : Ord.zip(exprs)) {
                expr.e.accept(new RexVisitorImpl<Void>(true) {
