...
public class PushProjector {
     ...
     */
    public PushProjector(Project origProj, RexNode origFilter, RelNode childRel, ExprCondition preserveExprCondition,
    ...
    public boolean locateAllRefs() {
        RexUtil.apply(new InputSpecialOpFinder(projRefs, childBitmap, rightBitmap, strongBitmap, preserveExprCondition,
                                               childPreserveExprs, rightPreserveExprs), origProjExprs, origFilter);
        ...
        // project cases
        if (projRefs.cardinality() == nChildFields && childPreserveExprs.size() == 0
    ...
    public Project createProjectRefsAndExprs(RelNode projChild, boolean adjust, boolean rightSide) {
        ...
        for (int i = 0; i < nInputRefs; i++) {
            ...
            newProjects.add(Pair.of((RexNode) rexBuilder.makeInputRef(destField.getType(), refIdx - offset),
                                    destField.getName()));
        }
        ...
        for (RexNode projExpr : preserveExprs) {
            ...
            if (adjust) {
                newExpr = projExpr.accept(
                        new RelOptUtil.RexInputConverter(rexBuilder, childFields, destFields, adjustments));
            } else {
                newExpr = projExpr;
            }
        ...
        return (Project) RelOptUtil.createProject(projChild, Pair.left(newProjects), Pair.right(newProjects), false,
                                                  relBuilder);
    }
    ...
    public RexNode convertRefsAndExprs(RexNode rex, List<RelDataTypeField> destFields, int[] adjustments) {
        return rex.accept(
                new RefAndExprConverter(rexBuilder, childFields, destFields, adjustments, childPreserveExprs, nProject,
                                        rightPreserveExprs, nProject + childPreserveExprs.size() + nRightProject));
    }
    ...
    public RelNode createNewProject(RelNode projChild, int[] adjustments) {
        ...
        if (origProj != null) {
            for (Pair<RexNode, String> p : origProj.getNamedProjects()) {
                projects.add(Pair.of(convertRefsAndExprs(p.left, projChild.getRowType().getFieldList(), adjustments),
                                     p.right));
            }
        } else {
        ...
        return RelOptUtil.createProject(projChild, Pair.left(projects), Pair.right(projects), true /* optimize to avoid trivial projections, as per javadoc */,
                                        relBuilder);
    }
    ...
    private class InputSpecialOpFinder extends RexVisitorImpl<Void> {
        ...
        InputSpecialOpFinder(BitSet rexRefs, ImmutableBitSet leftFields, ImmutableBitSet rightFields,
                             final ImmutableBitSet strongFields, ExprCondition preserveExprCondition,
    ...
    private class RefAndExprConverter extends RelOptUtil.RexInputConverter {
        ...
        RefAndExprConverter(RexBuilder rexBuilder, List<RelDataTypeField> srcFields, List<RelDataTypeField> destFields,
         ...
         * if no match
         */
        private int findExprInLists(RexNode rex, List<RexNode> rexList1, int adjust1, List<RexNode> rexList2,
                                    int adjust2) {
    ...
    public interface ExprCondition extends Predicate<RexNode> {
         ...
         */
        ExprCondition FALSE = new ExprConditionImpl() {
         ...
         */
        ExprCondition TRUE = new ExprConditionImpl() {
     ...
     */
    abstract static class ExprConditionImpl extends PredicateImpl<RexNode> implements ExprCondition {
