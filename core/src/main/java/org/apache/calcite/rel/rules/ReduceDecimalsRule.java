...
public class ReduceDecimalsRule extends RelOptRule {
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        RexProgramBuilder programBuilder = RexProgramBuilder.create(rexBuilder, calc.getInput().getRowType(),
                                                                    program.getExprList(), program.getProjectList(),
                                                                    program.getCondition(), program.getOutputRowType(),
                                                                    shuttle, true);
        ...
        protected RexNode ensureScale(RexNode value, int scale, int required) {
            ...
            if (scaleDiff >= maxPrecision) {
                throw Util.needToImplement(
                        ...
                        + " because the smallest value of the " + "source type is too large to be encoded by the "
                        + "target type");
            }
        ...
        public RexNode expand(RexCall call) {
            ...
            if (SqlTypeUtil.isIntType(fromType)) {
                ...
                if (fromScale <= toScale) {
                    ...
                    if ((toDigits == fromDigits) && (toScale < fromScale)) {
    ...
    private class BinaryArithmeticExpander extends RexExpander {
        ...
        private RexNode expandPlusMinus(RexCall call, List<RexNode> operands) {
            ...
            return encodeValue(
                    builder.makeCall(call.getOperator(), ensureScale(accessValue(operands.get(0)), scaleA, outScale),
                                     ensureScale(accessValue(operands.get(1)), scaleB, outScale)), outType);
        }
        ...
        private RexNode expandTimes(RexCall call, List<RexNode> operands) {
            ...
            if (builder.getTypeFactory().useDoubleMultiplication(typeA, typeB)) {
                ...
                //     / 10^divisor
                RexNode division = makeDivide(makeMultiply(ensureType(real8, accessValue(operands.get(0))),
                                                           ensureType(real8, accessValue(operands.get(1)))),
    ...
    private class ReinterpretExpander extends RexExpander {
        ...
        private boolean canSimplify(RexCall outer, RexCall inner, RexNode value) {
            ...
            if ((outerType.getSqlTypeName() != valueType.getSqlTypeName()) || (outerType.getPrecision()
                                                                               != valueType.getPrecision()) || (
