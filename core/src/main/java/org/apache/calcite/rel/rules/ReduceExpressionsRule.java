...
public abstract class ReduceExpressionsRule extends RelOptRule {
     ...
     */
    public static final ReduceExpressionsRule FILTER_INSTANCE = new FilterReduceExpressionsRule(LogicalFilter.class,
     ...
     */
    public static final ReduceExpressionsRule PROJECT_INSTANCE = new ProjectReduceExpressionsRule(LogicalProject.class,
                                                                                                  RelFactories.LOGICAL_BUILDER);
     ...
     */
    public static final ReduceExpressionsRule JOIN_INSTANCE = new JoinReduceExpressionsRule(Join.class,
                                                                                            RelFactories.LOGICAL_BUILDER);
     ...
     */
    public static final ReduceExpressionsRule CALC_INSTANCE = new CalcReduceExpressionsRule(LogicalCalc.class,
                                                                                            RelFactories.LOGICAL_BUILDER);
    ...
    public static class ProjectReduceExpressionsRule extends ReduceExpressionsRule {
        ...
        @Override public void onMatch(RelOptRuleCall call) {
            ...
            if (reduceExpressions(project, expList, predicates)) {
                call.transformTo(call.builder().push(project.getInput()).project(expList,
    ...
    public static class JoinReduceExpressionsRule extends ReduceExpressionsRule {
        ...
        @Override public void onMatch(RelOptRuleCall call) {
            ...
            final RelOptPredicateList predicates = leftPredicates.union(rexBuilder,
            ...
            call.transformTo(
                    join.copy(join.getTraitSet(), expList.get(0), join.getLeft(), join.getRight(), join.getJoinType(),
                              join.isSemiJoinDone()));
    ...
    public static class CalcReduceExpressionsRule extends ReduceExpressionsRule {
        ...
        @Override public void onMatch(RelOptRuleCall call) {
            ...
            if (reduceExpressions(calc, expandedExprList, predicates)) {
                final RexProgramBuilder builder = new RexProgramBuilder(calc.getInput().getRowType(),
    ...
    protected static boolean reduceExpressions(RelNode rel, List<RexNode> expList, RelOptPredicateList predicates,
        ...
        final RexSimplify simplify = new RexSimplify(rexBuilder, unknownAsFalse, executor);
    ...
    protected static boolean reduceExpressionsInternal(RelNode rel, RexSimplify simplify, List<RexNode> expList,
        ...
        findReducibleExps(rel.getCluster().getTypeFactory(), expList, predicates.constantMap, constExps, addCasts,
                          removableCasts);
        ...
        if (!removableCasts.isEmpty()) {
            ...
            RexReplacer replacer = new RexReplacer(simplify, removableCasts, reducedExprs,
        ...
        if (!predicates.constantMap.isEmpty()) {
            ...
            final List<Map.Entry<RexNode, RexNode>> pairs = (List<Map.Entry<RexNode, RexNode>>) (List) Lists.newArrayList(
                    predicates.constantMap.entrySet());
            RexReplacer replacer = new RexReplacer(simplify, Pair.left(pairs), Pair.right(pairs),
     ...
     */
    protected static void findReducibleExps(RelDataTypeFactory typeFactory, List<RexNode> exps,
                                            ImmutableMap<RexNode, RexNode> constants, List<RexNode> constExps,
                                            List<Boolean> addCasts, List<RexNode> removableCasts) {
        ReducibleExprLocator gardener = new ReducibleExprLocator(typeFactory, constants, constExps, addCasts,
                                                                 removableCasts);
    ...
    protected static class RexReplacer extends RexShuttle {
        ...
        RexReplacer(RexSimplify simplify, List<RexNode> reducibleExps, List<RexNode> reducedValues,
    ...
    protected static class ReducibleExprLocator extends RexVisitorImpl<Void> {
        ...
        ReducibleExprLocator(RelDataTypeFactory typeFactory, ImmutableMap<RexNode, RexNode> constants,
                             List<RexNode> constExprs, List<Boolean> addCasts, List<RexNode> removableCasts) {
            // go deep
            super(true);
