...
public class SemiJoinJoinTransposeRule extends RelOptRule {
    ...
    private SemiJoinJoinTransposeRule() {
        super(operand(SemiJoin.class, some(operand(Join.class, any()))));
    }
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        if (nKeysFromX > 0) {
            ...
            newSemiJoinFilter = semiJoin.getCondition().accept(
                    new RelOptUtil.RexInputConverter(semiJoin.getCluster().getRexBuilder(), fields, adjustments));
        ...
        SemiJoin newSemiJoin = SemiJoin.create(leftSemiJoinOp, semiJoin.getRight(), newSemiJoinFilter,
                                               ImmutableIntList.copyOf(newLeftKeys), rightKeys);
        ...
        RelNode newJoinRel = join.copy(join.getTraitSet(), join.getCondition(), leftJoinRel, rightJoinRel,
     ...
     */
    private void setJoinAdjustments(int[] adjustments, int nFieldsX, int nFieldsY, int nFieldsZ, int adjustY,
                                    int adjustZ) {
