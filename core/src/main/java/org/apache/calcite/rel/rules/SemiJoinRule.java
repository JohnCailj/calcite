...
public abstract class SemiJoinRule extends RelOptRule {
    ...
    private static final Predicate<Join> IS_LEFT_OR_INNER = new PredicateImpl<Join>() {
    ...
    private static final Predicate<Aggregate> IS_EMPTY_AGGREGATE = new PredicateImpl<Aggregate>() {
    ...
    public static final SemiJoinRule PROJECT = new ProjectToSemiJoinRule(Project.class, Join.class, Aggregate.class,
                                                                         RelFactories.LOGICAL_BUILDER,
                                                                         "SemiJoinRule:project");

    public static final SemiJoinRule JOIN = new JoinToSemiJoinRule(Join.class, Aggregate.class,
                                                                   RelFactories.LOGICAL_BUILDER, "SemiJoinRule:join");

    protected SemiJoinRule(Class<Project> projectClass, Class<Join> joinClass, Class<Aggregate> aggregateClass,
                           RelBuilderFactory relBuilderFactory, String description) {
        super(operand(projectClass, some(operand(joinClass, null, IS_LEFT_OR_INNER,
                                                 some(operand(RelNode.class, any()), operand(aggregateClass, any()))))),
              relBuilderFactory, description);
    }

    protected SemiJoinRule(Class<Join> joinClass, Class<Aggregate> aggregateClass, RelBuilderFactory relBuilderFactory,
                           String description) {
        super(operand(joinClass, null, IS_LEFT_OR_INNER,
                      some(operand(RelNode.class, any()), operand(aggregateClass, null, IS_EMPTY_AGGREGATE, any()))),
              relBuilderFactory, description);
    }
        ...
        switch (join.getJoinType()) {
            case INNER:
                ...
                final RexNode newCondition = RelOptUtil.createEquiJoinCondition(relBuilder.peek(2, 0),
                                                                                joinInfo.leftKeys,
                                                                                relBuilder.peek(2, 1), newRightKeys,
                                                                                rexBuilder);
    ...
    public static class ProjectToSemiJoinRule extends SemiJoinRule {
         ...
         */
        public ProjectToSemiJoinRule(Class<Project> projectClass, Class<Join> joinClass,
    ...
    public static class JoinToSemiJoinRule extends SemiJoinRule {
         ...
         */
        public JoinToSemiJoinRule(Class<Join> joinClass, Class<Aggregate> aggregateClass,
                                  RelBuilderFactory relBuilderFactory, String description) {
