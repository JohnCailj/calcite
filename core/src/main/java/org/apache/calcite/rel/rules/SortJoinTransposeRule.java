...
public class SortJoinTransposeRule extends RelOptRule {
    ...
    public static final SortJoinTransposeRule INSTANCE = new SortJoinTransposeRule(LogicalSort.class,
                                                                                   LogicalJoin.class);
    ...
    public SortJoinTransposeRule(Class<? extends Sort> sortClass, Class<? extends Join> joinClass) {
        super(operand(sortClass, operand(joinClass, any())));
    }
    ...
    @Override public boolean matches(RelOptRuleCall call) {
        ...
        if (join.getJoinType() == JoinRelType.LEFT) {
            ...
            if (sort.offset != null && !RelMdUtil.areColumnsDefinitelyUnique(mq, join.getRight(),
                                                                             joinInfo.rightSet())) {
                return false;
            }
    ...
    @Override public void onMatch(RelOptRuleCall call) {
        ...
        if (join.getJoinType() == JoinRelType.LEFT) {
            ...
            // If the input is already sorted and we are not reducing the number of tuples,
            // we bail out
            if (RelMdUtil.checkInputForCollationAndLimit(mq, join.getRight(), rightCollation, sort.offset,
                                                         sort.fetch)) {
                return;
            }
            newLeftInput = join.getLeft();
            newRightInput = sort.copy(sort.getTraitSet().replace(rightCollation), join.getRight(), rightCollation,
