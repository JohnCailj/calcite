...
public class SortProjectTransposeRule extends RelOptRule {
    ...
    public static final SortProjectTransposeRule INSTANCE = new SortProjectTransposeRule(Sort.class,
                                                                                         LogicalProject.class, null);
    ...
    public SortProjectTransposeRule(Class<? extends Sort> sortClass, Class<? extends Project> projectClass,
                                    String description) {
        super(operand(sortClass, operand(projectClass, any())), description);
    }
    ...
    public void onMatch(RelOptRuleCall call) {
        ...
        for (RelFieldCollation fc : sort.getCollation().getFieldCollations()) {
            ...
            if (node.isA(SqlKind.CAST)) {
                ...
                final RexCallBinding binding = RexCallBinding.create(cluster.getTypeFactory(), cast, ImmutableList.of(
                        RelCollations.of(RexUtil.apply(map, fc))));
        ...
        final Sort newSort = sort.copy(sort.getTraitSet().replace(newCollation), project.getInput(), newCollation,
        ...
        if (sort.offset == null && sort.fetch == null && cluster.getPlanner().getRelTraitDefs().contains(
