...
public class RexBuilder {
    ...
    private static final Function<RelDataTypeField, RexInputRef> TO_INPUT_REF = new Function<RelDataTypeField, RexInputRef>() {
    ...
    public RexBuilder(RelDataTypeFactory typeFactory) {
        ...
        this.booleanTrue = makeLiteral(Boolean.TRUE, typeFactory.createSqlType(SqlTypeName.BOOLEAN),
                                       SqlTypeName.BOOLEAN);
        this.booleanFalse = makeLiteral(Boolean.FALSE, typeFactory.createSqlType(SqlTypeName.BOOLEAN),
                                        SqlTypeName.BOOLEAN);
        this.charEmpty = makeLiteral(new NlsString("", null, null), typeFactory.createSqlType(SqlTypeName.CHAR, 0),
                                     SqlTypeName.CHAR);
        this.constantNull = makeLiteral(null, typeFactory.createSqlType(SqlTypeName.NULL), SqlTypeName.NULL);
    }
    ...
    private RexNode makeFieldAccessInternal(RexNode expr, final RelDataTypeField field) {
        if (expr instanceof RexRangeRef) {
            ...
            if (field.getIndex() < 0) {
                return makeCall(field.getType(), GET_OPERATOR, ImmutableList.of(expr, makeLiteral(field.getName())));
            }
    ...
    public RelDataType deriveReturnType(SqlOperator op, List<? extends RexNode> exprs) {
        return op.inferReturnType(new RexCallBinding(typeFactory, op, exprs, ImmutableList.<RelCollation>of()));
    }
     ...
     */
    public RexNode addAggCall(AggregateCall aggCall, int groupCount, boolean indicator, List<AggregateCall> aggCalls,
                              Map<AggregateCall, RexNode> aggCallMapping, final List<RelDataType> aggArgTypes) {
     ...
     */
    public RexNode makeOver(RelDataType type, SqlAggFunction operator, List<RexNode> exprs, List<RexNode> partitionKeys,
                            ImmutableList<RexFieldCollation> orderKeys, RexWindowBound lowerBound,
                            RexWindowBound upperBound, boolean physical, boolean allowPartial,
                            boolean nullWhenCountZero, boolean distinct) {
        ...
        if (nullWhenCountZero) {
            ...
            result = makeCall(SqlStdOperatorTable.CASE, makeCall(SqlStdOperatorTable.GREATER_THAN,
                                                                 new RexOver(bigintType, SqlStdOperatorTable.COUNT,
                                                                             exprs, window, distinct),
                                                                 makeLiteral(BigDecimal.ZERO, bigintType,
                                                                             SqlTypeName.DECIMAL)),
                              ensureType(type, // SUM0 is non-nullable, thus need a cast
                                         new RexOver(typeFactory.createTypeWithNullability(type, false), operator,
                                                     exprs, window, distinct), false), makeCast(type, constantNull()));
        }
        if (!allowPartial) {
            Preconditions.checkArgument(physical, "DISALLOW PARTIAL over RANGE");
            final RelDataType bigintType = getTypeFactory().createSqlType(SqlTypeName.BIGINT);
            // todo: read bound
            result = makeCall(SqlStdOperatorTable.CASE, makeCall(SqlStdOperatorTable.GREATER_THAN_OR_EQUAL,
                                                                 new RexOver(bigintType, SqlStdOperatorTable.COUNT,
                                                                             ImmutableList.<RexNode>of(), window,
                                                                             distinct),
                                                                 makeLiteral(BigDecimal.valueOf(2), bigintType,
                                                                             SqlTypeName.DECIMAL)), result,
                              constantNull);
        }
        return result;
    }
     ...
     */
    public RexWindow makeWindow(List<RexNode> partitionKeys, ImmutableList<RexFieldCollation> orderKeys,
    ...
    boolean canRemoveCastFromLiteral(RelDataType toType, Comparable value, SqlTypeName fromTypeName) {
        ...
        if (toType.getSqlTypeName() != fromTypeName && SqlTypeFamily.DATETIME.getTypeNames().contains(fromTypeName)) {
            return false;
        }
    ...
    private RexNode makeCastBooleanToExact(RelDataType toType, RexNode exp) {
        final RexNode casted = makeCall(SqlStdOperatorTable.CASE, exp, makeExactLiteral(BigDecimal.ONE, toType),
                                        makeZeroLiteral(toType));
        if (!exp.getType().isNullable()) {
            return casted;
        }
        return makeCall(toType, SqlStdOperatorTable.CASE,
                        ImmutableList.of(makeCall(SqlStdOperatorTable.IS_NOT_NULL, exp), casted,
    ...
    private RexNode makeCastIntervalToExact(RelDataType toType, RexNode exp) {
        ...
        if (scale > 0) {
            RelDataType decimalType = getTypeFactory().createSqlType(SqlTypeName.DECIMAL,
    ...
    public RexLiteral makeBinaryLiteral(ByteString byteString) {
        return makeLiteral(byteString, typeFactory.createSqlType(SqlTypeName.BINARY, byteString.length()),
                           SqlTypeName.BINARY);
    }
    ...
    protected RexLiteral makePreciseStringLiteral(String s) {
        ...
        if (s.equals("")) {
            ...
            return makeLiteral(new NlsString(s, null, null), typeFactory.createSqlType(SqlTypeName.CHAR, s.length()),
                               SqlTypeName.CHAR);
        }
    }
    ...
    public RexNode ensureType(RelDataType type, RexNode node, boolean matchNullability) {
        ...
        if (targetType.getSqlTypeName() == SqlTypeName.ANY && (!matchNullability || targetType.isNullable()
    ...
    public RexLiteral makeDateLiteral(DateString date) {
        return makeLiteral(Preconditions.checkNotNull(date), typeFactory.createSqlType(SqlTypeName.DATE),
                           SqlTypeName.DATE);
    }
    ...
    public RexLiteral makeTimeLiteral(TimeString time, int precision) {
        return makeLiteral(Preconditions.checkNotNull(time), typeFactory.createSqlType(SqlTypeName.TIME, precision),
                           SqlTypeName.TIME);
    }
    ...
    public RexLiteral makeTimestampLiteral(TimestampString timestamp, int precision) {
        return makeLiteral(Preconditions.checkNotNull(timestamp),
                           typeFactory.createSqlType(SqlTypeName.TIMESTAMP, precision), SqlTypeName.TIMESTAMP);
    }
    ...
    public RexLiteral makeIntervalLiteral(BigDecimal v, SqlIntervalQualifier intervalQualifier) {
        return makeLiteral(v, typeFactory.createSqlIntervalType(intervalQualifier), intervalQualifier.typeName());
    }
        ...
        switch (type.getSqlTypeName()) {
            ...
            case MULTISET:
                ...
                for (Object entry : (List) value) {
                    final RexNode e = entry instanceof RexLiteral ? (RexNode) entry : makeLiteral(entry,
            ...
            case ROW:
                ...
                for (Pair<RelDataTypeField, Object> pair : Pair.zip(type.getFieldList(), (List<Object>) value)) {
                    final RexNode e = pair.right instanceof RexLiteral ? (RexNode) pair.right : makeLiteral(pair.right,
                                                                                                            pair.left.getType(),
                                                                                                            allowCast);
                    operands.add(e);
                }
