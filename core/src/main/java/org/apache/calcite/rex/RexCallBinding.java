...
public class RexCallBinding extends SqlOperatorBinding {
     ...
     */
    public static RexCallBinding create(RelDataTypeFactory typeFactory, RexCall call,
                                        List<RelCollation> inputCollations) {
        switch (call.getKind()) {
            case CAST:
                return new RexCastCallBinding(typeFactory, call.getOperator(), call.getOperands(), call.getType(),
                                              inputCollations);
        }
    ...
    @SuppressWarnings("deprecation") @Override public String getStringLiteralOperand(int ordinal) {
        return RexLiteral.stringValue(operands.get(ordinal));
    }

    @SuppressWarnings("deprecation") @Override public int getIntLiteralOperand(int ordinal) {
    ...
    @Override public SqlMonotonicity getOperandMonotonicity(int ordinal) {
        ...
        if (operand instanceof RexInputRef) {
            ...
            final RexCallBinding binding = RexCallBinding.create(typeFactory, (RexCall) operand, inputCollations);
    ...
    private static class RexCastCallBinding extends RexCallBinding {
        ...
        RexCastCallBinding(RelDataTypeFactory typeFactory, SqlOperator sqlOperator, List<? extends RexNode> operands,
                           RelDataType type, List<RelCollation> inputCollations) {
