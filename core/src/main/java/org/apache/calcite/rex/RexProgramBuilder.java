...
public class RexProgramBuilder {
    ...
    private final Map<Pair<String, String>, RexLocalRef> exprMap         = new HashMap<>();
    ...
    private RexProgramBuilder(RexBuilder rexBuilder, final RelDataType inputRowType, final List<RexNode> exprList,
                              final Iterable<? extends RexNode> projectList, RexNode condition,
        ...
        if (condition != null) {
            if (simplify != null) {
                condition = simplify.simplify(
                        rexBuilder.makeCall(SqlStdOperatorTable.IS_TRUE, condition.accept(expander)));
    ...
    public RexProgram getProgram(boolean normalize) {
        ...
        if (normalize) {
            return create(rexBuilder, inputRowType, exprList, projectRefList, conditionRef, outputRowType,
                          true).getProgram(false);
        }
    ...
    public static RexProgramBuilder create(RexBuilder rexBuilder, final RelDataType inputRowType,
                                           final List<RexNode> exprList, final List<? extends RexNode> projectList,
                                           final RexNode condition, final RelDataType outputRowType, boolean normalize,
    ...
    public static RexProgramBuilder create(RexBuilder rexBuilder, final RelDataType inputRowType,
                                           ...
                                           boolean simplify) {
        return new RexProgramBuilder(rexBuilder, inputRowType, exprList, projectList, condition, outputRowType,
                                     normalize, simplify ? new RexSimplify(rexBuilder, false, RexUtil.EXECUTOR) : null);
    }
     ...
     */
    public static RexProgramBuilder create(RexBuilder rexBuilder, final RelDataType inputRowType,
                                           final List<RexNode> exprList, final List<RexLocalRef> projectRefList,
                                           final RexLocalRef conditionRef, final RelDataType outputRowType,
                                           final RexShuttle shuttle, final boolean updateRefs) {
    ...
    public void addIdentity() {
        ...
        for (RelDataTypeField field : inputRowType.getFieldList()) {
            addProject(new RexInputRef(field.getIndex(), field.getType()), field.getName());
        }
    }
        ...
        public RexNode visitInputRef(RexInputRef input) {
            ...
            if (valid) {
                ...
                if ((index < 0) || (index >= inputRowType.getFieldCount())) {
                    assert false :
                            "RexInputRef index " + index + " out of range 0.." + (inputRowType.getFieldCount() - 1);
                }
