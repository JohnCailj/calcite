...
public class RexSimplify {
    ...
    private RexNode simplifyComparison(RexCall e) {
        ...
        if (o0.isA(SqlKind.LITERAL) && o1.isA(SqlKind.LITERAL) && o0.getType().equals(o1.getType())) {
            ...
            @SuppressWarnings("unchecked") final int comparisonResult = v0.compareTo(v1);
            switch (e.getKind()) {
                case EQUALS:
    ...
    private RexNode simplifyNot(RexCall call) {
        ...
        if (a.getKind() != negateKind) {
            return simplify(
                    rexBuilder.makeCall(RexUtil.op(negateKind), ImmutableList.of(((RexCall) a).getOperands().get(0))));
        }
    ...
    private RexNode simplifyCase(RexCall call) {
        ...
        if (call.getType().getSqlTypeName() == SqlTypeName.BOOLEAN) {
            ...
            for (Ord<Pair<RexNode, RexNode>> pair : Ord.zip(pairs)) {
                ...
                if (!pair.e.getValue().isAlwaysTrue() && !pair.e.getValue().isAlwaysFalse() && (!unknownAsFalse
                                                                                                || !RexUtil.isNull(
                        pair.e.getValue()))) {
                    break trueFalse;
                }
    ...
    private static RexNode processRange(RexBuilder rexBuilder, List<RexNode> terms,
                                        Map<String, Pair<Range, List<RexNode>>> rangeTerms, RexNode term, RexNode ref,
                                        RexLiteral constant, SqlKind comparison) {
