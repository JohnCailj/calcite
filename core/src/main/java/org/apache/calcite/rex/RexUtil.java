...
public class RexUtil {
     ...
     */
    public static final RexExecutor EXECUTOR = new RexExecutorImpl(Schemas.createDataContext(null, null));
    ...
    private static <C extends RexNode> void gatherConstraint(Class<C> clazz, RexNode left, RexNode right,
                                                             Map<RexNode, C> map, Set<RexNode> excludeSet,
        ...
        if (existedValue == null) {
            ...
            if (existedValue instanceof RexLiteral && constant instanceof RexLiteral && !Objects.equals(
                    ((RexLiteral) existedValue).getValue(), ((RexLiteral) constant).getValue())) {
     ...
     */
    @Nonnull public static RexNode composeDisjunction(RexBuilder rexBuilder, Iterable<? extends RexNode> nodes) {
    ...
    private static boolean isFlat(List<? extends RexNode> exprs, final SqlOperator op) {
        return !isAssociative(op) || !exists(exprs, new Predicate1<RexNode>() {
    ...
    public static boolean isFlat(RexNode expr) {
        ...
        return isFlat(call.getOperands(), call.getOperator()) && all(call.getOperands(), IS_FLAT_PREDICATE);
    }
    ...
    public static RexNode simplifyPreservingType(RexBuilder rexBuilder, RexNode e) {
        return new RexSimplify(rexBuilder, false, EXECUTOR).simplifyPreservingType(e);
    }
    ...
    public static RexNode simplify(RexBuilder rexBuilder, RexNode e) {
        return new RexSimplify(rexBuilder, false, EXECUTOR).simplify(e);
    }
    ...
    public static RexNode simplify(RexBuilder rexBuilder, RexNode e, boolean unknownAsFalse) {
        return new RexSimplify(rexBuilder, unknownAsFalse, EXECUTOR).simplify(e);
    }
    ...
    public static RexNode simplifyAnds(RexBuilder rexBuilder, Iterable<? extends RexNode> nodes) {
        return new RexSimplify(rexBuilder, false, EXECUTOR).simplifyAnds(nodes);
    }

    @Deprecated // to be removed before 2.0
    public static RexNode simplifyAnds(RexBuilder rexBuilder, Iterable<? extends RexNode> nodes,
                                       boolean unknownAsFalse) {
        return new RexSimplify(rexBuilder, unknownAsFalse, EXECUTOR).simplifyAnds(nodes);
    }
    ...
    public static RexNode simplifyAnd(RexBuilder rexBuilder, RexCall e, boolean unknownAsFalse) {
        return new RexSimplify(rexBuilder, unknownAsFalse, EXECUTOR).simplifyAnd(e);
    }

    @Deprecated // to be removed before 2.0
    public static RexNode simplifyAnd2(RexBuilder rexBuilder, List<RexNode> terms, List<RexNode> notTerms) {
        return new RexSimplify(rexBuilder, false, EXECUTOR).simplifyAnd2(terms, notTerms);
    }

    @Deprecated // to be removed before 2.0
    public static RexNode simplifyAnd2ForUnknownAsFalse(RexBuilder rexBuilder, List<RexNode> terms,
                                                        List<RexNode> notTerms) {
        return new RexSimplify(rexBuilder, true, EXECUTOR).simplifyAnd2ForUnknownAsFalse(terms, notTerms);
    }
    ...
    public static RexNode simplifyOr(RexBuilder rexBuilder, RexCall call) {
        return new RexSimplify(rexBuilder, false, EXECUTOR).simplifyOr(call);
    }

    @Deprecated // to be removed before 2.0
    public static RexNode simplifyOrs(RexBuilder rexBuilder, List<RexNode> terms) {
        return new RexSimplify(rexBuilder, false, EXECUTOR).simplifyOrs(terms);
    }
    ...
    public static RexNode andNot(final RexBuilder rexBuilder, RexNode e, Iterable<? extends RexNode> notTerms) {
        ...
        switch (e.getKind()) {
            case EQUALS:
                ...
                if (call.getOperands().get(1) instanceof RexLiteral) {
                    notTerms = Iterables.filter(notTerms, new PredicateImpl<RexNode>() {
        ...
        return composeConjunction(rexBuilder, Iterables.concat(ImmutableList.of(e),
    ...
    public static Function<RexNode, RexNode> notFn(final RexBuilder rexBuilder) {
        return new Function<RexNode, RexNode>() {
            ...
            public RexNode apply(RexNode input) {
                return input.isAlwaysTrue() ? rexBuilder.makeLiteral(
                        false) : input.isAlwaysFalse() ? rexBuilder.makeLiteral(true) :
                        input.getKind() == SqlKind.NOT ? ((RexCall) input).operands.get(0) : rexBuilder.makeCall(
                                SqlStdOperatorTable.NOT, input);
            }
        };
    }
    ...
    private static class CnfHelper {
         ...
         */
        @SuppressWarnings("serial") private static class OverflowError extends ControlFlowException {

            @SuppressWarnings("ThrowableInstanceNeverThrown") protected static final OverflowError INSTANCE = new OverflowError();
    ...
    public static class SubQueryFinder extends RexVisitorImpl<Void> {
         ...
         */
        public static final Predicate<Join> JOIN_PREDICATE = new PredicateImpl<Join>() {
    ...
    public static class ExprSimplifier extends RexShuttle {
        ...
        @Override public RexNode visitCall(RexCall call) {
            ...
            RexNode simplifiedNode = simplify.withUnknownAsFalse(unknownAsFalseCall).simplify(node);
