...
public class Resources {
    ...
    private static <T> T create(final String base, final PropertyAccessor accessor, Class<T> clazz) {
            ...
            private Object create(Method method, Object[] args)
                ...
                try {
                    if (Inst.class.isAssignableFrom(returnType)) {
                        final Constructor<?> constructor = returnType.getConstructor(String.class, Locale.class,
                                                                                     Method.class, Object[].class);
                        final Locale locale = Resources.getThreadOrDefaultLocale();
                        return constructor.newInstance(base, locale, method, args != null ? args : new Object[0]);
                    } else {
                        final Constructor<?> constructor = returnType.getConstructor(PropertyAccessor.class,
                                                                                     Method.class);
    ...
    public static void validate(Object o, EnumSet<Validation> validations) {
        ...
        if (count == 0 && validations.contains(Validation.AT_LEAST_ONE)) {
            throw new AssertionError("resource object " + o + " contains no resources");
        }
    }

    private static Object zero(Class<?> clazz) {
        return clazz == String.class ? "" : clazz == byte.class ? (byte) 0 : clazz == char.class ? (char) 0 :
                clazz == short.class ? (short) 0 : clazz == int.class ? 0 : clazz == long.class ? 0L :
                        clazz == float.class ? 0F : clazz == double.class ? 0D : clazz == boolean.class ? false : null;
    }
    ...
    public static class Inst extends Element {
        ...
        @Override public boolean equals(Object obj) {
            return this == obj || obj != null && obj.getClass() == this.getClass() && locale == ((Inst) obj).locale
                                  && method == ((Inst) obj).method && Arrays.equals(args, ((Inst) obj).args);
        }
                    ...
                    case BUNDLE_HAS_RESOURCE:
                        if (!bundle.containsKey(key)) {
                            throw new AssertionError(
                                    "key '" + key + "' not found for resource '" + method.getName() + "' in bundle '"
                                    + bundle + "'");
                        }
                        break;
                    case MESSAGE_SPECIFIED:
                        final BaseMessage annotation1 = method.getAnnotation(BaseMessage.class);
                        if (annotation1 == null) {
                            throw new AssertionError("resource '" + method.getName() + "' must specify BaseMessage");
                        }
                        break;
                    case EVEN_QUOTES:
                        ...
                        if (countQuotesIn(message) % 2 == 1) {
                            throw new AssertionError(
                                    "resource '" + method.getName() + "' should have even number of quotes");
                        }
                        break;
                    case MESSAGE_MATCH:
                        final BaseMessage annotation2 = method.getAnnotation(BaseMessage.class);
                        if (annotation2 != null) {
                            final String value = annotation2.value();
                            final String value2 = bundle.containsKey(key) ? bundle.getString(key) : null;
                            if (!equal(value, value2)) {
                                throw new AssertionError("message for resource '" + method.getName()
                    ...
                    case ARGUMENT_MATCH:
                        ...
                        for (int i = 0; i < formats.length; i++) {
                            ...
                            if (format1 instanceof NumberFormat) {
                                e = parameterType == short.class || parameterType == int.class
                                    || parameterType == long.class || parameterType == float.class
                                    || parameterType == double.class || Number.class.isAssignableFrom(
                                        parameterType) ? parameterType : Number.class;
                        ...
                        if (!types.equals(parameterTypeList)) {
                            throw new AssertionError("type mismatch in method '" + method.getName()
                                                     + "' between message format elements " + types
                                                     + " and method parameters " + parameterTypeList);
                        }
                        break;
                }
            }
        ...
        public static Class getExceptionClass(Type type) {
            ...
            for (; ; ) {
                if (type instanceof ParameterizedType) {
                    ...
                    if (types.length >= 1 && types[0] instanceof Class && Throwable.class.isAssignableFrom(
                            (Class) types[0])) {
        ...
        protected void validateException(Callable<Exception> exSupplier) {
            ...
            if (cause != null) {
                AssertionError assertionError = new AssertionError(
                        "error instantiating exception for resource '" + method.getName() + "'");
    ...
    public abstract static class Prop extends Element {
        ...
        void checkDefault() {
            if (!hasDefault) {
                throw new NoDefaultValueException("Property " + key + " has no default value");
            }
        }
     ...
     */
    @Retention(RetentionPolicy.RUNTIME) @Target(ElementType.METHOD) public @interface BaseMessage {
     ...
     */
    @Retention(RetentionPolicy.RUNTIME) @Target(ElementType.METHOD) public @interface Resource {
     ...
     */
    @Retention(RetentionPolicy.RUNTIME) @Target(ElementType.METHOD) public @interface Property {
     ...
     */
    @Retention(RetentionPolicy.RUNTIME) @Target(ElementType.METHOD) public @interface Default {
    ...
    public abstract static class ShadowResourceBundle extends ResourceBundle {
        ...
        protected ShadowResourceBundle() throws IOException {
            ...
            while (true) {
                ...
                if (clazz == null || clazz == ShadowResourceBundle.class || !ResourceBundle.class.isAssignableFrom(
                        clazz)) {
                    break;
                }
        ...
        protected static ShadowResourceBundle instance(String baseName, Locale locale, ResourceBundle bundle) {
            if (bundle instanceof PropertyResourceBundle) {
                throw new ClassCastException(
                        "ShadowResourceBundle.instance('" + baseName + "','" + locale + "') found " + baseName + "_"
                        + locale + ".properties but not " + baseName + "_" + locale + ".class");
            }
        ...
        public static Method lookupMethod(Class clazz, String methodName, Class... argumentTypes) {
            try {
                ...
                throw new RuntimeException(
                        "while resolving method '" + methodName + Arrays.toString(argumentTypes) + "' in class "
                        + clazz, e);
            }
        }
    }
