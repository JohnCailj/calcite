...
public final class Schemas {
    ...
    private static final com.google.common.base.Function<CalciteSchema.LatticeEntry, CalciteSchema.TableEntry> TO_TABLE_ENTRY = new com.google.common.base.Function<CalciteSchema.LatticeEntry, CalciteSchema.TableEntry>() {
    ...
    private static final com.google.common.base.Function<CalciteSchema.LatticeEntry, Lattice> TO_LATTICE = new com.google.common.base.Function<CalciteSchema.LatticeEntry, Lattice>() {
    ...
    public static Expression subSchemaExpression(SchemaPlus schema, String name, Class type) {
        ...
        Expression call = Expressions.call(schemaExpression, BuiltInMethod.SCHEMA_GET_SUB_SCHEMA.method,
    ...
    public static Expression unwrap(Expression call, Class type) {
        return Expressions.convert_(
                Expressions.call(call, BuiltInMethod.SCHEMA_PLUS_UNWRAP.method, Expressions.constant(type)), type);
    }
    ...
    public static Expression tableExpression(SchemaPlus schema, Type elementType, String tableName, Class clazz) {
        ...
        if (Table.class.isAssignableFrom(clazz)) {
            expression = Expressions.call(expression(schema), BuiltInMethod.SCHEMA_GET_TABLE.method,
                                          Expressions.constant(tableName));
            if (ScannableTable.class.isAssignableFrom(clazz)) {
                return Expressions.call(BuiltInMethod.SCHEMAS_ENUMERABLE_SCANNABLE.method,
                                        Expressions.convert_(expression, ScannableTable.class), DataContext.ROOT);
            }
            if (FilterableTable.class.isAssignableFrom(clazz)) {
                return Expressions.call(BuiltInMethod.SCHEMAS_ENUMERABLE_FILTERABLE.method,
            ...
            if (ProjectableFilterableTable.class.isAssignableFrom(clazz)) {
                return Expressions.call(BuiltInMethod.SCHEMAS_ENUMERABLE_PROJECTABLE_FILTERABLE.method,
                                        Expressions.convert_(expression, ProjectableFilterableTable.class),
                                        DataContext.ROOT);
            }
        } else {
            expression = Expressions.call(BuiltInMethod.SCHEMAS_QUERYABLE.method, DataContext.ROOT, expression(schema),
                                          Expressions.constant(elementType), Expressions.constant(tableName));
        }
     ...
     */
    public static CalcitePrepare.ConvertResult convert(final CalciteConnection connection, final CalciteSchema schema,
                                                       final List<String> schemaPath, final String sql) {
        final CalcitePrepare prepare = CalcitePrepare.DEFAULT_FACTORY.apply();
        final ImmutableMap<CalciteConnectionProperty, String> propValues = ImmutableMap.of();
        final CalcitePrepare.Context context = makeContext(connection, schema, schemaPath, null, propValues);
     ...
     * @return Context
     */
    private static CalcitePrepare.Context makeContext(CalciteConnection connection, CalciteSchema schema,
        ...
        if (connection == null) {
            ...
            return makeContext(config, connection.getTypeFactory(), createDataContext(connection, schema.root().plus()),
                               schema, schemaPath, objectPath);
        }
    }
    ...
    private static CalcitePrepare.Context makeContext(final CalciteConnectionConfig connectionConfig,
                                                      final JavaTypeFactory typeFactory, final DataContext dataContext,
                                                      final CalciteSchema schema, final List<String> schemaPath,
