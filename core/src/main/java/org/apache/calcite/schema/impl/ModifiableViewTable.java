...
public class ModifiableViewTable extends ViewTable implements ModifiableView, Wrapper {
    ...
    public final ModifiableViewTable extend(List<RelDataTypeField> extendedColumns, RelDataTypeFactory typeFactory) {
        ...
        final ImmutableIntList newColumnMapping = getNewColumnMapping(underlying, getColumnMapping(), extendedColumns,
                                                                      typeFactory);
        ...
        final List<RelDataTypeField> columnsOfExtendedBaseTable = RelOptUtil.deduplicateColumns(underlyingColumns,
                                                                                                extendedColumns);
        final List<RelDataTypeField> extendColumnsOfBaseTable = columnsOfExtendedBaseTable.subList(
    ...
    private class ModifiableViewTableInitializerExpressionFactory extends NullInitializerExpressionFactory {
        ...
        @Override public RexNode newColumnDefaultValue(RelOptTable table, int iColumn, InitializerContext context) {
            ...
            if (schemaTable instanceof Wrapper) {
                final InitializerExpressionFactory initializerExpressionFactory = ((Wrapper) schemaTable).unwrap(
                        InitializerExpressionFactory.class);
                if (initializerExpressionFactory != null) {
                    final RexNode tableConstraint = initializerExpressionFactory.newColumnDefaultValue(table, iColumn,
                                                                                                       context);
