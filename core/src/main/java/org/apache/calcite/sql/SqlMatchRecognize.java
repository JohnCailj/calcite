...
public class SqlMatchRecognize extends SqlCall {
    ...
    public static final SqlPrefixOperator SKIP_TO_FIRST = new SqlPrefixOperator("SKIP TO FIRST", SqlKind.SKIP_TO_FIRST,
                                                                                20, null, null, null);

    public static final SqlPrefixOperator SKIP_TO_LAST = new SqlPrefixOperator("SKIP TO LAST", SqlKind.SKIP_TO_LAST, 20,
                                                                               null, null, null);
     ...
     */
    public SqlMatchRecognize(SqlParserPos pos, SqlNode tableRef, SqlNode pattern, SqlLiteral strictStart,
                             SqlLiteral strictEnd, SqlNodeList patternDefList, SqlNodeList measureList, SqlNode after,
                             SqlNodeList subsetList, SqlLiteral rowsPerMatch, SqlNodeList partitionList,
    ...
    @Override public List<SqlNode> getOperandList() {
        return ImmutableNullableList.of(tableRef, pattern, strictStart, strictEnd, patternDefList, measureList, after,
                                        subsetList, partitionList, orderList);
    }
    ...
    public static class SqlMatchRecognizeOperator extends SqlOperator {
        ...
        @Override public SqlCall createCall(SqlLiteral functionQualifier, SqlParserPos pos, SqlNode... operands) {
            ...
            return new SqlMatchRecognize(pos, operands[0], operands[1], (SqlLiteral) operands[2],
                                         (SqlLiteral) operands[3], (SqlNodeList) operands[4], (SqlNodeList) operands[5],
                                         operands[6], (SqlNodeList) operands[7], (SqlLiteral) operands[8],
                                         (SqlNodeList) operands[9], (SqlNodeList) operands[10],
                                         (SqlLiteral) operands[11]);
        }

        @Override public <R> void acceptCall(SqlVisitor<R> visitor, SqlCall call, boolean onlyExpressions,
        ...
        @Override public void unparse(SqlWriter writer, SqlCall call, int leftPrec, int rightPrec) {
            ...
            for (SqlNode node : pattern.getPatternDefList()) {
                ...
                newDefineList.add(
                        call2.getOperator().createCall(SqlParserPos.ZERO, call2.operand(1), call2.operand(0)));
            }
