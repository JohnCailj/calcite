...
public class SqlSelectOperator extends SqlOperator {
    ...
    public SqlCall createCall(SqlLiteral functionQualifier, SqlParserPos pos, SqlNode... operands) {
        ...
        return new SqlSelect(pos, (SqlNodeList) operands[0], (SqlNodeList) operands[1], operands[2], operands[3],
                             (SqlNodeList) operands[4], operands[5], (SqlNodeList) operands[6],
                             (SqlNodeList) operands[7], operands[8], operands[9]);
    }
     ...
     */
    public SqlSelect createCall(SqlNodeList keywordList, SqlNodeList selectList, SqlNode fromClause,
                                SqlNode whereClause, SqlNodeList groupBy, SqlNode having, SqlNodeList windowDecls,
                                SqlNodeList orderBy, SqlNode offset, SqlNode fetch, SqlParserPos pos) {
        return new SqlSelect(pos, keywordList, selectList, fromClause, whereClause, groupBy, having, windowDecls,
    ...
    public void unparse(SqlWriter writer, SqlCall call, int leftPrec, int rightPrec) {
        ...
        if (select.where != null) {
            ...
            if (!writer.isAlwaysUseParentheses()) {
                ...
                unparseListClause(writer, new SqlNodeList(list, select.where.getParserPosition()), whereSepKind);
