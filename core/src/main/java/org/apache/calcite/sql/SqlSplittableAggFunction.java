...
public interface SqlSplittableAggFunction {
     ...
     */
    AggregateCall topSplit(RexBuilder rexBuilder, Registry<RexNode> extra, int offset, RelDataType inputRowType,
    ...
    class CountSplitter implements SqlSplittableAggFunction {
        ...
        public AggregateCall other(RelDataTypeFactory typeFactory, AggregateCall e) {
            return AggregateCall.create(SqlStdOperatorTable.COUNT, false, ImmutableIntList.of(), -1,
                                        typeFactory.createSqlType(SqlTypeName.BIGINT), null);
        }

        public AggregateCall topSplit(RexBuilder rexBuilder, Registry<RexNode> extra, int offset,
                                      RelDataType inputRowType, AggregateCall aggregateCall, int leftSubTotal,
                                      int rightSubTotal) {
            ...
            return AggregateCall.create(SqlStdOperatorTable.SUM0, false, ImmutableList.of(ordinal), -1,
        ...
        public RexNode singleton(RexBuilder rexBuilder, RelDataType inputRowType, AggregateCall aggregateCall) {
            ...
            for (Integer arg : aggregateCall.getArgList()) {
                ...
                if (type.isNullable()) {
                    predicates.add(
                            rexBuilder.makeCall(SqlStdOperatorTable.IS_NOT_NULL, rexBuilder.makeInputRef(type, arg)));
                }
            }
    ...
    class SumSplitter implements SqlSplittableAggFunction {
        ...
        public AggregateCall topSplit(RexBuilder rexBuilder, Registry<RexNode> extra, int offset,
            ...
            int ordinal = extra.register(node);
            return AggregateCall.create(SqlStdOperatorTable.SUM, false, ImmutableList.of(ordinal), -1,
                                        aggregateCall.type, aggregateCall.name);
        }
    }
}
