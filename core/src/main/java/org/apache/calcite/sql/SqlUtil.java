...
public abstract class SqlUtil {
    ...
    public static void unparseBinarySyntax(SqlOperator operator, SqlCall call, SqlWriter writer, int leftPrec,
        ...
        final SqlWriter.Frame frame = writer.startList(
                (operator instanceof SqlSetOperator) ? SqlWriter.FrameTypeEnum.SETOP : SqlWriter.FrameTypeEnum.SIMPLE);
     ...
     */
    public static SqlOperator lookupRoutine(SqlOperatorTable opTab, SqlIdentifier funcName, List<RelDataType> argTypes,
                                            List<String> argNames, SqlFunctionCategory category, SqlSyntax syntax,
                                            SqlKind sqlKind) {
        ...
        return (Iterator) Iterators.filter(Iterators.filter(routines, SqlFunction.class),
                                           new PredicateImpl<SqlFunction>() {
                                               ...
                                               public boolean test(SqlFunction function) {
                                                   ...
                                                   for (Pair<RelDataType, RelDataType> p : Pair.zip(paramTypes,
                                                                                                    permutedArgTypes)) {
                                                       final RelDataType argType = p.right;
                                                       final RelDataType paramType = p.left;
                                                       if (argType != null && !SqlTypeUtil.canCastFrom(paramType,
                                                                                                       argType,
                                                                                                       false)) {
                                                           return false;
                                                       }
    ...
    public static CalciteContextException newContextException(int line, int col, int endLine, int endCol,
                                                              Resources.ExInst<?> e) {
        CalciteContextException contextExcn = (line == endLine && col == endCol ? RESOURCE.validatorContextPoint(line,
                                                                                                                 col) : RESOURCE.validatorContext(
                line, col, endLine, endCol)).ex(e.ex());
