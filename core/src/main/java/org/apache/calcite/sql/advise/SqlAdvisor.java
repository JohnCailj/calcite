...
public class SqlAdvisor {
    ...
    private static boolean isSelectListItem(SqlNode root, final SqlParserPos pos) {
        List<SqlNode> nodes = SqlUtil.getAncestry(root, new PredicateImpl<SqlNode>() {

            public boolean test(SqlNode input) {
                return input instanceof SqlIdentifier && Util.last(((SqlIdentifier) input).names).equals(
                        UPPER_HINT_TOKEN);
            }
        ...
        return nodes.size() > 2 && nodes.get(2) instanceof SqlSelect && nodes.get(1) == ((SqlSelect) nodes.get(
                2)).getSelectList();
    }
    ...
    public class ValidateErrorInfo {
         ...
         */
        public ValidateErrorInfo(int startLineNum, int startColumnNum, int endLineNum, int endColumnNum,
                                 String errorMsg) {
