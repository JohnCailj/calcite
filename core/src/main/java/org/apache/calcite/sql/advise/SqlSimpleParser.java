...
public class SqlSimpleParser {
    ...
    private void consumeSelect(ListIterator<Token> iter, List<Token> outList) {
        ...
        if (isQuery) {
            ...
            if ((outList.size() >= 2) && (outList.get(outList.size() - 2).type == TokenType.LPAREN)) {
    ...
    static class Query extends Token {
        ...
        private void purgeSelectListExcept(String hintToken) {
            ...
            if (found) {
                ...
                List<Token> selectItem = new ArrayList<Token>(sublist.subList(itemStart, itemEnd));
        ...
        private void purgeFromExcept(String hintToken) {
            ...
            if (found && (joinCount == 0)) {
                ...
                List<Token> fromItem = new ArrayList<Token>(sublist.subList(itemStart, itemEnd));
        ...
        private List<Token> findClause(TokenType keyword) {
            int start = -1;
            int k = -1;
            EnumSet<TokenType> clauses = EnumSet.of(TokenType.SELECT, TokenType.FROM, TokenType.WHERE, TokenType.GROUP,
                                                    TokenType.HAVING, TokenType.ORDER);
            for (Token token : tokenList) {
                ++k;
                if (token.type == keyword) {
                    start = k;
                } else if ((start >= 0) && clauses.contains(token.type)) {
