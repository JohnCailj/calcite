...
public final class SqlParserUtil {
    ...
    public static SqlDateLiteral parseDateLiteral(String s, SqlParserPos pos) {
        ...
        final Calendar cal = DateTimeUtils.parseDateFormat(dateStr, Format.PER_THREAD.get().date,
                                                           DateTimeUtils.UTC_ZONE);
        if (cal == null) {
            throw SqlUtil.newContextException(pos, RESOURCE.illegalLiteral("DATE", s, RESOURCE.badFormat(
                    DateTimeUtils.DATE_FORMAT_STRING).str()));
        }
    ...
    public static SqlTimeLiteral parseTimeLiteral(String s, SqlParserPos pos) {
        ...
        final DateTimeUtils.PrecisionTime pt = DateTimeUtils.parsePrecisionDateTimeLiteral(dateStr,
                                                                                           Format.PER_THREAD.get().time,
                                                                                           DateTimeUtils.UTC_ZONE, -1);
        if (pt == null) {
            throw SqlUtil.newContextException(pos, RESOURCE.illegalLiteral("TIME", s, RESOURCE.badFormat(
                    DateTimeUtils.TIME_FORMAT_STRING).str()));
        }
    ...
    public static SqlTimestampLiteral parseTimestampLiteral(String s, SqlParserPos pos) {
        final String dateStr = parseString(s);
        final DateTimeUtils.PrecisionTime pt = DateTimeUtils.parsePrecisionDateTimeLiteral(dateStr,
                                                                                           Format.PER_THREAD.get().timestamp,
                                                                                           DateTimeUtils.UTC_ZONE, -1);
        if (pt == null) {
            throw SqlUtil.newContextException(pos, RESOURCE.illegalLiteral("TIMESTAMP", s, RESOURCE.badFormat(
                    DateTimeUtils.TIMESTAMP_FORMAT_STRING).str()));
        }
    ...
    public static SqlIntervalLiteral parseIntervalLiteral(SqlParserPos pos, int sign, String s,
                                                          SqlIntervalQualifier intervalQualifier) {
        final String intervalStr = parseString(s);
        if (intervalStr.equals("")) {
            throw SqlUtil.newContextException(pos,
                                              RESOURCE.illegalIntervalLiteral(s + " " + intervalQualifier.toString(),
                                                                              pos.toString()));
        }
    ...
    public static long intervalToMonths(SqlIntervalLiteral.IntervalValue interval) {
        return intervalToMonths(interval.getIntervalLiteral(), interval.getIntervalQualifier());
    }
    ...
    public static StringAndPos findPos(String sql) {
        ...
        if (secondCaret < 0) {
            ...
            String sqlSansCaret =
                    sql.substring(0, firstCaret) + sql.substring(firstCaret + 1, secondCaret) + sql.substring(
                            secondCaret + 1);
            int[] start = indexToLineCol(sql, firstCaret);
     ...
     */
    public static SqlNode toTreeEx(SqlSpecialOperator.TokenSequence list, int start, final int minPrec,
                                   final SqlKind stopperKind) {
        final Predicate<PrecedenceClimbingParser.Token> predicate = new PredicateImpl<PrecedenceClimbingParser.Token>() {
            ...
            public boolean test(PrecedenceClimbingParser.Token t) {
                if (t instanceof PrecedenceClimbingParser.Op) {
                    ...
                    return stopperKind != SqlKind.OTHER && op.kind == stopperKind
                           || minPrec > 0 && op.getLeftPrec() < minPrec;
                } else {
                    return false;
                }
            }
    ...
    private static SqlNode convert(PrecedenceClimbingParser.Token token) {
        switch (token.type) {
            ...
            case CALL:
                ...
                if (item.op == SqlStdOperatorTable.UNARY_MINUS && list.size() == 1 && list.get(
                        0) instanceof SqlNumericLiteral) {
                    return SqlLiteral.createNegative((SqlNumericLiteral) list.get(0), item.pos.plusAll(list));
                }
                if (item.op == SqlStdOperatorTable.UNARY_PLUS && list.size() == 1 && list.get(
                        0) instanceof SqlNumericLiteral) {
                    return list.get(0);
                }
    ...
    public static char checkUnicodeEscapeChar(String s) {
        ...
        char c = s.charAt(0);
        if (Character.isDigit(c) || Character.isWhitespace(c) || (c == '+') || (c == '"') || ((c >= 'a') && (c <= 'f'))
            || ((c >= 'A') && (c <= 'F'))) {
                        ...
                        builder.special(item, op.getLeftPrec(), op.getRightPrec(),
                                        new PrecedenceClimbingParser.Special() {
                                            ...
                                            public PrecedenceClimbingParser.Result apply(
                                                ...
                                                final SqlSpecialOperator op1 = (SqlSpecialOperator) ((ToTreeListItem) op.o).op;
                                                SqlSpecialOperator.ReduceResult r = op1.reduceExpr(tokens.indexOf(op),
                                                                                                   new TokenSequenceImpl(
                                                                                                           parser));
                                                return new PrecedenceClimbingParser.Result(tokens.get(r.startOrdinal),
    ...
    private static class Format {
        ...
        final                DateFormat          timestamp  = new SimpleDateFormat(
                DateTimeUtils.TIMESTAMP_FORMAT_STRING, Locale.ROOT);
        final                DateFormat          time       = new SimpleDateFormat(DateTimeUtils.TIME_FORMAT_STRING,
                                                                                   Locale.ROOT);
        final                DateFormat          date       = new SimpleDateFormat(DateTimeUtils.DATE_FORMAT_STRING,
                                                                                   Locale.ROOT);
    }
}
