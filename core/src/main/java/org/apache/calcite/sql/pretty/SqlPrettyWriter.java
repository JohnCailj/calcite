...
public class SqlPrettyWriter implements SqlWriter {
    ...
    protected static final CalciteLogger LOGGER = new CalciteLogger(
    ...
    public boolean inQuery() {
        return (frame == null) || (frame.frameType == FrameTypeEnum.ORDER_BY) || (frame.frameType == FrameTypeEnum.WITH)
     ...
     * @return A frame
     */
    protected FrameImpl createListFrame(FrameType frameType, String keyword, String open, String close) {
        ...
        if (frameType instanceof FrameTypeEnum) {
            ...
            switch (frameTypeEnum) {
                ...
                case VALUES:
                    return new FrameImpl(frameType, keyword, open, close, indentation, false, false, indentation,
                                         windowDeclListNewline, false, false);

                case UPDATE_SET_LIST:
                    return new FrameImpl(frameType, keyword, open, close, indentation, false, updateSetListNewline,
                                         indentation, false, false, false);

                case SELECT_LIST:
                    return new FrameImpl(frameType, keyword, open, close, selectListExtraIndentFlag ? indentation : 0,
                                         selectListItemsOnSeparateLines, false, indentation,
                                         selectListItemsOnSeparateLines, false, false);

                case ORDER_BY_LIST:
                case GROUP_BY_LIST:
                    return new FrameImpl(frameType, keyword, open, close, indentation, selectListItemsOnSeparateLines,
                                         false, indentation, selectListItemsOnSeparateLines, false, false);
                ...
                case SUB_QUERY:
                    switch (subQueryStyle) {
                        case BLACK:
                            ...
                            return new FrameImpl(frameType, keyword, open, close, 0, false, true, indentation, false,
                                                 false, false) {
                        ...
                        case HYDE:
                            ...
                            // WHERE foo IN (
                            //     SELECT ...
                            return new FrameImpl(frameType, keyword, open, close, 0, false, true, 0, false, false,
                                                 false) {
                ...
                case SELECT:
                    return new FrameImpl(frameType, keyword, open, close, indentation, false, isClauseStartsLine(),
                                         // newline before FROM, WHERE etc.
                                         0, // all clauses appear below SELECT
                                         false, false, false);

                case SETOP:
                    return new FrameImpl(frameType, keyword, open, close, indentation, false, isClauseStartsLine(),
                                         // newline before UNION, EXCEPT
                                         0, // all clauses appear below SELECT
                                         isClauseStartsLine(), // newline after UNION, EXCEPT
                                         false, false);

                case WINDOW:
                    return new FrameImpl(frameType, keyword, open, close, indentation, false, windowNewline, 0, false,
                                         false, false);

                case FUN_CALL:
                    setNeedWhitespace(false);
                    return new FrameImpl(frameType, keyword, open, close, indentation, false, false, indentation, false,
                                         false, false);
                ...
                case WHERE_LIST:
                    return new FrameImpl(frameType, keyword, open, close, indentation, false,
                                         whereListItemsOnSeparateLines, 0, false, false, false);
                ...
                case JOIN:
                    return new FrameImpl(frameType, keyword, open, close, indentation, false, isClauseStartsLine(),
                        ...
                        protected void sep(boolean printFirst, String sep) {
                            boolean newlineBefore = newlineBeforeSep && !sep.equals(",");
                            boolean newlineAfter = newlineAfterSep && sep.equals(",");
        ...
        return new FrameImpl(frameType, keyword, open, close, indentation, newlineAfterOpen, newlineBeforeSep,
                             sepIndent, false, newlineBeforeClose, false);
    }
     ...
     */
    protected Frame startList(FrameType frameType, String keyword, String open, String close) {
    ...
    public void keyword(String s) {
        maybeWhitespace(s);
        pw.print(isKeywordsLowerCase() ? s.toLowerCase(Locale.ROOT) : s.toUpperCase(Locale.ROOT));
        charCount += s.length();
        if (!s.equals("")) {
    ...
    protected boolean tooLong(String s) {
        boolean result = lineLength > 0 && (charCount > currentIndent) && ((charCount + s.length()) >= lineLength);
        if (result) {
            nextWhitespace = NL;
        }
    ...
    public void identifier(String name) {
        String qName = name;
        if (isQuoteAllIdentifiers() || dialect.identifierNeedsToBeQuoted(name)) {
    ...
    public void setFormatOptions(SqlFormatOptions options) {
        ...
        setSelectListItemsOnSeparateLines(options.isSelectListItemsOnSeparateLines());
        setWhereListItemsOnSeparateLines(options.isWhereListItemsOnSeparateLines());
    ...
    protected class FrameImpl implements Frame {
        ...
        FrameImpl(FrameType frameType, String keyword, String open, String close, int extraIndent,
                  boolean newlineAfterOpen, boolean newlineBeforeSep, int sepIndent, boolean newlineAfterSep,
        ...
        protected void sep(boolean printFirst, String sep) {
            if ((newlineBeforeSep && (itemCount > 0)) || (newlineAfterOpen && (itemCount == 0))) {
                newlineAndIndent();
            }
            if ((itemCount > 0) || printFirst) {
                keyword(sep);
