...
class AggChecker extends SqlBasicVisitor<Void> {
     ...
     *                   DISTINCT clause
     */
    AggChecker(SqlValidatorImpl validator, AggregatingScope scope, List<SqlNode> extraExprs, List<SqlNode> groupExprs,
               boolean distinct) {
    ...
    public Void visit(SqlIdentifier id) {
        ...
        throw validator.newValidationError(originalExpr, distinct ? RESOURCE.notSelectDistinctExpr(
                exprString) : RESOURCE.notGroupExpr(exprString));
    }
    ...
    public Void visit(SqlCall call) {
        ...
        if (groupCall != null) {
            ...
            throw validator.newValidationError(groupCall,
                                               RESOURCE.auxiliaryWithoutMatchingGroupCall(call.getOperator().getName(),
                                                                                          groupCall.getOperator().getName()));
        }
