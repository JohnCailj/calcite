...
public abstract class DelegatingScope implements SqlValidatorScope {
    ...
    @SuppressWarnings("deprecation") public Pair<String, SqlValidatorNamespace> findQualifyingTableName(
            String columnName, SqlNode ctx) {
        //noinspection deprecation
        return parent.findQualifyingTableName(columnName, ctx);
    }

    public Map<String, ScopeChild> findQualifyingTableNames(String columnName, SqlNode ctx,
    ...
    @SuppressWarnings("deprecation") public SqlValidatorNamespace getTableNamespace(List<String> names) {
    ...
    public SqlQualified fullyQualify(SqlIdentifier identifier) {
        ...
        switch (identifier.names.size()) {
            case 1: {
                ...
                identifier = new SqlIdentifier(ImmutableList.of(tableName, columnName), null, pos,
                                ...
                                switch (field.getType().getStructKind()) {
                                    ...
                                    case PEEK_FIELDS_NO_EXPAND:
                                        ...
                                        if (resolved.count() == 1) {
                                            ...
                                            fromPath = resolve.path;
                                            fromRowType = resolve.rowType();
                                            identifier = identifier.setName(0, columnName).add(0, tableName2,
                                                                                               SqlParserPos.ZERO);
                                            ++i;
                                            ++size;
                                        }
                                    ...
                                    default:
                                        ...
                                        throw validator.newValidationError(prefix, RESOURCE.tableNameNotFound(
                                                prefix.toString()));
                                }
                            }
                        }
                    ...
                    case 0:
                        ...
                        if (nameMatcher.isCaseSensitive()) {
                            ...
                            if (resolved.count() > 0) {
                                ...
                                throw validator.newValidationError(suffix3, RESOURCE.columnNotFoundInTableDidYouMean(
                                        suffix3.toString(), prefix.toString(), step.name));
                            }
                        }
                        ...
                        final SqlIdentifier prefix = identifier.getComponent(0, i);
                        final SqlIdentifier suffix3 = identifier.getComponent(i, k + 1);
                        throw validator.newValidationError(suffix3, RESOURCE.columnNotFoundInTable(suffix3.toString(),
                                                                                                   prefix.toString()));
                    case 1:
