...
public abstract class ListScope extends DelegatingScope {
    ...
    private ScopeChild findChild(List<String> names, SqlNameMatcher nameMatcher) {
        for (ScopeChild child : children) {
            ...
            if (table != null) {
                ...
                if (resolved.count() == 1 && resolved.only().remainingNames.isEmpty()
    ...
    @SuppressWarnings("deprecation") @Override public Pair<String, SqlValidatorNamespace> findQualifyingTableName(
    ...
    @Override public void resolve(List<String> names, SqlNameMatcher nameMatcher, boolean deep, Resolved resolved) {
        ...
        if (child0 != null) {
            final Step path = Path.EMPTY.plus(child0.namespace.getRowType(), child0.ordinal, child0.name,
        ...
        if (deep) {
            for (ScopeChild child : children) {
                ...
                final List<String> names2 = nameMatcher.matches(child.name, names.get(0)) ? names.subList(1,
                                                                                                          names.size()) : names;
                resolveInNamespace(child.namespace, child.nullable, names2, nameMatcher, Path.EMPTY, resolved);
            }
            if (resolved.count() > 0) {
                return;
            }
