...
public class SqlValidatorImpl implements SqlValidatorWithHints {
     ...
     */
    protected SqlValidatorImpl(SqlOperatorTable opTab, SqlValidatorCatalogReader catalogReader,
    ...
    public SqlNodeList expandStar(SqlNodeList selectList, SqlSelect select, boolean includeSystemVars) {
        ...
        for (int i = 0; i < selectList.size(); i++) {
            ...
            expandSelectItem(selectItem, select, unknownType, list,
                             catalogReader.nameMatcher().isCaseSensitive() ? new LinkedHashSet<String>() : new TreeSet<>(
                                     String.CASE_INSENSITIVE_ORDER), types, includeSystemVars);
        }
     ...
     */
    private boolean expandSelectItem(final SqlNode selectItem, SqlSelect select, RelDataType targetType,
                                     List<SqlNode> selectItems, Set<String> aliases,
                                     List<Map.Entry<String, RelDataType>> types, final boolean includeSystemVars) {
        ...
        if (expanded != selectItem) {
            ...
            if (!newAlias.equals(alias)) {
                expanded = SqlStdOperatorTable.AS.createCall(selectItem.getParserPosition(), expanded,
    ...
    private boolean expandStar(List<SqlNode> selectItems, Set<String> aliases,
                               List<Map.Entry<String, RelDataType>> types, boolean includeSystemVars, SelectScope scope,
                               SqlNode node) {
        ...
        switch (identifier.names.size()) {
                ...
                for (ScopeChild child : scope.children) {
                    ...
                    if (child.namespace.getRowType().isDynamicStruct()) {
                        ...
                        final SqlNode exp = new SqlIdentifier(
                                ImmutableList.of(child.name, DynamicRecordType.DYNAMIC_STAR_PREFIX), startPosition);
                        ...
                        for (RelDataTypeField field : rowType.getFieldList()) {
                            ...
                            final SqlIdentifier exp = new SqlIdentifier(ImmutableList.of(child.name, columnName),
                                                                        startPosition);
                    ...
                    if (child.nullable) {
                        for (int i = before; i < types.size(); i++) {
                            ...
                            if (!type.isNullable()) {
                                types.set(i,
                                          Pair.of(entry.getKey(), typeFactory.createTypeWithNullability(type, true)));
                            }
                        }
                    }
                }
            ...
            default:
                ...
                if (rowType.isDynamicStruct()) {
                    // don't expand star if the underneath table is dynamic.
                    addToSelectList(selectItems, aliases, types,
                                    prefixId.plus(DynamicRecordType.DYNAMIC_STAR_PREFIX, startPosition), scope,
                                    includeSystemVars);
                    ...
                    for (RelDataTypeField field : rowType.getFieldList()) {
                        ...
                        // TODO: do real implicit collation here
                        addOrExpandField(selectItems, aliases, types, includeSystemVars, scope,
                                         prefixId.plus(columnName, startPosition), field);
                    }
                } else {
    ...
    private void lookupFromHints(SqlNode node, SqlValidatorScope scope, SqlParserPos pos,
    ...
    private void lookupJoinHints(SqlJoin join, SqlValidatorScope scope, SqlParserPos pos,
                                 Collection<SqlMoniker> hintList) {
     ...
     */
    public final void lookupNameCompletionHints(SqlValidatorScope scope, List<String> names, SqlParserPos pos,
                                                Collection<SqlMoniker> hintList) {
    ...
    private static void findAllValidUdfNames(List<String> names, SqlValidator validator,
    ...
    private static void findAllValidFunctionNames(List<String> names, SqlValidator validator,
                                                  Collection<SqlMoniker> result, SqlParserPos pos) {
    ...
    @VisibleForTesting public SqlValidatorScope getEmptyScope() {
    ...
    protected SqlNode performUnconditionalRewrites(SqlNode node, boolean underFrom) {
        ...
        if (node instanceof SqlCall) {
            ...
            if (call.getOperator() instanceof SqlUnresolvedFunction) {
                ...
                opTab.lookupOperatorOverloads(function.getNameAsId(), function.getFunctionType(), SqlSyntax.FUNCTION,
                                              overloads);
                if (overloads.size() == 1) {
        ...
        switch (kind) {
            case VALUES:
                ...
                if (underFrom || true) {
                    ...
                    return new SqlSelect(node.getParserPosition(), null, selectList, node, null, null, null, null, null,
                                         null, null);
                }
            ...
            case ORDER_BY: {
                    ...
                    for (int i = 0; i < orderList.size(); i++) {
                        ...
                        for (Ord<SqlNode> sel : Ord.zip(selectList2)) {
                            if (stripAs(sel.e).equalsDeep(sqlNode, Litmus.IGNORE)) {
                                orderList.set(i, SqlLiteral.createExactNumeric(Integer.toString(sel.i + 1),
                                                                               SqlParserPos.ZERO));
                            }
                        }
                    }
                ...
                return new SqlSelect(SqlParserPos.ZERO, null, selectList, orderBy.query, null, null, null, null,
                                     orderList, orderBy.offset, orderBy.fetch);
            }
            ...
            case EXPLICIT_TABLE: {
                ...
                final SqlNodeList selectList = new SqlNodeList(SqlParserPos.ZERO);
                selectList.add(SqlIdentifier.star(SqlParserPos.ZERO));
                return new SqlSelect(SqlParserPos.ZERO, null, selectList, call.operand(0), null, null, null, null, null,
                                     null, null);
            }
    ...
    private void rewriteMerge(SqlMerge call) {
        ...
        SqlNode outerJoin = new SqlJoin(SqlParserPos.ZERO, leftJoinTerm,
                                        SqlLiteral.createBoolean(false, SqlParserPos.ZERO),
                                        joinType.symbol(SqlParserPos.ZERO), targetTable,
                                        JoinConditionType.ON.symbol(SqlParserPos.ZERO), call.getCondition());
        SqlSelect select = new SqlSelect(SqlParserPos.ZERO, null, selectList, outerJoin, null, null, null, null, null,
                                         null, null);
        call.setSourceSelect(select);
        ...
        if (insertCall != null) {
            ...
            select = new SqlSelect(SqlParserPos.ZERO, null, selectList, insertSource, null, null, null, null, null,
                                   null, null);
    ...
    private SqlNode rewriteUpdateToMerge(SqlUpdate updateCall, SqlNode selfJoinSrcExpr) {
        ...
        SqlNode selfJoinTgtExpr = getSelfJoinExprForUpdate(updateCall.getTargetTable(),
        ...
        SqlNode selfJoinCond = SqlStdOperatorTable.EQUALS.createCall(SqlParserPos.ZERO, selfJoinSrcExpr,
                                                                     selfJoinTgtExpr);
        if (condition == null) {
        ...
        SqlMerge mergeCall = new SqlMerge(updateCall.getParserPosition(), target, condition, source, updateCall, null,
                                          null, updateCall.getAlias());
    ...
    protected SqlSelect createSourceSelectForUpdate(SqlUpdate call) {
        ...
        return new SqlSelect(SqlParserPos.ZERO, null, selectList, sourceTable, call.getCondition(), null, null, null,
                             null, null, null);
    }
     ...
     */
    @SuppressWarnings("deprecation") public final void setValidatedNodeType(SqlNode node, RelDataType type) {
    ...
    public RelDataType deriveConstructorType(SqlValidatorScope scope, SqlCall call, SqlFunction unresolvedConstructor,
                                             SqlFunction resolvedConstructor, List<RelDataType> argTypes) {
        ...
        if (shouldExpandIdentifiers()) {
            if (resolvedConstructor != null) {
                ...
                ((SqlBasicCall) call).setOperator(
                        new SqlFunction(type.getSqlIdentifier(), ReturnTypes.explicit(type), null, null, null,
    ...
    public CalciteException handleUnresolvedFunction(SqlCall call, SqlFunction unresolvedFunction,
    ...
    protected void addToSelectList(List<SqlNode> list, Set<String> aliases,
                                   List<Map.Entry<String, RelDataType>> fieldList, SqlNode exp, SqlValidatorScope scope,
    ...
    private void registerMatchRecognize(SqlValidatorScope parentScope, SqlValidatorScope usingScope,
                                        SqlMatchRecognize call, SqlNode enclosingNode, String alias,
        ...
        SqlNode newExpr = registerFrom(usingScope, matchRecognizeScope, expr, expr, null, null, forceNullable);
        if (expr != newExpr) {
     ...
     */
    private SqlNode registerFrom(SqlValidatorScope parentScope, SqlValidatorScope usingScope, final SqlNode node,
                                 SqlNode enclosingNode, String alias, SqlNodeList extendList, boolean forceNullable) {
        final SqlKind kind = node.getKind();
        ...
        switch (kind) {
            case AS:
                ...
                return node;
            case MATCH_RECOGNIZE:
                registerMatchRecognize(parentScope, usingScope, (SqlMatchRecognize) node, enclosingNode, alias,
                                       forceNullable);
                return node;
            case TABLESAMPLE:
                call = (SqlCall) node;
            ...
            case JOIN:
                ...
                final SqlNode newRight = registerFrom(rightParentScope, joinScope, right, right, null, null,
                                                      forceRightNullable);
                if (newRight != right) {
            ...
            case IDENTIFIER:
                final SqlIdentifier id = (SqlIdentifier) node;
                final IdentifierNamespace newNs = new IdentifierNamespace(this, id, extendList, enclosingNode,
                                                                          parentScope);
            ...
            case LATERAL:
                ...
                return registerFrom(parentScope, usingScope, ((SqlCall) node).operand(0), enclosingNode, alias,
                                    extendList, forceNullable);

            case COLLECTION_TABLE:
                call = (SqlCall) node;
                operand = call.operand(0);
                newOperand = registerFrom(tableScope == null ? parentScope : tableScope, usingScope, operand,
                                          enclosingNode, alias, extendList, forceNullable);
            ...
            case OVER:
                ...
                operand = call.operand(0);
                newOperand = registerFrom(parentScope, overScope, operand, enclosingNode, alias, extendList,
                                          forceNullable);
                if (newOperand != operand) {
                    call.setOperand(0, newOperand);
                }
            ...
            case EXTEND:
                ...
                return registerFrom(parentScope, usingScope, extend.getOperandList().get(0), extend, alias,
                                    (SqlNodeList) extend.getOperandList().get(1), forceNullable);
     ...
     *                    if usingScope != null
     */
    private void registerQuery(SqlValidatorScope parentScope, SqlValidatorScope usingScope, SqlNode node,
     ...
     */
    private void registerQuery(SqlValidatorScope parentScope, SqlValidatorScope usingScope, SqlNode node,
                               SqlNode enclosingNode, String alias, boolean forceNullable, boolean checkUpdate) {
        Preconditions.checkNotNull(node);
        ...
        switch (node.getKind()) {
            ...
            case VALUES:
                ...
                final TableConstructorNamespace tableConstructorNamespace = new TableConstructorNamespace(this, call,
                                                                                                          parentScope,
                                                                                                          enclosingNode);
            ...
            case MERGE:
                ...
                if (mergeCall.getUpdateCall() != null) {
                    registerQuery(whereScopes.get(mergeCall.getSourceSelect()), null, mergeCall.getUpdateCall(),
                                  enclosingNode, null, false, false);
                }
    ...
    private void registerSetop(SqlValidatorScope parentScope, SqlValidatorScope usingScope, SqlNode node,
                               SqlNode enclosingNode, String alias, boolean forceNullable) {
        SqlCall call = (SqlCall) node;
    ...
    private void registerWith(SqlValidatorScope parentScope, SqlValidatorScope usingScope, SqlWith with,
                              SqlNode enclosingNode, String alias, boolean forceNullable, boolean checkUpdate) {
        ...
        for (SqlNode withItem_ : with.withList) {
            ...
            registerNamespace(null, alias, new WithItemNamespace(this, withItem, enclosingNode), false);
            scope = withScope;
        }
    ...
    public boolean isAggregate(SqlSelect select) {
        ...
        for (SqlNode node : select.getSelectList()) {
            if (node instanceof SqlCall) {
                ...
                if (call != null && call.getOperator().getKind() == SqlKind.OVER && call.getOperandList().size() != 0) {
                    if (call.operand(0) instanceof SqlCall && isNestedAggregateWindow((SqlCall) call.operand(0))) {
                        return true;
                    }
                }
            }
    ...
    @SuppressWarnings("deprecation") public boolean isAggregate(SqlNode selectNode) {
    ...
    private void registerSubQueries(SqlValidatorScope parentScope, SqlNode node) {
        ...
        if (node.getKind().belongsTo(SqlKind.QUERY) || node.getKind() == SqlKind.MULTISET_QUERY_CONSTRUCTOR
        ...
        switch (literal.getTypeName()) {
            ...
            case INTERVAL_SECOND:
                if (literal instanceof SqlIntervalLiteral) {
                    ...
                    int[] values = intervalQualifier.evaluateIntervalLiteral(intervalStr, literal.getParserPosition(),
    ...
    public void validateIntervalQualifier(SqlIntervalQualifier qualifier) {
        ...
        if (startPrecisionOutOfRange) {
            throw newValidationError(qualifier, RESOURCE.intervalStartPrecisionOutOfRange(startPrecision,
                                                                                          "INTERVAL " + qualifier));
        } else if (fractionalSecondPrecisionOutOfRange) {
            throw newValidationError(qualifier, RESOURCE.intervalFractionalSecondPrecisionOutOfRange(fracPrecision,
                                                                                                     "INTERVAL "
                                                                                                     + qualifier));
        }
    }
            ...
            case USING:
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    if (!SqlTypeUtil.isComparable(leftColType, rightColType)) {
                        throw newValidationError(id, RESOURCE.naturalOrUsingColumnNotCompatible(id.getSimple(),
            ...
            for (String name : naturalColumnNames) {
                ...
                if (!SqlTypeUtil.isComparable(leftColType, rightColType)) {
                    throw newValidationError(join,
                                             RESOURCE.naturalOrUsingColumnNotCompatible(name, leftColType.toString(),
                                                                                        rightColType.toString()));
                }
            }
        }
    ...
    protected void validateSelect(SqlSelect select, RelDataType targetRowType) {
        ...
        if (!catalogReader.nameMatcher().isCaseSensitive()) {
            names = Lists.transform(names, new Function<String, String>() {
    ...
    private void checkRollUp(SqlNode grandParent, SqlNode parent, SqlNode current, SqlValidatorScope scope,
        ...
        if (current instanceof SqlCall && !(current instanceof SqlSelect)) {
            ...
            if (!id.isStar() && isRolledUpColumn(id, scope)) {
                if (!isAggregation(parent.getKind()) || !isRolledUpColumnAllowedInAgg(id, scope, (SqlCall) parent,
                                                                                      grandParent)) {
    ...
    private boolean isRolledUpColumnAllowedInAgg(SqlIdentifier identifier, SqlValidatorScope scope, SqlCall aggCall,
                                                 SqlNode parent) {
    ...
    private Table findTable(String alias) {
        ...
        CalciteSchema.TableEntry entry = schema.getTable(tableName, catalogReader.nameMatcher().isCaseSensitive());
    ...
    private void validateGroupItem(SqlValidatorScope groupScope, AggregatingSelectScope aggregatingScope,
                                   SqlNode groupItem) {
    ...
    private void validateGroupingSets(SqlValidatorScope groupScope, AggregatingSelectScope aggregatingScope,
                                      SqlCall groupItem) {
    ...
    protected RelDataType validateSelectList(final SqlNodeList selectItems, SqlSelect select,
        ...
        for (int i = 0; i < selectItems.size(); i++) {
            ...
            if (selectItem instanceof SqlSelect) {
                ...
                expandSelectItem(selectItem, select, targetRowType.isStruct() && targetRowType.getFieldCount()
                                                                                 >= i ? targetRowType.getFieldList().get(
                        i).getType() : unknownType, expandedSelectItems, aliases, fieldList, false);
            }
        }
     ...
     */
    private void handleScalarSubQuery(SqlSelect parentSelect, SqlSelect selectItem, List<SqlNode> expandedSelectItems,
    ...
    protected RelDataType createTargetRowType(SqlValidatorTable table, SqlNodeList targetColumnList, boolean append) {
        ...
        for (SqlNode node : targetColumnList) {
            ...
            RelDataTypeField targetField = SqlValidatorUtil.getTargetField(baseRowType, typeFactory, id, catalogReader,
                                                                           relOptTable);
            if (targetField == null) {
    ...
    public void validateInsert(SqlInsert insert) {
        ...
        final RelOptTable relOptTable = SqlValidatorUtil.getRelOptTable(targetNamespace, catalogReader.unwrap(
                Prepare.CatalogReader.class), null, null);
        final SqlValidatorTable table =
                relOptTable == null ? targetNamespace.getTable() : relOptTable.unwrap(SqlValidatorTable.class);
        ...
        checkFieldCount(insert.getTargetTable(), table, logicalSourceRowType, logicalTargetRowType);
    ...
    private void checkConstraint(SqlValidatorTable validatorTable, SqlNode source, RelDataType targetRowType) {
        ...
        if (modifiableViewTable != null && source instanceof SqlCall) {
            ...
            final Map<Integer, RelDataTypeField> tableIndexToTargetField = SqlValidatorUtil.getIndexToFieldMap(
                    tableFields, targetRowType);
            final Map<Integer, RexNode> projectMap = RelOptUtil.getColumnConstraints(modifiableViewTable, targetRowType,
                                                                                     typeFactory);
            ...
            for (final int colIndex : constrainedTargetColumns.asList()) {
                ...
                for (SqlNode row : values) {
                    ...
                    final ValidationError validationError = new ValidationError(sourceValue,
                                                                                RESOURCE.viewConstraintNotSatisfied(
                                                                                        colName, Util.last(
                                                                                                validatorTable.getQualifiedName())));
        ...
        if (modifiableViewTable != null) {
            ...
            for (final Pair<SqlNode, SqlNode> column : Pair.zip(targets, sources)) {
                ...
                if (projectMap.containsKey(columnIndex)) {
                    ...
                    final ValidationError validationError = new ValidationError(column.right,
                                                                                RESOURCE.viewConstraintNotSatisfied(
                                                                                        columnName, Util.last(
                                                                                                validatorTable.getQualifiedName())));
    ...
    private void checkFieldCount(SqlNode node, SqlValidatorTable table, RelDataType logicalSourceRowType,
        ...
        for (final RelDataTypeField field : table.getRowType().getFieldList()) {
            if (!field.getType().isNullable()) {
                ...
                if (targetField == null && !table.columnHasDefaultValue(table.getRowType(), field.getIndex(),
                                                                        rexBuilder)) {
    ...
    protected RelDataType getLogicalTargetRowType(RelDataType targetRowType, SqlInsert insert) {
        if (insert.getTargetColumnList() == null && conformance.isInsertSubsetColumnsAllowed()) {
            ...
            final RelDataType sourceRowType = getNamespace(source).getRowType();
            final RelDataType logicalSourceRowType = getLogicalSourceRowType(sourceRowType, insert);
            final RelDataType implicitTargetRowType = typeFactory.createStructType(
                    targetRowType.getFieldList().subList(0, logicalSourceRowType.getFieldCount()));
            final SqlValidatorNamespace targetNamespace = getNamespace(insert);
        ...
        for (int i = 0; i < sourceCount; ++i) {
            ...
            if (!SqlTypeUtil.canAssignFrom(targetType, sourceType)) {
                ...
                throw newValidationError(node,
                                         RESOURCE.typeNotAssignable(targetFields.get(i).getName(), targetTypeString,
    ...
    private void validateAccess(SqlNode node, SqlValidatorTable table, SqlAccessEnum requiredAccess) {
        if (table != null) {
            ...
            if (!access.allowsAccess(requiredAccess)) {
                throw newValidationError(node, RESOURCE.accessNotAllowed(requiredAccess.name(),
        ...
        for (SqlNode operand : operands) {
            ...
            if (conformance.isInsertSubsetColumnsAllowed() && targetRowType.isStruct()
                ...
                targetRowType = typeFactory.createStructType(
                        targetRowType.getFieldList().subList(0, rowConstructor.operandCount()));
            ...
            if (targetRowType.isStruct()) {
                for (Pair<SqlNode, RelDataTypeField> pair : Pair.zip(rowConstructor.getOperandList(),
    ...
    class ValidationErrorFunction
        ...
        @Override public CalciteContextException apply(SqlNode v0, Resources.ExInst<SqlValidatorException> v1) {
    ...
    private List<Map.Entry<String, RelDataType>> validateMeasure(SqlMatchRecognize mr, MatchRecognizeScope scope,
                                                                 boolean allRows) {
        ...
        for (SqlNode measure : measures) {
            ...
            sqlNodes.add(SqlStdOperatorTable.AS.createCall(SqlParserPos.ZERO, expand,
                                                           new SqlIdentifier(alias, SqlParserPos.ZERO)));
        }
    ...
    private void validateDefinitions(SqlMatchRecognize mr, MatchRecognizeScope scope) {
        final Set<String> aliases = catalogReader.nameMatcher().isCaseSensitive() ? new LinkedHashSet<String>() : new TreeSet<>(
    ...
    private static boolean isAggregation(SqlKind kind) {
        return kind == SqlKind.SUM || kind == SqlKind.SUM0 || kind == SqlKind.AVG || kind == SqlKind.COUNT
    ...
    public static class DmlNamespace extends IdentifierNamespace {
        ...
        protected DmlNamespace(SqlValidatorImpl validator, SqlNode id, SqlNode enclosingNode,
    ...
    private static class InsertNamespace extends DmlNamespace {
        ...
        InsertNamespace(SqlValidatorImpl validator, SqlInsert node, SqlNode enclosingNode,
                        SqlValidatorScope parentScope) {
    ...
    private static class UpdateNamespace extends DmlNamespace {
        ...
        UpdateNamespace(SqlValidatorImpl validator, SqlUpdate node, SqlNode enclosingNode,
                        SqlValidatorScope parentScope) {
            super(validator, node.getTargetTable(), enclosingNode, parentScope);
            this.node = Preconditions.checkNotNull(node);
        }
    ...
    private static class DeleteNamespace extends DmlNamespace {
        ...
        DeleteNamespace(SqlValidatorImpl validator, SqlDelete node, SqlNode enclosingNode,
                        SqlValidatorScope parentScope) {
            super(validator, node.getTargetTable(), enclosingNode, parentScope);
            this.node = Preconditions.checkNotNull(node);
        }
    ...
    private static class MergeNamespace extends DmlNamespace {
        ...
        MergeNamespace(SqlValidatorImpl validator, SqlMerge node, SqlNode enclosingNode,
                       SqlValidatorScope parentScope) {
            super(validator, node.getTargetTable(), enclosingNode, parentScope);
            this.node = Preconditions.checkNotNull(node);
        }
    ...
    static class ExtendedExpander extends Expander {
        ...
        ExtendedExpander(SqlValidatorImpl validator, SqlValidatorScope scope, SqlSelect select, SqlNode root,
                         boolean havingExpr) {
        ...
        @Override public SqlNode visit(SqlIdentifier id) {
            if (id.isSimple()
                && (havingExpr ? validator.getConformance().isHavingAlias() : validator.getConformance().isGroupByAlias())) {
                ...
                if (isPhysicalNavigation(kind)) {
                    ...
                    if (isPhysicalNavigation(innerKind)) {
                        ...
                        SqlOperator newOperator =
                                innerKind == kind ? SqlStdOperatorTable.PLUS : SqlStdOperatorTable.MINUS;
    ...
    private static class NavigationReplacer extends NavigationModifier {
            ...
            switch (kind) {
                case PREV:
                    ...
                    if (operands.get(0) instanceof SqlIdentifier) {
                        ...
                        return name.equals(alpha) ? call : SqlStdOperatorTable.LAST.createCall(SqlParserPos.ZERO,
                                                                                               operands);
                    }
            }
        ...
        @Override public SqlNode visit(SqlIdentifier id) {
            ...
            SqlOperator operator = id.names.get(0).equals(alpha) ? SqlStdOperatorTable.PREV : SqlStdOperatorTable.LAST;
    ...
    private class PatternValidator extends SqlBasicVisitor<Set<String>> {
        ...
        @Override public Set<String> visit(SqlCall call) {
            ...
            for (SqlNode node : operands) {
                if (node != null) {
                    vars.addAll(node.accept(
                            new PatternValidator(isMeasure, firstLastCount, prevNextCount, aggregateCount)));
                }
            }
            ...
            if (isSingle) {
                switch (kind) {
                    case COUNT:
                        ...
                        break;
                    default:
                        if (operands.size() == 0 || !(operands.get(0) instanceof SqlCall)
