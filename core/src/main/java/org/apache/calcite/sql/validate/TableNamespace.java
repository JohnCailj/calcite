...
class TableNamespace extends AbstractNamespace {
    ...
    private void checkExtendedColumnTypes(SqlNodeList extendList) {
        final List<RelDataTypeField> extendedFields = SqlValidatorUtil.getExtendedColumns(validator.getTypeFactory(),
                                                                                          table, extendList);
        ...
        for (final RelDataTypeField extendedField : extendedFields) {
            ...
            if (nameToIndex.containsKey(extFieldName)) {
                ...
                if (!extType.equals(baseType)) {
                    ...
                    throw validator.getValidationErrorFunction().apply(extColNode, RESOURCE.typeNotAssignable(
                            baseFields.get(baseIndex).getName(), baseType.getFullTypeString(), extendedField.getName(),
