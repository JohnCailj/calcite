...
public class RelDecorrelator implements ReflectiveVisitor {
    ...
    public static RelNode decorrelateQuery(RelNode rootRel) {
        ...
        final RelDecorrelator decorrelator = new RelDecorrelator(cluster, corelMap, cluster.getPlanner().getContext());

        RelNode newRootRel = decorrelator.removeCorrelationViaRule(rootRel);

        if (SQL2REL_LOGGER.isDebugEnabled()) {
            SQL2REL_LOGGER.debug(
                    RelOptUtil.dumpPlan("Plan after removing Correlator", newRootRel, SqlExplainFormat.TEXT,
    ...
    private RelNode decorrelate(RelNode root) {
        ...
        HepProgram program = HepProgram.builder().addRuleInstance(
                new AdjustProjectForCountAggregateRule(false)).addRuleInstance(
                new AdjustProjectForCountAggregateRule(true)).addRuleInstance(
                FilterJoinRule.FILTER_ON_JOIN).addRuleInstance(FilterProjectTransposeRule.INSTANCE).addRuleInstance(
                FilterCorrelateRule.INSTANCE).build();
        ...
        if (frame != null) {
            ...
            final HepProgram program2 = HepProgram.builder().addRuleInstance(
                    FilterJoinRule.FILTER_ON_JOIN).addRuleInstance(FilterJoinRule.JOIN).build();
    ...
    public RelNode removeCorrelationViaRule(RelNode root) {
        HepProgram program = HepProgram.builder().addRuleInstance(new RemoveSingleAggregateRule()).addRuleInstance(
                new RemoveCorrelationForScalarProjectRule()).addRuleInstance(
                new RemoveCorrelationForScalarAggregateRule()).build();
    ...
    protected RexNode removeCorrelationExpr(RexNode exp, boolean projectPulledAboveLeftCorrelator) {
        RemoveCorrelationRexShuttle shuttle = new RemoveCorrelationRexShuttle(rexBuilder,
                                                                              projectPulledAboveLeftCorrelator, null,
        ...
        for (CorRef corVar : correlations) {
            ...
            if (!joinedInputs.contains(newInput)) {
                ...
                if (r == null) {
                    ...
                    r = LogicalJoin.create(r, distinct, cluster.getRexBuilder().makeLiteral(true),
    ...
    private Frame decorrelateInputWithValueGenerator(RelNode rel, Frame frame) {
        ...
        RelNode join = LogicalJoin.create(frame.r, valueGen, rexBuilder.makeLiteral(true),
                                          ImmutableSet.<CorrelationId>of(), JoinRelType.INNER);
    ...
    public Frame decorrelateRel(LogicalCorrelate rel) {
        ...
        for (Map.Entry<CorDef, Integer> rightOutput : new ArrayList<>(corDefOutputs.entrySet())) {
            ...
            conditions.add(rexBuilder.makeCall(SqlStdOperatorTable.EQUALS, RexInputRef.of(newLeftPos, newLeftOutput),
        ...
        RelNode newJoin = LogicalJoin.create(leftFrame.r, rightFrame.r, condition, ImmutableSet.<CorrelationId>of(),
    ...
    private RelNode projectJoinOutputWithNullability(LogicalJoin join, LogicalProject project, int nullIndicatorPos) {
        ...
        RexInputRef nullIndicator = new RexInputRef(nullIndicatorPos, typeFactory.createTypeWithNullability(
    ...
    private class RemoveCorrelationRexShuttle extends RexShuttle {
        ...
        private RexNode createCaseExpression(RexInputRef nullInputRef, RexLiteral lit, RexNode rexNode) {
            ...
            caseOperands[0] = rexBuilder.makeCall(SqlStdOperatorTable.IS_NULL, new RexInputRef(nullInputRef.getIndex(),
    ...
    private final class RemoveSingleAggregateRule extends RelOptRule {
        ...
        public void onMatch(RelOptRuleCall call) {
            ...
            if ((!singleAggregate.getGroupSet().isEmpty()) || (singleAggregate.getAggCallList().size() != 1)
                || !(singleAggregate.getAggCallList().get(0).getAggregation() instanceof SqlSingleValueAggFunction)) {
                return;
            }
            ...
            RelNode newProject = RelOptUtil.createProject(aggregate, ImmutableList.of(rexBuilder.makeCast(
                    cluster.getTypeFactory().createTypeWithNullability(projExprs.get(0).getType(), true),
                    projExprs.get(0))), null);
    ...
    private final class RemoveCorrelationForScalarProjectRule extends RelOptRule {
        ...
        RemoveCorrelationForScalarProjectRule() {
            super(operand(LogicalCorrelate.class, operand(RelNode.class, any()), operand(LogicalAggregate.class,
        ...
        public void onMatch(RelOptRuleCall call) {
            ...
            if ((right instanceof LogicalFilter) && cm.mapRefRelToCorRef.containsKey(right)) {
                ...
                right = createProjectWithAdditionalExprs(right, ImmutableList.of(
                        Pair.<RexNode, String>of(rexBuilder.makeLiteral(true), "nullIndicator")));
    ...
    private final class RemoveCorrelationForScalarAggregateRule extends RelOptRule {
        ...
        RemoveCorrelationForScalarAggregateRule() {
            super(operand(LogicalCorrelate.class, operand(RelNode.class, any()), operand(LogicalProject.class,
        ...
        public void onMatch(RelOptRuleCall call) {
            ...
            RexInputRef nullIndicator = new RexInputRef(nullIndicatorPos,
                                                        cluster.getTypeFactory().createTypeWithNullability(
                                                                join.getRowType().getFieldList().get(
                                                                        nullIndicatorPos).getType(), true));
            ...
            for (RexNode aggInputProjExpr : aggInputProjects) {
                joinOutputProjects.add(
                        removeCorrelationExpr(aggInputProjExpr, joinType.generatesNullsOnRight(), nullIndicator));
            }
    ...
    private final class AdjustProjectForCountAggregateRule extends RelOptRule {
        ...
        AdjustProjectForCountAggregateRule(boolean flavor) {
            super(flavor ? operand(LogicalCorrelate.class, operand(RelNode.class, any()),
                                   operand(LogicalProject.class, operand(LogicalAggregate.class, any()))) : operand(
                    LogicalCorrelate.class, operand(RelNode.class, any()), operand(LogicalAggregate.class, any())));
            this.flavor = flavor;
        }
    ...
    private static class CorelMap {
        ...
        @Override public boolean equals(Object obj) {
            return obj == this || obj instanceof CorelMap && mapRefRelToCorRef.equals(
                    ((CorelMap) obj).mapRefRelToCorRef) && mapCorToCorRel.equals(((CorelMap) obj).mapCorToCorRel)
    ...
    private static class CorelMapBuilder extends RelShuttleImpl {
        ...
        final SortedSetMultimap<RelNode, CorRef> mapRefRelToCorRef = Multimaps.newSortedSetMultimap(
                new HashMap<RelNode, Collection<CorRef>>(), new Supplier<TreeSet<CorRef>>() {
