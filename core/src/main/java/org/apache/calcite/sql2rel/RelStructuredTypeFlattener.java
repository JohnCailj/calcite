...
public class RelStructuredTypeFlattener implements ReflectiveVisitor {
    ...
    public RelNode rewrite(RelNode root) {
        ...
        if (restructured) {
            ...
            // structured types.
            return RelOptUtil.createProject(flattened, structuringExps, root.getRowType().getFieldNames());
        } else {
            return flattened;
        }
    }
    ...
    private Mappings.TargetMapping getNewForOldInputMapping(RelNode oldRel) {
        ...
        return Mappings.target(new Function<Integer, Integer>() {
    ...
    public void rewriteRel(LogicalCorrelate rel) {
        ...
        LogicalCorrelate newRel = LogicalCorrelate.create(getNewForOldRel(rel.getLeft()),
                                                          getNewForOldRel(rel.getRight()), rel.getCorrelationId(),
                                                          newPos.build(), rel.getJoinType());
        setNewForOldRel(rel, newRel);
    }
    ...
    public void rewriteRel(LogicalProject rel) {
        ...
        flattenProjections(new RewriteRexShuttle(), rel.getProjects(), rel.getRowType().getFieldNames(), "",
                           flattenedExpList);
        RelNode newRel = RelOptUtil.createProject(getNewForOldRel(rel.getInput()), flattenedExpList, false);
        setNewForOldRel(rel, newRel);
    }
    ...
    public void rewriteRel(LogicalCalc rel) {
        ...
        if (conditionRef != null) {
            programBuilder.addCondition(
                    new RexLocalRef(getNewForOldInput(conditionRef.getIndex()), conditionRef.getType()));
        }
        ...
        if (exp.getType().isStruct()) {
            if (exp instanceof RexInputRef) {
                ...
                for (int j = 0; j < n; ++j) {
                    ...
                    flattenedExps.add(
                            Pair.<RexNode, String>of(new RexInputRef(newOffset + j, field.getType()), fieldName));
                }
                ...
                flattenProjections(new RewriteRexShuttle(), call.getOperands(),
                                   Collections.<String>nCopies(call.getOperands().size(), null), fieldName,
                                   flattenedExps);
                ...
                for (RelDataTypeField field : exp.getType().getFieldList()) {
                    flattenedExps.add(
                            Pair.of(rexBuilder.makeFieldAccess(exp, field.getIndex()), fieldName + "$" + (j++)));
                }
            } else {
    ...
    private void flattenNullLiteral(RelDataType type, List<Pair<RexNode, String>> flattenedExps) {
        ...
        for (RelDataTypeField field : flattenedType.getFieldList()) {
            flattenedExps.add(
                    Pair.of(rexBuilder.makeCast(field.getType(), rexBuilder.constantNull()), field.getName()));
        }
    }
    ...
    public void rewriteRel(TableScan rel) {
        ...
        if (!SqlTypeUtil.isFlat(rel.getRowType())) {
            ...
            newRel = RelOptUtil.createProject(newRel, flattenedExpList, false);
        }
        setNewForOldRel(rel, newRel);
    }
    ...
    private class RewriteRelVisitor extends RelVisitor {
        ...
        private final ReflectiveVisitDispatcher<RelStructuredTypeFlattener, RelNode> dispatcher = ReflectUtil.createDispatcher(
    ...
    private class RewriteRexShuttle extends RexShuttle {
        ...
        @Override public RexNode visitFieldAccess(RexFieldAccess fieldAccess) {
            ...
            for (; ; ) {
                ...
                if (refExp.isA(SqlKind.NEW_SPECIFICATION)) {
                    return ((RexCall) refExp).operands.get(fieldAccess.getField().getIndex());
                }
        ...
        @Override public RexNode visitSubQuery(RexSubQuery subQuery) {
            ...
            RelStructuredTypeFlattener flattener = new RelStructuredTypeFlattener(rexBuilder, toRelContext,
                                                                                  restructure);
