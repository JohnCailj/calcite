...
public class SqlToRelConverter {
            ...
            if ((sqlCall.getOperator() == SqlStdOperatorTable.AND) || (sqlCall.getOperator()
                ...
                if (childSqlCall.getOperator() == SqlStdOperatorTable.AND) {
                    ...
                    if (inOp.kind == SqlKind.NOT_IN) {
                        ...
                        return reg(scope, SqlStdOperatorTable.NOT_IN.createCall(SqlParserPos.ZERO, inOperands[0],
                                                                                inOperands[1]));
                    }
                } else {
                ...
                if (converted.indicator) {
                    ...
                    LogicalAggregate aggregate = LogicalAggregate.create(seek, ImmutableBitSet.of(), null,
                                                                         ImmutableList.of(AggregateCall.create(
                                                                                 ...
                                                                                 ImmutableList.<Integer>of(), -1,
                                                                                 longType, null), AggregateCall.create(
                                                                                 SqlStdOperatorTable.COUNT, false, args,
                                                                                 -1, longType, null)));
                ...
                rexComparison = RexUtil.composeConjunction(rexBuilder, Iterables.transform(
                        Pair.zip(leftKeys, call.getOperandList()), new Function<Pair<RexNode, SqlNode>, RexNode>() {
                            ...
                            public RexNode apply(Pair<RexNode, SqlNode> pair) {
                                return rexBuilder.makeCall(comparisonOp, pair.left, ensureSqlType(pair.left.getType(),
    ...
    private RelOptUtil.Exists convertExists(SqlNode seek, RelOptUtil.SubQueryType subQueryType, RelOptUtil.Logic logic,
        ...
        final SqlValidatorScope seekScope = (seek instanceof SqlSelect) ? validator.getSelectScope(
                (SqlSelect) seek) : null;
    ...
    private RexLiteral convertLiteralInValuesList(SqlNode sqlNode, Blackboard bb, RelDataType rowType, int iField) {
        ...
        if ((value instanceof NlsString) && (type.getSqlTypeName() == SqlTypeName.CHAR)) {
            ...
            return rexBuilder.makeCharLiteral(
                    new NlsString(Spaces.padRight(unpadded.getValue(), type.getPrecision()), unpadded.getCharsetName(),
        ...
        switch (from.getKind()) {
            ...
            case JOIN:
                ...
                if (isNatural) {
                    ...
                    conditionExp = convertJoinCondition(fromBlackboard, leftNamespace, rightNamespace,
                                                        join.getCondition(), join.getConditionType(), leftRel,
                                                        rightRel);
                }
            ...
            case UNNEST:
                ...
                final RelNode input = RelOptUtil.createProject(
                        (null != bb.root) ? bb.root : LogicalValues.createOneRow(cluster), exprs, fieldNames, true);

                Uncollect uncollect = new Uncollect(cluster, cluster.traitSetOf(Convention.NONE), input,
    ...
    protected void convertMatchRecognize(Blackboard bb, SqlCall call) {
        ...
        final RelNode rel = factory.createMatchRecognize(input, patternNode, rowType,
    ...
    private void convertIdentifier(Blackboard bb, SqlIdentifier id, SqlNodeList extendedColumns) {
        ...
        if (extendedColumns != null && extendedColumns.size() > 0) {
            ...
            final List<RelDataTypeField> extendedFields = SqlValidatorUtil.getExtendedColumns(
                    validator.getTypeFactory(), validatorTable, extendedColumns);
        ...
        try {
            ...
            for (SqlNode expr : selectList) {
                projects.add(Pair.of(bb.convertExpression(expr),
                                     k < sysFieldCount ? validator.deriveAlias(expr, k++) : names.get(
                                             k++ - sysFieldCount)));
            }
    ...
    private RelNode convertMultisets(final List<SqlNode> operands, Blackboard bb) {
        ...
        for (int i = 0; i < joinList.size(); i++) {
            ...
            if (o instanceof List) {
                ...
                RelNode projRel = RelOptUtil.createProject(LogicalValues.createOneRow(cluster), selectList,
                                                           fieldNameList);
                    ...
                    case ALL:
                        ...
                        if (!(query instanceof SqlNodeList)) {
                            ...
                            switch (kind) {
                                ...
                                case ALL:
                                    return rexBuilder.makeCall(SqlStdOperatorTable.NOT, RexSubQuery.some(root.rel, list,
    ...
    protected class AggConverter implements SqlVisitor<Void> {
        ...
        private void translateAgg(SqlCall call, SqlNode filter, SqlCall outerCall) {
            ...
            int filterArg = -1;
            final List<RelDataType> argTypes = call.getOperator() instanceof SqlCountAggFunction ? new ArrayList<RelDataType>(
            ...
            final AggregateCall aggCall = AggregateCall.create(aggFunction, distinct, args, filterArg, type,
        ...
        public RexNode visitCall(RexCall call) {
            ...
            if (histogramOp != null) {
                ...
                if (histogramType != type) {
                    ...
                    exprs.set(0, reinterpretCast ? rexBuilder.makeReinterpretCast(histogramType, exprs.get(0),
                                                                                  rexBuilder.makeLiteral(
                                                                                          false)) : rexBuilder.makeCast(
                            histogramType, exprs.get(0)));
                }

                RexCallBinding bind = new RexCallBinding(rexBuilder.getTypeFactory(), SqlStdOperatorTable.HISTOGRAM_AGG,
                                                         exprs, ImmutableList.<RelCollation>of());

                RexNode over = rexBuilder.makeOver(SqlStdOperatorTable.HISTOGRAM_AGG.inferReturnType(bind),
                                                   SqlStdOperatorTable.HISTOGRAM_AGG, exprs, partitionKeys, orderKeys,
                                                   lowerBound, upperBound, window.isRows(), window.isAllowPartial(),
                                                   false, distinct);
