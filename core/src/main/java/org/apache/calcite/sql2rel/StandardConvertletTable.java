...
public class StandardConvertletTable extends ReflectiveConvertletTable {
    ...
    private StandardConvertletTable() {
        ...
        registerOp(SqlStdOperatorTable.CAST, new SqlRexConvertlet() {

            public RexNode convertCall(SqlRexContext cx, SqlCall call) {
                return convertCast(cx, call);
            }
        });
        registerOp(SqlStdOperatorTable.IS_DISTINCT_FROM, new SqlRexConvertlet() {

            public RexNode convertCall(SqlRexContext cx, SqlCall call) {
                return convertIsDistinctFrom(cx, call, false);
            }
        });
        registerOp(SqlStdOperatorTable.IS_NOT_DISTINCT_FROM, new SqlRexConvertlet() {
        ...
        registerOp(SqlStdOperatorTable.PLUS, new SqlRexConvertlet() {
        ...
        registerOp(OracleSqlOperatorTable.NVL, new SqlRexConvertlet() {
            ...
            public RexNode convertCall(SqlRexContext cx, SqlCall call) {
                ...
                return rexBuilder.makeCall(type, SqlStdOperatorTable.CASE, ImmutableList.of(
                        rexBuilder.makeCall(SqlStdOperatorTable.IS_NOT_NULL, operand0),
                        rexBuilder.makeCast(type, operand0), rexBuilder.makeCast(type, operand1)));
            }
        });

        registerOp(OracleSqlOperatorTable.DECODE, new SqlRexConvertlet() {

            public RexNode convertCall(SqlRexContext cx, SqlCall call) {
                final RexBuilder rexBuilder = cx.getRexBuilder();
                final List<RexNode> operands = convertExpressionList(cx, call.getOperandList(),
        ...
        registerOp(SqlStdOperatorTable.NOT_LIKE, new SqlRexConvertlet() {

            public RexNode convertCall(SqlRexContext cx, SqlCall call) {
                final SqlCall expanded = SqlStdOperatorTable.NOT.createCall(SqlParserPos.ZERO,
        ...
        registerOp(SqlStdOperatorTable.NOT_SIMILAR_TO, new SqlRexConvertlet() {
        ...
        registerOp(SqlStdOperatorTable.UNARY_PLUS, new SqlRexConvertlet() {
        ...
        registerOp(SqlStdOperatorTable.AS, new SqlRexConvertlet() {
        ...
        registerOp(SqlStdOperatorTable.SQRT, new SqlRexConvertlet() {

            public RexNode convertCall(SqlRexContext cx, SqlCall call) {
                SqlNode expanded = SqlStdOperatorTable.POWER.createCall(SqlParserPos.ZERO, call.operand(0),
        ...
        if (false) {
            registerOp(SqlStdOperatorTable.ELEMENT, new SqlRexConvertlet() {
                ...
                public RexNode convertCall(SqlRexContext cx, SqlCall call) {
                    ...
                    if (!type.getComponentType().isStruct()) {
                        return cx.convertExpression(
                                SqlStdOperatorTable.ELEMENT_SLICE.createCall(SqlParserPos.ZERO, operand));
                    }
        ...
        if (false) {
            registerOp(SqlStdOperatorTable.ELEMENT_SLICE, new SqlRexConvertlet() {
    ...
    protected RexNode convertCast(SqlRexContext cx, final SqlCall call) {
        ...
        if (null != dataType.getCollectionsTypeName()) {
            ...
            if (argComponentType.isStruct() && !componentType.isStruct()) {
                RelDataType tt = typeFactory.builder().add(argComponentType.getFieldList().get(0).getName(),
                                                           componentType).build();
    ...
    protected RexNode convertFloorCeil(SqlRexContext cx, SqlCall call) {
        ...
        if (call.operandCount() == 1 && call.operand(0) instanceof SqlIntervalLiteral) {
            ...
            RexNode kase = floor ? case_(rexBuilder, rexInterval, cond, sum) : case_(rexBuilder, sum, cond,
                                                                                     rexInterval);
    ...
    public RexNode convertExtract(SqlRexContext cx, SqlExtractFunction op, SqlCall call) {
        ...
        final TimeUnit unit = ((SqlIntervalQualifier) operands.get(0)).getStartUnit();
            ...
            case WEEK:
                switch (sqlTypeName) {
                    ...
                    case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
                        RelDataType type = cx.getTypeFactory().createSqlType(SqlTypeName.TIMESTAMP);
                        type = cx.getTypeFactory().createTypeWithNullability(type, exprs.get(1).getType().isNullable());
            ...
            case EPOCH:
                switch (sqlTypeName) {
                    case DATE:
                        ...
                        res = rexBuilder.makeCall(resType, SqlStdOperatorTable.MULTIPLY, ImmutableList.of(res,
                    ...
                    case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
                        RelDataType type = cx.getTypeFactory().createSqlType(SqlTypeName.TIMESTAMP);
                        type = cx.getTypeFactory().createTypeWithNullability(type, exprs.get(1).getType().isNullable());
                        return divide(rexBuilder, rexBuilder.makeCast(type, res), TimeUnit.SECOND.multiplier);
                    case INTERVAL_YEAR:
                    case INTERVAL_YEAR_MONTH:
                    case INTERVAL_MONTH:
                    case INTERVAL_DAY:
    ...
    public RexNode convertDatetimeMinus(SqlRexContext cx, SqlDatetimeSubtractionOperator op, SqlCall call) {
        ...
        casts[0] = rexBuilder.makeCast(
                cx.getTypeFactory().createTypeWithNullability(int8Type, exprs.get(0).getType().isNullable()),
                exprs.get(0));
        casts[1] = rexBuilder.makeCast(
                cx.getTypeFactory().createTypeWithNullability(int8Type, exprs.get(1).getType().isNullable()),
                exprs.get(1));
    ...
    public RexNode convertAggregateFunction(SqlRexContext cx, SqlAggFunction fun, SqlCall call) {
        ...
        if (returnType == null) {
            RexCallBinding binding = new RexCallBinding(cx.getTypeFactory(), fun, exprs,
    ...
    private static RexNode makeConstructorCall(SqlRexContext cx, SqlFunction constructor, List<RexNode> exprs) {
        ...
        for (int i = 0; i < n; ++i) {
            initializationExprs.add(
                    cx.getInitializerExpressionFactory().newAttributeInitializer(type, constructor, i, exprs,
                                                                                 initializerContext));
        }
    ...
    private RexNode convertCall(SqlRexContext cx, SqlCall call, SqlOperator op) {
        final List<SqlNode> operands = call.getOperandList();
        final RexBuilder rexBuilder = cx.getRexBuilder();
        final SqlOperandTypeChecker.Consistency consistency = op.getOperandTypeChecker()
                                                              == null ? SqlOperandTypeChecker.Consistency.NONE : op.getOperandTypeChecker().getConsistency();
    ...
    private static RelDataType consistentType(SqlRexContext cx, SqlOperandTypeChecker.Consistency consistency,
    ...
    public RexNode convertBetween(SqlRexContext cx, SqlBetweenOperator op, SqlCall call) {
        final List<RexNode> list = convertExpressionList(cx, call.getOperandList(),
    ...
    public RexNode convertOverlaps(SqlRexContext cx, SqlOverlapsOperator op, SqlCall call) {
        ...
        final Pair<RexNode, RexNode> left = convertOverlapsOperand(cx, call.getParserPosition(), call.operand(0));
        final RexNode r0 = left.left;
        final RexNode r1 = left.right;
        final Pair<RexNode, RexNode> right = convertOverlapsOperand(cx, call.getParserPosition(), call.operand(1));
    ...
    private static class AvgVarianceConvertlet implements SqlRexConvertlet {
        ...
        private SqlNode expandAvg(final SqlNode arg, final RelDataType avgType, final SqlRexContext cx) {
            ...
            if (!sumRex.getType().equals(avgType)) {
                sumCast = SqlStdOperatorTable.CAST.createCall(pos, new SqlDataTypeSpec(
                        new SqlIdentifier(avgType.getSqlTypeName().getName(), pos), avgType.getPrecision(),
                        avgType.getScale(), null, null, pos));
            } else {
                sumCast = sum;
            }
        ...
        private SqlNode expandVariance(final SqlNode argInput, final RelDataType varType, final SqlRexContext cx,
                                       boolean biased, boolean sqrt) {
            // stddev_pop(x) ==>
            //   power(
            ...
            if (!argRex.getType().equals(varType)) {
                arg = SqlStdOperatorTable.CAST.createCall(pos, new SqlDataTypeSpec(
                        new SqlIdentifier(varType.getSqlTypeName().getName(), pos), varType.getPrecision(),
                        varType.getScale(), null, null, pos));
            } else {
                arg = argInput;
            }
    ...
    private static class TrimConvertlet implements SqlRexConvertlet {
        ...
        public RexNode convertCall(SqlRexContext cx, SqlCall call) {
            ...
            return rexBuilder.makeCall(SqlStdOperatorTable.TRIM, rexBuilder.makeFlag(flag), rexBuilder.makeLiteral(" "),
                                       operand);
        }
    }
    ...
    private static class GreatestConvertlet implements SqlRexConvertlet {
        ...
        public RexNode convertCall(SqlRexContext cx, SqlCall call) {
            ...
            final List<RexNode> exprs = convertExpressionList(cx, call.getOperandList(),
                                                              SqlOperandTypeChecker.Consistency.NONE);
    ...
    private static class TimestampAddConvertlet implements SqlRexConvertlet {
        ...
        public RexNode convertCall(SqlRexContext cx, SqlCall call) {
            ...
            return rexBuilder.makeCall(SqlStdOperatorTable.DATETIME_PLUS, cx.convertExpression(call.operand(2)),
                                       multiply(rexBuilder, rexBuilder.makeIntervalLiteral(unit.multiplier,
    ...
    private static class TimestampDiffConvertlet implements SqlRexConvertlet {
        ...
        public RexNode convertCall(SqlRexContext cx, SqlCall call) {
            ...
            final SqlIntervalQualifier qualifier = new SqlIntervalQualifier(unit, null, SqlParserPos.ZERO);
            ...
            final RexCall rexCall = (RexCall) rexBuilder.makeCall(intervalType, SqlStdOperatorTable.MINUS_DATE,
