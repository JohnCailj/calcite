...
public class PigRelBuilder extends RelBuilder {
    ...
    public PigRelBuilder group(GroupOption option, Partitioner partitioner, int parallel,
                               Iterable<? extends GroupKey> groupKeys) {
        @SuppressWarnings("unchecked") final List<GroupKeyImpl> groupKeyList = ImmutableList.copyOf(
                (Iterable) groupKeys);
        ...
        for (Ord<GroupKeyImpl> groupKey : Ord.reverse(groupKeyList)) {
            ...
            final RexNode row = cluster.getRexBuilder().makeCall(peek(1, 0).getRowType(), SqlStdOperatorTable.ROW,
                                                                 fields());
            aggregate(groupKey.e, aggregateCall(SqlStdOperatorTable.COLLECT, false, null, getAlias(), row));
            if (groupKey.i < n - 1) {
                push(r);
