...
public class RelBuilder {
    ...
    private RexNode call(SqlOperator operator, List<RexNode> operandList) {
        ...
        if (type == null) {
            throw new IllegalArgumentException(
                    "cannot derive type: " + operator + "; operands: " + Lists.transform(operandList, FN_TYPE));
        }
    ...
    private GroupKey groupKey_(ImmutableBitSet groupSet, boolean indicator, ImmutableList<ImmutableBitSet> groupSets) {
        ...
        final List<ImmutableList<RexNode>> nodeLists = Lists.transform(groupSets,
                                                                       new Function<ImmutableBitSet, ImmutableList<RexNode>>() {

                                                                           public ImmutableList<RexNode> apply(
                                                                                   ImmutableBitSet input) {
                                                                               return fields(ImmutableIntList.of(
                                                                                       input.toArray()));
                                                                           }
                                                                       });
    ...
    public AggCall aggregateCall(SqlAggFunction aggFunction, boolean distinct, RexNode filter, String alias,
                                 RexNode... operands) {
        return aggregateCall(aggFunction, distinct, filter, alias, ImmutableList.copyOf(operands));
    }
    ...
    public AggCall count(boolean distinct, String alias, RexNode... operands) {
        return aggregateCall(SqlStdOperatorTable.COUNT, distinct, null, alias, operands);
    }
    ...
    public AggCall sum(boolean distinct, String alias, RexNode operand) {
        return aggregateCall(SqlStdOperatorTable.SUM, distinct, null, alias, operand);
    }
    ...
    public AggCall avg(boolean distinct, String alias, RexNode operand) {
        return aggregateCall(SqlStdOperatorTable.AVG, distinct, null, alias, operand);
    }
    ...
    public RelBuilder project(Iterable<? extends RexNode> nodes, Iterable<String> fieldNames, boolean force) {
        ...
        final Set<String> uniqueNameList = getTypeFactory().getTypeSystem().isSchemaCaseSensitive() ? new HashSet<String>() : new TreeSet<>(
    ...
    public RelBuilder aggregate(GroupKey groupKey, Iterable<AggCall> aggCalls) {
        ...
        if (groupKey_.nodeLists != null) {
            ...
            if (extraNodes.size() > sizeBefore) {
                throw new IllegalArgumentException(
                        "group sets contained expressions not in group key: " + extraNodes.subList(sizeBefore,
                                                                                                   extraNodes.size()));
            }
        } else {
            groupSets = ImmutableList.of(groupSet);
        ...
        for (AggCall aggCall : aggCalls) {
            ...
            if (aggCall instanceof AggCallImpl) {
                ...
                aggregateCall = AggregateCall.create(aggCall1.aggFunction, aggCall1.distinct, args, filterArg,
    ...
    public RelBuilder join(JoinRelType joinType, Iterable<? extends RexNode> conditions) {
        return join(joinType, and(conditions), ImmutableSet.<CorrelationId>of());
    }
    ...
    public RelBuilder as(final String alias) {
        ...
        List<Field> newFields = Lists.transform(pair.fields, new Function<Field, Field>() {

            public Field apply(Field field) {
                return new Field(ImmutableSet.<String>builder().addAll(field.left).add(alias).build(), field.right);
            }
        });
    ...
    public RelBuilder aggregate(GroupKey groupKey, List<AggregateCall> aggregateCalls) {
        return aggregate(groupKey, Lists.transform(aggregateCalls, new Function<AggregateCall, AggCall>() {
    ...
    private static class AggCallImpl implements AggCall {
        ...
        AggCallImpl(SqlAggFunction aggFunction, boolean distinct, RexNode filter, String alias,
    ...
    private static class Frame {
        ...
        private Frame(RelNode rel) {
            ...
            ImmutableSet<String> aliases = tableAlias == null ? ImmutableSet.<String>of() : ImmutableSet.of(tableAlias);
