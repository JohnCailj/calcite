...
public class ImmutableBitSet implements Iterable<Integer>, Serializable, Comparable<ImmutableBitSet> {
     ...
     */
    public static final Comparator<ImmutableBitSet> COMPARATOR = new Comparator<ImmutableBitSet>() {
    ...
    public static final Function<? super BitSet, ImmutableBitSet> FROM_BIT_SET = new Function<BitSet, ImmutableBitSet>() {
    ...
    public Iterable<ImmutableBitSet> powerSet() {
        ...
        return Iterables.transform(Linq4j.product(singletons), new Function<List<ImmutableBitSet>, ImmutableBitSet>() {
    ...
    public ImmutableBitSet union(BitSet other) {
        return rebuild() // remember "this" and try to re-use later
                         .addAll(BitSets.toIter(other)).build();
    }
    ...
    public int length() {
        ...
        return BITS_PER_WORD * (words.length - 1) + (BITS_PER_WORD - Long.numberOfLeadingZeros(
     ...
     */
    public static Iterable<ImmutableBitSet> permute(Iterable<ImmutableBitSet> bitSets,
                                                    final Map<Integer, Integer> map) {
        return Iterables.transform(bitSets, new Function<ImmutableBitSet, ImmutableBitSet>() {
    ...
    private static class Closure {
        ...
        private final SortedMap<Integer, ImmutableBitSet> closure = Maps.newTreeMap();
