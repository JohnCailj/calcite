...
public class PartiallyOrderedSet<E> extends AbstractSet<E> {
    ...
    @SuppressWarnings("NullableProblems") @Override public Iterator<E> iterator() {
    ...
    private Set<Node<E>> findParentsChildren(E e, Deque<Node<E>> ancestors, boolean up) {
        ...
        while (!ancestors.isEmpty()) {
            ...
            assert ancestor.e == null || (up ? !ordering.lessThan(ancestor.e, e) : !ordering.lessThan(e, ancestor.e));
     ...
     */
    @SuppressWarnings({ "ConstantConditions" }) public boolean isValid(boolean fail) {
        // Top has no parents.
        ...
        for (Node<E> node : map.values()) {
            ...
            for (int i = 0; i < node.childList.size(); i++) {
                ...
                for (int i2 = 0; i2 < node.childList.size(); i2++) {
                    ...
                    if (child.e != null && child2.e != null && child != child2 && ordering.lessThan(child.e,
                                                                                                    child2.e)) {
                        assert !fail :
                                "relation between children " + child.e + " and " + child2.e + " of node " + node.e;
                        return false;
                    }
                }
            }
        ...
        for (Node<E> node1 : map.values()) {
            for (Node<E> node2 : map.values()) {
                ...
                if (lt12 && lt21) {
                    if (!(node1 == node2)) {
                        assert !fail : "node " + node1.e + " and node " + node2.e
                                       + " are less than each other but are not the same" + " value";
                        return false;
                    }
                }
                if (lt12 && !lt21) {
                    if (!nodeAncestors.get(node1).contains(node2.e)) {
                        assert !fail : node1.e + " is less than " + node2.e + " but " + node2.e
                                       + " is not in the ancestor set of " + node1.e;
                        return false;
                    }
                    if (!nodeDescendants.get(node2).contains(node1.e)) {
                        assert !fail : node1.e + " is less than " + node2.e + " but " + node1.e
                                       + " is not in the descendant set of " + node2.e;
                        return false;
                    }
                }
                if (lt21 && !lt12) {
                    if (!nodeAncestors.get(node2).contains(node1.e)) {
                        assert !fail : node2.e + " is less than " + node1.e + " but " + node1.e
                                       + " is not in the ancestor set of " + node2.e;
                        return false;
                    }
                    if (!nodeDescendants.get(node1).contains(node2.e)) {
                        assert !fail : node2.e + " is less than " + node1.e + " but " + node2.e
                                       + " is not in the descendant set of " + node1.e;
                        return false;
                    }
                }
            }
    ...
    public List<E> getNonChildren() {
        if (topNode.childList.size() == 1 && topNode.childList.get(0).e == null) {
            return Collections.emptyList();
        }
    ...
    public List<E> getNonParents() {
        if (bottomNode.parentList.size() == 1 && bottomNode.parentList.get(0).e == null) {
            return Collections.emptyList();
        }
