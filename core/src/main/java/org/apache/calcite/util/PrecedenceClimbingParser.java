...
public class PrecedenceClimbingParser {
    ...
    private Op highest() {
        ...
        for (Token t = first; t != null; t = t.next) {
            if ((t.left > p || t.right > p) && (t.left < 0 || t.left >= prevRight(t.previous)) && (t.right < 0
                                                                                                   || t.right
                                                                                                      >= nextLeft(
                    t.next))) {
    ...
    public static class Builder {
        ...
        final         List<Token>              tokens = new ArrayList<>();
        private final PrecedenceClimbingParser dummy  = new PrecedenceClimbingParser(ImmutableList.<Token>of());
