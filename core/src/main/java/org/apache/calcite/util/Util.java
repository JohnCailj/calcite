...
public class Util {
     ...
     */
    private static final LoadingCache<Class, Map<String, Enum>> ENUM_CONSTANTS = CacheBuilder.newBuilder().weakKeys().build(
    ...
    public static void print(PrintWriter pw, Object o, int indent) {
        ...
        } else if ((clazz == Integer.class) || (clazz == Boolean.class) || (clazz == Character.class) || (clazz
                                                                                                          == Byte.class)
                   || (clazz == Short.class) || (clazz == Long.class) || (clazz == Float.class) || (clazz
                                                                                                    == Double.class)
                   || (clazz == Void.class)) {
            pw.print(o.toString());
    ...
    public static String toJavaId(String s, int ordinal) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (c == '_') {
                buf.append("__");
            } else if ((c < 0x7F) /* Normal ascii character */ && !Character.isISOControl(c) && ((i
                                                                                                  == 0) ? Character.isJavaIdentifierStart(
    ...
    public static String getMessages(Throwable t) {
        ...
        for (Throwable curr = t; curr != null; curr = curr.getCause()) {
            String msg = ((curr instanceof CalciteException)
                          || (curr instanceof SQLException)) ? curr.getMessage() : curr.toString();
            if (sb.length() > 0) {
                sb.append("\n");
            }
            sb.append(msg);
    ...
    public static String toPosix(TimeZone tz, boolean verbose) {
        ...
        String patternString = ".*," + "startMode=([0-9]*)," + "startMonth=([0-9]*)," + "startDay=([-0-9]*),"
                               + "startDayOfWeek=([0-9]*)," + "startTime=([0-9]*)," + "startTimeMode=([0-9]*),"
                               + "endMode=([0-9]*)," + "endMonth=([0-9]*)," + "endDay=([-0-9]*),"
                               + "endDayOfWeek=([0-9]*)," + "endTime=([0-9]*)," + "endTimeMode=([0-9]*).*";
        ...
        appendPosixDaylightTransition(tz, buf, startMode, startDay, startMonth, startDayOfWeek, startTime,
                                      startTimeMode, verbose, false);
        appendPosixDaylightTransition(tz, buf, endMode, endDay, endMonth, endDayOfWeek, endTime, endTimeMode, verbose,
                                      true);
        return buf.toString();
    }
     ...
     */
    private static void appendPosixDaylightTransition(TimeZone tz, StringBuilder buf, int mode, int day, int month,
                                                      int dayOfWeek, int time, int timeMode, boolean verbose,
                                                      boolean isEnd) {
        buf.append(',');
        int week = day;
        switch (mode) {
    ...
    public static <E extends Enum<E>> Error unexpected(E value) {
        return new AssertionError(
                "Was not expecting value '" + value + "' for enumeration '" + value.getDeclaringClass().getName()
                + "' in this context");
    }
     ...
     */
    public static <K, V> Map<K, V> asIndexMap(final Collection<V> values, final Function<V, K> function) {
        final Collection<Map.Entry<K, V>> entries = Collections2.transform(values, new Function<V, Map.Entry<K, V>>() {
    ...
    public static class FoundOne extends ControlFlowException {
         ...
         */
        @SuppressWarnings("ThrowableInstanceNeverThrown") public static final FoundOne NULL = new FoundOne(null);
