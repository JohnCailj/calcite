...
public abstract class Mappings {
    ...
    public static Mapping divide(Mapping mapping1, Mapping mapping2) {
        ...
        Mapping remaining = create(MappingType.INVERSE_SURJECTION, mapping2.getTargetCount(),
    ...
    public static Mapping multiply(Mapping mapping1, Mapping mapping2) {
        ...
        Mapping product = create(MappingType.INVERSE_SURJECTION, mapping1.getSourceCount(), mapping2.getTargetCount());
     ...
     */
    public static ImmutableList<ImmutableBitSet> apply2(final Mapping mapping, Iterable<ImmutableBitSet> bitSets) {
        return ImmutableList.copyOf(Iterables.transform(bitSets, new Function<ImmutableBitSet, ImmutableBitSet>() {
    ...
    public static <T> List<T> apply(final Mapping mapping, final List<T> list) {
        if (mapping.getSourceCount() != list.size()) {
            // REVIEW: too strict?
            throw new IllegalArgumentException(
                    "mapping source count " + mapping.getSourceCount() + " does not match list size " + list.size());
        }
    ...
    public static TargetMapping target(Map<Integer, Integer> map, int sourceCount, int targetCount) {
        final PartialFunctionImpl mapping = new PartialFunctionImpl(sourceCount, targetCount, MappingType.FUNCTION);
    ...
    public static TargetMapping target(Function<Integer, Integer> function, int sourceCount, int targetCount) {
        final PartialFunctionImpl mapping = new PartialFunctionImpl(sourceCount, targetCount, MappingType.FUNCTION);
    ...
    public static TargetMapping createShiftMapping(int sourceCount, int... ints) {
        ...
        final TargetMapping mapping = create(MappingType.INVERSE_SURJECTION, sourceCount, // aCount + bCount + cCount,
     ...
     */
    public static TargetMapping offsetSource(final TargetMapping mapping, final int offset) {
     ...
     * @return Shifted mapping
     */
    public static TargetMapping offsetSource(final TargetMapping mapping, final int offset, final int sourceCount) {
        if (sourceCount < mapping.getSourceCount() + offset) {
            throw new IllegalArgumentException("new source count too low");
        }
        return target(new Function<Integer, Integer>() {

            public Integer apply(Integer source) {
                int source2 = source - offset;
                return source2 < 0 || source2 >= mapping.getSourceCount() ? null : mapping.getTargetOpt(source2);
            }
     ...
     * @return Shifted mapping
     */
    public static TargetMapping offsetTarget(final TargetMapping mapping, final int offset, final int targetCount) {
     ...
     * @return Shifted mapping
     */
    public static TargetMapping offset(final TargetMapping mapping, final int offset, final int sourceCount) {
        if (sourceCount < mapping.getSourceCount() + offset) {
            throw new IllegalArgumentException("new source count too low");
        }
        return target(new Function<Integer, Integer>() {
    ...
    public abstract static class AbstractMapping implements Mapping {
        ...
        public int getTarget(int source) {
            ...
            if (target == -1) {
                throw new NoElementException("source #" + source + " has no target in mapping " + toString());
            }
            return target;
        }
        ...
        public int getSource(int target) {
            ...
            if (source == -1) {
                throw new NoElementException("target #" + target + " has no source in mapping " + toString());
            }
            return source;
        }
        ...
        public String toString() {
            ...
            buf.append("[size=").append(size()).append(", sourceCount=").append(getSourceCount()).append(
                    ", targetCount=").append(getTargetCount()).append(", elements=[");
            int i = 0;
    ...
    public static class OverridingTargetMapping extends AbstractMapping implements TargetMapping {
        ...
        public boolean isIdentity() {
            ...
            // fixes it.
            return (source == target) && ((Mapping) parent).isIdentity();
        }
     ...
     */
    private static class PartialFunctionImpl extends AbstractMapping implements TargetMapping {
