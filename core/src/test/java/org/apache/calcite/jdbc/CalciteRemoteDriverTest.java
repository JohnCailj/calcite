...
public class CalciteRemoteDriverTest {
    ...
    private final PrintWriter out = CalcitePrepareImpl.DEBUG ? Util.printWriter(System.out) : new PrintWriter(
            new StringWriter());
    ...
    @Test public void testRemoteSchemas() throws Exception {
        CalciteAssert.hr().with(REMOTE_CONNECTION_FACTORY).metaData(GET_SCHEMAS).returns(
                "TABLE_SCHEM=POST; TABLE_CATALOG=null\n" + "TABLE_SCHEM=foodmart; TABLE_CATALOG=null\n"
    ...
    @Test public void testRemoteColumns() throws Exception {
        CalciteAssert.hr().with(REMOTE_CONNECTION_FACTORY).metaData(GET_COLUMNS).returns(
                CalciteAssert.checkResultContains("COLUMN_NAME=EMPNO"));
    }

    @Test public void testRemoteTypeInfo() throws Exception {
        CalciteAssert.hr().with(REMOTE_CONNECTION_FACTORY).metaData(GET_TYPEINFO).returns(
                CalciteAssert.checkResultCount(is(45)));
    }

    @Test public void testRemoteTableTypes() throws Exception {
        CalciteAssert.hr().with(REMOTE_CONNECTION_FACTORY).metaData(GET_TABLE_TYPES).returns(
                "TABLE_TYPE=TABLE\n" + "TABLE_TYPE=VIEW\n");
    }

    @Test public void testRemoteExecuteQuery() throws Exception {
        CalciteAssert.hr().with(REMOTE_CONNECTION_FACTORY).query(
                "values (1, 'a'), (cast(null as integer), 'b')").returnsUnordered("EXPR$0=1; EXPR$1=a",
    ...
    @Test public void testRemoteExecuteQuery2() throws Exception {
        ...
        final ResultSet resultSet = statement.executeQuery("values (1, 'a'), (cast(null as integer), 'b')");
        int n = 0;
    ...
    @Test public void testParameterConvert() throws Exception {
        ...
        for (Map.Entry<Class, SqlType> entry : SqlType.getSetConversions()) {
            ...
            for (Object sampleValue : values(sqlType.boxedClass())) {
                ...
                out.println("check " + o + " (originally " + sampleValue.getClass() + ", now " + o.getClass()
                            + ") converted to " + sqlType);
                if (o instanceof Double && o.equals(Double.POSITIVE_INFINITY) || o instanceof Float && o.equals(
    ...
    @Test public void testTableB5() {
        SqlType[] columns = { SqlType.TINYINT, SqlType.SMALLINT, SqlType.INTEGER, SqlType.BIGINT, SqlType.REAL,
                              SqlType.FLOAT, SqlType.DOUBLE, SqlType.DECIMAL, SqlType.NUMERIC, SqlType.BIT,
                              SqlType.BOOLEAN, SqlType.CHAR, SqlType.VARCHAR, SqlType.LONGVARCHAR, SqlType.BINARY,
                              SqlType.VARBINARY, SqlType.LONGVARBINARY, SqlType.DATE, SqlType.TIME, SqlType.TIMESTAMP,
                              SqlType.ARRAY, SqlType.BLOB, SqlType.CLOB, SqlType.STRUCT, SqlType.REF, SqlType.DATALINK,
                              SqlType.JAVA_OBJECT, SqlType.ROWID, SqlType.NCHAR, SqlType.NVARCHAR, SqlType.LONGNVARCHAR,
                              SqlType.NCLOB, SqlType.SQLXML };
        Class[] rows = { String.class, BigDecimal.class, Boolean.class, Byte.class, Short.class, Integer.class,
                         Long.class, Float.class, Double.class, byte[].class, BigInteger.class, java.sql.Date.class,
                         Time.class, Timestamp.class, Array.class, Blob.class, Clob.class, Struct.class, Ref.class,
                         URL.class, Class.class, RowId.class, NClob.class, SQLXML.class, Calendar.class,
    ...
    @Test public void testTableB6() {
        SqlType[] columns = { SqlType.TINYINT, SqlType.SMALLINT, SqlType.INTEGER, SqlType.BIGINT, SqlType.REAL,
                              ...
                              SqlType.BOOLEAN, SqlType.CHAR, SqlType.VARCHAR, SqlType.LONGVARCHAR, SqlType.BINARY,
                              SqlType.VARBINARY, SqlType.LONGVARBINARY, SqlType.DATE, SqlType.TIME, SqlType.TIMESTAMP,
                              SqlType.CLOB, SqlType.BLOB, SqlType.ARRAY, SqlType.REF, SqlType.DATALINK, SqlType.STRUCT,
                              SqlType.JAVA_OBJECT, SqlType.ROWID, SqlType.NCHAR, SqlType.NVARCHAR, SqlType.LONGNVARCHAR,
                              SqlType.NCLOB, SqlType.SQLXML };
        final PrintWriter out = CalcitePrepareImpl.DEBUG ? Util.printWriter(System.out) : new PrintWriter(
                new StringWriter());
    ...
    @Test(expected = SQLException.class) public void testAvaticaConnectionException() throws Exception {
        remoteConnection.isValid(0);
    }

    @Test(expected = SQLException.class) public void testAvaticaStatementException() throws Exception {
     ...
     */
    private static final List<Object> SAMPLE_VALUES = ImmutableList.<Object>of(false, true,
                                                                               // byte
                                                                               (byte) 0, (byte) 1, Byte.MIN_VALUE,
                                                                               Byte.MAX_VALUE,
                                                                               // short
                                                                               (short) 0, (short) 1, Short.MIN_VALUE,
                                                                               Short.MAX_VALUE, (short) Byte.MIN_VALUE,
                                                                               (short) Byte.MAX_VALUE,
                                                                               // int
                                                                               0, 1, -3, Integer.MIN_VALUE,
                                                                               Integer.MAX_VALUE, (int) Short.MIN_VALUE,
                                                                               (int) Short.MAX_VALUE,
                                                                               (int) Byte.MIN_VALUE,
                                                                               (int) Byte.MAX_VALUE,
                                                                               // long
                                                                               0L, 1L, -2L, Long.MIN_VALUE,
                                                                               Long.MAX_VALUE, (long) Integer.MIN_VALUE,
                                                                               ...
                                                                               (long) Byte.MAX_VALUE,
                                                                               // float
                                                                               0F, 1.5F, -10F, Float.MIN_VALUE,
                                                                               Float.MAX_VALUE,
                                                                               // double
                                                                               0D, Math.PI, Double.MIN_VALUE,
                                                                               Double.MAX_VALUE,
                                                                               ...
                                                                               // datetime
                                                                               new Timestamp(0), new java.sql.Date(0),
                                                                               new Time(0),
                                                                               // string
                                                                               "", "foo", " foo! Baz ",
                                                                               // byte[]
    ...
    private Object convert(Object o, Class clazz) throws ParseException {
        ...
        throw new AssertionError("cannot convert " + o + "(" + o.getClass() + ") to " + clazz);
    }
    ...
    public static class Factory2 implements Service.Factory {
        ...
        public Service create(AvaticaConnection connection) {
            try {
                ...
                final Meta meta = CalciteConnectionImpl.TROJAN.getMeta((CalciteConnectionImpl) localConnection);
