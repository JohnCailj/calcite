...
public class RelWriterTest {
    ...
    @Test public void testWriter() {
        String s = Frameworks.withPlanner(new Frameworks.PlannerAction<String>() {
            ...
            public String apply(RelOptCluster cluster, RelOptSchema relOptSchema, SchemaPlus rootSchema) {
                ...
                LogicalTableScan scan = LogicalTableScan.create(cluster, relOptSchema.getTableForMember(
                        Arrays.asList("hr", "emps")));
                final RexBuilder rexBuilder = cluster.getRexBuilder();
                LogicalFilter filter = LogicalFilter.create(scan, rexBuilder.makeCall(SqlStdOperatorTable.EQUALS,
                ...
                LogicalAggregate aggregate = LogicalAggregate.create(filter, ImmutableBitSet.of(0), null,
                                                                     ImmutableList.of(AggregateCall.create(
                                                                             SqlStdOperatorTable.COUNT, true,
                                                                             ImmutableList.of(1), -1, bigIntType, "c"),
    ...
    @Test public void testReader() {
        ...
        assertThat(Util.toLinux(s), is("LogicalAggregate(group=[{0}], agg#0=[COUNT(DISTINCT $1)], agg#1=[COUNT()])\n"
