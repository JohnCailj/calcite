...
import static org.apache.calcite.plan.volcano.PlannerTests.*;
...
public class CollationConversionTest {
    ...
    private static final TestRelCollationImpl LEAF_COLLATION = new TestRelCollationImpl(
            ImmutableList.of(new RelFieldCollation(0, Direction.CLUSTERED)));

    private static final TestRelCollationImpl ROOT_COLLATION = new TestRelCollationImpl(
            ImmutableList.of(new RelFieldCollation(0)));
    ...
    @Test public void testCollationConversion() {
        ...
        final RelNode convertedRel = planner.changeTraits(singleRel, cluster.traitSetOf(PHYS_CALLING_CONVENTION).plus(
                ROOT_COLLATION));
    ...
    private class SingleNodeRule extends RelOptRule {
        ...
        public void onMatch(RelOptRuleCall call) {
            ...
            RelNode physInput = convert(input,
                                        single.getTraitSet().replace(PHYS_CALLING_CONVENTION).plus(ROOT_COLLATION));
    ...
    private static class TestRelCollationTraitDef extends RelTraitDef<RelCollation> {
        ...
        public RelNode convert(RelOptPlanner planner, RelNode rel, RelCollation toCollation,
            ...
            return new PhysicalSort(rel.getCluster(), rel.getTraitSet().replace(toCollation), rel, toCollation, null,
                                    null);
        }
    ...
    private static class PhysicalSort extends Sort {
        ...
        PhysicalSort(RelOptCluster cluster, RelTraitSet traits, RelNode input, RelCollation collation, RexNode offset,
                     RexNode fetch) {
            super(cluster, traits, input, collation, offset, fetch);
        }

        public Sort copy(RelTraitSet traitSet, RelNode newInput, RelCollation newCollation, RexNode offset,
                         RexNode fetch) {
