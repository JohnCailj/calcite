...
public class TraitPropagationTest {
    ...
    static final RelCollation COLLATION = RelCollations.of(
            new RelFieldCollation(0, RelFieldCollation.Direction.ASCENDING, RelFieldCollation.NullDirection.FIRST));

    static final RuleSet RULES = RuleSets.ofList(PhysAggRule.INSTANCE, PhysProjRule.INSTANCE, PhysTableRule.INSTANCE,
                                                 PhysSortRule.INSTANCE, SortRemoveRule.INSTANCE,
                                                 ExpandConversionRule.INSTANCE);

    @Test public void testOne() throws Exception {
        RelNode planned = run(new PropAction(), RULES);
        if (CalcitePrepareImpl.DEBUG) {
            System.out.println(RelOptUtil.dumpPlan("LOGICAL PLAN", planned, SqlExplainFormat.TEXT,
        ...
        public RelNode apply(RelOptCluster cluster, RelOptSchema relOptSchema, SchemaPlus rootSchema) {
            ...
            final Table table = new AbstractTable() {
                ...
                @Override public Statistic getStatistic() {
                    return Statistics.of(100d, ImmutableList.<ImmutableBitSet>of(), ImmutableList.of(COLLATION));
                }
            };
            ...
            RelNode project = LogicalProject.create(rt1,
                                                    ...
                                                    typeFactory.builder().add("s", stringType).add("i",
                                                                                                   integerType).build());

            // aggregate on s, count
            AggregateCall aggCall = AggregateCall.create(SqlStdOperatorTable.COUNT, false, Collections.singletonList(1),
                                                         -1, sqlBigInt, "cnt");
            RelNode agg = new LogicalAggregate(cluster, cluster.traitSetOf(Convention.NONE), project, false,
    ...
    private static class PhysAggRule extends RelOptRule {
        ...
        public void onMatch(RelOptRuleCall call) {
            ...
            RelTrait collation = RelCollations.of(new RelFieldCollation(aggIndex, RelFieldCollation.Direction.ASCENDING,
            ...
            call.transformTo(new PhysAgg(rel.getCluster(), empty.replace(PHYSICAL), convertedInput, rel.indicator,
            ...
            if (subsetHack && input instanceof RelSubset) {
                ...
                for (RelNode child : subset.getRels()) {
                    ...
                    if (child.getTraitSet().getTrait(ConventionTraitDef.INSTANCE) == Convention.NONE) {
                        ...
                        call.transformTo(
                                new PhysProj(rel.getCluster(), outcome, convert(child, outcome), rel.getChildExps(),
                                             rel.getRowType()));
                    }
                }
            } else {
    ...
    private static class PhysSortRule extends ConverterRule {
        ...
        public RelNode convert(RelNode rel) {
            ...
            return new PhysSort(rel.getCluster(), input.getTraitSet().plus(sort.getCollation()),
                                convert(input, input.getTraitSet().replace(PHYSICAL)), sort.getCollation(), null, null);
        }
    }
  ...
  /* RELS */
    ...
    private static class PhysAgg extends Aggregate implements Phys {
        ...
        PhysAgg(RelOptCluster cluster, RelTraitSet traits, RelNode child, boolean indicator, ImmutableBitSet groupSet,
    ...
    private static class PhysProj extends Project implements Phys {
        ...
        public static PhysProj create(final RelNode input, final List<RexNode> projects, RelDataType rowType) {
            ...
            final RelMetadataQuery mq = RelMetadataQuery.instance();
            final RelTraitSet traitSet = cluster.traitSet().replace(PHYSICAL).replaceIfs(RelCollationTraitDef.INSTANCE,
                                                                                         new Supplier<List<RelCollation>>() {

                                                                                             public List<RelCollation> get() {
                                                                                                 return RelMdCollation.project(
                                                                                                         mq, input,
                                                                                                         projects);
                                                                                             }
                                                                                         });
    ...
    private static class PhysSort extends Sort implements Phys {
        ...
        PhysSort(RelOptCluster cluster, RelTraitSet traits, RelNode child, RelCollation collation, RexNode offset,
                 RexNode fetch) {
            super(cluster, traits, child, collation, offset, fetch);

        }

        public PhysSort copy(RelTraitSet traitSet, RelNode newInput, RelCollation newCollation, RexNode offset,
                             RexNode fetch) {
