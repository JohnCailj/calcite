...
public class VolcanoPlannerTraitTest {
    ...
    @Ignore @Test public void testDoubleConversion() {
        ...
        RelNode convertedRel = planner.changeTraits(noneRel, cluster.traitSetOf(EnumerableConvention.INSTANCE).replace(
                ALT_TRAIT2));
    ...
    @Ignore @Test public void testTraitPropagation() {
        VolcanoPlanner planner = new VolcanoPlanner();
    ...
    private static class AltTraitDef extends RelTraitDef<AltTrait> {
        ...
        public RelNode convert(RelOptPlanner planner, RelNode rel, AltTrait toTrait,
            ...
            if (conversionMap.containsKey(fromTrait)) {
                ...
                for (Pair<RelTrait, ConverterRule> traitAndRule : conversionMap.get(fromTrait)) {
                    ...
                    if (trait == toTrait) {
                        ...
                        if ((converted != null) && (!planner.getCost(converted, mq).isInfinite()
    ...
    private static class AltTraitConverter extends ConverterImpl {
        ...
        public RelNode copy(RelTraitSet traitSet, List<RelNode> inputs) {
            return new AltTraitConverter(getCluster(), sole(inputs), toTrait);
        }
    }
    ...
    private static class PhysToIteratorConverter extends ConverterImpl {
        ...
        PhysToIteratorConverter(RelOptCluster cluster, RelNode child) {
            super(cluster, ConventionTraitDef.INSTANCE, child.getTraitSet().replace(EnumerableConvention.INSTANCE),
                  child);
        }
