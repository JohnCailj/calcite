...
public class DateRangeRulesTest {
    ...
    @Test public void testExtractYearFromDateColumn() {
        ...
        assertThat(DateRangeRules.extractTimeUnits(e), is(set(TimeUnitRange.YEAR)));
        ...
        checkDateRange(f, f.eq(f.exYear, f.literal(2014)), is("AND(>=($8, 2014-01-01), <($8, 2015-01-01))"));
        checkDateRange(f, f.ge(f.exYear, f.literal(2014)), is(">=($8, 2014-01-01)"));
        checkDateRange(f, f.gt(f.exYear, f.literal(2014)), is(">=($8, 2015-01-01)"));
        checkDateRange(f, f.lt(f.exYear, f.literal(2014)), is("<($8, 2014-01-01)"));
        checkDateRange(f, f.le(f.exYear, f.literal(2014)), is("<($8, 2015-01-01)"));
        checkDateRange(f, f.ne(f.exYear, f.literal(2014)), is("<>(EXTRACT_DATE(FLAG(YEAR), $8), 2014)"));
    }

    @Test public void testExtractYearFromTimestampColumn() {
        final Fixture2 f = new Fixture2();
        checkDateRange(f, f.eq(f.exYearTs, f.literal(2014)), is("AND(>=($9, 2014-01-01), <($9, 2015-01-01))"));
        checkDateRange(f, f.ge(f.exYearTs, f.literal(2014)), is(">=($9, 2014-01-01)"));
        checkDateRange(f, f.gt(f.exYearTs, f.literal(2014)), is(">=($9, 2015-01-01)"));
        checkDateRange(f, f.lt(f.exYearTs, f.literal(2014)), is("<($9, 2014-01-01)"));
        checkDateRange(f, f.le(f.exYearTs, f.literal(2014)), is("<($9, 2015-01-01)"));
        checkDateRange(f, f.ne(f.exYearTs, f.literal(2014)), is("<>(EXTRACT(FLAG(YEAR), $9), 2014)"));
    }

    @Test public void testExtractYearAndMonthFromDateColumn() {
        final Fixture2 f = new Fixture2();
        checkDateRange(f, f.and(f.eq(f.exYear, f.literal(2014)), f.eq(f.exMonth, f.literal(6))),
                       is("AND(AND(>=($8, 2014-01-01), <($8, 2015-01-01)),"
                          + " AND(>=($8, 2014-06-01), <($8, 2014-07-01)))"),
                       is("AND(>=($8, 2014-01-01), <($8, 2015-01-01)," + " >=($8, 2014-06-01), <($8, 2014-07-01))"));
    }
    ...
    @Test public void testExtractYearAndMonthFromDateColumn2() {
        final Fixture2 f = new Fixture2();
        final String s1 = "AND(" + "AND(>=($8, 2000-01-01), <($8, 2001-01-01))," + " OR("
                          + "AND(>=($8, 2000-02-01), <($8, 2000-03-01)), "
                          + "AND(>=($8, 2000-03-01), <($8, 2000-04-01)), "
                          + "AND(>=($8, 2000-05-01), <($8, 2000-06-01))))";
        final String s2 =
                "AND(>=($8, 2000-01-01), <($8, 2001-01-01)," + " OR(" + "AND(>=($8, 2000-02-01), <($8, 2000-03-01)), "
                + "AND(>=($8, 2000-03-01), <($8, 2000-04-01)), " + "AND(>=($8, 2000-05-01), <($8, 2000-06-01))))";
        final RexNode e = f.and(f.eq(f.exYear, f.literal(2000)),
                                f.or(f.eq(f.exMonth, f.literal(2)), f.eq(f.exMonth, f.literal(3)),
    ...
    @Test public void testExtractYearAndDayFromDateColumn() {
        final Fixture2 f = new Fixture2();
        checkDateRange(f, f.and(f.eq(f.exYear, f.literal(2010)), f.eq(f.exDay, f.literal(31))),
    ...
    @Test public void testExtractYearMonthDayFromDateColumn() {
        ...
        // folding intervals.
        checkDateRange(f, f.and(f.gt(f.exYear, f.literal(2010)), f.lt(f.exYear, f.literal(2020)),
                                f.eq(f.exMonth, f.literal(2)), f.eq(f.exDay, f.literal(29))),
                       is("AND(>=($8, 2011-01-01)," + " AND(>=($8, 2011-01-01), <($8, 2020-01-01)),"
    ...
    @Test public void testExtractYearMonthDayFromTimestampColumn() {
        final Fixture2 f = new Fixture2();
        checkDateRange(f, f.and(f.gt(f.exYearTs, f.literal(2010)), f.lt(f.exYearTs, f.literal(2020)),
                                f.eq(f.exMonthTs, f.literal(2)), f.eq(f.exDayTs, f.literal(29))),
                       is("AND(>=($9, 2011-01-01)," + " AND(>=($9, 2011-01-01), <($9, 2020-01-01)),"
    ...
    private void checkDateRange(Fixture f, RexNode e, Matcher<String> matcher, Matcher<String> simplifyMatcher) {
        ...
        final List<TimeUnitRange> timeUnits = Ordering.natural().sortedCopy(DateRangeRules.extractTimeUnits(e));
        for (TimeUnitRange timeUnit : timeUnits) {
            e = e.accept(new DateRangeRules.ExtractShuttle(f.rexBuilder, timeUnit, operandRanges));
        }
    ...
    private static class Fixture2 extends Fixture {
        ...
        Fixture2() {
            exYear = rexBuilder.makeCall(intRelDataType, SqlStdOperatorTable.EXTRACT_DATE,
                                         ImmutableList.of(rexBuilder.makeFlag(TimeUnitRange.YEAR), dt));
            exMonth = rexBuilder.makeCall(intRelDataType, SqlStdOperatorTable.EXTRACT_DATE,
                                          ImmutableList.of(rexBuilder.makeFlag(TimeUnitRange.MONTH), dt));
            exDay = rexBuilder.makeCall(intRelDataType, SqlStdOperatorTable.EXTRACT_DATE,
            ...
            exMonthTs = rexBuilder.makeCall(intRelDataType, SqlStdOperatorTable.EXTRACT,
                                            ImmutableList.of(rexBuilder.makeFlag(TimeUnitRange.MONTH), ts));
            exDayTs = rexBuilder.makeCall(intRelDataType, SqlStdOperatorTable.EXTRACT,
