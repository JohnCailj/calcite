...
public class RexExecutorTest {
    ...
    @Test public void testConstant() throws Exception {
        check(new Action() {
            ...
            public void check(RexBuilder rexBuilder, RexExecutorImpl executor) {
                ...
                executor.reduce(rexBuilder, ImmutableList.<RexNode>of(ten), reducedValues);
    ...
    @Test public void testConstant2() throws Exception {
        ...
        checkConstant(10L, new Function<RexBuilder, RexNode>() {
        ...
        // 10 + 1 -> 11
        checkConstant(11L, new Function<RexBuilder, RexNode>() {

            public RexNode apply(RexBuilder rexBuilder) {
                return rexBuilder.makeCall(SqlStdOperatorTable.PLUS, rexBuilder.makeExactLiteral(BigDecimal.TEN),
                                           rexBuilder.makeExactLiteral(BigDecimal.ONE));
            }
        });
        // date 'today' <= date 'today' -> true
        checkConstant(true, new Function<RexBuilder, RexNode>() {

            public RexNode apply(RexBuilder rexBuilder) {
                final DateString d = DateString.fromCalendarFields(Util.calendar());
                return rexBuilder.makeCall(SqlStdOperatorTable.LESS_THAN_OR_EQUAL, rexBuilder.makeDateLiteral(d),
                                           rexBuilder.makeDateLiteral(d));
            }
        });
        // date 'today' < date 'today' -> false
        checkConstant(false, new Function<RexBuilder, RexNode>() {

            public RexNode apply(RexBuilder rexBuilder) {
                final DateString d = DateString.fromCalendarFields(Util.calendar());
                return rexBuilder.makeCall(SqlStdOperatorTable.LESS_THAN, rexBuilder.makeDateLiteral(d),
                                           rexBuilder.makeDateLiteral(d));
            }
        });
    }
    ...
    @Test public void testBinarySubstring() throws Exception {
        check(new Action() {
            ...
            public void check(RexBuilder rexBuilder, RexExecutorImpl executor) {
                ...
                final RexLiteral binaryHello = rexBuilder.makeBinaryLiteral(
    ...
    @Test public void testDeterministic2() throws Exception {
        check(new Action() {
            ...
            public void check(RexBuilder rexBuilder, RexExecutorImpl executor) {
                final RexNode plus = rexBuilder.makeCall(PLUS_RANDOM, rexBuilder.makeExactLiteral(BigDecimal.ONE),
                                                         rexBuilder.makeExactLiteral(BigDecimal.ONE));
    ...
    @Test public void testDeterministic3() throws Exception {
        check(new Action() {
            ...
            public void check(RexBuilder rexBuilder, RexExecutorImpl executor) {
                final RexNode plus = rexBuilder.makeCall(SqlStdOperatorTable.PLUS, rexBuilder.makeCall(PLUS_RANDOM,
    ...
    private static final SqlBinaryOperator PLUS_RANDOM = new SqlMonotonicBinaryOperator("+", SqlKind.PLUS, 40, true,
