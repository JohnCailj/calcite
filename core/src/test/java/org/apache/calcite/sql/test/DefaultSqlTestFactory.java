...
public class DefaultSqlTestFactory implements SqlTestFactory {
    ...
    public static final ImmutableMap<String, Object> DEFAULT_OPTIONS = ImmutableMap.<String, Object>builder().put(
            "quoting", Quoting.DOUBLE_QUOTE).put("quotedCasing", Casing.UNCHANGED).put("unquotedCasing",
                                                                                       Casing.TO_UPPER).put(
            "caseSensitive", true).put("conformance", SqlConformanceEnum.DEFAULT).put("operatorTable",
                                                                                      SqlStdOperatorTable.instance()).put(
            "connectionFactory", CalciteAssert.EMPTY_CONNECTION_FACTORY.with(
                    new CalciteAssert.AddSchemaSpecPostProcessor(CalciteAssert.SchemaSpec.HR))).build();
     ...
     */
    private final LoadingCache<SqlTestFactory, Xyz> cache = CacheBuilder.newBuilder().build(
            new CacheLoader<SqlTestFactory, Xyz>() {
                ...
                public Xyz load(@Nonnull SqlTestFactory factory) throws Exception {
                    ...
                    final JavaTypeFactory typeFactory = new JavaTypeFactoryImpl(RelDataTypeSystem.DEFAULT);
                    final MockCatalogReader catalogReader = factory.createCatalogReader(factory, typeFactory);
    ...
    public MockCatalogReader createCatalogReader(SqlTestFactory testFactory, JavaTypeFactory typeFactory) {
    ...
    public SqlParser createParser(SqlTestFactory factory, String sql) {
        return SqlParser.create(sql, SqlParser.configBuilder().setQuoting(
                (Quoting) factory.get("quoting")).setUnquotedCasing(
                (Casing) factory.get("unquotedCasing")).setQuotedCasing(
                (Casing) factory.get("quotedCasing")).setConformance(
                (SqlConformance) factory.get("conformance")).build());
    }
