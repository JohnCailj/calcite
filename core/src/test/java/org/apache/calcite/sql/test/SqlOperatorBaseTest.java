...
public abstract class SqlOperatorBaseTest {
    ...
    public static final String OUT_OF_RANGE_MESSAGE = Bug.FNL3_FIXED ? null : "(?s).*";

    // TODO: Change message when Fnl3Fixed to something like
    // "Division by zero: PC=0 Code=22012"
    public static final String DIVISION_BY_ZERO_MESSAGE = Bug.FNL3_FIXED ? null : "(?s).*";

    // TODO: Change message when Fnl3Fixed to something like
    // "String right truncation: PC=0 Code=22001"
    public static final String STRING_TRUNC_MESSAGE = Bug.FNL3_FIXED ? null : "(?s).*";

    // TODO: Change message when Fnl3Fixed to something like
    // "Invalid datetime format: PC=0 Code=22007"
    public static final String BAD_DATETIME_MESSAGE = Bug.FNL3_FIXED ? null : "(?s).*";
    ...
    public static final String[] NUMERIC_TYPE_NAMES = { "TINYINT", "SMALLINT", "INTEGER", "BIGINT", "DECIMAL(5, 2)",
                                                        "REAL", "FLOAT", "DOUBLE" };

    // REVIEW jvs 27-Apr-2006:  for Float and Double, MIN_VALUE
    // is the smallest positive value, not the smallest negative value
    public static final String[] MIN_NUMERIC_STRINGS = { Long.toString(Byte.MIN_VALUE), Long.toString(Short.MIN_VALUE),
    ...
    public static final String[] MIN_OVERFLOW_NUMERIC_STRINGS = { Long.toString(Byte.MIN_VALUE - 1),
                                                                  Long.toString(Short.MIN_VALUE - 1),
                                                                  Long.toString((long) Integer.MIN_VALUE - 1),
                                                                  new BigDecimal(Long.MIN_VALUE).subtract(
                                                                          BigDecimal.ONE).toString(), "-1000.00",
                                                                  "1e-46", "1e-324", "1e-324" };

    public static final String[] MAX_NUMERIC_STRINGS = { Long.toString(Byte.MAX_VALUE), Long.toString(Short.MAX_VALUE),
    ...
    public static final    String[]         MAX_OVERFLOW_NUMERIC_STRINGS = { Long.toString(Byte.MAX_VALUE + 1),
                                                                             Long.toString(Short.MAX_VALUE + 1),
                                                                             Long.toString(
                                                                                     (long) Integer.MAX_VALUE + 1),
                                                                             (new BigDecimal(Long.MAX_VALUE)).add(
                                                                                     BigDecimal.ONE).toString(),
                                                                             "1000.00", "1e39", "-1e309", "1e309" };
    ...
    protected SqlTester oracleTester() {
        return tester.withOperatorTable(ChainedSqlOperatorTable.of(OracleSqlOperatorTable.instance(),
                                                                   SqlStdOperatorTable.instance())).withConnectionFactory(
                CalciteAssert.EMPTY_CONNECTION_FACTORY.with(
                        new CalciteAssert.AddSchemaSpecPostProcessor(CalciteAssert.SchemaSpec.HR)).with("fun",
                                                                                                        "oracle"));
    }
    ...
    @Test public void testSqlOperatorOverloading() {
        ...
        for (SqlOperator sqlOperator : operatorTable.getOperatorList()) {
            ...
            operatorTable.lookupOperatorOverloads(new SqlIdentifier(operatorName, SqlParserPos.ZERO), null,
                                                  sqlOperator.getSyntax(), routines);
    ...
    @Test public void testBetween() {
        tester.setFor(SqlStdOperatorTable.BETWEEN, SqlTester.VmName.EXPAND);
        ...
        tester.checkBoolean("1.5e1 between 1.6e1 and 1.7e1", Boolean.FALSE);
    ...
    @Test public void testNotBetween() {
        ...
        tester.checkBoolean("1.2e0 not between 1.1 and 1.3", Boolean.FALSE);
    ...
    private void checkCastToApproxOkay(String value, String targetType, double expected, double delta) {
        tester.checkScalarApprox(getCastString(value, targetType, false), targetType + " NOT NULL", expected, delta);
    }

    private void checkCastToStringOkay(String value, String targetType, String expected) {
        tester.checkString(getCastString(value, targetType, false), expected, targetType + " NOT NULL");
    }

    private void checkCastToScalarOkay(String value, String targetType, String expected) {
        tester.checkScalarExact(getCastString(value, targetType, false), targetType + " NOT NULL", expected);
    }
    ...
    private void checkCastFails(String value, String targetType, String expectedError, boolean runtime) {
        tester.checkFails(getCastString(value, targetType, !runtime), expectedError, runtime);
    }
    ...
    private void checkCastToString(String value, String type, String expected) {
        ...
        if (Bug.DT239_FIXED) {
            checkCastFails(value, "VARCHAR(" + (len - 1) + ")", STRING_TRUNC_MESSAGE, true);
        }
        ...
        // currently no exception thrown for truncation
        if (Bug.DT239_FIXED) {
            checkCastFails(value, "CHAR(" + (len - 1) + ")", STRING_TRUNC_MESSAGE, true);
        }
    ...
    @Test public void testCastToString() {
        ...
        tester.checkString("cast(1.29 as varchar(10))", "1.29", "VARCHAR(10) NOT NULL");
        tester.checkString("cast(.48 as varchar(10))", ".48", "VARCHAR(10) NOT NULL");
        tester.checkFails("cast(2.523 as char(2))", STRING_TRUNC_MESSAGE, true);

        tester.checkString("cast(-0.29 as varchar(10))", "-.29", "VARCHAR(10) NOT NULL");
        tester.checkString("cast(-1.29 as varchar(10))", "-1.29", "VARCHAR(10) NOT NULL");
        ...
        if (TODO) {
            checkCastToString("cast(-45e-2 as varchar(17))", "CHAR(7)", "-4.5E-1");
        }
        if (TODO) {
            checkCastToString("cast(4683442.3432498375e0 as varchar(20))", "CHAR(19)", "4.683442343249838E6");
        }
        if (TODO) {
            checkCastToString("cast(-0.1 as real)", "CHAR(5)", "-1E-1");
        }

        tester.checkFails("cast(1.3243232e0 as varchar(4))", STRING_TRUNC_MESSAGE, true);
        tester.checkFails("cast(1.9e5 as char(4))", STRING_TRUNC_MESSAGE, true);
        ...
        checkCastToString("cast(cast('abc' as char(4)) as varchar(6))", null, "abc ");
        tester.checkString("cast(cast('a' as char(2)) as varchar(3)) || 'x' ", "a x", "VARCHAR(4) NOT NULL");
        tester.checkString("cast(cast('a' as char(3)) as varchar(5)) || 'x' ", "a  x", "VARCHAR(6) NOT NULL");
        tester.checkString("cast('a' as char(3)) || 'x'", "a  x", "CHAR(4) NOT NULL");

        tester.checkScalar("char_length(cast(' x ' as char(4)))", 4, "INTEGER NOT NULL");
        tester.checkScalar("char_length(cast(' x ' as varchar(3)))", 3, "INTEGER NOT NULL");
        tester.checkScalar("char_length(cast(' x ' as varchar(4)))", 3, "INTEGER NOT NULL");
        tester.checkScalar("char_length(cast(cast(' x ' as char(4)) as varchar(5)))", 4, "INTEGER NOT NULL");
        tester.checkScalar("char_length(cast(' x ' as varchar(3)))", 3, "INTEGER NOT NULL");

        // date & time
        checkCastToString("date '2008-01-01'", "CHAR(10)", "2008-01-01");
        checkCastToString("time '1:2:3'", "CHAR(8)", "01:02:03");
        checkCastToString("timestamp '2008-1-1 1:2:3'", "CHAR(19)", "2008-01-01 01:02:03");
        checkCastToString("timestamp '2008-1-1 1:2:3'", "VARCHAR(30)", "2008-01-01 01:02:03");

        checkCastToString("interval '3-2' year to month", "CHAR(5)", "+3-02");
        checkCastToString("interval '32' month", "CHAR(3)", "+32");
        checkCastToString("interval '1 2:3:4' day to second", "CHAR(11)", "+1 02:03:04");
        checkCastToString("interval '1234.56' second(4,2)", "CHAR(8)", "+1234.56");
        checkCastToString("interval '60' day", "CHAR(8)", "+60     ");
        ...
        tester.checkFails("cast(true as char(3))", INVALID_CHAR_MESSAGE, true);
        tester.checkFails("cast(false as char(4))", INVALID_CHAR_MESSAGE, true);
        tester.checkFails("cast(true as varchar(3))", INVALID_CHAR_MESSAGE, true);
        tester.checkFails("cast(false as varchar(4))", INVALID_CHAR_MESSAGE, true);
    }
    ...
    @Test public void testCastExactNumericLimits() {
        ...
        for (int i = 0; i < NUMERIC_TYPE_NAMES.length; i++) {
            ...
            if (type.equalsIgnoreCase("DOUBLE") || type.equalsIgnoreCase("FLOAT") || type.equalsIgnoreCase("REAL")) {
                // Skip approx types
                continue;
            }
            ...
            if (type.equalsIgnoreCase("BIGINT")) {
                // Literal of range
                checkCastFails(MAX_OVERFLOW_NUMERIC_STRINGS[i], type, LITERAL_OUT_OF_RANGE_MESSAGE, false);
                checkCastFails(MIN_OVERFLOW_NUMERIC_STRINGS[i], type, LITERAL_OUT_OF_RANGE_MESSAGE, false);
            } else {
                checkCastFails(MAX_OVERFLOW_NUMERIC_STRINGS[i], type, OUT_OF_RANGE_MESSAGE, true);
                checkCastFails(MIN_OVERFLOW_NUMERIC_STRINGS[i], type, OUT_OF_RANGE_MESSAGE, true);
            }
    ...
    @Test public void testCastToExactNumeric() {
        ...
        tester.checkScalarExact("cast('654342432412312' as bigint)", "BIGINT NOT NULL", "654342432412312");
    }
    ...
    @Test public void testCastStringToDecimal() {
        ...
        // string to decimal
        tester.checkScalarExact("cast('1.29' as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "1.3");
        tester.checkScalarExact("cast(' 1.25 ' as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "1.3");
        tester.checkScalarExact("cast('1.21' as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "1.2");
        tester.checkScalarExact("cast(' -1.29 ' as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "-1.3");
        tester.checkScalarExact("cast('-1.25' as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "-1.3");
        tester.checkScalarExact("cast(' -1.21 ' as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "-1.2");
        tester.checkFails("cast(' -1.21e' as decimal(2,1))", INVALID_CHAR_MESSAGE, true);
    }
    ...
    @Test public void testCastIntervalToNumeric() {
        ...
        // interval to decimal
        if (DECIMAL) {
            tester.checkScalarExact("cast(INTERVAL '1.29' second(1,2) as decimal(2,1))", "DECIMAL(2, 1) NOT NULL",
                                    "1.3");
            tester.checkScalarExact("cast(INTERVAL '1.25' second as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "1.3");
            tester.checkScalarExact("cast(INTERVAL '-1.29' second as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "-1.3");
            tester.checkScalarExact("cast(INTERVAL '-1.25' second as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "-1.3");
            tester.checkScalarExact("cast(INTERVAL '-1.21' second as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "-1.2");
            tester.checkScalarExact("cast(INTERVAL '5' minute as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "5.0");
            tester.checkScalarExact("cast(INTERVAL '5' hour as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "5.0");
            tester.checkScalarExact("cast(INTERVAL '5' day as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "5.0");
            tester.checkScalarExact("cast(INTERVAL '5' month as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "5.0");
            tester.checkScalarExact("cast(INTERVAL '5' year as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "5.0");
            tester.checkScalarExact("cast(INTERVAL '-5' day as decimal(2,1))", "DECIMAL(2, 1) NOT NULL", "-5.0");
        }

        // Interval to bigint
        tester.checkScalarExact("cast(INTERVAL '1.25' second as bigint)", "BIGINT NOT NULL", "1");
        tester.checkScalarExact("cast(INTERVAL '-1.29' second(1,2) as bigint)", "BIGINT NOT NULL", "-1");
        tester.checkScalarExact("cast(INTERVAL '5' day as bigint)", "BIGINT NOT NULL", "5");

        // Interval to integer
        tester.checkScalarExact("cast(INTERVAL '1.25' second as integer)", "INTEGER NOT NULL", "1");
        tester.checkScalarExact("cast(INTERVAL '-1.29' second(1,2) as integer)", "INTEGER NOT NULL", "-1");
        tester.checkScalarExact("cast(INTERVAL '5' day as integer)", "INTEGER NOT NULL", "5");
    }

    @Test public void testCastToInterval() {
        tester.setFor(SqlStdOperatorTable.CAST);
        tester.checkScalar("cast(5 as interval second)", "+5.000000", "INTERVAL SECOND NOT NULL");
        tester.checkScalar("cast(5 as interval minute)", "+5", "INTERVAL MINUTE NOT NULL");
        tester.checkScalar("cast(5 as interval hour)", "+5", "INTERVAL HOUR NOT NULL");
        tester.checkScalar("cast(5 as interval day)", "+5", "INTERVAL DAY NOT NULL");
        tester.checkScalar("cast(5 as interval month)", "+5", "INTERVAL MONTH NOT NULL");
        tester.checkScalar("cast(5 as interval year)", "+5", "INTERVAL YEAR NOT NULL");
        if (DECIMAL) {
            // Due to DECIMAL rounding bugs, currently returns "+5"
            tester.checkScalar("cast(5.7 as interval day)", "+6", "INTERVAL DAY NOT NULL");
            tester.checkScalar("cast(-5.7 as interval day)", "-6", "INTERVAL DAY NOT NULL");
        } else {
            // An easier case
            tester.checkScalar("cast(6.2 as interval day)", "+6", "INTERVAL DAY NOT NULL");
        }
        tester.checkScalar("cast(3456 as interval month(4))", "+3456", "INTERVAL MONTH(4) NOT NULL");
        tester.checkScalar("cast(-5723 as interval minute(4))", "-5723", "INTERVAL MINUTE(4) NOT NULL");
    }

    @Test public void testCastIntervalToInterval() {
        tester.checkScalar("cast(interval '2 5' day to hour as interval hour to minute)", "+53:00",
                           "INTERVAL HOUR TO MINUTE NOT NULL");
        tester.checkScalar("cast(interval '2 5' day to hour as interval day to minute)", "+2 05:00",
                           "INTERVAL DAY TO MINUTE NOT NULL");
        tester.checkScalar("cast(interval '2 5' day to hour as interval hour to second)", "+53:00:00.000000",
                           "INTERVAL HOUR TO SECOND NOT NULL");
        tester.checkScalar("cast(interval '2 5' day to hour as interval hour)", "+53", "INTERVAL HOUR NOT NULL");
        tester.checkScalar("cast(interval '-29:15' hour to minute as interval day to hour)", "-1 05",
    ...
    @Test public void testCastWithRoundingToScalar() {
        ...
        tester.checkFails("cast(9.99 as decimal(2,1))", OUT_OF_RANGE_MESSAGE, true);
    }

    @Test public void testCastDecimalToDoubleToInteger() {
        tester.setFor(SqlStdOperatorTable.CAST);

        tester.checkScalarExact("cast( cast(1.25 as double) as integer)", "1");
        tester.checkScalarExact("cast( cast(-1.25 as double) as integer)", "-1");
        if (!enable) {
            return;
        }
        tester.checkScalarExact("cast( cast(1.75 as double) as integer)", "2");
        tester.checkScalarExact("cast( cast(-1.75 as double) as integer)", "-2");
        tester.checkScalarExact("cast( cast(1.5 as double) as integer)", "2");
        tester.checkScalarExact("cast( cast(-1.5 as double) as integer)", "-2");
    }
    ...
    @Test public void testCastApproxNumericLimits() {
        ...
        for (int i = 0; i < NUMERIC_TYPE_NAMES.length; i++) {
            ...
            // Convert from literal to type
            checkCastToApproxOkay(MAX_NUMERIC_STRINGS[i], type, Double.parseDouble(MAX_NUMERIC_STRINGS[i]),
                                  isFloat ? 1E32 : 0);
            checkCastToApproxOkay(MIN_NUMERIC_STRINGS[i], type, Double.parseDouble(MIN_NUMERIC_STRINGS[i]), 0);
            ...
            checkCastToApproxOkay("'" + MAX_NUMERIC_STRINGS[i] + "'", type, Double.parseDouble(MAX_NUMERIC_STRINGS[i]),
                                  isFloat ? 1E32 : 0);
            checkCastToApproxOkay("'" + MIN_NUMERIC_STRINGS[i] + "'", type, Double.parseDouble(MIN_NUMERIC_STRINGS[i]),
                                  0);
            ...
            // Treated as DOUBLE
            checkCastToString(MAX_NUMERIC_STRINGS[i], null, isFloat ? null : "1.79769313486231E308");
            ...
            if (false /* fennel calc*/) { // Treated as FLOAT or DOUBLE
                checkCastToString(MAX_NUMERIC_STRINGS[i], type,
                                  // Treated as DOUBLE
                                  isFloat ? "3.402824E38" : "1.797693134862316E308");
                checkCastToString(MIN_NUMERIC_STRINGS[i], null,
                                  // Treated as FLOAT or DOUBLE
                                  isFloat ? null : "4.940656458412465E-324");
                checkCastToString(MIN_NUMERIC_STRINGS[i], type, isFloat ? "1.401299E-45" : "4.940656458412465E-324");
            } else if (false /* JavaCalc */) {
                // Treated as FLOAT or DOUBLE
                checkCastToString(MAX_NUMERIC_STRINGS[i], type,
                                  // Treated as DOUBLE
                                  isFloat ? "3.402823E38" : "1.797693134862316E308");
                checkCastToString(MIN_NUMERIC_STRINGS[i], null, isFloat ? null : null); // Treated as FLOAT or DOUBLE
                checkCastToString(MIN_NUMERIC_STRINGS[i], type, isFloat ? "1.401298E-45" : null);
            }
    ...
    @Ignore("[CALCITE-1439] Handling errors during constant reduction") @Test public void testCastInvalid() {
    ...
    @Test public void testCastDateTime() {
        ...
        tester.checkScalar("cast(TIMESTAMP '1945-02-24 12:42:25.34' as TIMESTAMP)", "1945-02-24 12:42:25",
                           "TIMESTAMP(0) NOT NULL");

        tester.checkScalar("cast(TIME '12:42:25.34' as TIME)", "12:42:25", "TIME(0) NOT NULL");

        // test rounding
        if (enable) {
            tester.checkScalar("cast(TIME '12:42:25.9' as TIME)", "12:42:26", "TIME(0) NOT NULL");
        }

        if (Bug.FRG282_FIXED) {
            // test precision
            tester.checkScalar("cast(TIME '12:42:25.34' as TIME(2))", "12:42:25.34", "TIME(2) NOT NULL");
        }

        tester.checkScalar("cast(DATE '1945-02-24' as DATE)", "1945-02-24", "DATE NOT NULL");

        // timestamp <-> time
        tester.checkScalar("cast(TIMESTAMP '1945-02-24 12:42:25.34' as TIME)", "12:42:25", "TIME(0) NOT NULL");
        ...
        tester.checkScalar("cast(DATE '1945-02-24' as TIMESTAMP)", "1945-02-24 00:00:00", "TIMESTAMP(0) NOT NULL");

        // Note: Casting to time(0) should lose date info and fractional
        // seconds, then casting back to timestamp should initialize to
        // current_date.
        tester.checkScalar("cast(cast(TIMESTAMP '1945-02-24 12:42:25.34' as TIME) as TIMESTAMP)", today + " 12:42:25",
                           "TIMESTAMP(0) NOT NULL");

        tester.checkScalar("cast(TIME '12:42:25.34' as TIMESTAMP)", today + " 12:42:25", "TIMESTAMP(0) NOT NULL");

        // timestamp <-> date
        tester.checkScalar("cast(TIMESTAMP '1945-02-24 12:42:25.34' as DATE)", "1945-02-24", "DATE NOT NULL");

        // Note: casting to Date discards Time fields
        tester.checkScalar("cast(cast(TIMESTAMP '1945-02-24 12:42:25.34' as DATE) as TIMESTAMP)", "1945-02-24 00:00:00",
                           "TIMESTAMP(0) NOT NULL");
    }

    @Test public void testCastStringToDateTime() {
        tester.checkScalar("cast('12:42:25' as TIME)", "12:42:25", "TIME(0) NOT NULL");
        tester.checkScalar("cast('1:42:25' as TIME)", "01:42:25", "TIME(0) NOT NULL");
        tester.checkScalar("cast('1:2:25' as TIME)", "01:02:25", "TIME(0) NOT NULL");
        tester.checkScalar("cast('  12:42:25  ' as TIME)", "12:42:25", "TIME(0) NOT NULL");
        tester.checkScalar("cast('12:42:25.34' as TIME)", "12:42:25", "TIME(0) NOT NULL");

        if (Bug.FRG282_FIXED) {
            tester.checkScalar("cast('12:42:25.34' as TIME(2))", "12:42:25.34", "TIME(2) NOT NULL");
        }

        tester.checkFails("cast('nottime' as TIME)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('1241241' as TIME)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('12:54:78' as TIME)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('12:34:5' as TIME)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('12:3:45' as TIME)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('1:23:45' as TIME)", BAD_DATETIME_MESSAGE, true);

        // timestamp <-> string
        checkCastToString("TIMESTAMP '1945-02-24 12:42:25'", null, "1945-02-24 12:42:25");

        if (TODO) {
            // TODO: casting allows one to discard precision without error
            checkCastToString("TIMESTAMP '1945-02-24 12:42:25.34'", null, "1945-02-24 12:42:25.34");
        }

        tester.checkScalar("cast('1945-02-24 12:42:25' as TIMESTAMP)", "1945-02-24 12:42:25", "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("cast('1945-2-2 12:2:5' as TIMESTAMP)", "1945-02-02 12:02:05", "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("cast('  1945-02-24 12:42:25  ' as TIMESTAMP)", "1945-02-24 12:42:25",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("cast('1945-02-24 12:42:25.34' as TIMESTAMP)", "1945-02-24 12:42:25",
                           "TIMESTAMP(0) NOT NULL");

        if (Bug.FRG282_FIXED) {
            tester.checkScalar("cast('1945-02-24 12:42:25.34' as TIMESTAMP(2))", "1945-02-24 12:42:25.34",
                               "TIMESTAMP(2) NOT NULL");
        }
        tester.checkFails("cast('nottime' as TIMESTAMP)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('1241241' as TIMESTAMP)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('1945-20-24 12:42:25.34' as TIMESTAMP)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('1945-01-24 25:42:25.34' as TIMESTAMP)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('1945-1-24 12:23:34.454' as TIMESTAMP)", BAD_DATETIME_MESSAGE, true);

        // date <-> string
        checkCastToString("DATE '1945-02-24'", null, "1945-02-24");
        checkCastToString("DATE '1945-2-24'", null, "1945-02-24");

        tester.checkScalar("cast('1945-02-24' as DATE)", "1945-02-24", "DATE NOT NULL");
        tester.checkScalar("cast(' 1945-2-4 ' as DATE)", "1945-02-04", "DATE NOT NULL");
        tester.checkScalar("cast('  1945-02-24  ' as DATE)", "1945-02-24", "DATE NOT NULL");
        tester.checkFails("cast('notdate' as DATE)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('52534253' as DATE)", BAD_DATETIME_MESSAGE, true);
        tester.checkFails("cast('1945-30-24' as DATE)", BAD_DATETIME_MESSAGE, true);
            ...
            try {
                switch (timeUnit) {
                    case Calendar.DAY_OF_MONTH:
                        ...
                        if ((cal.get(Calendar.HOUR_OF_DAY) == 23) && (cal.get(Calendar.MINUTE) >= 58)) {
    ...
    @Test public void testCastToBoolean() {
        ...
        tester.checkBoolean("cast('  fALse' as boolean)", Boolean.FALSE);
        tester.checkFails("cast('unknown' as boolean)", INVALID_CHAR_MESSAGE, true);

        tester.checkBoolean("cast(cast('true' as varchar(10))  as boolean)", Boolean.TRUE);
        tester.checkBoolean("cast(cast('false' as varchar(10)) as boolean)", Boolean.FALSE);
        tester.checkFails("cast(cast('blah' as varchar(10)) as boolean)", INVALID_CHAR_MESSAGE, true);
    }
    ...
    @Test public void testCase() {
        ...
        tester.checkString("case 2 when 1 then 'a' when 2 then 'bcd' end", "bcd", "CHAR(3)");
        tester.checkString("case 1 when 1 then 'a' when 2 then 'bcd' end", "a  ", "CHAR(3)");
        tester.checkString("case 1 when 1 then cast('a' as varchar(1)) " + "when 2 then cast('bcd' as varchar(3)) end",
                           "a", "VARCHAR(3)");
        if (DECIMAL) {
            tester.checkScalarExact("case 2 when 1 then 11.2 when 2 then 4.543 else null end", "DECIMAL(5, 3)",
                                    "4.543");
            tester.checkScalarExact("case 1 when 1 then 11.2 when 2 then 4.543 else null end", "DECIMAL(5, 3)",
                                    "11.200");
        }
        tester.checkScalarExact("case 'a' when 'a' then 1 end", "1");
        tester.checkScalarApprox("case 1 when 1 then 11.2e0 when 2 then cast(4 as bigint) else 3 end",
                                 "DOUBLE NOT NULL", 11.2, 0);
        tester.checkScalarApprox("case 1 when 1 then 11.2e0 when 2 then 4 else null end", "DOUBLE", 11.2, 0);
        tester.checkScalarApprox("case 2 when 1 then 11.2e0 when 2 then 4 else null end", "DOUBLE", 4, 0);
        tester.checkScalarApprox("case 1 when 1 then 11.2e0 when 2 then 4.543 else null end", "DOUBLE", 11.2, 0);
        tester.checkScalarApprox("case 2 when 1 then 11.2e0 when 2 then 4.543 else null end", "DOUBLE", 4.543, 0);
        ...
        tester.checkString(
                ...
                "nulls do not match", "CHAR(18) NOT NULL");

        tester.checkScalarExact("case when 'a'=cast(null as varchar(1)) then 1 else 2 end", "2");

        // equivalent to "nullif('a',cast(null as varchar(1)))"
        tester.checkString("case when 'a' = cast(null as varchar(1)) then null else 'a' end", "a", "CHAR(1)");

        if (TODO) {
            tester.checkScalar("case 1 when 1 then row(1,2) when 2 then row(2,3) end",
                               "ROW(INTEGER NOT NULL, INTEGER NOT NULL)", "row(1,2)");
            tester.checkScalar("case 1 when 1 then row('a','b') when 2 then row('ab','cd') end",
        ...
        tester.checkString(
                ...
                + "else 'none of the above' " + "end", "1 or 2           ", "CHAR(17) NOT NULL");
        tester.checkString(
                "case 2 " + "when 1, 2 then '1 or 2' " + "when 2 then 'not possible' " + "when 3, 2 then '3' "
                + "else 'none of the above' " + "end", "1 or 2           ", "CHAR(17) NOT NULL");
        tester.checkString(
                "case 3 " + "when 1, 2 then '1 or 2' " + "when 2 then 'not possible' " + "when 3, 2 then '3' "
                + "else 'none of the above' " + "end", "3                ", "CHAR(17) NOT NULL");
        tester.checkString(
                "case 4 " + "when 1, 2 then '1 or 2' " + "when 2 then 'not possible' " + "when 3, 2 then '3' "
                + "else 'none of the above' " + "end", "none of the above", "CHAR(17) NOT NULL");
    ...
    @Test public void testCaseType() {
        tester.setFor(SqlStdOperatorTable.CASE);
        tester.checkType("case 1 when 1 then current_timestamp else null end", "TIMESTAMP(0)");
        tester.checkType("case 1 when 1 then current_timestamp else current_timestamp end", "TIMESTAMP(0) NOT NULL");
        tester.checkType("case when true then current_timestamp else null end", "TIMESTAMP(0)");
        tester.checkType("case when true then current_timestamp end", "TIMESTAMP(0)");
        tester.checkType("case 'x' when 'a' then 3 when 'b' then null else 4.5 end", "DECIMAL(11, 1)");
    }
    ...
    @Test public void testJdbcFn() {
        ...
        tester.checkFails("^{fn ROUND(1251)}^",
                          "Cannot apply '\\{fn ROUND\\}' to " + "arguments of type '\\{fn ROUND\\}\\(<INTEGER>\\)'.*",
                          false);
        ...
        tester.checkScalar("{fn TIMESTAMPADD(HOUR, 5," + " TIMESTAMP '2014-03-29 12:34:56')}", "2014-03-29 17:34:56",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar(
                "{fn TIMESTAMPDIFF(HOUR," + " TIMESTAMP '2014-03-29 12:34:56'," + " TIMESTAMP '2014-03-29 12:34:56')}",
                "0", "INTEGER NOT NULL");
        ...
        tester.checkScalar("{fn CONVERT(INTERVAL '1' DAY, SQL_INTERVAL_DAY_TO_SECOND)}", "+1 00:00:00.000000",
    ...
    @Test public void testSelect() {
        ...
        if (Bug.FRG189_FIXED || (getClass() != SqlOperatorTest.class) && Bug.TODO_FIXED) {
            tester.checkType("SELECT *,(SELECT * FROM (VALUES(1))) FROM (VALUES(2))",
                             "RecordType(INTEGER NOT NULL EXPR$0, INTEGER EXPR$1) NOT NULL");
            tester.checkType(
                    "SELECT *,(SELECT * FROM (VALUES(CAST(10 as BIGINT)))) " + "FROM (VALUES(CAST(10 as bigint)))",
                    "RecordType(BIGINT NOT NULL EXPR$0, BIGINT EXPR$1) NOT NULL");
            tester.checkType(" SELECT *,(SELECT * FROM (VALUES(10.5))) FROM (VALUES(10.5))",
                             "RecordType(DECIMAL(3, 1) NOT NULL EXPR$0, DECIMAL(3, 1) EXPR$1) NOT NULL");
            tester.checkType(
                    "SELECT *,(SELECT * FROM (VALUES('this is a char'))) " + "FROM (VALUES('this is a char too'))",
                    "RecordType(CHAR(18) NOT NULL EXPR$0, CHAR(14) EXPR$1) NOT NULL");
            tester.checkType("SELECT *,(SELECT * FROM (VALUES(true))) FROM (values(false))",
                             "RecordType(BOOLEAN NOT NULL EXPR$0, BOOLEAN EXPR$1) NOT NULL");
            tester.checkType(" SELECT *,(SELECT * FROM (VALUES(cast('abcd' as varchar(10))))) "
                             + "FROM (VALUES(CAST('abcd' as varchar(10))))",
                             "RecordType(VARCHAR(10) NOT NULL EXPR$0, VARCHAR(10) EXPR$1) NOT NULL");
            tester.checkType("SELECT *," + "  (SELECT * FROM (VALUES(TIMESTAMP '2006-01-01 12:00:05'))) "
    ...
    @Test public void testAndOperator2() {
        tester.checkBoolean("case when false then unknown else true end and true", Boolean.TRUE);
        tester.checkBoolean("case when false then cast(null as boolean) else true end and true", Boolean.TRUE);
        tester.checkBoolean("case when false then null else true end and true", Boolean.TRUE);
    }
    ...
    @Test public void testAndOperatorLazy() {
        ...
        // both are valid
        tester.check("values 1 > 2 and sqrt(-4) = -2", SqlTests.BOOLEAN_TYPE_CHECKER, SqlTests.ANY_PARAMETER_CHECKER,
                     new ValueOrExceptionResultChecker(Boolean.FALSE, INVALID_ARG_FOR_POWER, CODE_2201F));
    }
    ...
    @Test public void testConcatOperator() {
        ...
        tester.checkNull(" cast(null as char(1)) || cast(null as char(2)) ");

        tester.checkString(" x'fe'||x'df' ", "fedf", "BINARY(2) NOT NULL");
        tester.checkString(" cast('fe' as char(2)) || cast('df' as varchar)", "fedf", "VARCHAR NOT NULL");
        // Precision is larger than VARCHAR allows, so result is unbounded
        tester.checkString(" cast('fe' as char(2)) || cast('df' as varchar(65535))", "fedf", "VARCHAR NOT NULL");
        tester.checkString(" cast('fe' as char(2)) || cast('df' as varchar(33333))", "fedf", "VARCHAR(33335) NOT NULL");
    ...
    @Test public void testModOperator() {
        ...
        tester1.checkScalarExact("cast(9 as decimal(2, 0)) % 7", "INTEGER NOT NULL", "2");
        tester1.checkScalarExact("7 % cast(9 as decimal(2, 0))", "DECIMAL(2, 0) NOT NULL", "7");
        tester1.checkScalarExact("cast(-9 as decimal(2, 0)) % cast(7 as decimal(1, 0))", "DECIMAL(1, 0) NOT NULL",
                                 "-2");
    }
    ...
    @Test public void testDivideOperator() {
        ...
        tester.checkScalarApprox(" cast(10.0 as double) / 5", "DOUBLE NOT NULL", 2.0, 0);
        tester.checkScalarApprox(" cast(10.0 as real) / 5", "REAL NOT NULL", 2.0, 0);
        tester.checkScalarApprox(" 6.0 / cast(10.0 as real) ", "DOUBLE NOT NULL", 0.6, 0);
        tester.checkScalarExact("10.0 / 5.0", "DECIMAL(9, 6) NOT NULL", "2");
        if (DECIMAL) {
            tester.checkScalarExact("1.0 / 3.0", "DECIMAL(8, 6) NOT NULL", "0.333333");
            tester.checkScalarExact("100.1 / 0.0001", "DECIMAL(14, 7) NOT NULL", "1001000.0000000");
            tester.checkScalarExact("100.1 / 0.00000001", "DECIMAL(19, 8) NOT NULL", "10010000000.00000000");
        }
        tester.checkNull("1e1 / cast(null as float)");

        tester.checkFails("100.1 / 0.00000000000000001", OUT_OF_RANGE_MESSAGE, true);
    }

    @Test public void testDivideOperatorIntervals() {
        tester.checkScalar("interval '-2:2' hour to minute / 3", "-0:41", "INTERVAL HOUR TO MINUTE NOT NULL");
        tester.checkScalar("interval '2:5:12' hour to second / 2 / -3", "-0:20:52.000000",
                           "INTERVAL HOUR TO SECOND NOT NULL");
        tester.checkNull("interval '2' day / cast(null as bigint)");
        tester.checkNull("cast(null as interval month) / 2");
        tester.checkScalar("interval '3-3' year to month / 15e-1", "+2-02", "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkScalar("interval '3-4' year to month / 4.5", "+0-09", "INTERVAL YEAR TO MONTH NOT NULL");
    }
    ...
    @Test public void testEqualsOperator() {
        ...
        tester.checkBoolean("cast(1e2 as real)=cast(101 as bigint)", Boolean.FALSE);
        ...
        tester.checkBoolean("cast('a ' as varchar(30))=cast('a' as varchar(30))", Boolean.FALSE);
        tester.checkBoolean("cast(' a' as varchar(30))=cast(' a' as varchar(30))", Boolean.TRUE);
        tester.checkBoolean("cast('a ' as varchar(15))=cast('a ' as varchar(30))", Boolean.TRUE);
        tester.checkBoolean("cast(' ' as varchar(3))=cast(' ' as varchar(2))", Boolean.TRUE);
        tester.checkBoolean("cast('abcd' as varchar(2))='ab'", Boolean.TRUE);
    ...
    @Test public void testEqualsOperatorInterval() {
        ...
        tester.checkBoolean("interval '2:2:2' hour to second = interval '2' hour", Boolean.FALSE);
        tester.checkNull("cast(null as interval hour) = interval '2' minute");
    }
    ...
    @Test public void testGreaterThanOperator() {
        ...
        tester.checkBoolean("cast(-1 as TINYINT)>cast(1 as TINYINT)", Boolean.FALSE);
        tester.checkBoolean("cast(1 as SMALLINT)>cast(1 as SMALLINT)", Boolean.FALSE);
        ...
        tester.checkBoolean("cast(-1.1 as real) > cast(-1.2 as real)", Boolean.TRUE);
        ...
        tester.checkBoolean("DATE '2013-02-23' > DATE '1945-02-24'", Boolean.TRUE);
        tester.checkBoolean("DATE '2013-02-23' > CAST(NULL AS DATE)", null);
    ...
    @Test public void testGreaterThanOperatorIntervals() {
        ...
        tester.checkBoolean("interval '2' day > interval '5' day", Boolean.FALSE);
        tester.checkBoolean("interval '2 2:2:2' day to second > interval '2' day", Boolean.TRUE);
        tester.checkBoolean("interval '2' day > interval '2' day", Boolean.FALSE);
        tester.checkBoolean("interval '2' day > interval '-2' day", Boolean.TRUE);
        tester.checkBoolean("interval '2' day > interval '2' hour", Boolean.TRUE);
        tester.checkBoolean("interval '2' minute > interval '2' hour", Boolean.FALSE);
        tester.checkBoolean("interval '2' second > interval '2' minute", Boolean.FALSE);
        tester.checkNull("cast(null as interval hour) > interval '2' minute");
        tester.checkNull("interval '2:2' hour to minute > cast(null as interval second)");
    }

    @Test public void testIsDistinctFromOperator() {
        tester.setFor(SqlStdOperatorTable.IS_DISTINCT_FROM, VM_EXPAND);
        ...
        tester.checkBoolean("-23e0 is distinct from -2.3e1", Boolean.FALSE);

        // IS DISTINCT FROM not implemented for ROW yet
        if (false) {
            tester.checkBoolean("row(1,1) is distinct from row(1,1)", true);
            tester.checkBoolean("row(1,1) is distinct from row(1,2)", false);
        }

        // Intervals
        tester.checkBoolean("interval '2' day is distinct from interval '1' day", Boolean.TRUE);
        tester.checkBoolean("interval '10' hour is distinct from interval '10' hour", Boolean.FALSE);
    }

    @Test public void testIsNotDistinctFromOperator() {
        tester.setFor(SqlStdOperatorTable.IS_NOT_DISTINCT_FROM, VM_EXPAND);
        ...
        tester.checkBoolean("1.23 is not distinct from 1.23", Boolean.TRUE);
        tester.checkBoolean("1.23 is not distinct from 5.23", Boolean.FALSE);
        tester.checkBoolean("-23e0 is not distinct from -2.3e1", Boolean.TRUE);

        // IS NOT DISTINCT FROM not implemented for ROW yet
        if (false) {
            tester.checkBoolean("row(1,1) is not distinct from row(1,1)", false);
            tester.checkBoolean("row(1,1) is not distinct from row(1,2)", true);
        }

        // Intervals
        tester.checkBoolean("interval '2' day is not distinct from interval '1' day", Boolean.FALSE);
        tester.checkBoolean("interval '10' hour is not distinct from interval '10' hour", Boolean.TRUE);
    }
    ...
    @Test public void testInOperator() {
        ...
        tester.checkBoolean("cast(null as integer) in (0, cast(null as integer), 2)", null);
        if (Bug.FRG327_FIXED) {
            tester.checkBoolean("cast(null as integer) in (0, null, 2)", null);
        ...
        tester.checkFails("'foo' in (^)^", "(?s).*Encountered \"\\)\" at .*", false);
    }
    ...
    @Test public void testNotInOperator() {
        ...
        tester.checkBoolean("cast(null as integer) not in (0, 1, 2)", null);
        tester.checkBoolean("cast(null as integer) not in (0, cast(null as integer), 2)", null);
        if (Bug.FRG327_FIXED) {
            tester.checkBoolean("cast(null as integer) not in (0, null, 2)", null);
        ...
        tester.checkFails("'foo' not in (^)^", "(?s).*Encountered \"\\)\" at .*", false);
    }
    ...
    @Test public void testOverlapsOperator() {
        ...
        tester.checkBoolean("(date '1-2-3', date '1-2-3') overlaps (date '4-5-6', interval '1' year)", Boolean.FALSE);
        tester.checkBoolean("(date '1-2-3', date '4-5-6') overlaps (date '2-2-3', date '3-4-5')", Boolean.TRUE);
        tester.checkNull("(cast(null as date), date '1-2-3') overlaps (date '1-2-3', interval '1' year)");
        tester.checkNull("(date '1-2-3', date '1-2-3') overlaps (date '1-2-3', cast(null as date))");

        tester.checkBoolean("(time '1:2:3', interval '1' second) overlaps (time '23:59:59', time '1:2:3')",
                            Boolean.TRUE);
        tester.checkBoolean("(time '1:2:3', interval '1' second) overlaps (time '23:59:59', time '1:2:2')",
                            Boolean.TRUE);
        tester.checkBoolean("(time '1:2:3', interval '1' second) overlaps (time '23:59:59', interval '2' hour)",
                            Boolean.FALSE);
        tester.checkNull("(time '1:2:3', cast(null as time)) overlaps (time '23:59:59', time '1:2:3')");
        tester.checkNull("(time '1:2:3', interval '1' second) overlaps (time '23:59:59', cast(null as interval hour))");
    ...
    @Test public void testPeriodOperators() {
        String[] times = { "TIME '01:00:00'", "TIME '02:00:00'", "TIME '03:00:00'", "TIME '04:00:00'", };
        String[] dates = { "DATE '1970-01-01'", "DATE '1970-02-01'", "DATE '1970-03-01'", "DATE '1970-04-01'", };
        String[] timestamps = { "TIMESTAMP '1970-01-01 00:00:00'", "TIMESTAMP '1970-02-01 00:00:00'",
                                "TIMESTAMP '1970-03-01 00:00:00'", "TIMESTAMP '1970-04-01 00:00:00'", };
    ...
    @Test public void testLessThanOperator() {
        ...
        tester.checkBoolean("cast(1.1 as real)<cast(1.1 as double)", Boolean.FALSE);
        tester.checkBoolean("true<false", Boolean.FALSE);
        tester.checkBoolean("true<true", Boolean.FALSE);
        tester.checkBoolean("false<false", Boolean.FALSE);
    ...
    @Test public void testLessThanOperatorInterval() {
        ...
        tester.checkBoolean("interval '2 2:2:2' day to second < interval '2' day", Boolean.FALSE);
        tester.checkBoolean("interval '2' day < interval '2' day", Boolean.FALSE);
        tester.checkBoolean("interval '2' day < interval '-2' day", Boolean.FALSE);
        tester.checkBoolean("interval '2' day < interval '2' hour", Boolean.FALSE);
        tester.checkBoolean("interval '2' minute < interval '2' hour", Boolean.TRUE);
        tester.checkBoolean("interval '2' second < interval '2' minute", Boolean.TRUE);
        tester.checkNull("cast(null as interval hour) < interval '2' minute");
        tester.checkNull("interval '2:2' hour to minute < cast(null as interval second)");
    }
    ...
    @Test public void testMinusOperator() {
        ...
        tester.checkScalarApprox("cast(2.0 as double) -1", "DOUBLE NOT NULL", 1, 0);
        tester.checkScalarApprox("cast(1 as smallint)-cast(2.0 as real)", "REAL NOT NULL", -1, 0);
        tester.checkScalarApprox("2.4-cast(2.0 as real)", "DOUBLE NOT NULL", 0.4, 0.00000001);
        tester.checkScalarExact("1-2", "-1");
        tester.checkScalarExact("10.0 - 5.0", "DECIMAL(4, 1) NOT NULL", "5.0");
        tester.checkScalarExact("19.68 - 4.2", "DECIMAL(5, 2) NOT NULL", "15.48");
        ...
        if (Bug.FNL25_FIXED) {
            ...
            tester.checkFails("cast(100 as tinyint) - cast(-100 as tinyint)", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(-20000 as smallint) - cast(20000 as smallint)", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(1.5e9 as integer) - cast(-1.5e9 as integer)", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(-5e18 as bigint) - cast(5e18 as bigint)", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(5e18 as decimal(19,0)) - cast(-5e18 as decimal(19,0))", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(-5e8 as decimal(19,10)) - cast(5e8 as decimal(19,10))", OUT_OF_RANGE_MESSAGE, true);
        }
    }

    @Test public void testMinusIntervalOperator() {
        tester.setFor(SqlStdOperatorTable.MINUS);
        tester.checkScalar("interval '2' day - interval '1' day", "+1", "INTERVAL DAY NOT NULL");
        tester.checkScalar("interval '2' day - interval '1' minute", "+1 23:59", "INTERVAL DAY TO MINUTE NOT NULL");
        tester.checkScalar("interval '2' year - interval '1' month", "+1-11", "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkScalar("interval '2' year - interval '1' month - interval '3' year", "-1-01",
                           "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkNull("cast(null as interval day) + interval '2' hour");

        // Datetime minus interval
        tester.checkScalar("time '12:03:01' - interval '1:1' hour to minute", "11:02:01", "TIME(0) NOT NULL");
        // Per [CALCITE-1632] Return types of datetime + interval
        // make sure that TIME values say in range
        tester.checkScalar("time '12:03:01' - interval '1' day", "12:03:01", "TIME(0) NOT NULL");
        tester.checkScalar("time '12:03:01' - interval '25' hour", "11:03:01", "TIME(0) NOT NULL");
        tester.checkScalar("time '12:03:03' - interval '25:0:1' hour to second", "11:03:02", "TIME(0) NOT NULL");
        tester.checkScalar("date '2005-03-02' - interval '5' day", "2005-02-25", "DATE NOT NULL");
        tester.checkScalar("date '2005-03-02' - interval '5' day", "2005-02-25", "DATE NOT NULL");
        tester.checkScalar("date '2005-03-02' - interval '5' hour", "2005-03-02", "DATE NOT NULL");
        tester.checkScalar("date '2005-03-02' - interval '25' hour", "2005-03-01", "DATE NOT NULL");
        tester.checkScalar("date '2005-03-02' - interval '25:45' hour to minute", "2005-03-01", "DATE NOT NULL");
        tester.checkScalar("date '2005-03-02' - interval '25:45:54' hour to second", "2005-03-01", "DATE NOT NULL");
        tester.checkScalar("timestamp '2003-08-02 12:54:01' - interval '-4 2:4' day to minute", "2003-08-06 14:58:01",
                           "TIMESTAMP(0) NOT NULL");

        // Datetime minus year-month interval
        tester.checkScalar("timestamp '2003-08-02 12:54:01' - interval '12' year", "1991-08-02 12:54:01",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("date '2003-08-02' - interval '12' year", "1991-08-02", "DATE NOT NULL");
        tester.checkScalar("date '2003-08-02' - interval '12-3' year to month", "1991-05-02", "DATE NOT NULL");
    }

    @Test public void testMinusDateOperator() {
        tester.setFor(SqlStdOperatorTable.MINUS_DATE);
        tester.checkScalar("(time '12:03:34' - time '11:57:23') minute to second", "+6:11.000000",
                           "INTERVAL MINUTE TO SECOND NOT NULL");
        tester.checkScalar("(time '12:03:23' - time '11:57:23') minute", "+6", "INTERVAL MINUTE NOT NULL");
        tester.checkScalar("(time '12:03:34' - time '11:57:23') minute", "+6", "INTERVAL MINUTE NOT NULL");
        tester.checkScalar("(timestamp '2004-05-01 12:03:34' - timestamp '2004-04-29 11:57:23') day to second",
                           "+2 00:06:11.000000", "INTERVAL DAY TO SECOND NOT NULL");
        tester.checkScalar("(timestamp '2004-05-01 12:03:34' - timestamp '2004-04-29 11:57:23') day to hour", "+2 00",
                           "INTERVAL DAY TO HOUR NOT NULL");
        tester.checkScalar("(date '2004-12-02' - date '2003-12-01') day", "+367", "INTERVAL DAY NOT NULL");
        tester.checkNull("(cast(null as date) - date '2003-12-01') day");

        // combine '<datetime> + <interval>' with '<datetime> - <datetime>'
        tester.checkScalar("timestamp '1969-04-29 0:0:0' +" + " (timestamp '2008-07-15 15:28:00' - "
                           + "  timestamp '1969-04-29 0:0:0') day to second / 2", "1988-12-06 07:44:00",
                           "TIMESTAMP(0) NOT NULL");

        tester.checkScalar("date '1969-04-29' +" + " (date '2008-07-15' - " + "  date '1969-04-29') day / 2",
                           "1988-12-06", "DATE NOT NULL");

        tester.checkScalar("time '01:23:44' +" + " (time '15:28:00' - " + "  time '01:23:44') hour to second / 2",
                           "08:25:52", "TIME(0) NOT NULL");

        if (Bug.DT1684_FIXED) {
            tester.checkBoolean(
                    "(date '1969-04-29' +" + " (CURRENT_DATE - " + "  date '1969-04-29') day / 2) is not null",
                    Boolean.TRUE);
        }
    ...
    @Test public void testMultiplyOperator() {
        ...
        tester.checkScalarApprox("cast(2.0 as float)*3", "FLOAT NOT NULL", 6, 0);
        tester.checkScalarApprox("3*cast(2.0 as real)", "REAL NOT NULL", 6, 0);
        tester.checkScalarApprox("cast(2.0 as real)*3.2", "DOUBLE NOT NULL", 6.4, 0);
        tester.checkScalarExact("10.0 * 5.0", "DECIMAL(5, 2) NOT NULL", "50.00");
        tester.checkScalarExact("19.68 * 4.2", "DECIMAL(6, 3) NOT NULL", "82.656");
        ...
        if (Bug.FNL25_FIXED) {
            // Should throw out of range error
            tester.checkFails("cast(100 as tinyint) * cast(-2 as tinyint)", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(200 as smallint) * cast(200 as smallint)", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(1.5e9 as integer) * cast(-2 as integer)", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(5e9 as bigint) * cast(2e9 as bigint)", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(2e9 as decimal(19,0)) * cast(-5e9 as decimal(19,0))", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(5e4 as decimal(19,10)) * cast(2e4 as decimal(19,10))", OUT_OF_RANGE_MESSAGE, true);
        }
    }

    @Test public void testMultiplyIntervals() {
        tester.checkScalar("interval '2:2' hour to minute * 3", "+6:06", "INTERVAL HOUR TO MINUTE NOT NULL");
        tester.checkScalar("3 * 2 * interval '2:5:12' hour to second", "+12:31:12.000000",
                           "INTERVAL HOUR TO SECOND NOT NULL");
        tester.checkNull("interval '2' day * cast(null as bigint)");
        tester.checkNull("cast(null as interval month) * 2");
        if (TODO) {
            tester.checkScalar("interval '3-2' year to month * 15e-1", "+04-09", "INTERVAL YEAR TO MONTH NOT NULL");
            tester.checkScalar("interval '3-4' year to month * 4.5", "+15-00", "INTERVAL YEAR TO MONTH NOT NULL");
        }
    }

    @Test public void testDatePlusInterval() {
        tester.checkScalar("date '2014-02-11' + interval '2' day", "2014-02-13", "DATE NOT NULL");
        // 60 days is more than 2^32 milliseconds
        tester.checkScalar("date '2014-02-11' + interval '60' day", "2014-04-12", "DATE NOT NULL");
    }
    ...
    @Test public void testNotEqualsOperator() {
        ...
        tester.checkFails("1 != 1", "Bang equal '!=' is not allowed under the current SQL conformance level", false);
        // "!=" is allowed under ORACLE_10 SQL conformance level
        final SqlTester tester1 = tester.withConformance(SqlConformanceEnum.ORACLE_10);
    ...
    @Test public void testNotEqualsOperatorIntervals() {
        tester.checkBoolean("interval '2' day <> interval '1' day", Boolean.TRUE);
        tester.checkBoolean("interval '2' day <> interval '2' day", Boolean.FALSE);
        tester.checkBoolean("interval '2:2:2' hour to second <> interval '2' hour", Boolean.TRUE);
        tester.checkNull("cast(null as interval hour) <> interval '2' minute");
    }
    ...
    @Test public void testOrOperatorLazy() {
        ...
        // get error
        tester.check("values 1 < cast(null as integer) or sqrt(-4) = -2", SqlTests.BOOLEAN_TYPE_CHECKER,
                     SqlTests.ANY_PARAMETER_CHECKER,
                     new ValueOrExceptionResultChecker(null, INVALID_ARG_FOR_POWER, CODE_2201F));

        // Do not need to evaluate 2nd argument if first evaluates to true.
        // In eager evaluation, get error;
        // lazy evaluation returns true;
        // both are valid.
        tester.check("values 1 < 2 or sqrt(-4) = -2", SqlTests.BOOLEAN_TYPE_CHECKER, SqlTests.ANY_PARAMETER_CHECKER,
                     new ValueOrExceptionResultChecker(Boolean.TRUE, INVALID_ARG_FOR_POWER, CODE_2201F));
        ...
        // NULL OR TRUE --> TRUE
        tester.checkBoolean("1 < cast(null as integer) or sqrt(4) = 2", Boolean.TRUE);
    }
    ...
    @Test public void testPlusOperator() {
        ...
        tester.checkScalarApprox("1+cast(2.0 as double)", "DOUBLE NOT NULL", 3, 0);
        tester.checkScalarApprox("1+cast(2.0 as double)+cast(6.0 as float)", "DOUBLE NOT NULL", 9, 0);
        tester.checkScalarExact("10.0 + 5.0", "DECIMAL(4, 1) NOT NULL", "15.0");
        tester.checkScalarExact("19.68 + 4.2", "DECIMAL(5, 2) NOT NULL", "23.88");
        tester.checkScalarExact("19.68 + 4.2 + 6", "DECIMAL(13, 2) NOT NULL", "29.88");
        tester.checkScalarApprox("19.68 + cast(4.2 as float)", "DOUBLE NOT NULL", 23.88, 0.02);
        ...
        if (Bug.FNL25_FIXED) {
            ...
            tester.checkFails("cast(1.5e9 as integer) + cast(1.5e9 as integer)", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(5e18 as bigint) + cast(5e18 as bigint)", OUT_OF_RANGE_MESSAGE, true);
            tester.checkFails("cast(-5e18 as decimal(19,0)) + cast(-5e18 as decimal(19,0))", OUT_OF_RANGE_MESSAGE,
                              true);
            tester.checkFails("cast(5e8 as decimal(19,10)) + cast(5e8 as decimal(19,10))", OUT_OF_RANGE_MESSAGE, true);
        }
    }
    ...
    @Test public void testPlusIntervalOperator() {
        tester.setFor(SqlStdOperatorTable.PLUS);
        tester.checkScalar("interval '2' day + interval '1' day", "+3", "INTERVAL DAY NOT NULL");
        tester.checkScalar("interval '2' day + interval '1' minute", "+2 00:01", "INTERVAL DAY TO MINUTE NOT NULL");
        tester.checkScalar("interval '2' day + interval '5' minute + interval '-3' second", "+2 00:04:57.000000",
                           "INTERVAL DAY TO SECOND NOT NULL");
        tester.checkScalar("interval '2' year + interval '1' month", "+2-01", "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkNull("interval '2' year + cast(null as interval month)");

        // Datetime plus interval
        tester.checkScalar("time '12:03:01' + interval '1:1' hour to minute", "13:04:01", "TIME(0) NOT NULL");
        // Per [CALCITE-1632] Return types of datetime + interval
        // make sure that TIME values say in range
        tester.checkScalar("time '12:03:01' + interval '1' day", "12:03:01", "TIME(0) NOT NULL");
        tester.checkScalar("time '12:03:01' + interval '25' hour", "13:03:01", "TIME(0) NOT NULL");
        tester.checkScalar("time '12:03:01' + interval '25:0:1' hour to second", "13:03:02", "TIME(0) NOT NULL");
        tester.checkScalar("interval '5' day + date '2005-03-02'", "2005-03-07", "DATE NOT NULL");
        tester.checkScalar("date '2005-03-02' + interval '5' day", "2005-03-07", "DATE NOT NULL");
        tester.checkScalar("date '2005-03-02' + interval '5' hour", "2005-03-02", "DATE NOT NULL");
        tester.checkScalar("date '2005-03-02' + interval '25' hour", "2005-03-03", "DATE NOT NULL");
        tester.checkScalar("date '2005-03-02' + interval '25:45' hour to minute", "2005-03-03", "DATE NOT NULL");
        tester.checkScalar("date '2005-03-02' + interval '25:45:54' hour to second", "2005-03-03", "DATE NOT NULL");
        tester.checkScalar("timestamp '2003-08-02 12:54:01' + interval '-4 2:4' day to minute", "2003-07-29 10:50:01",
                           "TIMESTAMP(0) NOT NULL");

        // Datetime plus year-to-month interval
        tester.checkScalar("interval '5-3' year to month + date '2005-03-02'", "2010-06-02", "DATE NOT NULL");
        tester.checkScalar("timestamp '2003-08-02 12:54:01' + interval '5-3' year to month", "2008-11-02 12:54:01",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("interval '5-3' year to month + timestamp '2003-08-02 12:54:01'", "2008-11-02 12:54:01",
                           "TIMESTAMP(0) NOT NULL");
    }
    ...
    @Test public void testIsNotNullOperator() {
        ...
        tester.checkBoolean("cast(null as boolean) is not null", Boolean.FALSE);
    }
    ...
    @Test public void testIsNullOperator() {
        ...
        tester.checkBoolean("cast(null as boolean) is null", Boolean.TRUE);
    }
    ...
    @Test public void testIsNotTrueOperator() {
        ...
        tester.checkBoolean("cast(null as boolean) is not true", Boolean.TRUE);
        tester.checkFails("select ^'a string' is not true^ from (values (1))",
    ...
    @Test public void testIsTrueOperator() {
        ...
        tester.checkBoolean("cast(null as boolean) is true", Boolean.FALSE);
    }
    ...
    @Test public void testIsNotFalseOperator() {
        ...
        tester.checkBoolean("cast(null as boolean) is not false", Boolean.TRUE);
    }
    ...
    @Test public void testIsFalseOperator() {
        ...
        tester.checkBoolean("cast(null as boolean) is false", Boolean.FALSE);
    }
    ...
    @Test public void testIsNotUnknownOperator() {
        ...
        tester.checkBoolean("cast(null as boolean) is not unknown", Boolean.FALSE);
        tester.checkBoolean("unknown is not unknown", Boolean.FALSE);
        tester.checkFails("^'abc' IS NOT UNKNOWN^", "(?s).*Cannot apply 'IS NOT UNKNOWN'.*", false);
    }
    ...
    @Test public void testIsUnknownOperator() {
        ...
        tester.checkBoolean("cast(null as boolean) is unknown", Boolean.TRUE);
        tester.checkBoolean("unknown is unknown", Boolean.TRUE);
        tester.checkFails("0 = 1 AND ^2 IS UNKNOWN^ AND 3 > 4", "(?s).*Cannot apply 'IS UNKNOWN'.*", false);
    }
    ...
    @Test public void testPrefixMinusOperator() {
        ...
        tester.checkFails("'a' + ^- 'b'^ + 'c'", "(?s)Cannot apply '-' to arguments of type '-<CHAR\\(1\\)>'.*", false);
    ...
    @Test public void testPrefixMinusOperatorIntervals() {
        tester.checkScalar("-interval '-6:2:8' hour to second", "+6:02:08.000000", "INTERVAL HOUR TO SECOND NOT NULL");
        tester.checkScalar("- -interval '-6:2:8' hour to second", "-6:02:08.000000",
                           "INTERVAL HOUR TO SECOND NOT NULL");
        tester.checkScalar("-interval '5' month", "-5", "INTERVAL MONTH NOT NULL");
        tester.checkNull("-cast(null as interval day to minute)");
    }
    ...
    @Test public void testPrefixPlusOperatorIntervals() {
        ...
        tester.checkScalar("++interval '-6:2:8' hour to second", "-6:02:08.000000", "INTERVAL HOUR TO SECOND NOT NULL");
        if (Bug.FRG254_FIXED) {
            tester.checkScalar("+interval '6:2:8.234' hour to second", "+06:02:08.234",
                               "INTERVAL HOUR TO SECOND NOT NULL");
        }
        tester.checkScalar("+interval '5' month", "+5", "INTERVAL MONTH NOT NULL");
        tester.checkNull("+cast(null as interval day to minute)");
    ...
    @Test public void testExplicitTableOperator() {
        tester.setFor(SqlStdOperatorTable.EXPLICIT_TABLE, VM_EXPAND);
    }

    @Test public void testValuesOperator() {
        tester.setFor(SqlStdOperatorTable.VALUES, VM_EXPAND);
        tester.check("select 'abc' from (values(true))", new SqlTests.StringTypeChecker("CHAR(3) NOT NULL"), "abc", 0);
    }
    ...
    @Ignore("[CALCITE-525] Exception-handling in built-in functions") @Test public void testLikeEscape2() {
    ...
    @Test public void testNotSimilarToOperator() {
        ...
        tester.checkBoolean("cast(null as varchar(2)) not similar to 'a_'", null);
        tester.checkBoolean("cast(null as varchar(3)) not similar to cast(null as char(2))", null);
    }
    ...
    @Test public void testSimilarToOperator() {
        ...
        tester.checkBoolean("'123aXYZ' similar to '_%_a%_' ", Boolean.TRUE);

        tester.checkBoolean("'xy' similar to '(xy)' ", Boolean.TRUE);

        tester.checkBoolean("'abd' similar to '[ab][bcde]d' ", Boolean.TRUE);

        tester.checkBoolean("'bdd' similar to '[ab][bcde]d' ", Boolean.TRUE);
        ...
        tester.checkBoolean("'mike' similar to 'amy|fred' ", Boolean.FALSE);
        ...
        tester.checkBoolean("'abbbbb' similar to 'a(b{3})' ", Boolean.FALSE);

        tester.checkBoolean("'abbbbb' similar to 'ab{3,6}' ", Boolean.TRUE);

        tester.checkBoolean("'abbbbbbbb' similar to 'ab{3,6}' ", Boolean.FALSE);
        ...
        tester.checkBoolean("'ab' similar to 'a\\_' ESCAPE '\\' ", Boolean.FALSE);

        tester.checkBoolean("'ab' similar to 'a\\%' ESCAPE '\\' ", Boolean.FALSE);

        tester.checkBoolean("'a_' similar to 'a\\_' ESCAPE '\\' ", Boolean.TRUE);

        tester.checkBoolean("'a%' similar to 'a\\%' ESCAPE '\\' ", Boolean.TRUE);

        tester.checkBoolean("'a(b{3})' similar to 'a(b{3})' ", Boolean.FALSE);

        tester.checkBoolean("'a(b{3})' similar to 'a\\(b\\{3\\}\\)' ESCAPE '\\' ", Boolean.TRUE);
        ...
        // some negative tests
        tester.checkFails("'yd' similar to '[x-ze-a]d'",
        ...
        tester.checkFails("'cd' similar to '[(a-e)]d' ", "Invalid regular expression: \\[\\(a-e\\)\\]d at 1", true);

        tester.checkFails("'yd' similar to '[(a-e)]d' ", "Invalid regular expression: \\[\\(a-e\\)\\]d at 1", true);

        // all the following tests wrong results due to missing functionality
        // or defect (FRG-375, 377).

        if (Bug.FRG375_FIXED) {
            tester.checkBoolean("'cd' similar to '[a-e^c]d' ", Boolean.FALSE); // FRG-375
        }

        // following tests use regular character set identifiers.
        // Not implemented yet. FRG-377.
        if (Bug.FRG377_FIXED) {
            tester.checkBoolean("'y' similar to '[:ALPHA:]*'", Boolean.TRUE);

            tester.checkBoolean("'yd32' similar to '[:LOWER:]{2}[:DIGIT:]*'", Boolean.TRUE);

            tester.checkBoolean("'yd32' similar to '[:ALNUM:]*'", Boolean.TRUE);

            tester.checkBoolean("'yd32' similar to '[:ALNUM:]*[:DIGIT:]?'", Boolean.TRUE);

            tester.checkBoolean("'yd32' similar to '[:ALNUM:]?[:DIGIT:]*'", Boolean.FALSE);

            tester.checkBoolean("'yd3223' similar to '([:LOWER:]{2})[:DIGIT:]{2,5}'", Boolean.TRUE);

            tester.checkBoolean("'yd3223' similar to '[:LOWER:]{2}[:DIGIT:]{2,}'", Boolean.TRUE);

            tester.checkBoolean("'yd3223' similar to '[:LOWER:]{2}||[:DIGIT:]{4}'", Boolean.TRUE);

            tester.checkBoolean("'yd3223' similar to '[:LOWER:]{2}[:DIGIT:]{3}'", Boolean.FALSE);

            tester.checkBoolean("'yd  3223' similar to '[:UPPER:]{2}  [:DIGIT:]{3}'", Boolean.FALSE);

            tester.checkBoolean("'YD  3223' similar to '[:UPPER:]{2}  [:DIGIT:]{3}'", Boolean.FALSE);

            tester.checkBoolean("'YD  3223' similar to " + "'[:UPPER:]{2}||[:WHITESPACE:]*[:DIGIT:]{4}'", Boolean.TRUE);

            tester.checkBoolean("'YD\t3223' similar to '[:UPPER:]{2}[:SPACE:]*[:DIGIT:]{4}'", Boolean.FALSE);

            tester.checkBoolean("'YD\t3223' similar to " + "'[:UPPER:]{2}[:WHITESPACE:]*[:DIGIT:]{4}'", Boolean.TRUE);

            tester.checkBoolean("'YD\t\t3223' similar to " + "'([:UPPER:]{2}[:WHITESPACE:]+)||[:DIGIT:]{4}'",
                                Boolean.TRUE);
        }
    }
    ...
    @Test public void testConvertFunc() {
        tester.setFor(SqlStdOperatorTable.CONVERT, VM_FENNEL, VM_JAVA);
    }

    @Test public void testTranslateFunc() {
        tester.setFor(SqlStdOperatorTable.TRANSLATE, VM_FENNEL, VM_JAVA);
    }
    ...
    @Test public void testOverlayFunc() {
        ...
        tester.checkString("overlay('ABCdef' placing 'abc' from 1)", "abcdef", "VARCHAR(9) NOT NULL");
        tester.checkString("overlay('ABCdef' placing 'abc' from 1 for 2)", "abcCdef", "VARCHAR(9) NOT NULL");
        if (enable) {
            tester.checkString(
                    "overlay(cast('ABCdef' as varchar(10)) placing " + "cast('abc' as char(5)) from 1 for 2)",
                    "abc  Cdef", "VARCHAR(15) NOT NULL");
        }
        if (enable) {
            tester.checkString("overlay(cast('ABCdef' as char(10)) placing " + "cast('abc' as char(5)) from 1 for 2)",
                               "abc  Cdef    ", "VARCHAR(15) NOT NULL");
        }
        tester.checkNull("overlay('ABCdef' placing 'abc' from 1 for cast(null as integer))");
        tester.checkNull("overlay(cast(null as varchar(1)) placing 'abc' from 1)");

        tester.checkString("overlay(x'ABCdef' placing x'abcd' from 1)", "abcdef", "VARBINARY(5) NOT NULL");
        tester.checkString("overlay(x'ABCDEF1234' placing x'2345' from 1 for 2)", "2345ef1234",
                           "VARBINARY(7) NOT NULL");
        if (enable) {
            tester.checkString(
                    "overlay(cast(x'ABCdef' as varbinary(5)) placing " + "cast(x'abcd' as binary(3)) from 1 for 2)",
                    "abc  Cdef", "VARBINARY(8) NOT NULL");
        }
        if (enable) {
            tester.checkString(
                    "overlay(cast(x'ABCdef' as binary(5)) placing " + "cast(x'abcd' as binary(3)) from 1 for 2)",
                    "abc  Cdef    ", "VARBINARY(8) NOT NULL");
        }
        tester.checkNull("overlay(x'ABCdef' placing x'abcd' from 1 for cast(null as integer))");
        tester.checkNull("overlay(cast(null as varbinary(1)) placing x'abcd' from 1)");

        tester.checkNull("overlay(x'abcd' placing x'abcd' from cast(null as integer))");
    }
    ...
    @Test public void testPositionFunc() {
        ...
        tester.checkScalar("position(cast('a' as char) in cast('bca' as varchar))", 3, "INTEGER NOT NULL");
    }
    ...
    @Test public void testReplaceFunc() {
        ...
        tester.checkString("REPLACE('hello world', 'o', '')", "hell wrld", "VARCHAR(11) NOT NULL");
    ...
    @Test public void testInitcapFunc() {
        ...
        tester.checkString("initcap('ab cd Ef 12')", "Ab Cd Ef 12", "CHAR(11) NOT NULL");
        tester.checkNull("initcap(cast(null as varchar(1)))");

        // dtbug 232
        tester.checkFails("^initcap(cast(null as date))^",
    ...
    @Test public void testPowerFunc() {
        ...
        tester.checkScalarApprox("power(2,-2)", "DOUBLE NOT NULL", 0.25, 0);
        ...
        tester.checkFails("^pow(2,-2)^", "No match found for function signature POW\\(<NUMERIC>, <NUMERIC>\\)", false);
    }

    @Test public void testSqrtFunc() {
        tester.setFor(SqlStdOperatorTable.SQRT, SqlTester.VmName.EXPAND);
        tester.checkType("sqrt(2)", "DOUBLE NOT NULL");
        tester.checkType("sqrt(cast(2 as float))", "DOUBLE NOT NULL");
        tester.checkType("sqrt(case when false then 2 else null end)", "DOUBLE");
        tester.checkFails("^sqrt('abc')^",
                          "Cannot apply 'SQRT' to arguments of type 'SQRT\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'SQRT\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("sqrt(2)", "DOUBLE NOT NULL", 1.4142d, 0.0001d);
        tester.checkScalarApprox("sqrt(cast(2 as decimal(2, 0)))", "DOUBLE NOT NULL", 1.4142d, 0.0001d);
    ...
    @Test public void testExpFunc() {
        ...
        tester.checkScalarApprox("exp(-2)", "DOUBLE NOT NULL", 0.1353, 0.0001);
    ...
    @Test public void testModFunc() {
        ...
        tester.checkScalarExact("mod(-12,-7)", "-5");
        tester.checkScalarExact("mod(12,-7)", "5");
        tester.checkScalarExact("mod(cast(12 as tinyint), cast(-7 as tinyint))", "TINYINT NOT NULL", "5");

        if (!DECIMAL) {
            return;
        }
        tester.checkScalarExact("mod(cast(9 as decimal(2, 0)), 7)", "INTEGER NOT NULL", "2");
        tester.checkScalarExact("mod(7, cast(9 as decimal(2, 0)))", "DECIMAL(2, 0) NOT NULL", "7");
        tester.checkScalarExact("mod(cast(-9 as decimal(2, 0)), cast(7 as decimal(1, 0)))", "DECIMAL(1, 0) NOT NULL",
                                "-2");
    }
    ...
    @Test public void testModFuncDivByZero() {
        ...
        // submit as non-runtime because the janino exception does not have
        // error position information and the framework is unhappy with that.
        tester.checkFails("mod(3,case 'a' when 'a' then 0 end)", DIVISION_BY_ZERO_MESSAGE, true);
    }

    @Test public void testLnFunc() {
        tester.setFor(SqlStdOperatorTable.LN);
        tester.checkScalarApprox("ln(2.71828)", "DOUBLE NOT NULL", 1.0, 0.000001);
    ...
    @Test public void testLogFunc() {
        ...
        tester.checkScalarApprox("log10(10)", "DOUBLE NOT NULL", 1.0, 0.000001);
        tester.checkScalarApprox("log10(100.0)", "DOUBLE NOT NULL", 2.0, 0.000001);
        tester.checkScalarApprox("log10(cast(10e8 as double))", "DOUBLE NOT NULL", 9.0, 0.000001);
        tester.checkScalarApprox("log10(cast(10e2 as float))", "DOUBLE NOT NULL", 3.0, 0.000001);
        tester.checkScalarApprox("log10(cast(10e-3 as real))", "DOUBLE NOT NULL", -2.0, 0.000001);
    ...
    @Test public void testAbsFunc() {
        ...
        tester.checkScalarExact("abs(cast(10 as TINYINT))", "TINYINT NOT NULL", "10");
        tester.checkScalarExact("abs(cast(-20 as SMALLINT))", "SMALLINT NOT NULL", "20");
        tester.checkScalarExact("abs(cast(-100 as INT))", "INTEGER NOT NULL", "100");
        tester.checkScalarExact("abs(cast(1000 as BIGINT))", "BIGINT NOT NULL", "1000");
        tester.checkScalarExact("abs(54.4)", "DECIMAL(3, 1) NOT NULL", "54.4");
        tester.checkScalarExact("abs(-54.4)", "DECIMAL(3, 1) NOT NULL", "54.4");
        tester.checkScalarApprox("abs(-9.32E-2)", "DOUBLE NOT NULL", 0.0932, 0);
        tester.checkScalarApprox("abs(cast(-3.5 as double))", "DOUBLE NOT NULL", 3.5, 0);
        tester.checkScalarApprox("abs(cast(-3.5 as float))", "FLOAT NOT NULL", 3.5, 0);
        tester.checkScalarApprox("abs(cast(3.5 as real))", "REAL NOT NULL", 3.5, 0);

        tester.checkNull("abs(cast(null as double))");
    }

    @Test public void testAbsFuncIntervals() {
        tester.checkScalar("abs(interval '-2' day)", "+2", "INTERVAL DAY NOT NULL");
        tester.checkScalar("abs(interval '-5-03' year to month)", "+5-03", "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkNull("abs(cast(null as interval hour))");
    }

    @Test public void testAcosFunc() {
        tester.setFor(SqlStdOperatorTable.ACOS);
        tester.checkType("acos(0)", "DOUBLE NOT NULL");
        tester.checkType("acos(cast(1 as float))", "DOUBLE NOT NULL");
        tester.checkType("acos(case when false then 0.5 else null end)", "DOUBLE");
        tester.checkFails("^acos('abc')^",
                          "Cannot apply 'ACOS' to arguments of type 'ACOS\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'ACOS\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("acos(0.5)", "DOUBLE NOT NULL", 1.0472d, 0.0001d);
        tester.checkScalarApprox("acos(cast(0.5 as decimal(1, 1)))", "DOUBLE NOT NULL", 1.0472d, 0.0001d);
        tester.checkNull("acos(cast(null as integer))");
        tester.checkNull("acos(cast(null as double))");
    }

    @Test public void testAsinFunc() {
        tester.setFor(SqlStdOperatorTable.ASIN);
        tester.checkType("asin(0)", "DOUBLE NOT NULL");
        tester.checkType("asin(cast(1 as float))", "DOUBLE NOT NULL");
        tester.checkType("asin(case when false then 0.5 else null end)", "DOUBLE");
        tester.checkFails("^asin('abc')^",
                          "Cannot apply 'ASIN' to arguments of type 'ASIN\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'ASIN\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("asin(0.5)", "DOUBLE NOT NULL", 0.5236d, 0.0001d);
        tester.checkScalarApprox("asin(cast(0.5 as decimal(1, 1)))", "DOUBLE NOT NULL", 0.5236d, 0.0001d);
        tester.checkNull("asin(cast(null as integer))");
        tester.checkNull("asin(cast(null as double))");
    }

    @Test public void testAtanFunc() {
        tester.setFor(SqlStdOperatorTable.ATAN);
        tester.checkType("atan(2)", "DOUBLE NOT NULL");
        tester.checkType("atan(cast(2 as float))", "DOUBLE NOT NULL");
        tester.checkType("atan(case when false then 2 else null end)", "DOUBLE");
        tester.checkFails("^atan('abc')^",
                          "Cannot apply 'ATAN' to arguments of type 'ATAN\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'ATAN\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("atan(2)", "DOUBLE NOT NULL", 1.1071d, 0.0001d);
        tester.checkScalarApprox("atan(cast(2 as decimal(1, 0)))", "DOUBLE NOT NULL", 1.1071d, 0.0001d);
        tester.checkNull("atan(cast(null as integer))");
        tester.checkNull("atan(cast(null as double))");
    }

    @Test public void testAtan2Func() {
        tester.setFor(SqlStdOperatorTable.ATAN2);
        tester.checkType("atan2(2, -2)", "DOUBLE NOT NULL");
        tester.checkType("atan2(cast(1 as float), -1)", "DOUBLE NOT NULL");
        tester.checkType("atan2(case when false then 0.5 else null end, -1)", "DOUBLE");
        tester.checkFails("^atan2('abc', 'def')^",
                          "Cannot apply 'ATAN2' to arguments of type 'ATAN2\\(<CHAR\\(3\\)>, <CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'ATAN2\\(<NUMERIC>, <NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("atan2(0.5, -0.5)", "DOUBLE NOT NULL", 2.3562d, 0.0001d);
        tester.checkScalarApprox("atan2(cast(0.5 as decimal(1, 1)), cast(-0.5 as decimal(1, 1)))", "DOUBLE NOT NULL",
                                 2.3562d, 0.0001d);
    ...
    @Test public void testCosFunc() {
        tester.setFor(SqlStdOperatorTable.COS);
        tester.checkType("cos(1)", "DOUBLE NOT NULL");
        tester.checkType("cos(cast(1 as float))", "DOUBLE NOT NULL");
        tester.checkType("cos(case when false then 1 else null end)", "DOUBLE");
        tester.checkFails("^cos('abc')^",
                          "Cannot apply 'COS' to arguments of type 'COS\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'COS\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("cos(1)", "DOUBLE NOT NULL", 0.5403d, 0.0001d);
        tester.checkScalarApprox("cos(cast(1 as decimal(1, 0)))", "DOUBLE NOT NULL", 0.5403d, 0.0001d);
        tester.checkNull("cos(cast(null as integer))");
        tester.checkNull("cos(cast(null as double))");
    }

    @Test public void testCotFunc() {
        tester.setFor(SqlStdOperatorTable.COT);
        tester.checkType("cot(1)", "DOUBLE NOT NULL");
        tester.checkType("cot(cast(1 as float))", "DOUBLE NOT NULL");
        tester.checkType("cot(case when false then 1 else null end)", "DOUBLE");
        tester.checkFails("^cot('abc')^",
                          "Cannot apply 'COT' to arguments of type 'COT\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'COT\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("cot(1)", "DOUBLE NOT NULL", 0.6421d, 0.0001d);
        tester.checkScalarApprox("cot(cast(1 as decimal(1, 0)))", "DOUBLE NOT NULL", 0.6421d, 0.0001d);
        tester.checkNull("cot(cast(null as integer))");
        tester.checkNull("cot(cast(null as double))");
    }

    @Test public void testDegreesFunc() {
        tester.setFor(SqlStdOperatorTable.DEGREES);
        tester.checkType("degrees(1)", "DOUBLE NOT NULL");
        tester.checkType("degrees(cast(1 as float))", "DOUBLE NOT NULL");
        tester.checkType("degrees(case when false then 1 else null end)", "DOUBLE");
        tester.checkFails("^degrees('abc')^",
                          "Cannot apply 'DEGREES' to arguments of type 'DEGREES\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'DEGREES\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("degrees(1)", "DOUBLE NOT NULL", 57.2958d, 0.0001d);
        tester.checkScalarApprox("degrees(cast(1 as decimal(1, 0)))", "DOUBLE NOT NULL", 57.2958d, 0.0001d);
    ...
    @Test public void testPiFunc() {
        ...
        tester.checkFails("^PI()^", "No match found for function signature PI\\(\\)", false);
    }

    @Test public void testRadiansFunc() {
        tester.setFor(SqlStdOperatorTable.RADIANS);
        tester.checkType("radians(42)", "DOUBLE NOT NULL");
        tester.checkType("radians(cast(42 as float))", "DOUBLE NOT NULL");
        tester.checkType("radians(case when false then 42 else null end)", "DOUBLE");
        tester.checkFails("^radians('abc')^",
                          "Cannot apply 'RADIANS' to arguments of type 'RADIANS\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'RADIANS\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("radians(42)", "DOUBLE NOT NULL", 0.7330d, 0.0001d);
        tester.checkScalarApprox("radians(cast(42 as decimal(2, 0)))", "DOUBLE NOT NULL", 0.7330d, 0.0001d);
        tester.checkNull("radians(cast(null as integer))");
        tester.checkNull("radians(cast(null as double))");
    }

    @Test public void testRoundFunc() {
        tester.setFor(SqlStdOperatorTable.ROUND);
        tester.checkType("round(42, -1)", "INTEGER NOT NULL");
        tester.checkType("round(cast(42 as float), 1)", "FLOAT NOT NULL");
        tester.checkType("round(case when false then 42 else null end, -1)", "INTEGER");
        tester.checkFails("^round('abc', 'def')^",
                          "Cannot apply 'ROUND' to arguments of type 'ROUND\\(<CHAR\\(3\\)>, <CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'ROUND\\(<NUMERIC>, <INTEGER>\\)'",
                          false);
        tester.checkScalar("round(42, -1)", 40, "INTEGER NOT NULL");
        tester.checkScalar("round(cast(42.346 as decimal(2, 3)), 2)", BigDecimal.valueOf(4235, 2),
        ...
        tester.checkScalar("round(cast(42.346 as decimal(2, 3)))", BigDecimal.valueOf(42, 0), "DECIMAL(2, 3) NOT NULL");
        tester.checkScalar("round(42.324)", BigDecimal.valueOf(42, 0), "DECIMAL(5, 3) NOT NULL");
        tester.checkScalar("round(42.724)", BigDecimal.valueOf(43, 0), "DECIMAL(5, 3) NOT NULL");
    }

    @Test public void testSignFunc() {
        tester.setFor(SqlStdOperatorTable.SIGN);
        tester.checkType("sign(1)", "INTEGER NOT NULL");
        tester.checkType("sign(cast(1 as float))", "FLOAT NOT NULL");
        tester.checkType("sign(case when false then 1 else null end)", "INTEGER");
        tester.checkFails("^sign('abc')^",
                          "Cannot apply 'SIGN' to arguments of type 'SIGN\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'SIGN\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalar("sign(1)", 1, "INTEGER NOT NULL");
        tester.checkScalar("sign(cast(-1 as decimal(1, 0)))", BigDecimal.valueOf(-1), "DECIMAL(1, 0) NOT NULL");
        tester.checkScalar("sign(cast(0 as float))", 0d, "FLOAT NOT NULL");
        tester.checkNull("sign(cast(null as integer))");
        tester.checkNull("sign(cast(null as double))");
    }

    @Test public void testSinFunc() {
        tester.setFor(SqlStdOperatorTable.SIN);
        tester.checkType("sin(1)", "DOUBLE NOT NULL");
        tester.checkType("sin(cast(1 as float))", "DOUBLE NOT NULL");
        tester.checkType("sin(case when false then 1 else null end)", "DOUBLE");
        tester.checkFails("^sin('abc')^",
                          "Cannot apply 'SIN' to arguments of type 'SIN\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'SIN\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("sin(1)", "DOUBLE NOT NULL", 0.8415d, 0.0001d);
        tester.checkScalarApprox("sin(cast(1 as decimal(1, 0)))", "DOUBLE NOT NULL", 0.8415d, 0.0001d);
        tester.checkNull("sin(cast(null as integer))");
        tester.checkNull("sin(cast(null as double))");
    }

    @Test public void testTanFunc() {
        tester.setFor(SqlStdOperatorTable.TAN);
        tester.checkType("tan(1)", "DOUBLE NOT NULL");
        tester.checkType("tan(cast(1 as float))", "DOUBLE NOT NULL");
        tester.checkType("tan(case when false then 1 else null end)", "DOUBLE");
        tester.checkFails("^tan('abc')^",
                          "Cannot apply 'TAN' to arguments of type 'TAN\\(<CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'TAN\\(<NUMERIC>\\)'",
                          false);
        tester.checkScalarApprox("tan(1)", "DOUBLE NOT NULL", 1.5574d, 0.0001d);
        tester.checkScalarApprox("tan(cast(1 as decimal(1, 0)))", "DOUBLE NOT NULL", 1.5574d, 0.0001d);
        tester.checkNull("tan(cast(null as integer))");
        tester.checkNull("tan(cast(null as double))");
    }

    @Test public void testTruncateFunc() {
        tester.setFor(SqlStdOperatorTable.TRUNCATE);
        tester.checkType("truncate(42, -1)", "INTEGER NOT NULL");
        tester.checkType("truncate(cast(42 as float), 1)", "FLOAT NOT NULL");
        tester.checkType("truncate(case when false then 42 else null end, -1)", "INTEGER");
        tester.checkFails("^truncate('abc', 'def')^",
                          "Cannot apply 'TRUNCATE' to arguments of type 'TRUNCATE\\(<CHAR\\(3\\)>, <CHAR\\(3\\)>\\)'\\. Supported form\\(s\\): 'TRUNCATE\\(<NUMERIC>, <INTEGER>\\)'",
                          false);
        tester.checkScalar("truncate(42, -1)", 40, "INTEGER NOT NULL");
        tester.checkScalar("truncate(cast(42.345 as decimal(2, 3)), 2)", BigDecimal.valueOf(4234, 2),
                           "DECIMAL(2, 3) NOT NULL");
        tester.checkScalar("truncate(cast(-42.345 as decimal(2, 3)), 2)", BigDecimal.valueOf(-4234, 2),
                           "DECIMAL(2, 3) NOT NULL");
        ...
        tester.checkScalar("truncate(42.324)", BigDecimal.valueOf(42, 0), "DECIMAL(5, 3) NOT NULL");
        tester.checkScalar("truncate(cast(42.324 as float))", 42F, "FLOAT NOT NULL");
        tester.checkScalar("truncate(cast(42.345 as decimal(2, 3)))", BigDecimal.valueOf(42, 0),
                           "DECIMAL(2, 3) NOT NULL");
    ...
    @Test public void testNullifFunc() {
        ...
        tester.checkScalarExact("nullif(1.5, 13.56)", "DECIMAL(2, 1)", "1.5");
        tester.checkScalarExact("nullif(13.56, 1.5)", "DECIMAL(4, 2)", "13.56");
        ...
        tester.checkScalarApprox("nullif(1.5, cast(3e0 as REAL))", "DECIMAL(2, 1)", 1.5, 0);
        tester.checkScalarExact("nullif(3, 1.5e0)", "INTEGER", "3");
        tester.checkScalarExact("nullif(3, cast(1.5e0 as REAL))", "INTEGER", "3");
        tester.checkScalarApprox("nullif(1.5e0, 3.4)", "DOUBLE", 1.5, 0);
        tester.checkScalarExact("nullif(3.4, 1.5e0)", "DECIMAL(2, 1)", "3.4");
        tester.checkString("nullif('a','bc')", "a", "CHAR(1)");
        ...
        tester.checkFails("1 + ^nullif(1, date '2005-8-4')^ + 2",
                          "(?s)Cannot apply '=' to arguments of type '<INTEGER> = <DATE>'\\..*", false);

        tester.checkFails("1 + ^nullif(1, 2, 3)^ + 2",
                          "Invalid number of arguments to function 'NULLIF'\\. Was expecting 2 arguments", false);
    }
    ...
    @Test public void testNullIfOperatorIntervals() {
        ...
        tester.checkNull("nullif(interval '3' day, interval '3' day)");
    }
    ...
    @Test public void testCoalesceFunc() {
        ...
        tester.checkFails("1 + ^coalesce('a', 'b', 1, null)^ + 2",
                          "Illegal mixing of types in CASE or COALESCE statement", false);
    }
    ...
    @Test public void testLocalTimeFunc() {
        ...
        tester.checkFails("^LOCALTIME()^", "No match found for function signature LOCALTIME\\(\\)", false);
        tester.checkScalar("LOCALTIME(1)", TIME_PATTERN, "TIME(1) NOT NULL");

        final Pair<String, Hook.Closeable> pair = currentTimeString(LOCAL_TZ);
        tester.checkScalar("CAST(LOCALTIME AS VARCHAR(30))",
                           Pattern.compile(pair.left.substring(11) + "[0-9][0-9]:[0-9][0-9]"), "VARCHAR(30) NOT NULL");
        tester.checkScalar("LOCALTIME", Pattern.compile(pair.left.substring(11) + "[0-9][0-9]:[0-9][0-9]"),
                           "TIME(0) NOT NULL");
        pair.right.close();
    }
    ...
    @Test public void testLocalTimestampFunc() {
        ...
        tester.checkScalar("LOCALTIMESTAMP", TIMESTAMP_PATTERN, "TIMESTAMP(0) NOT NULL");
        tester.checkFails("^LOCALTIMESTAMP()^", "No match found for function signature LOCALTIMESTAMP\\(\\)", false);
        tester.checkFails("^LOCALTIMESTAMP(4000000000)^", LITERAL_OUT_OF_RANGE_MESSAGE, false);
        tester.checkFails("^LOCALTIMESTAMP(9223372036854775807)^", LITERAL_OUT_OF_RANGE_MESSAGE, false);
        tester.checkScalar("LOCALTIMESTAMP(1)", TIMESTAMP_PATTERN, "TIMESTAMP(1) NOT NULL");

        // Check that timestamp is being generated in the right timezone by
        // generating a specific timestamp.
        final Pair<String, Hook.Closeable> pair = currentTimeString(LOCAL_TZ);
        tester.checkScalar("CAST(LOCALTIMESTAMP AS VARCHAR(30))", Pattern.compile(pair.left + "[0-9][0-9]:[0-9][0-9]"),
                           "VARCHAR(30) NOT NULL");
        tester.checkScalar("LOCALTIMESTAMP", Pattern.compile(pair.left + "[0-9][0-9]:[0-9][0-9]"),
                           "TIMESTAMP(0) NOT NULL");
        pair.right.close();
    }
    ...
    @Test public void testCurrentTimeFunc() {
        ...
        tester.checkScalar("CURRENT_TIME", TIME_PATTERN, "TIME(0) NOT NULL");
        tester.checkFails("^CURRENT_TIME()^", "No match found for function signature CURRENT_TIME\\(\\)", false);
        tester.checkScalar("CURRENT_TIME(1)", TIME_PATTERN, "TIME(1) NOT NULL");

        final Pair<String, Hook.Closeable> pair = currentTimeString(CURRENT_TZ);
        tester.checkScalar("CAST(CURRENT_TIME AS VARCHAR(30))",
                           Pattern.compile(pair.left.substring(11) + "[0-9][0-9]:[0-9][0-9]"), "VARCHAR(30) NOT NULL");
        tester.checkScalar("CURRENT_TIME", Pattern.compile(pair.left.substring(11) + "[0-9][0-9]:[0-9][0-9]"),
                           "TIME(0) NOT NULL");
        pair.right.close();
    }
    ...
    @Test public void testCurrentTimestampFunc() {
        ...
        tester.checkScalar("CURRENT_TIMESTAMP", TIMESTAMP_PATTERN, "TIMESTAMP(0) NOT NULL");
        tester.checkFails("^CURRENT_TIMESTAMP()^", "No match found for function signature CURRENT_TIMESTAMP\\(\\)",
                          false);
        tester.checkFails("^CURRENT_TIMESTAMP(4000000000)^", LITERAL_OUT_OF_RANGE_MESSAGE, false);
        tester.checkScalar("CURRENT_TIMESTAMP(1)", TIMESTAMP_PATTERN, "TIMESTAMP(1) NOT NULL");

        final Pair<String, Hook.Closeable> pair = currentTimeString(CURRENT_TZ);
        tester.checkScalar("CAST(CURRENT_TIMESTAMP AS VARCHAR(30))",
                           Pattern.compile(pair.left + "[0-9][0-9]:[0-9][0-9]"), "VARCHAR(30) NOT NULL");
        tester.checkScalar("CURRENT_TIMESTAMP", Pattern.compile(pair.left + "[0-9][0-9]:[0-9][0-9]"),
                           "TIMESTAMP(0) NOT NULL");
        pair.right.close();
    }
    ...
    @Test public void testCurrentDateFunc() {
        ...
        tester.checkScalar("(CURRENT_DATE - CURRENT_DATE) DAY", "+0", "INTERVAL DAY NOT NULL");
        ...
        tester.checkFails("^CURRENT_DATE()^", "No match found for function signature CURRENT_DATE\\(\\)", false);
        ...
        try (Hook.Closeable ignore = pair.right) {
            tester.checkScalar("CAST(CURRENT_DATE AS VARCHAR(30))", dateString.substring(0, 10),
                               "VARCHAR(30) NOT NULL");
            tester.checkScalar("CURRENT_DATE", dateString.substring(0, 10), "DATE NOT NULL");

            tester1.checkScalar("CAST(CURRENT_DATE AS VARCHAR(30))", dateString.substring(0, 10),
                                "VARCHAR(30) NOT NULL");
            tester1.checkScalar("CAST(CURRENT_DATE() AS VARCHAR(30))", dateString.substring(0, 10),
                                "VARCHAR(30) NOT NULL");
    ...
    @Test public void testSubstringFunction() {
        ...
        tester.checkString("substring('abc' from 2)", "bc", "VARCHAR(3) NOT NULL");

        tester.checkString("substring(x'aabbcc' from 1 for 2)", "aabb", "VARBINARY(3) NOT NULL");
        tester.checkString("substring(x'aabbcc' from 2)", "bbcc", "VARBINARY(3) NOT NULL");

        if (Bug.FRG296_FIXED) {
            // substring regexp not supported yet
            tester.checkString("substring('foobar' from '%#\"o_b#\"%' for'#')", "oob", "xx");
        }
    ...
    @Test public void testTrimFunc() {
        ...
        tester.checkString("trim('a' from 'aAa')", "A", "VARCHAR(3) NOT NULL");
        tester.checkString("trim(both 'a' from 'aAa')", "A", "VARCHAR(3) NOT NULL");
        tester.checkString("trim(leading 'a' from 'aAa')", "Aa", "VARCHAR(3) NOT NULL");
        tester.checkString("trim(trailing 'a' from 'aAa')", "aA", "VARCHAR(3) NOT NULL");
        ...
        if (Bug.FNL3_FIXED) {
            ...
            tester.checkFails("trim('xy' from 'abcde')",
                              "could not calculate results for the following row:\n" + "\\[ 0 \\]\n" + "Messages:\n"
                              + "\\[0\\]:PC=0 Code=22027 ", true);
            tester.checkFails("trim('' from 'abcde')",
                              "could not calculate results for the following row:\n" + "\\[ 0 \\]\n" + "Messages:\n"
                              + "\\[0\\]:PC=0 Code=22027 ", true);
        }
    }
    ...
    @Test public void testWindow() {
        ...
        tester.check("select sum(1) over (order by x) from (select 1 as x, 2 as y from (values (true)))",
                     new SqlTests.StringTypeChecker("INTEGER"), "1", 0);
    }

    @Test public void testElementFunc() {
        tester.setFor(SqlStdOperatorTable.ELEMENT, VM_FENNEL, VM_JAVA);
        if (TODO) {
    ...
    @Test public void testCardinalityFunc() {
        tester.setFor(SqlStdOperatorTable.CARDINALITY, VM_FENNEL, VM_JAVA);
        if (TODO) {
        ...
        // applied to array
        tester.checkScalarExact("cardinality(array['foo', 'bar'])", "2");

        // applied to map
        tester.checkScalarExact("cardinality(map['foo', 1, 'bar', 2])", "2");
    }

    @Test public void testMemberOfOperator() {
        tester.setFor(SqlStdOperatorTable.MEMBER_OF, VM_FENNEL, VM_JAVA);
        if (TODO) {
    ...
    @Test public void testCollectFunc() {
        ...
        checkAggType(tester, "collect(1.2)", "DECIMAL(2, 1) NOT NULL MULTISET NOT NULL");
        checkAggType(tester, "collect(DISTINCT 1.5)", "DECIMAL(2, 1) NOT NULL MULTISET NOT NULL");
        tester.checkFails("^collect()^", "Invalid number of arguments to function 'COLLECT'. Was expecting 1 arguments",
                          false);
        tester.checkFails("^collect(1, 2)^",
                          "Invalid number of arguments to function 'COLLECT'. Was expecting 1 arguments", false);
    ...
    @Test public void testYear() {
        tester.setFor(SqlStdOperatorTable.YEAR, VM_FENNEL, VM_JAVA);

        tester.checkScalar("year(date '2008-1-23')", "2008", "BIGINT NOT NULL");
        tester.checkNull("year(cast(null as date))");
    }

    @Test public void testQuarter() {
        tester.setFor(SqlStdOperatorTable.QUARTER, VM_FENNEL, VM_JAVA);

        tester.checkScalar("quarter(date '2008-1-23')", "1", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-2-23')", "1", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-3-23')", "1", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-4-23')", "2", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-5-23')", "2", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-6-23')", "2", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-7-23')", "3", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-8-23')", "3", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-9-23')", "3", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-10-23')", "4", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-11-23')", "4", "BIGINT NOT NULL");
        tester.checkScalar("quarter(date '2008-12-23')", "4", "BIGINT NOT NULL");
        tester.checkNull("quarter(cast(null as date))");
    }

    @Test public void testMonth() {
        tester.setFor(SqlStdOperatorTable.MONTH, VM_FENNEL, VM_JAVA);

        tester.checkScalar("month(date '2008-1-23')", "1", "BIGINT NOT NULL");
        tester.checkNull("month(cast(null as date))");
    }

    @Test public void testWeek() {
        tester.setFor(SqlStdOperatorTable.WEEK, VM_FENNEL, VM_JAVA);
        // TODO: Not implemented in operator test execution code
        tester.checkFails("week(date '2008-1-23')", "cannot translate call EXTRACT.*", true);
        tester.checkFails("week(cast(null as date))", "cannot translate call EXTRACT.*", true);
    }

    @Test public void testDayOfYear() {
        tester.setFor(SqlStdOperatorTable.DAYOFYEAR, VM_FENNEL, VM_JAVA);
        // TODO: Not implemented in operator test execution code
        tester.checkFails("dayofyear(date '2008-1-23')", "cannot translate call EXTRACT.*", true);
        tester.checkFails("dayofyear(cast(null as date))", "cannot translate call EXTRACT.*", true);
    }

    @Test public void testDayOfMonth() {
        tester.setFor(SqlStdOperatorTable.DAYOFMONTH, VM_FENNEL, VM_JAVA);
        tester.checkScalar("dayofmonth(date '2008-1-23')", "23", "BIGINT NOT NULL");
        tester.checkNull("dayofmonth(cast(null as date))");
    }

    @Test public void testDayOfWeek() {
        tester.setFor(SqlStdOperatorTable.DAYOFWEEK, VM_FENNEL, VM_JAVA);
        // TODO: Not implemented in operator test execution code
        tester.checkFails("dayofweek(date '2008-1-23')", "cannot translate call EXTRACT.*", true);
        tester.checkFails("dayofweek(cast(null as date))", "cannot translate call EXTRACT.*", true);
    }

    @Test public void testHour() {
        tester.setFor(SqlStdOperatorTable.HOUR, VM_FENNEL, VM_JAVA);

        tester.checkScalar("hour(timestamp '2008-1-23 12:34:56')", "12", "BIGINT NOT NULL");
        tester.checkNull("hour(cast(null as timestamp))");
    }

    @Test public void testMinute() {
        tester.setFor(SqlStdOperatorTable.MINUTE, VM_FENNEL, VM_JAVA);

        tester.checkScalar("minute(timestamp '2008-1-23 12:34:56')", "34", "BIGINT NOT NULL");
        tester.checkNull("minute(cast(null as timestamp))");
    }

    @Test public void testSecond() {
        tester.setFor(SqlStdOperatorTable.SECOND, VM_FENNEL, VM_JAVA);

        tester.checkScalar("second(timestamp '2008-1-23 12:34:56')", "56", "BIGINT NOT NULL");
        tester.checkNull("second(cast(null as timestamp))");
    }

    @Test public void testExtractIntervalYearMonth() {
        tester.setFor(SqlStdOperatorTable.EXTRACT, VM_FENNEL, VM_JAVA);

        if (TODO) {
            // Not supported, fails in type validation because the extract
            // unit is not YearMonth interval type.

            tester.checkScalar("extract(epoch from interval '4-2' year to month)",
                               // number of seconds elapsed since timestamp
                               // '1970-01-01 00:00:00' + input interval
                               "131328000", "BIGINT NOT NULL");

            tester.checkScalar("extract(second from interval '4-2' year to month)", "0", "BIGINT NOT NULL");

            tester.checkScalar("extract(minute from interval '4-2' year to month)", "0", "BIGINT NOT NULL");

            tester.checkScalar("extract(hour from interval '4-2' year to month)", "0", "BIGINT NOT NULL");

            tester.checkScalar("extract(day from interval '4-2' year to month)", "0", "BIGINT NOT NULL");
        }
        ...
        tester.checkFails("^extract(doy from interval '4-2' year to month)^", INVALID_EXTRACT_UNIT_VALIDATION_ERROR,
                          false);
        tester.checkFails("^extract(dow from interval '4-2' year to month)^", INVALID_EXTRACT_UNIT_VALIDATION_ERROR,
                          false);
        tester.checkFails("^extract(week from interval '4-2' year to month)^", INVALID_EXTRACT_UNIT_VALIDATION_ERROR,
                          false);

        tester.checkScalar("extract(month from interval '4-2' year to month)", "2", "BIGINT NOT NULL");

        tester.checkScalar("extract(quarter from interval '4-2' year to month)", "1", "BIGINT NOT NULL");

        tester.checkScalar("extract(year from interval '4-2' year to month)", "4", "BIGINT NOT NULL");

        tester.checkScalar("extract(decade from interval '426-3' year(3) to month)", "42", "BIGINT NOT NULL");

        tester.checkScalar("extract(century from interval '426-3' year(3) to month)", "4", "BIGINT NOT NULL");

        tester.checkScalar("extract(millennium from interval '2005-3' year(4) to month)", "2", "BIGINT NOT NULL");
    }
    ...
    @Test public void testExtractIntervalDayTime() {
        ...
        if (TODO) {
            ...
            tester.checkScalar("extract(epoch from interval '2 3:4:5.678' day to second)",
                               // number of seconds elapsed since timestamp
                               // '1970-01-01 00:00:00' + input interval
                               "183845.678", "BIGINT NOT NULL");
        }

        tester.checkScalar("extract(second from interval '2 3:4:5.678' day to second)", "5", "BIGINT NOT NULL");

        tester.checkScalar("extract(minute from interval '2 3:4:5.678' day to second)", "4", "BIGINT NOT NULL");

        tester.checkScalar("extract(hour from interval '2 3:4:5.678' day to second)", "3", "BIGINT NOT NULL");

        tester.checkScalar("extract(day from interval '2 3:4:5.678' day to second)", "2", "BIGINT NOT NULL");
        ...
        tester.checkFails("^extract(month from interval '2 3:4:5.678' day to second)^",
                          "(?s)Cannot apply 'EXTRACT' to arguments of type 'EXTRACT\\(<INTERVAL "
                          + "MONTH> FROM <INTERVAL DAY TO SECOND>\\)'\\. Supported " + "form\\(s\\):.*", false);

        tester.checkFails("^extract(quarter from interval '2 3:4:5.678' day to second)^",
                          "(?s)Cannot apply 'EXTRACT' to arguments of type 'EXTRACT\\(<INTERVAL "
                          + "QUARTER> FROM <INTERVAL DAY TO SECOND>\\)'\\. Supported " + "form\\(s\\):.*", false);

        tester.checkFails("^extract(year from interval '2 3:4:5.678' day to second)^",
                          "(?s)Cannot apply 'EXTRACT' to arguments of type 'EXTRACT\\(<INTERVAL "
                          + "YEAR> FROM <INTERVAL DAY TO SECOND>\\)'\\. Supported " + "form\\(s\\):.*", false);

        tester.checkFails("^extract(century from interval '2 3:4:5.678' day to second)^",
                          "(?s)Cannot apply 'EXTRACT' to arguments of type 'EXTRACT\\(<INTERVAL "
                          + "CENTURY> FROM <INTERVAL DAY TO SECOND>\\)'\\. Supported " + "form\\(s\\):.*", false);
    }

    @Test public void testExtractDate() {
        tester.setFor(SqlStdOperatorTable.EXTRACT, VM_FENNEL, VM_JAVA);

        tester.checkScalar("extract(epoch from date '2008-2-23')", "1203724800",
                           // number of seconds elapsed since timestamp
        ...
        if (TODO) {
            ...
            // instead of 0
            tester.checkScalar("extract(second from date '2008-2-23')", "0", "BIGINT NOT NULL");
        }

        tester.checkScalar("extract(minute from date '2008-2-23')", "0", "BIGINT NOT NULL");

        tester.checkScalar("extract(hour from date '2008-2-23')", "0", "BIGINT NOT NULL");

        tester.checkScalar("extract(day from date '2008-2-23')", "23", "BIGINT NOT NULL");

        tester.checkScalar("extract(month from date '2008-2-23')", "2", "BIGINT NOT NULL");

        tester.checkScalar("extract(quarter from date '2008-4-23')", "2", "BIGINT NOT NULL");

        tester.checkScalar("extract(year from date '2008-2-23')", "2008", "BIGINT NOT NULL");

        tester.checkScalar("extract(doy from date '2008-2-23')", "54", "BIGINT NOT NULL");

        tester.checkScalar("extract(dow from date '2008-2-23')", "7", "BIGINT NOT NULL");

        tester.checkScalar("extract(dow from date '2008-2-24')", "1", "BIGINT NOT NULL");

        tester.checkScalar("extract(week from date '2008-2-23')", "8", "BIGINT NOT NULL");

        tester.checkScalar("extract(week from timestamp '2008-2-23 01:23:45')", "8", "BIGINT NOT NULL");

        tester.checkScalar("extract(week from cast(null as date))", null, "BIGINT");

        tester.checkScalar("extract(decade from date '2008-2-23')", "200", "BIGINT NOT NULL");

        tester.checkScalar("extract(century from date '2008-2-23')", "21", "BIGINT NOT NULL");
        tester.checkScalar("extract(century from date '2001-01-01')", "21", "BIGINT NOT NULL");
        tester.checkScalar("extract(century from date '2000-12-31')", "20", "BIGINT NOT NULL");
        tester.checkScalar("extract(century from date '1852-06-07')", "19", "BIGINT NOT NULL");
        tester.checkScalar("extract(century from date '0001-02-01')", "1", "BIGINT NOT NULL");

        tester.checkScalar("extract(millennium from date '2000-2-23')", "2", "BIGINT NOT NULL");
        tester.checkScalar("extract(millennium from date '1969-2-23')", "2", "BIGINT NOT NULL");
        tester.checkScalar("extract(millennium from date '2000-12-31')", "2", "BIGINT NOT NULL");
        tester.checkScalar("extract(millennium from date '2001-01-01')", "3", "BIGINT NOT NULL");
    }

    @Test public void testExtractTimestamp() {
        tester.setFor(SqlStdOperatorTable.EXTRACT, VM_FENNEL, VM_JAVA);

        tester.checkScalar("extract(epoch from timestamp '2008-2-23 12:34:56')", "1203770096",
                           // number of seconds elapsed since timestamp
                           // '1970-01-01 00:00:00' for given date
                           "BIGINT NOT NULL");

        tester.checkScalar("extract(second from timestamp '2008-2-23 12:34:56')", "56", "BIGINT NOT NULL");

        tester.checkScalar("extract(minute from timestamp '2008-2-23 12:34:56')", "34", "BIGINT NOT NULL");

        tester.checkScalar("extract(hour from timestamp '2008-2-23 12:34:56')", "12", "BIGINT NOT NULL");

        tester.checkScalar("extract(day from timestamp '2008-2-23 12:34:56')", "23", "BIGINT NOT NULL");

        tester.checkScalar("extract(month from timestamp '2008-2-23 12:34:56')", "2", "BIGINT NOT NULL");

        tester.checkScalar("extract(quarter from timestamp '2008-7-23 12:34:56')", "3", "BIGINT NOT NULL");

        tester.checkScalar("extract(year from timestamp '2008-2-23 12:34:56')", "2008", "BIGINT NOT NULL");

        // TODO: Not implemented in operator test execution code
        tester.checkFails("extract(doy from timestamp '2008-2-23 12:34:56')", "cannot translate call EXTRACT.*", true);

        // TODO: Not implemented in operator test execution code
        tester.checkFails("extract(dow from timestamp '2008-2-23 12:34:56')", "cannot translate call EXTRACT.*", true);

        // TODO: Not implemented in operator test execution code
        tester.checkFails("extract(week from timestamp '2008-2-23 12:34:56')", "cannot translate call EXTRACT.*", true);

        tester.checkScalar("extract(decade from timestamp '2008-2-23 12:34:56')", "200", "BIGINT NOT NULL");

        tester.checkScalar("extract(century from timestamp '2008-2-23 12:34:56')", "21", "BIGINT NOT NULL");

        tester.checkScalar("extract(century from timestamp '2001-01-01 12:34:56')", "21", "BIGINT NOT NULL");

        tester.checkScalar("extract(century from timestamp '2000-12-31 12:34:56')", "20", "BIGINT NOT NULL");

        tester.checkScalar("extract(millennium from timestamp '2008-2-23 12:34:56')", "3", "BIGINT NOT NULL");

        tester.checkScalar("extract(millennium from timestamp '2000-2-23 12:34:56')", "2", "BIGINT NOT NULL");
    }
    ...
    @Test public void testExtractFunc() {
        ...
        tester.checkScalar("extract(day from interval '2 3:4:5.678' day to second)", "2", "BIGINT NOT NULL");
        tester.checkScalar("extract(day from interval '23456 3:4:5.678' day(5) to second)", "23456", "BIGINT NOT NULL");
        tester.checkScalar("extract(hour from interval '2 3:4:5.678' day to second)", "3", "BIGINT NOT NULL");
        tester.checkScalar("extract(minute from interval '2 3:4:5.678' day to second)", "4", "BIGINT NOT NULL");

        // TODO: Seconds should include precision
        tester.checkScalar("extract(second from interval '2 3:4:5.678' day to second)", "5", "BIGINT NOT NULL");
        tester.checkNull("extract(month from cast(null as interval year))");
    }
    ...
    @Test public void testExtractFuncFromDateTime() {
        ...
        tester.checkScalar("extract(minute from time '12:23:34')", "23", "BIGINT NOT NULL");

        tester.checkNull("extract(month from cast(null as timestamp))");

        tester.checkNull("extract(month from cast(null as date))");

        tester.checkNull("extract(second from cast(null as time))");
    }

    @Test public void testArrayValueConstructor() {
        tester.setFor(SqlStdOperatorTable.ARRAY_VALUE_CONSTRUCTOR);
        tester.checkScalar("Array['foo', 'bar']", "[foo, bar]", "CHAR(3) NOT NULL ARRAY NOT NULL");
    ...
    @Test public void testItemOp() {
        ...
        tester.checkNull("ARRAY ['foo', 'bar'][1 + CAST(NULL AS INTEGER)]");
        tester.checkFails("^ARRAY ['foo', 'bar']['baz']^",
        ...
        // Map item
        tester.checkScalarExact("map['foo', 3, 'bar', 7]['bar']", "INTEGER", "7");
        tester.checkScalarExact("map['foo', CAST(NULL AS INTEGER), 'bar', 7]['bar']", "INTEGER", "7");
        tester.checkScalarExact("map['foo', CAST(NULL AS INTEGER), 'bar', 7]['baz']", "INTEGER", null);
        tester.checkColumnType("select cast(null as any)['x'] from (values(1))", "ANY");
    }

    @Test public void testMapValueConstructor() {
        tester.setFor(SqlStdOperatorTable.MAP_VALUE_CONSTRUCTOR, VM_JAVA);

        tester.checkFails("^Map[]^", "Map requires at least 2 arguments", false);

        tester.checkFails("^Map[1, 'x', 2]^", "Map requires an even number of arguments", false);

        tester.checkFails("^map[1, 1, 2, 'x']^", "Parameters must be of the same type", false);
        tester.checkScalarExact("map['washington', 1, 'obama', 44]",
                                "(CHAR(10) NOT NULL, INTEGER NOT NULL) MAP NOT NULL", "{washington=1, obama=44}");
    }
    ...
    @Test public void testCeilFunc() {
        ...
        tester.checkScalarApprox("ceil(cast(-11.2e0 as real))", "REAL NOT NULL", -11, 0);
        tester.checkScalarExact("ceil(100)", "INTEGER NOT NULL", "100");
        tester.checkScalarExact("ceil(1.3)", "DECIMAL(2, 0) NOT NULL", "2");
        tester.checkScalarExact("ceil(-1.7)", "DECIMAL(2, 0) NOT NULL", "-1");
    ...
    @Test public void testCeilFuncInterval() {
        ...
        tester.checkScalar("ceil(interval '3:4:5' hour to second)", "+4:00:00.000000",
                           "INTERVAL HOUR TO SECOND NOT NULL");
        tester.checkScalar("ceil(interval '-6.3' second)", "-6.000000", "INTERVAL SECOND NOT NULL");
        tester.checkScalar("ceil(interval '5-1' year to month)", "+6-00", "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkScalar("ceil(interval '-5-1' year to month)", "-5-00", "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkNull("ceil(cast(null as interval year))");
    }
    ...
    @Test public void testFloorFunc() {
        ...
        tester.checkScalarApprox("floor(cast(-1.2e0 as real))", "REAL NOT NULL", -2, 0);
        tester.checkScalarExact("floor(100)", "INTEGER NOT NULL", "100");
        tester.checkScalarExact("floor(1.7)", "DECIMAL(2, 0) NOT NULL", "1");
        tester.checkScalarExact("floor(-1.7)", "DECIMAL(2, 0) NOT NULL", "-2");
    ...
    @Test public void testFloorFuncDateTime() {
        ...
        tester.checkFails("^floor(time '12:34:56')^", "(?s)Cannot apply 'FLOOR' to arguments .*", false);
        tester.checkFails("^floor(123.45 to minute)^", "(?s)Cannot apply 'FLOOR' to arguments .*", false);
        tester.checkFails("^floor('abcde' to minute)^", "(?s)Cannot apply 'FLOOR' to arguments .*", false);
        tester.checkScalar("floor(time '12:34:56' to minute)", "12:34:00", "TIME(0) NOT NULL");
        tester.checkScalar("floor(timestamp '2015-02-19 12:34:56.78' to second)", "2015-02-19 12:34:56",
                           "TIMESTAMP(2) NOT NULL");
        tester.checkScalar("floor(timestamp '2015-02-19 12:34:56' to minute)", "2015-02-19 12:34:00",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("floor(timestamp '2015-02-19 12:34:56' to year)", "2015-01-01 00:00:00",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("floor(timestamp '2015-02-19 12:34:56' to month)", "2015-02-01 00:00:00",
                           "TIMESTAMP(0) NOT NULL");
    ...
    @Test public void testCeilFuncDateTime() {
        ...
        tester.checkFails("^ceil(time '12:34:56')^", "(?s)Cannot apply 'CEIL' to arguments .*", false);
        tester.checkFails("^ceil(123.45 to minute)^", "(?s)Cannot apply 'CEIL' to arguments .*", false);
        tester.checkFails("^ceil('abcde' to minute)^", "(?s)Cannot apply 'CEIL' to arguments .*", false);
        tester.checkScalar("ceil(time '12:34:56' to minute)", "12:35:00", "TIME(0) NOT NULL");
        tester.checkScalar("ceil(time '12:59:56' to minute)", "13:00:00", "TIME(0) NOT NULL");
        tester.checkScalar("ceil(timestamp '2015-02-19 12:34:56.78' to second)", "2015-02-19 12:34:57",
                           "TIMESTAMP(2) NOT NULL");
        tester.checkScalar("ceil(timestamp '2015-02-19 12:34:56.00' to second)", "2015-02-19 12:34:56",
                           "TIMESTAMP(2) NOT NULL");
        tester.checkScalar("ceil(timestamp '2015-02-19 12:34:56' to minute)", "2015-02-19 12:35:00",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("ceil(timestamp '2015-02-19 12:34:56' to year)", "2016-01-01 00:00:00",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("ceil(timestamp '2015-02-19 12:34:56' to month)", "2015-03-01 00:00:00",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkNull("ceil(cast(null as timestamp) to month)");

        // ceiling alias
        tester.checkScalar("ceiling(timestamp '2015-02-19 12:34:56' to month)", "2015-03-01 00:00:00",
                           "TIMESTAMP(0) NOT NULL");
    ...
    @Test public void testFloorFuncInterval() {
        ...
        tester.checkScalar("floor(interval '3:4:5' hour to second)", "+3:00:00.000000",
                           "INTERVAL HOUR TO SECOND NOT NULL");
        tester.checkScalar("floor(interval '-6.3' second)", "-7.000000", "INTERVAL SECOND NOT NULL");
        tester.checkScalar("floor(interval '5-1' year to month)", "+5-00", "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkScalar("floor(interval '-5-1' year to month)", "-6-00", "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkScalar("floor(interval '-6.3' second to second)", "-7.000000", "INTERVAL SECOND NOT NULL");
        tester.checkScalar("floor(interval '6-3' minute to second to minute)", "-7-0",
                           "INTERVAL MINUTE TO SECOND NOT NULL");
        tester.checkScalar("floor(interval '6-3' hour to minute to hour)", "7-0", "INTERVAL HOUR TO MINUTE NOT NULL");
        tester.checkScalar("floor(interval '6 3' day to hour to day)", "7 00", "INTERVAL DAY TO HOUR NOT NULL");
        tester.checkScalar("floor(interval '102-7' year to month to month)", "102-07",
                           "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkScalar("floor(interval '102-7' year to month to quarter)", "102-10",
                           "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkScalar("floor(interval '102-1' year to month to century)", "201",
                           "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkScalar("floor(interval '1004-1' year to month to millennium)", "2001-00",
                           "INTERVAL YEAR TO MONTH NOT NULL");
        tester.checkNull("floor(cast(null as interval year))");
    }

    @Test public void testTimestampAdd() {
        tester.setFor(SqlStdOperatorTable.TIMESTAMP_ADD);
        tester.checkScalar("timestampadd(SQL_TSI_SECOND, 2, timestamp '2016-02-24 12:42:25')", "2016-02-24 12:42:27",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("timestampadd(MINUTE, 2, timestamp '2016-02-24 12:42:25')", "2016-02-24 12:44:25",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("timestampadd(HOUR, -2000, timestamp '2016-02-24 12:42:25')", "2015-12-03 04:42:25",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkNull("timestampadd(HOUR, CAST(NULL AS INTEGER)," + " timestamp '2016-02-24 12:42:25')");
        tester.checkNull("timestampadd(HOUR, -200, CAST(NULL AS TIMESTAMP))");
        tester.checkScalar("timestampadd(MONTH, 3, timestamp '2016-02-24 12:42:25')", "2016-05-24 12:42:25",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("timestampadd(MONTH, 3, cast(null as timestamp))", null, "TIMESTAMP(0)");

        // TIMESTAMPADD with DATE; returns a TIMESTAMP value for sub-day intervals.
        tester.checkScalar("timestampadd(MONTH, 1, date '2016-06-15')", "2016-07-15", "DATE NOT NULL");
        tester.checkScalar("timestampadd(DAY, 1, date '2016-06-15')", "2016-06-16", "DATE NOT NULL");
        tester.checkScalar("timestampadd(HOUR, -1, date '2016-06-15')", "2016-06-14 23:00:00", "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("timestampadd(MINUTE, 1, date '2016-06-15')", "2016-06-15 00:01:00",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("timestampadd(SQL_TSI_SECOND, -1, date '2016-06-15')", "2016-06-14 23:59:59",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("timestampadd(SECOND, 1, date '2016-06-15')", "2016-06-15 00:00:01",
                           "TIMESTAMP(0) NOT NULL");
        tester.checkScalar("timestampadd(SECOND, 1, cast(null as date))", null, "TIMESTAMP(0)");
        tester.checkScalar("timestampadd(DAY, 1, cast(null as date))", null, "DATE");

        // Round to the last day of previous month
        tester.checkScalar("timestampadd(MONTH, 1, date '2016-05-31')", "2016-06-30", "DATE NOT NULL");
        tester.checkScalar("timestampadd(MONTH, 5, date '2016-01-31')", "2016-06-30", "DATE NOT NULL");
        tester.checkScalar("timestampadd(MONTH, -1, date '2016-03-31')", "2016-02-29", "DATE NOT NULL");
    }

    @Test public void testTimestampDiff() {
        tester.setFor(SqlStdOperatorTable.TIMESTAMP_DIFF);
        tester.checkScalar(
                "timestampdiff(HOUR, " + "timestamp '2016-02-24 12:42:25', " + "timestamp '2016-02-24 15:42:25')", "3",
                "INTEGER NOT NULL");
        tester.checkScalar("timestampdiff(MICROSECOND, " + "timestamp '2016-02-24 12:42:25', "
                           + "timestamp '2016-02-24 12:42:20')", "-5000000", "INTEGER NOT NULL");
        tester.checkScalar("timestampdiff(SQL_TSI_FRAC_SECOND, " + "timestamp '2016-02-24 12:42:25', "
                           + "timestamp '2016-02-24 12:42:20')", "-5000000", "INTEGER NOT NULL");
        tester.checkScalar(
                "timestampdiff(YEAR, " + "timestamp '2014-02-24 12:42:25', " + "timestamp '2016-02-24 12:42:25')", "2",
                "INTEGER NOT NULL");
        tester.checkScalar(
        ...
        tester.checkScalar("timestampdiff(QUARTER, " + "timestamp '2014-02-24 12:42:25', " + "cast(null as timestamp))",
                           null, "INTEGER");
        tester.checkScalar("timestampdiff(QUARTER, " + "cast(null as timestamp), " + "timestamp '2014-02-24 12:42:25')",
                           null, "INTEGER");

        // timestampdiff with date
        tester.checkScalar("timestampdiff(MONTH, date '2016-03-15', date '2016-06-14')", "2", "INTEGER NOT NULL");
        tester.checkScalar("timestampdiff(DAY, date '2016-06-15', date '2016-06-14')", "-1", "INTEGER NOT NULL");
        tester.checkScalar("timestampdiff(HOUR, date '2016-06-15', date '2016-06-14')", "-24", "INTEGER NOT NULL");
        tester.checkScalar("timestampdiff(MINUTE, date '2016-06-15',  date '2016-06-15')", "0", "INTEGER NOT NULL");
        tester.checkScalar("timestampdiff(SECOND, cast(null as date), date '2016-06-15')", null, "INTEGER");
        tester.checkScalar("timestampdiff(DAY, date '2016-06-15', cast(null as date))", null, "INTEGER");
    }

    @Test public void testDenseRankFunc() {
        tester.setFor(SqlStdOperatorTable.DENSE_RANK, VM_FENNEL, VM_JAVA);
    }

    @Test public void testPercentRankFunc() {
        tester.setFor(SqlStdOperatorTable.PERCENT_RANK, VM_FENNEL, VM_JAVA);
    }
    ...
    @Test public void testCumeDistFunc() {
        tester.setFor(SqlStdOperatorTable.CUME_DIST, VM_FENNEL, VM_JAVA);
    }

    @Test public void testRowNumberFunc() {
        tester.setFor(SqlStdOperatorTable.ROW_NUMBER, VM_FENNEL, VM_JAVA);
    }
    ...
    @Test public void testCountFunc() {
        ...
        tester.checkFails("^COUNT()^", "Invalid number of arguments to function 'COUNT'. Was expecting 1 arguments",
                          false);
        tester.checkType("count(1, 2)", "BIGINT NOT NULL");
    ...
    @Test public void testSumFunc() {
        ...
        tester.checkFails("sum(^*^)", "Unknown identifier '\\*'", false);
        tester.checkFails("^sum('name')^",
        ...
        tester.checkFails("^sum()^", "Invalid number of arguments to function 'SUM'. Was expecting 1 arguments", false);
        tester.checkFails("^sum(1, 2)^", "Invalid number of arguments to function 'SUM'. Was expecting 1 arguments",
                          false);
        tester.checkFails("^sum(cast(null as varchar(2)))^",
    ...
    @Test public void testAvgFunc() {
        ...
        tester.checkFails("avg(^*^)", "Unknown identifier '\\*'", false);
        tester.checkFails("^avg(cast(null as varchar(2)))^",
        ...
        Object result = -1;
        tester.checkAgg("avg(DISTINCT CASE x WHEN 0 THEN NULL ELSE -1 END)", values, result, 0d);
    }
    ...
    @Test public void testCovarPopFunc() {
        ...
        tester.checkFails("^covar_pop(cast(null as varchar(2)),cast(null as varchar(2)))^",
    ...
    @Test public void testCovarSampFunc() {
        ...
        tester.checkFails("covar_samp(^*^)", "Unknown identifier '\\*'", false);
        tester.checkFails("^covar_samp(cast(null as varchar(2)),cast(null as varchar(2)))^",
    ...
    @Test public void testRegrSxxFunc() {
        ...
        tester.checkFails("regr_sxx(^*^)", "Unknown identifier '\\*'", false);
        tester.checkFails("^regr_sxx(cast(null as varchar(2)), cast(null as varchar(2)))^",
                          "(?s)Cannot apply 'REGR_SXX' to arguments of type 'REGR_SXX\\(<VARCHAR\\(2\\)>, <VARCHAR\\(2\\)>\\)'\\. Supported form\\(s\\): 'REGR_SXX\\(<NUMERIC>, <NUMERIC>\\)'.*",
                          false);
        tester.checkType("regr_sxx(CAST(NULL AS INTEGER), CAST(NULL AS INTEGER))", "INTEGER");
    ...
    @Test public void testRegrSyyFunc() {
        ...
        tester.checkFails("regr_syy(^*^)", "Unknown identifier '\\*'", false);
        tester.checkFails("^regr_syy(cast(null as varchar(2)), cast(null as varchar(2)))^",
                          "(?s)Cannot apply 'REGR_SYY' to arguments of type 'REGR_SYY\\(<VARCHAR\\(2\\)>, <VARCHAR\\(2\\)>\\)'\\. Supported form\\(s\\): 'REGR_SYY\\(<NUMERIC>, <NUMERIC>\\)'.*",
                          false);
        tester.checkType("regr_syy(CAST(NULL AS INTEGER), CAST(NULL AS INTEGER))", "INTEGER");
    ...
    @Test public void testStddevPopFunc() {
        ...
        if (enable) {
            ...
            tester.checkAgg("stddev_pop(x)", values, 1.414213562373095d, 0.000000000000001d);
            // Oracle does not allow distinct
            tester.checkAgg("stddev_pop(DISTINCT x)", values, 1.5d, 0d);
            tester.checkAgg("stddev_pop(DISTINCT CASE x WHEN 0 THEN NULL ELSE -1 END)", values, 0, 0d);
        }
        // with one value
    ...
    @Test public void testStddevSampFunc() {
        ...
        tester.checkFails("stddev_samp(^*^)", "Unknown identifier '\\*'", false);
        tester.checkFails("^stddev_samp(cast(null as varchar(2)))^",
        ...
        if (enable) {
            // verified on Oracle 10g
            tester.checkAgg("stddev_samp(x)", values, 1.732050807568877d, 0.000000000000001d);
            // Oracle does not allow distinct
            tester.checkAgg("stddev_samp(DISTINCT x)", values, 2.121320343559642d, 0.000000000000001d);
            tester.checkAgg("stddev_samp(DISTINCT CASE x WHEN 0 THEN NULL ELSE -1 END)", values, null, 0d);
        }
        // with one value
        tester.checkAgg("stddev_samp(x)", new String[] { "5" }, null, 0d);
        // with zero values
        tester.checkAgg("stddev_samp(x)", new String[] {}, null, 0d);
    }

    @Test public void testStddevFunc() {
        tester.setFor(SqlStdOperatorTable.STDDEV, VM_EXPAND);
        tester.checkFails("stddev(^*^)", "Unknown identifier '\\*'", false);
        tester.checkFails("^stddev(cast(null as varchar(2)))^",
        ...
        // with zero values
        tester.checkAgg("stddev(x)", new String[] {}, null, 0d);
    }

    @Test public void testVarPopFunc() {
        tester.setFor(SqlStdOperatorTable.VAR_POP, VM_EXPAND);
        tester.checkFails("var_pop(^*^)", "Unknown identifier '\\*'", false);
        tester.checkFails("^var_pop(cast(null as varchar(2)))^",
        ...
        tester.checkAgg("var_pop(x)", values, 2d, // verified on Oracle 10g
                        0d);
        tester.checkAgg("var_pop(DISTINCT x)", // Oracle does not allow distinct
                        values, 2.25d, 0.0001d);
        tester.checkAgg("var_pop(DISTINCT CASE x WHEN 0 THEN NULL ELSE -1 END)", values, 0, 0d);
        // with one value
        tester.checkAgg("var_pop(x)", new String[] { "5" }, 0, 0d);
        // with zero values
        tester.checkAgg("var_pop(x)", new String[] {}, null, 0d);
    }

    @Test public void testVarSampFunc() {
        tester.setFor(SqlStdOperatorTable.VAR_SAMP, VM_EXPAND);
        tester.checkFails("var_samp(^*^)", "Unknown identifier '\\*'", false);
        tester.checkFails("^var_samp(cast(null as varchar(2)))^",
        ...
        tester.checkAgg("var_samp(x)", values, 3d, // verified on Oracle 10g
                        0d);
        tester.checkAgg("var_samp(DISTINCT x)", // Oracle does not allow distinct
                        values, 4.5d, 0.0001d);
        tester.checkAgg("var_samp(DISTINCT CASE x WHEN 0 THEN NULL ELSE -1 END)", values, null, 0d);
        // with one value
        tester.checkAgg("var_samp(x)", new String[] { "5" }, null, 0d);
        // with zero values
        tester.checkAgg("var_samp(x)", new String[] {}, null, 0d);
    }

    @Test public void testVarFunc() {
        tester.setFor(SqlStdOperatorTable.VARIANCE, VM_EXPAND);
        tester.checkFails("variance(^*^)", "Unknown identifier '\\*'", false);
        tester.checkFails("^variance(cast(null as varchar(2)))^",
        ...
        tester.checkAgg("variance(x)", values, 3d, // verified on Oracle 10g
                        0d);
        tester.checkAgg("variance(DISTINCT x)", // Oracle does not allow distinct
                        values, 4.5d, 0.0001d);
        tester.checkAgg("variance(DISTINCT CASE x WHEN 0 THEN NULL ELSE -1 END)", values, null, 0d);
        // with one value
        tester.checkAgg("variance(x)", new String[] { "5" }, null, 0d);
        // with zero values
        tester.checkAgg("variance(x)", new String[] {}, null, 0d);
    }

    @Test public void testMinFunc() {
        tester.setFor(SqlStdOperatorTable.MIN, VM_EXPAND);
        tester.checkFails("min(^*^)", "Unknown identifier '\\*'", false);
        tester.checkType("min(1)", "INTEGER");
        tester.checkType("min(1.2)", "DECIMAL(2, 1)");
        tester.checkType("min(DISTINCT 1.5)", "DECIMAL(2, 1)");
        tester.checkFails("^min()^", "Invalid number of arguments to function 'MIN'. Was expecting 1 arguments", false);
        tester.checkFails("^min(1, 2)^", "Invalid number of arguments to function 'MIN'. Was expecting 1 arguments",
                          false);
        final String[] values = { "0", "CAST(null AS INTEGER)", "2", "2" };
        if (!enable) {
            return;
        }
        tester.checkAgg("min(x)", values, "0", 0d);
        tester.checkAgg("min(CASE x WHEN 0 THEN NULL ELSE -1 END)", values, "-1", 0d);
        tester.checkAgg("min(DISTINCT CASE x WHEN 0 THEN NULL ELSE -1 END)", values, "-1", 0d);
        tester.checkAgg("min(DISTINCT x)", values, "0", 0d);
    }

    @Test public void testMaxFunc() {
        tester.setFor(SqlStdOperatorTable.MAX, VM_EXPAND);
        tester.checkFails("max(^*^)", "Unknown identifier '\\*'", false);
        tester.checkType("max(1)", "INTEGER");
        tester.checkType("max(1.2)", "DECIMAL(2, 1)");
        tester.checkType("max(DISTINCT 1.5)", "DECIMAL(2, 1)");
        tester.checkFails("^max()^", "Invalid number of arguments to function 'MAX'. Was expecting 1 arguments", false);
        tester.checkFails("^max(1, 2)^", "Invalid number of arguments to function 'MAX'. Was expecting 1 arguments",
                          false);
        final String[] values = { "0", "CAST(null AS INTEGER)", "2", "2" };
        if (!enable) {
            return;
        }
        tester.checkAgg("max(x)", values, "2", 0d);
        tester.checkAgg("max(CASE x WHEN 0 THEN NULL ELSE -1 END)", values, "-1", 0d);
        tester.checkAgg("max(DISTINCT CASE x WHEN 0 THEN NULL ELSE -1 END)", values, "-1", 0d);
        tester.checkAgg("max(DISTINCT x)", values, "2", 0d);
    }
    ...
    @Test public void testLastValueFunc() {
        ...
        tester.checkWinAgg("last_value(x)", values, "ROWS 3 PRECEDING", "INTEGER", Arrays.asList("3", "0"), 0d);
        final String[] values2 = { "1.6", "1.2" };
        tester.checkWinAgg("last_value(x)", values2, "ROWS 3 PRECEDING", "DECIMAL(2, 1) NOT NULL",
                           Arrays.asList("1.6", "1.2"), 0d);
        final String[] values3 = { "'foo'", "'bar'", "'name'" };
        tester.checkWinAgg("last_value(x)", values3, "ROWS 3 PRECEDING", "CHAR(4) NOT NULL",
                           Arrays.asList("foo ", "bar ", "name"), 0d);
    }
    ...
    @Test public void testFirstValueFunc() {
        ...
        final String[] values2 = { "1.6", "1.2" };
        tester.checkWinAgg("first_value(x)", values2, "ROWS 3 PRECEDING", "DECIMAL(2, 1) NOT NULL",
                           Arrays.asList("1.6"), 0d);
        final String[] values3 = { "'foo'", "'bar'", "'name'" };
        tester.checkWinAgg("first_value(x)", values3, "ROWS 3 PRECEDING", "CHAR(4) NOT NULL", Arrays.asList("foo "),
                           0d);
    }
        ...
        for (RelDataType type : types) {
            for (Object o : getValues((BasicSqlType) type, false)) {
                ...
                if ((type.getSqlTypeName() == SqlTypeName.BIGINT) || ((type.getSqlTypeName() == SqlTypeName.DECIMAL)
                    ...
                    tester.checkFails("CAST(" + literalString + " AS " + type + ")",
                                      "(?s).*(Overflow during calculation or cast\\.|Code=22003).*", true);
                }
            }
        }
    }
    ...
    @Test public void testCastTruncates() {
        ...
        tester.checkScalar("CAST('ABCD' AS VARCHAR(2))", "AB", "VARCHAR(2) NOT NULL");
        tester.checkScalar("CAST('ABCD' AS VARCHAR)", "ABCD", "VARCHAR NOT NULL");
        tester.checkScalar("CAST(CAST('ABCD' AS VARCHAR) AS VARCHAR(3))", "ABC", "VARCHAR(3) NOT NULL");

        tester.checkScalar("CAST(x'ABCDEF12' AS BINARY(2))", "abcd", "BINARY(2) NOT NULL");
        tester.checkScalar("CAST(x'ABCDEF12' AS VARBINARY(2))", "abcd", "VARBINARY(2) NOT NULL");
        tester.checkScalar("CAST(x'ABCDEF12' AS VARBINARY)", "abcdef12", "VARBINARY NOT NULL");
        tester.checkScalar("CAST(CAST(x'ABCDEF12' AS VARBINARY) AS VARBINARY(3))", "abcdef", "VARBINARY(3) NOT NULL");

        if (!enable) {
            return;
        }
        tester.checkBoolean("CAST(X'' AS BINARY(3)) = X'000000'", true);
        ...
        for (SqlOperator op : SqlStdOperatorTable.instance().getOperatorList()) {
            ...
            for (int n = range.getMin(), max = range.getMax(); n <= max; n++) {
                ...
                for (final List<ValueType> args : Linq4j.product(argValues)) {
                    ...
                    final SqlPrettyWriter writer = new SqlPrettyWriter(SqlDialect.CALCITE);
                    ...
                    try {
                        if (nullCount > 0 && policy == Strong.Policy.ANY) {
                            ...
                            tester.check(query, SqlTests.ANY_TYPE_CHECKER, SqlTests.ANY_PARAMETER_CHECKER,
    ...
    protected static class TesterImpl extends SqlTesterImpl {
        ...
        @Override public void check(String query, TypeChecker typeChecker, ParameterChecker parameterChecker,
                                    ResultChecker resultChecker) {
            super.check(query, typeChecker, parameterChecker, resultChecker);
            //noinspection unchecked
            final CalciteAssert.ConnectionFactory connectionFactory = (CalciteAssert.ConnectionFactory) getFactory().get(
                    "connectionFactory");
            try (Connection connection = connectionFactory.createConnection(); Statement statement = connection.createStatement()) {
        ...
        @Override protected TesterImpl with(final String name2, final Object value) {
            return new TesterImpl(new DelegatingSqlTestFactory(factory) {
    ...
    static class ValueType {
        ...
        private SqlNode literal(RelDataType type, Object value) {
            if (value == null) {
                ...
                return SqlStdOperatorTable.CAST.createCall(SqlParserPos.ZERO, SqlLiteral.createNull(SqlParserPos.ZERO),
                                                           new SqlDataTypeSpec(
                                                                   new SqlIdentifier(type.getSqlTypeName().getName(),
                                                                                     SqlParserPos.ZERO), precision,
                                                                   scale, null, null, SqlParserPos.ZERO));
            }
    ...
    class OverlapChecker {
        ...
        private String sub(String s) {
            return s.replace("$0", values[0]).replace("$1", values[1]).replace("$2", values[2]).replace("$3",
                                                                                                        values[3]);
        }
    }
}
