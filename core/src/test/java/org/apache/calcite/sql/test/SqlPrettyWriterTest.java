...
public class SqlPrettyWriterTest {
    ...
    protected void assertPrintsTo(boolean newlines, final String sql, String expected) {
        ...
        final SqlPrettyWriter prettyWriter = new SqlPrettyWriter(SqlDialect.DUMMY);
    ...
    protected void checkSimple(SqlPrettyWriter prettyWriter, String expectedDesc, String expected) throws Exception {
        final SqlNode node = parseQuery(
                "select x as a, b as b, c as c, d," + " 'mixed-Case string'," + " unquotedCamelCaseId,"
                + " \"quoted id\" " + "from" + " (select *" + " from t" + " where x = y and a > 5" + " group by z, zz"
                + " window w as (partition by c)," + "  w1 as (partition by c,d order by a, b"
    ...
    @Ignore("default SQL parser cannot parse DDL") @Test public void testExplain() {
    ...
    @Test public void testCase() {
        ...
        assertExprPrintsTo(true, "case 1 when 2 + 3 then 4 when case a when b then c else d end then 6 else 7 end",
                           "CASE" + NL + "WHEN 1 = 2 + 3" + NL + "THEN 4" + NL + "WHEN 1 = CASE" + NL
                           + "        WHEN `A` = `B`" + NL // todo: indent should be 4 not 8
                           + "        THEN `C`" + NL + "        ELSE `D`" + NL + "        END" + NL + "THEN 6" + NL
                           + "ELSE 7" + NL + "END");
    }

    @Test public void testCase2() {
        assertExprPrintsTo(false, "case 1 when 2 + 3 then 4 when case a when b then c else d end then 6 else 7 end",
                           "CASE WHEN 1 = 2 + 3 THEN 4 WHEN 1 = CASE WHEN `A` = `B` THEN `C` ELSE `D` END THEN 6 ELSE 7 END");
    }

    @Test public void testBetween() {
        assertExprPrintsTo(true, "x not between symmetric y and z",
    ...
    @Test public void testOverlaps() {
        assertExprPrintsTo(true, "(x,xx) overlaps (y,yy) or x is not null",
                           "PERIOD (`X`, `XX`) OVERLAPS PERIOD (`Y`, `YY`) OR `X` IS NOT NULL");
    }

    @Test public void testUnion() {
        assertPrintsTo(true,
                       "select * from t " + "union select * from (" + "  select * from u " + "  union select * from v) "
                       + "union select * from w " + "order by a, b",
    ...
    @Test public void testWhereListItemsOnSeparateLinesOr() throws Exception {
        checkPrettySeparateLines(
                "select x" + " from y" + " where h is not null and i < j" + " or ((a or b) is true) and d not in (f,g)"
                + " or x <> z");
    }

    @Test public void testWhereListItemsOnSeparateLinesAnd() throws Exception {
        checkPrettySeparateLines("select x" + " from y" + " where h is not null and (i < j"
                                 + " or ((a or b) is true)) and (d not in (f,g)" + " or v <> ((w * x) + y) * z)");
    }
