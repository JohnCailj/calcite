...
public class SqlTesterImpl implements SqlTester, AutoCloseable {
        ...
        try {
            ...
            if (expectedMsgPattern == null) {
                ...
                throw new RuntimeException(
                        "Error did not match expected [" + expectedMsgPattern + "] while parsing query [" + sap.sql
                        + "]", e);
            }
            return;
        } catch (Throwable e) {
    ...
    public void checkIntervalConv(String sql, String expected) {
        ...
        long l = interval.getIntervalQualifier().isYearMonth() ? SqlParserUtil.intervalToMonths(
                interval) : SqlParserUtil.intervalToMillis(interval);
        String actual = l + "";
        assertEquals(expected, actual);
    }
    ...
    public SqlTester withLex(Lex lex) {
        return withQuoting(lex.quoting).withCaseSensitive(lex.caseSensitive).withQuotedCasing(
                lex.quotedCasing).withUnquotedCasing(lex.unquotedCasing);
    }
    ...
    public SqlTesterImpl withConformance(SqlConformance conformance) {
        ...
        return with("conformance", conformance).withConnectionFactory(
                CalciteAssert.EMPTY_CONNECTION_FACTORY.with("conformance", conformance));
    }
    ...
    public SqlTesterImpl withConnectionFactory(CalciteAssert.ConnectionFactory connectionFactory) {
        return with("connectionFactory", connectionFactory);
    }

    protected SqlTesterImpl with(final String name2, final Object value) {
        return new SqlTesterImpl(new DelegatingSqlTestFactory(factory) {
    ...
    public void checkWinAgg(String expr, String[] inputValues, String windowSpec, String type, Object result,
                            double delta) {
    ...
    public void check(String query, TypeChecker typeChecker, ParameterChecker parameterChecker,
    ...
    private String buildQuery2(String expression) {
        ...
        x.accept(new SqlShuttle() {
            ...
            private final List<SqlOperator> ops = ImmutableList.of(SqlStdOperatorTable.LITERAL_CHAIN,
        ...
        return "select " + sql2.substring("values (".length(), sql2.length() - 1) + " from (values (" + Util.commaList(
                Pair.left(values)) + ")) as t(" + Util.commaList(Pair.right(values)) + ")";
    }
