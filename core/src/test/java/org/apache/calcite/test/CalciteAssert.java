...
public class CalciteAssert {
     ...
     * java command line.
     */
    public static final DatabaseInstance DB = DatabaseInstance.valueOf(
            Util.first(System.getProperty("calcite.test.db"), "HSQLDB").toUpperCase(Locale.ROOT));
    ...
    private static final AssertThat DISABLED = new AssertThat(EMPTY_CONNECTION_FACTORY) {
        ...
        @Override public AssertThat connectThrows(Function<Throwable, Void> exceptionChecker) {
            return this;
        }

        @Override public <T> AssertThat doWithConnection(Function<CalciteConnection, T> fn) throws Exception {
            return this;
        }
        ...
        @Override public AssertThat with(ConnectionPostProcessor postProcessor) {
            return this;
        }
    ...
    static void assertQuery(Connection connection, String sql, int limit, boolean materializationsEnabled,
                            ...
                            Function<Integer, Void> updateChecker, Function<Throwable, Void> exceptionChecker)
            throws Exception {
        ...
        try (final Closer closer = new Closer()) {
            if (connection instanceof CalciteConnection) {
                ...
                calciteConnection.getProperties().setProperty(
                        ...
                        Boolean.toString(materializationsEnabled));
                calciteConnection.getProperties().setProperty(CalciteConnectionProperty.TIME_ZONE.camelName(),
    ...
    public static SchemaPlus addSchema(SchemaPlus rootSchema, SchemaSpec schema) {
        ...
        switch (schema) {
            ...
            case GEO:
                ModelHandler.addFunctions(rootSchema, null, ImmutableList.<String>of(), GeoFunctions.class.getName(),
                                          "*", true);
                ...
                final ViewTableMacro viewMacro = ViewTable.viewMacro(rootSchema, sql, ImmutableList.of("GEO"),
            ...
            case POST:
                ...
                post.add("EMP", ViewTable.viewMacro(post, "select * from (values\n" + "    ('Jane', 10, 'F'),\n"
                                                          ...
                                                          + "  as t(ename, deptno, gender)", ImmutableList.<String>of(),
                                                    ImmutableList.of("POST", "EMP"), null));
    ...
    public static class AssertThat {
        ...
        public AssertThat with(Config config) {
            ...
            switch (config) {
                ...
                case GEO:
                    return with(SchemaSpec.GEO).with(CalciteConnectionProperty.CONFORMANCE.camelName(),
    ...
    private static class PoolingConnectionFactory extends ConnectionFactory {
        ...
        private static class Pool {
            ...
            private static final LoadingCache<ConnectionFactory, Connection> POOL = CacheBuilder.newBuilder().build(
    ...
    public static class AssertQuery {
        ...
        public AssertQuery planHasSql(String expected) {
            return planContains(
                    "getDataSource(), \"" + expected.replace("\\", "\\\\").replace("\"", "\\\"").replaceAll("\n",
                                                                                                            "\\\\n")
                    + "\"");
        }
        ...
        private void ensurePlan(Function<Integer, Void> checkUpdate) {
            ...
            try {
                assertQuery(createConnection(), sql, limit, materializationsEnabled, hooks, null, checkUpdate, null);
                assertNotNull(plan);
            } catch (Exception e) {
                throw new RuntimeException("exception while executing [" + sql + "]", e);
            }
        ...
        public AssertQuery withRel(final Function<RelBuilder, RelNode> relFn) {
            return withHook(Hook.STRING_TO_QUERY,
                            new Function<Pair<FrameworkConfig, Holder<CalcitePrepare.Query>>, Void>() {
    ...
    private static class NopAssertQuery extends AssertQuery {
        ...
        @Override public AssertQuery returns(String sql, Function<ResultSet, Void> checker) {
            return this;
        }
        ...
        @Override public AssertQuery convertMatches(Function<RelNode, Void> checker) {
            return this;
        }

        @Override public AssertQuery substitutionMatches(Function<RelNode, Void> checker) {
            return this;
        }
        ...
        @Override public AssertQuery queryContains(Function<List, Void> predicate1) {
            return this;
        }
    }
    ...
    public enum DatabaseInstance {
        HSQLDB(new ConnectionSpec(FoodmartHsqldb.URI, "FOODMART", "FOODMART", "org.hsqldb.jdbcDriver", "foodmart"),
               new ConnectionSpec(ScottHsqldb.URI, ScottHsqldb.USER, ScottHsqldb.PASSWORD, "org.hsqldb.jdbcDriver",
                                  "SCOTT")), H2(new ConnectionSpec(
                "jdbc:h2:" + getDataSetPath() + "/h2/target/foodmart;user=foodmart;password=foodmart", "foodmart",
                "foodmart", "org.h2.Driver", "foodmart"), null), MYSQL(
                new ConnectionSpec("jdbc:mysql://localhost/foodmart", "foodmart", "foodmart", "com.mysql.jdbc.Driver",
                                   "foodmart"), null), ORACLE(
                new ConnectionSpec("jdbc:oracle:thin:@localhost:1521:XE", "foodmart", "foodmart",
                                   "oracle.jdbc.OracleDriver", "FOODMART"), null), POSTGRESQL(new ConnectionSpec(
                "jdbc:postgresql://localhost/foodmart?user=foodmart&password=foodmart&searchpath=foodmart", "foodmart",
                "foodmart", "org.postgresql.Driver", "foodmart"), null);
    ...
    public enum SchemaSpec {
        REFLECTIVE_FOODMART, JDBC_FOODMART, CLONE_FOODMART, JDBC_FOODMART_WITH_LATTICE, GEO, HR, JDBC_SCOTT, SCOTT, BLANK, LINGUAL, POST, ORINOCO
    }
