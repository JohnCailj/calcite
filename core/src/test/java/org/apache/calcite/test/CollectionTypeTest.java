...
public class CollectionTypeTest {
    ...
    @Test public void testAccessNestedMap() throws Exception {
        ...
        final String sql =
                ...
                + "from \"s\".\"nested\" " + "where \"NESTEDMAPFIELD\"['a']['b'] = 2 AND \"ARRAYFIELD\"[2] = 200";
    ...
    @Test public void testAccessNonExistKeyFromMap() throws Exception {
        ...
        final String sql =
                "select \"ID\"," + " \"MAPFIELD\", \"NESTEDMAPFIELD\", \"ARRAYFIELD\" " + "from \"s\".\"nested\" "
                + "where \"MAPFIELD\"['a'] = 2";
        final ResultSet resultSet = statement.executeQuery(sql);
        final List<String> resultStrings = CalciteAssert.toList(resultSet);
    ...
    @Test public void testAccessNestedMapWithAnyType() throws Exception {
        ...
        final String sql =
                "select \"ID\", \"MAPFIELD\"['c'] AS \"MAPFIELD_C\"," + " \"NESTEDMAPFIELD\", \"ARRAYFIELD\" "
                + "from \"s\".\"nested\" " + "where CAST(\"NESTEDMAPFIELD\"['a']['b'] AS INTEGER) = 2"
    ...
    @Test public void testAccessNestedMapWithAnyTypeWithoutCast() throws Exception {
        ...
        final String sql =
                "select \"ID\", \"MAPFIELD\"['c'] AS \"MAPFIELD_C\"," + " \"NESTEDMAPFIELD\", \"ARRAYFIELD\" "
                + "from \"s\".\"nested\" " + "where \"NESTEDMAPFIELD\"['a']['b'] = 2 AND 200.0 = \"ARRAYFIELD\"[2]";
    ...
    @Test public void testArithmeticToAnyTypeWithoutCast() throws Exception {
        ...
        final String sql =
                "select \"ID\", \"MAPFIELD\"['c'] AS \"MAPFIELD_C\"," + " \"NESTEDMAPFIELD\", \"ARRAYFIELD\" "
                + "from \"s\".\"nested\" " + "where \"NESTEDMAPFIELD\"['a']['b'] + 1.0 = 3 "
                + "AND \"NESTEDMAPFIELD\"['a']['b'] * 2.0 = 4 " + "AND \"NESTEDMAPFIELD\"['a']['b'] > 1"
                + "AND \"NESTEDMAPFIELD\"['a']['b'] >= 2" + "AND 100.1 <> \"ARRAYFIELD\"[2] - 100.0"
                + "AND 100.0 = \"ARRAYFIELD\"[2] / 2" + "AND 99.9 < \"ARRAYFIELD\"[2] / 2"
                + "AND 100.0 <= \"ARRAYFIELD\"[2] / 2" + "AND '200' <> \"STRINGARRAYFIELD\"[1]"
                + "AND '200' = \"STRINGARRAYFIELD\"[2]" + "AND '100' < \"STRINGARRAYFIELD\"[2]";
    ...
    public static class NestedCollectionTable implements ScannableTable {
        ...
        public RelDataType getRowType(RelDataTypeFactory typeFactory) {
            ...
            RelDataType nullableVarcharType = typeFactory.createTypeWithNullability(
                    typeFactory.createSqlType(SqlTypeName.VARCHAR), true);
            RelDataType nullableIntegerType = typeFactory.createTypeWithNullability(
                    typeFactory.createSqlType(SqlTypeName.INTEGER), true);
            RelDataType nullableMapType = typeFactory.createTypeWithNullability(
                    typeFactory.createMapType(nullableVarcharType, nullableIntegerType), true);
            return typeFactory.builder().add("ID", SqlTypeName.INTEGER).add("MAPFIELD",
                                                                            typeFactory.createTypeWithNullability(
                                                                                    typeFactory.createMapType(
                                                                                            nullableVarcharType,
                                                                                            nullableIntegerType),
                                                                                    true)).add("NESTEDMAPFIELD",
                                                                                               typeFactory.createTypeWithNullability(
                                                                                                       typeFactory.createMapType(
                                                                                                               nullableVarcharType,
                                                                                                               nullableMapType),
                                                                                                       true)).add(
                    "ARRAYFIELD",
                    typeFactory.createTypeWithNullability(typeFactory.createArrayType(nullableIntegerType, -1L),
                                                          true)).add("STRINGARRAYFIELD",
                                                                     typeFactory.createTypeWithNullability(
                                                                             typeFactory.createArrayType(
                                                                                     nullableVarcharType, -1L),
                                                                             true)).build();
        }
    ...
    public static class NestedCollectionWithAnyTypeTable implements ScannableTable {
        ...
        public RelDataType getRowType(RelDataTypeFactory typeFactory) {
            return typeFactory.builder().add("ID", SqlTypeName.INTEGER).add("MAPFIELD", SqlTypeName.ANY).add(
                    "NESTEDMAPFIELD", SqlTypeName.ANY).add("ARRAYFIELD", SqlTypeName.ANY).add("STRINGARRAYFIELD",
                                                                                              SqlTypeName.ANY).build();
        }
