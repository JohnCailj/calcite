...
public class JdbcFrontJdbcBackLinqMiddleTest {
    ...
    @Test public void testTable() {
        that().with(CalciteAssert.Config.JDBC_FOODMART).query("select * from \"foodmart\".\"days\"").returns(
    ...
    @Test public void testWhere() {
        that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                "select * from \"foodmart\".\"days\" where \"day\" < 3").returns(
                "day=1; week_day=Sunday\n" + "day=2; week_day=Monday\n");
    }

    @Test public void testWhere2() {
        that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                "select * from \"foodmart\".\"days\"\n" + "where not (lower(\"week_day\") = 'wednesday')").returns(
                "day=1; week_day=Sunday\n" + "day=2; week_day=Monday\n" + "day=5; week_day=Thursday\n"
                + "day=3; week_day=Tuesday\n" + "day=6; week_day=Friday\n" + "day=7; week_day=Saturday\n");
    }

    @Test public void testCase() {
        that().with(CalciteAssert.Config.FOODMART_CLONE).query(
    ...
    @Test public void testGroup() {
        that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                ...
                + "group by s").returnsUnordered("S=T; C=2; MW=Thursday", "S=F; C=1; MW=Friday",
                                                 "S=W; C=1; MW=Wednesday", "S=S; C=2; MW=Saturday",
                                                 "S=M; C=1; MW=Monday");
    }
     ...
     * cartesian product).</p>
     */
    @Ignore("non-deterministic on JDK 1.7 vs 1.8") @Test public void testJoinTheta() {
        that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select count(*) from (\n" + "  select *\n" + "  from \"foodmart\".\"sales_fact_1997\" as s\n"
                + "  join \"foodmart\".\"customer\" as c\n"
                + "  on s.\"customer_id\" - c.\"customer_id\" = 0)").explainContains(
    ...
    @Test public void testJoinGroupByEmpty() {
        if (CalciteAssert.DB == CalciteAssert.DatabaseInstance.MYSQL && !Bug.CALCITE_673_FIXED) {
            return;
        }
        that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                "select count(*) from (\n" + "  select *\n" + "  from \"foodmart\".\"sales_fact_1997\" as s\n"
                + "  join \"foodmart\".\"customer\" as c\n" + "  on s.\"customer_id\" = c.\"customer_id\")").returns(
                "EXPR$0=86837\n");
    }
    ...
    @Test public void testJoinGroupByOrderBy() {
        ...
        that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                ...
                + "order by c.\"state_province\"").returns2(
    ...
    @Ignore @Test public void testDistinctCount() {
        ...
        that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                ...
                + "  join \"foodmart\".\"customer\" as c\n" + "  on s.\"customer_id\" = c.\"customer_id\"\n"
                + "group by c.\"state_province\", c.\"country\"\n" + "order by c.\"state_province\", 2").planHasSql(
    ...
    @Ignore @Test public void testPlan2() {
        that().with(CalciteAssert.Config.JDBC_FOODMART).withDefaultSchema("foodmart").query(
                ...
                + "order by \"customer\".\"state_province\" ASC").planContains(
                "          public boolean moveNext() {\n" + "            while (inputEnumerator.moveNext()) {\n"
    ...
    @Test public void testPlan3() {
        ...
        that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"store\".\"store_country\" as \"c0\", sum(\"inventory_fact_1997\".\"supply_time\") as \"m0\" from \"store\" as \"store\", \"inventory_fact_1997\" as \"inventory_fact_1997\" where \"inventory_fact_1997\".\"store_id\" = \"store\".\"store_id\" group by \"store\".\"store_country\"").planContains(
