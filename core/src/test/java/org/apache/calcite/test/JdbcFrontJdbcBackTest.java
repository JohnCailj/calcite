...
public class JdbcFrontJdbcBackTest {
    ...
    @Ignore @Test public void testTables() throws Exception {
        that().with(CalciteAssert.Config.JDBC_FOODMART).doWithConnection(new Function<CalciteConnection, Object>() {
    ...
    @Test public void testTablesByType() throws Exception {
        that().with(CalciteAssert.Config.REGULAR_PLUS_METADATA).doWithConnection(
                new Function<CalciteConnection, Object>() {

                    public Object apply(CalciteConnection a0) {
                        try {
                            ResultSet rset = a0.getMetaData().getTables(null, null, null,
                                                                        new String[] { "SYSTEM_TABLE" });
                            StringBuilder buf = new StringBuilder();
                            while (rset.next()) {
                                buf.append(rset.getString(3)).append(';');
                            }
                            assertEquals("COLUMNS;TABLES;", buf.toString());
                        } catch (SQLException e) {
                            throw new RuntimeException(e);
                        }
                        return null;
