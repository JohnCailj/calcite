...
public class JdbcFrontLinqBackTest {
    ...
    @Test public void testJoin() {
        hr().query("select *\n" + "from \"foodmart\".\"sales_fact_1997\" as s\n" + "join \"hr\".\"emps\" as e\n"
                   + "on e.\"empid\" = s.\"cust_id\"").returnsUnordered(
    ...
    @Test public void testGroupBy() {
        hr().query("select \"deptno\", sum(\"empid\") as s, count(*) as c\n" + "from \"hr\".\"emps\" as e\n"
    ...
    @Test public void testOrderBy() {
        hr().query("select upper(\"name\") as un, \"deptno\"\n" + "from \"hr\".\"emps\" as e\n"
                   + "order by \"deptno\", \"name\" desc").explainContains(
    ...
    @Test public void testUnion() {
        hr().query("select substring(\"name\" from 1 for 1) as x\n" + "from \"hr\".\"emps\" as e\n" + "union\n"
                   + "select substring(\"name\" from 1 for 1) as y\n" + "from \"hr\".\"depts\"").returnsUnordered("X=T",
                                                                                                                  "X=E",
                                                                                                                  "X=S",
                                                                                                                  "X=B",
                                                                                                                  "X=M",
     ...
     * Tests EXCEPT.
     */
    @Ignore @Test public void testExcept() {
        hr().query("select substring(\"name\" from 1 for 1) as x\n" + "from \"hr\".\"emps\" as e\n" + "except\n"
                   + "select substring(\"name\" from 1 for 1) as y\n" + "from \"hr\".\"depts\"").returnsUnordered("X=T",
                                                                                                                  "X=E",
                                                                                                                  "X=B");
    ...
    @Test public void testWhereOr() {
        hr().query("select * from \"hr\".\"emps\"\n" + "where (\"empid\" = 100 or \"empid\" = 200)\n"
                   + "and \"deptno\" = 10").returns(
                "empid=100; deptno=10; name=Bill; salary=10000.0; commission=1000\n");
    }

    @Test public void testWhereLike() {
        hr().query("select *\n" + "from \"hr\".\"emps\" as e\n"
                   + "where e.\"empid\" < 120 or e.\"name\" like 'S%'").returns(
                "" + "empid=100; deptno=10; name=Bill; salary=10000.0; commission=1000\n"
    ...
    @Test public void testInsert() {
        ...
        with.query("select * from \"foo\".\"bar\"").returns(
        ...
        with.query("select \"name\", count(*) as c from \"foo\".\"bar\" " + "group by \"name\"").returnsUnordered(
                "name=Bill; C=2", "name=Eric; C=1", "name=Theodore; C=2", "name=first; C=1", "name=Sebastian; C=2");
    }
    ...
    @Test public void testInsertBind() throws Exception {
        ...
        with.query("select * from \"foo\".\"bar\"").returnsUnordered(
                "empid=0; deptno=0; name=first; salary=0.0; commission=null",
    ...
    private CalciteAssert.AssertThat mutable(final List<JdbcTest.Employee> employees) {
        employees.add(new JdbcTest.Employee(0, 0, "first", 0f, null));
        return that().with(CalciteAssert.Config.REGULAR).with(makePostProcessor(employees));
    }
