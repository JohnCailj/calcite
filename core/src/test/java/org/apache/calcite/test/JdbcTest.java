...
public class JdbcTest {
    ...
    public static final String FOODMART_SCHEMA =
            "     {\n" + "       type: 'jdbc',\n" + "       name: 'foodmart',\n" + "       jdbcDriver: " + q(
                    CalciteAssert.DB.foodmart.driver) + ",\n" + "       jdbcUser: " + q(
                    CalciteAssert.DB.foodmart.username) + ",\n" + "       jdbcPassword: " + q(
                    CalciteAssert.DB.foodmart.password) + ",\n" + "       jdbcUrl: " + q(CalciteAssert.DB.foodmart.url)
            + ",\n" + "       jdbcCatalog: " + q(CalciteAssert.DB.foodmart.catalog) + ",\n" + "       jdbcSchema: " + q(
                    CalciteAssert.DB.foodmart.schema) + "\n" + "     }\n";

    public static final String FOODMART_MODEL =
            "{\n" + "  version: '1.0',\n" + "  defaultSchema: 'foodmart',\n" + "   schemas: [\n" + FOODMART_SCHEMA
            + "   ]\n" + "}";

    public static final ConnectionSpec SCOTT = Util.first(CalciteAssert.DB.scott,
    ...
    @Test public void testModelWithModifiableView() throws Exception {
        ...
        try (final TryThreadLocal.Memo ignore = EmpDeptTableFactory.THREAD_COLLECTION.push(employees)) {
            final CalciteAssert.AssertThat with = modelWithView(
                    "select \"name\", \"empid\" as e, \"salary\" " + "from \"MUTABLE_EMPLOYEES\" where \"deptno\" = 10",
                    null);
            with.query("select \"name\" from \"adhoc\".V order by \"name\"").returns("name=Simon\n");
            with.doWithConnection(new Function<CalciteConnection, Object>() {
                ...
                @Override public Object apply(CalciteConnection input) {
                    try {
                        ...
                        assertThat(Util.toLinux(resultSet.getString(1)),
                                   is("EnumerableTableModify(table=[[adhoc, MUTABLE_EMPLOYEES]], operation=[INSERT], flattened=[false])\n"
                        ...
                        // Mis-named column
                        try {
                            final PreparedStatement s = input.prepareStatement(
                                    "explain plan for\n" + "insert into \"adhoc\".V (empno, \"salary\", \"name\")\n"
                                    + "values (56, 123.4, 'Fred')");
                            fail("expected error, got " + s);
                        } catch (SQLException e) {
                            assertThat(e.getMessage(), startsWith("Error while preparing statement"));
                        }
    ...
    @Test public void testModelWithInvalidModifiableView() throws Exception {
        ...
        try (final TryThreadLocal.Memo ignore = EmpDeptTableFactory.THREAD_COLLECTION.push(employees)) {
            ...
            modelWithView("select \"name\", \"empid\" as e, \"salary\" "
                    ...
                    "select \"name\" from \"adhoc\".V order by \"name\"").throws_(
            ...
            modelWithView("select \"name\", \"empid\" as e, \"salary\" "
                          + "from \"MUTABLE_EMPLOYEES\" where \"commission\" = 10", null).query(
                    "select \"name\" from \"adhoc\".V order by \"name\"").runs();
            ...
            modelWithView("select \"name\", \"empid\" as e, \"salary\" " + "from \"MUTABLE_EMPLOYEES\"\n"
                          + "where \"deptno\" = 10 AND (\"deptno\" < 20 OR \"commission\" > 1000)", true).query(
                    "insert into \"adhoc\".v values ('n',1,2)").throws_(
                    "Modifiable view must be predicated only on equality expressions");
            modelWithView("select \"name\", \"empid\" as e, \"salary\" " + "from \"MUTABLE_EMPLOYEES\"\n"
                          + "where \"deptno\" = 10 AND (\"deptno\" > 20 AND \"commission\" > 1000)", true).query(
                    "insert into \"adhoc\".v values ('n',1,2)").throws_(
                    "Modifiable view must be predicated only on equality expressions");
            ...
            modelWithView("select \"name\", \"empid\" as e, \"salary\", \"empid\" + 3 as e3, 1 as uno\n"
                          ...
                          true).query("select \"name\" from \"adhoc\".V order by \"name\"").runs();

            Util.discard(RESOURCE.moreThanOneMappedColumn(null, null));
            modelWithView("select \"name\", \"empid\" as e, \"salary\", \"name\" as n2 "
                          + "from \"MUTABLE_EMPLOYEES\" where \"deptno\" IN (10, 20)", true).query(
                    "select \"name\" from \"adhoc\".V order by \"name\"").throws_(
            ...
            // no error if we do not claim that the view is modifiable
            modelWithView("select \"name\", \"empid\" as e, \"salary\", \"name\" as n2 "
                          + "from \"MUTABLE_EMPLOYEES\" where \"deptno\" IN (10, 20)", null).query(
                    "select \"name\" from \"adhoc\".V order by \"name\"").runs();
        }
    }
     ...
     */
    @Test public void testSqlAdvisorGetHintsFunction() throws SQLException, ClassNotFoundException {
        adviseSql("select e.e^ from \"emps\" e", CalciteAssert.checkResultUnordered("id=e; names=null; type=MATCH",
     ...
     * Tests {@link org.apache.calcite.sql.advise.SqlAdvisorGetHintsFunction}.
     */
    @Test public void testSqlAdvisorSchemaNames() throws SQLException, ClassNotFoundException {
        adviseSql("select empid from \"emps\" e, ^",
                  CalciteAssert.checkResultUnordered("id=; names=null; type=MATCH", "id=(; names=[(]; type=KEYWORD",
     ...
     */
    @Test public void testTableMacro() throws SQLException, ClassNotFoundException {
     ...
     */
    @Test public void testTableMacroMap() throws SQLException, ClassNotFoundException {
        Connection connection = DriverManager.getConnection("jdbc:calcite:");
        CalciteConnection calciteConnection = connection.unwrap(CalciteConnection.class);
        SchemaPlus rootSchema = calciteConnection.getRootSchema();
        SchemaPlus schema = rootSchema.add("s", new AbstractSchema());
    ...
    @Test public void testTableMacroWithNamedParameters() throws Exception {
        ...
        with.query("select * from table(\"adhoc\".\"View\"('(5)'))").throws_(
                "No match found for function signature View(<CHARACTER>)");
        ...
        with.query("select * from table(\"adhoc\".\"View\"(t=>'5', s=>'6'))").throws_(
    ...
    private void checkTableMacroInModel(Class clazz) {
        assertWithMacro(clazz).query("select * from table(\"adhoc\".\"View\"('(30)'))").returns(
                "" + "c=1\n" + "c=3\n" + "c=30\n");
    }
    ...
    private void checkTableFunctionInModel(Class clazz) {
        ...
        assertWithMacro(clazz).query(
                "select \"a\".\"c\" a, \"b\".\"c\" b\n" + "  from table(\"adhoc\".\"View\"('(30)')) \"a\",\n"
                + " lateral(select *\n" + "   from table(\"adhoc\".\"View\"('('||\n"
                + "          cast(\"a\".\"c\" as varchar(10))||')'))) \"b\"").returnsUnordered("A=1; B=1", "A=1; B=3",
                                                                                               "A=1; B=1", "A=3; B=1",
                                                                                               "A=3; B=3", "A=3; B=3",
                                                                                               "A=30; B=1", "A=30; B=3",
                                                                                               "A=30; B=30");
    ...
    @Test public void testMockDdl() throws Exception {
        ...
        try (Connection connection = driver.connect("jdbc:calcite:",
                                                    new Properties()); Statement statement = connection.createStatement()) {
     ...
     */
    @Test public void testConnectionProperties() throws ClassNotFoundException, SQLException {
    ...
    @Test public void testVersion() throws ClassNotFoundException, SQLException {
        ...
        assertTrue(driverVersion.equals(mm(driverMajor, driverMinor)) || driverVersion.startsWith(
                mm(driverMajor, driverMinor) + ".") || driverVersion.startsWith(mm(driverMajor, driverMinor) + "-")
                   || driverVersion.endsWith("-SNAPSHOT") && driverVersion.startsWith(
                mm(driverMajor, driverMinor + 1)));

        assertTrue(databaseVersion.startsWith("1."));
        assertTrue(databaseVersion.split("\\.").length >= 2);
        assertTrue(databaseVersion.equals(mm(databaseMajor, databaseMinor)) || databaseVersion.startsWith(
                mm(databaseMajor, databaseMinor) + ".") || databaseVersion.startsWith(
                mm(databaseMajor, databaseMinor) + "-")
                   || databaseVersion.endsWith("-SNAPSHOT") && databaseVersion.startsWith(
                mm(driverMajor, driverMinor + 1)));
    ...
    @Test public void testGetByName() throws Exception {
        ...
        CalciteAssert.that().doWithConnection(new Function<CalciteConnection, Object>() {
            ...
            public Object apply(CalciteConnection c) {
                try {
                    ...
                    try {
                        ...
                        assertThat(e.getMessage(), equalTo("invalid column ordinal: 0"));
                    }
                    try {
                        int x = rs.getInt(5);
                        fail("expected error, got " + x);
                    } catch (SQLException e) {
                        assertThat(e.getMessage(), equalTo("invalid column ordinal: 5"));
                    }
                    return null;
                } catch (SQLException e) {
                    throw new RuntimeException(e);
    ...
    @Test public void testCloneGroupBy() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"the_year\", count(*) as c, min(\"the_month\") as m\n" + "from \"foodmart2\".\"time_by_day\"\n"
                + "group by \"the_year\"\n" + "order by 1, 2").returns(
                "" + "the_year=1997; C=365; M=April\n" + "the_year=1998; C=365; M=April\n");
    }

    @Ignore("The test returns expected results. Not sure why it is disabled") @Test public void testCloneGroupBy2() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"time_by_day\".\"the_year\" as \"c0\", \"time_by_day\".\"quarter\" as \"c1\", \"product_class\".\"product_family\" as \"c2\", sum(\"sales_fact_1997\".\"unit_sales\") as \"m0\" from \"time_by_day\" as \"time_by_day\", \"sales_fact_1997\" as \"sales_fact_1997\", \"product_class\" as \"product_class\", \"product\" as \"product\" where \"sales_fact_1997\".\"time_id\" = \"time_by_day\".\"time_id\" and \"time_by_day\".\"the_year\" = 1997 and \"sales_fact_1997\".\"product_id\" = \"product\".\"product_id\" and \"product\".\"product_class_id\" = \"product_class\".\"product_class_id\" group by \"time_by_day\".\"the_year\", \"time_by_day\".\"quarter\", \"product_class\".\"product_family\"").returnsUnordered(
                "c0=1997; c1=Q2; c2=Drink; m0=5895.0000", "c0=1997; c1=Q1; c2=Food; m0=47809.0000",
                "c0=1997; c1=Q3; c2=Drink; m0=6065.0000", "c0=1997; c1=Q4; c2=Drink; m0=6661.0000",
                "c0=1997; c1=Q4; c2=Food; m0=51866.0000", "c0=1997; c1=Q1; c2=Drink; m0=5976.0000",
                "c0=1997; c1=Q3; c2=Non-Consumable; m0=12343.0000", "c0=1997; c1=Q4; c2=Non-Consumable; m0=13497.0000",
                "c0=1997; c1=Q2; c2=Non-Consumable; m0=11890.0000", "c0=1997; c1=Q2; c2=Food; m0=44825.0000",
                "c0=1997; c1=Q3; c2=Food; m0=47440.0000", "c0=1997; c1=Q1; c2=Non-Consumable; m0=12506.0000");
    }

    /**
     * Tests plan for a query with 4 tables, 3 joins.
     */
    @Ignore("The actual and expected plan differ") @Test public void testCloneGroupBy2Plan() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "explain plan for select \"time_by_day\".\"the_year\" as \"c0\", \"time_by_day\".\"quarter\" as \"c1\", \"product_class\".\"product_family\" as \"c2\", sum(\"sales_fact_1997\".\"unit_sales\") as \"m0\" from \"time_by_day\" as \"time_by_day\", \"sales_fact_1997\" as \"sales_fact_1997\", \"product_class\" as \"product_class\", \"product\" as \"product\" where \"sales_fact_1997\".\"time_id\" = \"time_by_day\".\"time_id\" and \"time_by_day\".\"the_year\" = 1997 and \"sales_fact_1997\".\"product_id\" = \"product\".\"product_id\" and \"product\".\"product_class_id\" = \"product_class\".\"product_class_id\" group by \"time_by_day\".\"the_year\", \"time_by_day\".\"quarter\", \"product_class\".\"product_family\"").returns(
                "PLAN=EnumerableAggregate(group=[{0, 1, 2}], m0=[SUM($3)])\n"
    ...
    @Test public void testOrderByCase() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"time_by_day\".\"the_year\" as \"c0\" from \"time_by_day\" as \"time_by_day\" group by \"time_by_day\".\"the_year\" order by CASE WHEN \"time_by_day\".\"the_year\" IS NULL THEN 1 ELSE 0 END, \"time_by_day\".\"the_year\" ASC").returns(
                "c0=1997\n" + "c0=1998\n");
    }
    ...
    @Test public void testAlmostBushy() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                ...
                + "and p.\"brand_name\" = 'Washington'").explainMatches("including all attributes ",
     ...
     */
    @Ignore("extremely slow - a bit better if you disable ProjectMergeRule") @Test public void testBushy() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select *\n" + "from \"sales_fact_1997\" as s\n" + "  join \"customer\" as c using (\"customer_id\")\n"
                + "  join \"product\" as p using (\"product_id\")\n"
                + "  join \"product_class\" as pc using (\"product_class_id\")\n"
                + "where c.\"city\" = 'San Francisco'\n" + "and pc.\"product_department\" = 'Snacks'\n").explainMatches(
                "including all attributes ", CalciteAssert.checkMaskedResultContains(""
    ...
    public static final List<Pair<String, String>> FOODMART_QUERIES = querify(QUERIES);
    ...
    @Test public void testJanino169() {
        CalciteAssert.that().with(CalciteAssert.Config.JDBC_FOODMART).query(
    ...
    @Test public void testJdbcDate() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                ...
                + "  and \"gender\" = 'F')").enable(CalciteAssert.DB != CalciteAssert.DatabaseInstance.ORACLE).returns2(
                "C=1\n");
    }
    ...
    @Test public void testExtract() {
        CalciteAssert.that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                "values extract(year from date '2008-2-23')").returns(new Function<ResultSet, Void>() {
    ...
    @Test public void testFloorDate() {
        CalciteAssert.that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                "select floor(timestamp '2011-9-14 19:27:23' to month) as c \n"
    ...
    @Test public void testSelfJoinDifferentColumns() {
        CalciteAssert.that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                ...
                + "order by e1.\"last_name\" limit 3").returns(
    ...
    @Test public void testThetaJoin() {
        CalciteAssert.hr().query("select e.\"empid\", d.\"name\", e.\"name\"\n" + "from \"hr\".\"emps\" as e\n"
                                 ...
                                 + "on e.\"deptno\" < d.\"deptno\"\n").returnsUnordered(
                "empid=100; name=Marketing; name=Bill", "empid=100; name=HR; name=Bill",
                "empid=200; name=Marketing; name=Eric", "empid=200; name=HR; name=Eric",
                "empid=150; name=Marketing; name=Sebastian", "empid=150; name=HR; name=Sebastian",
                "empid=110; name=Marketing; name=Theodore", "empid=110; name=HR; name=Theodore");
    }
     ...
     */
    @Ignore @Test public void testJoinJoin() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
     ...
     */
    @Ignore @Test public void testJoinFiveWay() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"store\".\"store_country\" as \"c0\",\n" + " \"time_by_day\".\"the_year\" as \"c1\",\n"
                ...
                + "and \"product\".\"product_class_id\" = \"product_class\".\"product_class_id\"\n"
                + "group by \"store\".\"store_country\",\n" + " \"time_by_day\".\"the_year\",\n"
                + " \"product_class\".\"product_family\"").explainContains(
    ...
    private static void checkJoinNWay(int n) {
        ...
        for (int i = 1; i < n; i++) {
            buf.append(i == 1 ? "\nwhere" : "\nand").append(" d").append(i).append(".\"deptno\" = d").append(
                    i - 1).append(".\"deptno\"");
        }
    ...
    @Test public void testArray2() {
        CalciteAssert.hr().query(
                "select \"deptno\", cardinality(\"employees\") as c\n" + "from \"hr\".\"depts\"").returnsUnordered(
                "deptno=10; C=2", "deptno=30; C=0", "deptno=40; C=1");
    }
    ...
    @Test public void testNestedArray() throws Exception {
        CalciteAssert.hr().doWithConnection(new Function<CalciteConnection, Object>() {
            ...
            public Object apply(CalciteConnection a0) {
                try {
                    ...
                    assertThat(resultSet.getString(2), equalTo("[[x, y, z], [Bill]]"));
                    final Array array = resultSet.getArray(2);
                    assertThat(array.getBaseType(), equalTo(java.sql.Types.ARRAY));
                    ...
                    assertThat(subArray.getBaseType(), equalTo(java.sql.Types.VARCHAR));
                    ...
                    try {
                        ...
                        assertThat(e.getMessage(), equalTo("invalid column ordinal: 2"));
                    }
                    assertThat(subResultSet.next(), is(true));
                    assertThat(subResultSet.next(), is(true));
    ...
    @Test public void testMultisetQuery() {
        CalciteAssert.hr().query("select multiset(\n" + "  select \"deptno\", \"empid\" from \"hr\".\"emps\") as a\n"
                                 + "from (values (1))").returnsUnordered(
                "A=[{10, 100}, {20, 200}, {10, 150}, {10, 110}]");
    }
    ...
    @Test public void testUnnestArrayWithOrdinality() {
        CalciteAssert.that().query("select*from unnest(array[10,20]) with ordinality as t(i, o)").returnsUnordered(
                "I=10; O=1", "I=20; O=2");
    }

    @Test public void testUnnestMultiset() {
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(
                "select*from unnest(multiset[1,2]) as t(c)").returnsUnordered("C=1", "C=2");
    }

    @Test public void testUnnestMultiset2() {
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(
                "select*from unnest(\n" + " select \"employees\" from \"hr\".\"depts\"\n"
                + " where \"deptno\" = 10)").returnsUnordered(
    ...
    @Test public void testArrayElement() {
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(
                ...
                + "where cardinality(\"employees\") < 2").returnsUnordered("EXPR$0={200, 20, Eric, 8000.0, 500}",
                                                                           "EXPR$0=null");
    }

    @Test public void testLateral() {
        CalciteAssert.hr().query("select * from \"hr\".\"emps\",\n"
                                 + " LATERAL (select * from \"hr\".\"depts\" where \"emps\".\"deptno\" = \"depts\".\"deptno\")").returnsUnordered(
    ...
    @Test public void testUnnestArrayColumn() {
        CalciteAssert.hr().query("select d.\"name\", e.*\n" + "from \"hr\".\"depts\" as d,\n"
                                 + " UNNEST(d.\"employees\") as e").returnsUnordered(
    ...
    @Test public void testUnnestArrayScalarArray() {
        CalciteAssert.hr().query("select d.\"name\", e.*\n" + "from \"hr\".\"depts\" as d,\n"
                                 + " UNNEST(d.\"employees\", array[1, 2]) as e").returnsUnordered(
    ...
    @Test public void testUnnestArrayScalarArrayAliased() {
        CalciteAssert.hr().query("select d.\"name\", e.*\n" + "from \"hr\".\"depts\" as d,\n"
                                 ...
                                 + "where ei + i > 151").returnsUnordered(
    ...
    @Test public void testUnnestItemsInMapWithNoAliasAndAdditionalArgument() throws SQLException {
        Connection connection = DriverManager.getConnection("jdbc:calcite:");
    ...
    private CalciteAssert.AssertQuery withFoodMartQuery(int id) throws IOException {
        final FoodmartTest.FoodMartQuerySet set = FoodmartTest.FoodMartQuerySet.instance();
        return CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(set.queries.get(id).sql);
    }
     ...
     */
    @Ignore @Test public void testNoCalcBetweenJoins() throws IOException {
        final FoodmartTest.FoodMartQuerySet set = FoodmartTest.FoodMartQuerySet.instance();
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(set.queries.get(16).sql).explainContains(""
     ...
     * possible.
     */
    @Ignore @Test public void testExplainJoin() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                FOODMART_QUERIES.get(48).left).explainContains(
     ...
     * hash-joins.
     */
    @Ignore @Test public void testExplainJoin2() throws IOException {
        withFoodMartQuery(2482).explainContains(""
    ...
    @Test public void testExplainJoin3() throws IOException {
        withFoodMartQuery(8).explainContains(""
    ...
    @Test public void testFoodmartLattice() throws IOException {
        ...
        final FoodmartTest.FoodmartQuery query = FoodmartTest.FoodMartQuerySet.instance().queries.get(8);
        CalciteAssert.that().with(CalciteAssert.Config.JDBC_FOODMART_WITH_LATTICE).withDefaultSchema(
                "foodmart").pooled().query(query.sql).enableMaterializations(true).explainContains(""
     ...
     */
    @Ignore @Test public void testExplainJoin4() throws IOException {
        withFoodMartQuery(5217).explainContains("" + "EnumerableAggregateRel(group=[{0, 1, 2, 3}], m0=[COUNT($4)])\n"
     ...
     */
    @Ignore @Test public void testExplainJoinOrderingWithOr() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                FOODMART_QUERIES.get(47).left).explainContains("xxx");
    }
    ...
    private void checkNullableTimestamp(CalciteAssert.Config config) {
        CalciteAssert.that().with(config).query(
                "select \"hire_date\", \"end_date\", \"birth_date\" from \"foodmart\".\"employee\" where \"employee_id\" = 1").returns2(
                "hire_date=1994-12-01; end_date=null; birth_date=1961-08-26\n");
    }

    @Test public void testReuseExpressionWhenNullChecking() {
        CalciteAssert.hr().query(
                "select upper((case when \"empid\">\"deptno\"*10 then 'y' else null end)) T from \"hr\".\"emps\"").planContains(
                "static final String " + "$L4J$C$org_apache_calcite_runtime_SqlFunctions_upper_y_ = "
                + "org.apache.calcite.runtime.SqlFunctions.upper(\"y\");").planContains(
                "return current.empid <= current.deptno * 10 " + "? (String) null "
                + ": $L4J$C$org_apache_calcite_runtime_SqlFunctions_upper_y_;").returns(
                "T=null\n" + "T=null\n" + "T=Y\n" + "T=Y\n");
    }

    @Test public void testReuseExpressionWhenNullChecking2() {
        CalciteAssert.hr().query(
                "select upper((case when \"empid\">\"deptno\"*10 then \"name\" end)) T from \"hr\".\"emps\"").planContains(
                "final String inp2_ = current.name;").planContains(
                "return current.empid <= current.deptno * 10 " + "|| inp2_ == null " + "? (String) null "
                + ": org.apache.calcite.runtime.SqlFunctions.upper(inp2_);").returns(
                "T=null\n" + "T=null\n" + "T=SEBASTIAN\n" + "T=THEODORE\n");
    }

    @Test public void testReuseExpressionWhenNullChecking3() {
        CalciteAssert.hr().query(
                "select substring(\"name\", \"deptno\"+case when user <> 'sa' then 1 end) from \"hr\".\"emps\"").planContains(
                "final String inp2_ = current.name;").planContains(
                "static final boolean " + "$L4J$C$org_apache_calcite_runtime_SqlFunctions_ne_sa_sa_ = "
                + "org.apache.calcite.runtime.SqlFunctions.ne(\"sa\", \"sa\");").planContains(
                "static final boolean " + "$L4J$C$_org_apache_calcite_runtime_SqlFunctions_ne_sa_sa_ = "
                + "!$L4J$C$org_apache_calcite_runtime_SqlFunctions_ne_sa_sa_;").planContains(
                "return inp2_ == null " + "|| $L4J$C$_org_apache_calcite_runtime_SqlFunctions_ne_sa_sa_ ? (String) null"
                + " : org.apache.calcite.runtime.SqlFunctions.substring(inp2_, " + "current.deptno + 1);");
    }
    ...
    @Test public void testReuseExpressionWhenNullChecking4() {
        CalciteAssert.hr().query("select substring(trim(\n" + "substring(\"name\",\n"
                                 ...
                                 + "\"hr\".\"emps\"").planContains("final String inp2_ = current.name;").planContains(
                "final int inp1_ = current.deptno;").planContains(
                "static final boolean " + "$L4J$C$org_apache_calcite_runtime_SqlFunctions_eq_sa_sa_ = "
                + "org.apache.calcite.runtime.SqlFunctions.eq(\"sa\", \"sa\");").planContains(
                "static final boolean " + "$L4J$C$_org_apache_calcite_runtime_SqlFunctions_eq_sa_sa_ = "
                + "!$L4J$C$org_apache_calcite_runtime_SqlFunctions_eq_sa_sa_;").planContains(
                "return inp2_ == null " + "|| $L4J$C$_org_apache_calcite_runtime_SqlFunctions_eq_sa_sa_ "
    ...
    @Test public void testReuseExpressionWhenNullChecking5() {
        CalciteAssert.hr().query("select substring(trim(\n" + "substring(\"name\",\n"
                                 ...
                                 + "     case when \"deptno\"*8>8 then 5 end\n" + "   end-2) T\n" + "from\n"
                                 + "\"hr\".\"emps\"").planContains("final String inp2_ = current.name;").planContains(
                "final int inp1_ = current.deptno;").planContains("static final int $L4J$C$5_2 = 5 - 2;").planContains(
                "static final boolean " + "$L4J$C$org_apache_calcite_runtime_SqlFunctions_eq_sa_sa_ = "
                + "org.apache.calcite.runtime.SqlFunctions.eq(\"sa\", \"sa\");").planContains(
                "static final boolean " + "$L4J$C$_org_apache_calcite_runtime_SqlFunctions_eq_sa_sa_ = "
                + "!$L4J$C$org_apache_calcite_runtime_SqlFunctions_eq_sa_sa_;").planContains(
                ...
                + "org.apache.calcite.runtime.SqlFunctions.trim(true, true, \" \", "
                + "org.apache.calcite.runtime.SqlFunctions.substring(inp2_, "
                + "inp1_ * 0 + 1)), $L4J$C$5_2);").returns("T=ll\n" + "T=ic\n" + "T=bastian\n" + "T=eodore\n");
    }
    ...
    @Test public void testValuesAlias() {
        CalciteAssert.that().query(
                "select \"desc\" from (VALUES ROW(1, 'SameName')) AS \"t\" (\"id\", \"desc\")").returns(
                "desc=SameName\n");
    }
    ...
    @Test public void testValuesComposite() {
        CalciteAssert.that().query("values (1, 'a'), (2, 'abc')").returns(
                "EXPR$0=1; EXPR$1=a  \n" + "EXPR$0=2; EXPR$1=abc\n");
    }
    ...
    @Test public void testValuesCompositeRenamed() {
        CalciteAssert.that().query("select EXPR$0 q, EXPR$1 w from (values (1, 'a'), (2, 'abc'))").explainContains(
    ...
    @Test public void testValuesCompositeRenamedSameNames() {
        CalciteAssert.that().query("select EXPR$0 q, EXPR$1 q from (values (1, 'a'), (2, 'abc'))").explainContains(
                "PLAN=EnumerableValues(tuples=[[{ 1, 'a  ' }, { 2, 'abc' }]])\n").returnsUnordered("Q=1; Q=a  ",
                                                                                                   "Q=2; Q=abc");
    }
    ...
    @Test public void testUnionWithSameColumnNames() {
        CalciteAssert.hr().query(
                "select \"deptno\", \"deptno\" from \"hr\".\"depts\" union select \"deptno\", \"empid\" from \"hr\".\"emps\"").explainContains(
                ...
                + "    EnumerableTableScan(table=[[hr, emps]])\n").returnsUnordered("deptno=10; deptno=110",
                                                                                    "deptno=10; deptno=10",
    ...
    @Test public void testInnerJoinValues() {
        CalciteAssert.that().with(CalciteAssert.Config.LINGUAL).query("select empno, desc from sales.emps,\n"
                                                                      ...
                                                                      + "where emps.deptno = sn.id and sn.desc = 'SameName' group by empno, desc").explainContains(
                "EnumerableCalc(expr#0..1=[{inputs}], EMPNO=[$t1], DESC=[$t0])\n"
    ...
    @Test public void testMergeJoin() {
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(
                ...
                + "from \"hr\".\"emps\"\n" + " join \"hr\".\"depts\" using (\"deptno\")").explainContains(
                ...
                + "      EnumerableTableScan(table=[[hr, emps]])").returns(
                "empid=100; deptno=10; name=Sales\n" + "empid=150; deptno=10; name=Sales\n"
    ...
    @Test public void testCartesianJoin() {
        CalciteAssert.hr().query(
                "select * from \"hr\".\"emps\", \"hr\".\"depts\" where \"emps\".\"empid\" < 140 and \"depts\".\"deptno\" > 20").returnsUnordered(
    ...
    @Test public void testDistinctCountSimple() {
        final String s = "select count(distinct \"sales_fact_1997\".\"unit_sales\") as \"m0\"\n"
                         + "from \"sales_fact_1997\" as \"sales_fact_1997\"";
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(s).explainContains(
    ...
    @Test public void testDistinctCount2() {
        ...
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(s).explainContains(
                ...
                + "      EnumerableTableScan(table=[[foodmart2, sales_fact_1997]])").returnsUnordered("u=1; m0=523",
                                                                                                      "u=5; m0=1059",
                                                                                                      "u=4; m0=4459",
                                                                                                      "u=6; m0=19",
                                                                                                      "u=3; m0=4895",
    ...
    @Test public void testDistinctCount() {
        final String s = "select \"time_by_day\".\"the_year\" as \"c0\",\n"
                         ...
                         + "where \"sales_fact_1997\".\"time_id\" = \"time_by_day\".\"time_id\"\n"
                         + "and \"time_by_day\".\"the_year\" = 1997\n" + "group by \"time_by_day\".\"the_year\"";
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(s).enable(
                CalciteAssert.DB != CalciteAssert.DatabaseInstance.ORACLE).explainContains(
    ...
    @Test public void testDistinctCountComposite() {
        final String s = "select \"time_by_day\".\"the_year\" as \"c0\",\n"
                         ...
                         + "where \"sales_fact_1997\".\"time_id\" = \"time_by_day\".\"time_id\"\n"
                         + "and \"time_by_day\".\"the_year\" = 1997\n" + "group by \"time_by_day\".\"the_year\"";
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(s).returns("c0=1997; m0=85452\n");
    }
    ...
    @Test public void testAggregateFilter() {
        ...
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(s).returns(
                "the_month=April; c=30; c2=10\n" + "the_month=August; c=31; c2=11\n"
    ...
    @Test public void testSimpleIn() {
        CalciteAssert.hr().query(
                ...
                + "    EnumerableTableScan(table=[[hr, emps]])").returnsUnordered(
     ...
     */
    @Ignore @Test public void testIn() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
    ...
    @Test public void testOrderByDesc() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"store_id\", \"grocery_sqft\" from \"store\"\n"
                + "where \"store_id\" < 3 order by 2 desc").returns(
                "store_id=0; grocery_sqft=null\n" + "store_id=2; grocery_sqft=22271\n"
                + "store_id=1; grocery_sqft=17475\n");
    }
    ...
    @Test public void testOrderStarByExpr() {
        CalciteAssert.hr().query("select * from \"hr\".\"emps\"\n" + "order by - \"empid\"").explainContains(
    ...
    @Test public void testOrderByDescNullsFirst() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"store_id\", \"grocery_sqft\" from \"store\"\n"
                + "where \"store_id\" < 3 order by 2 desc nulls first").returns(
                "store_id=0; grocery_sqft=null\n" + "store_id=2; grocery_sqft=22271\n"
                + "store_id=1; grocery_sqft=17475\n");
    }
    ...
    @Test public void testOrderByNullsFirst() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"store_id\", \"grocery_sqft\" from \"store\"\n"
                + "where \"store_id\" < 3 order by 2 nulls first").returns(
                "store_id=0; grocery_sqft=null\n" + "store_id=1; grocery_sqft=17475\n"
                + "store_id=2; grocery_sqft=22271\n");
    }
    ...
    @Test public void testOrderByDescNullsLast() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"store_id\", \"grocery_sqft\" from \"store\"\n"
                + "where \"store_id\" < 3 order by 2 desc nulls last").returns(
                "store_id=2; grocery_sqft=22271\n" + "store_id=1; grocery_sqft=17475\n"
                + "store_id=0; grocery_sqft=null\n");
    }
    ...
    @Test public void testOrderByNullsLast() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"store_id\", \"grocery_sqft\" from \"store\"\n"
                + "where \"store_id\" < 3 order by 2 nulls last").returns(
                "store_id=1; grocery_sqft=17475\n" + "store_id=2; grocery_sqft=22271\n"
                + "store_id=0; grocery_sqft=null\n");
    }
    ...
    public void checkOrderBy(final boolean desc, final NullCollation nullCollation) {
        ...
        final CalciteAssert.AssertThat with = CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).with(
                "defaultNullCollation", nullCollation.name());
        final String sql =
    ...
    @Test public void testOrderByFetch() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"store_id\", \"grocery_sqft\" from \"store\"\n" + "where \"store_id\" < 10\n"
                + "order by 1 fetch first 5 rows only").explainContains(
    ...
    @Test public void testOrderByOffsetFetch() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"store_id\", \"grocery_sqft\" from \"store\"\n" + "where \"store_id\" < 10\n"
                + "order by 1 offset 2 rows fetch next 5 rows only").returns(
                "store_id=2; grocery_sqft=22271\n" + "store_id=3; grocery_sqft=24390\n"
    ...
    @Test public void testSelfJoinCount() {
        CalciteAssert.that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                "select count(*) as c from \"foodmart\".\"store\" as p1 join \"foodmart\".\"store\" as p2 using (\"store_id\")").returns(
    ...
    @Test public void testGroupByNull() {
        CalciteAssert.hr().query("select \"deptno\", \"commission\", sum(\"salary\") s\n" + "from \"hr\".\"emps\"\n"
                                 + "group by \"deptno\", \"commission\"").returnsUnordered(
                "deptno=10; commission=null; S=7000.0", "deptno=20; commission=500; S=8000.0",
                "deptno=10; commission=1000; S=10000.0", "deptno=10; commission=250; S=11500.0");
    }

    @Test public void testGroupingSets() {
        CalciteAssert.hr().query("select \"deptno\", count(*) as c, sum(\"salary\") as s\n" + "from \"hr\".\"emps\"\n"
                                 + "group by grouping sets((\"deptno\"), ())").returnsUnordered(
                "deptno=null; C=4; S=36500.0", "deptno=10; C=3; S=28500.0", "deptno=20; C=1; S=8000.0");
    }

    @Test public void testRollup() {
        CalciteAssert.hr().query("select \"deptno\", count(*) as c, sum(\"salary\") as s\n" + "from \"hr\".\"emps\"\n"
                                 + "group by rollup(\"deptno\")").returnsUnordered("deptno=null; C=4; S=36500.0",
    ...
    @Test public void testSelectDistinctComposite() {
        CalciteAssert.hr().query(
                ...
                "C=false; deptno=10", "C=true; deptno=10", "C=true; deptno=20").planContains(".distinct(");
    }
    ...
    @Test public void testGroupByNoAggregates() {
        CalciteAssert.hr().query(
                "select \"deptno\"\n" + "from \"hr\".\"emps\"\n" + "group by \"deptno\"").returnsUnordered("deptno=10",
                                                                                                           "deptno=20");
    }
    ...
    @Test public void testGroupByNoAggregatesAllColumns() {
        CalciteAssert.hr().query("select \"deptno\"\n" + "from \"hr\".\"emps\"\n"
                                 + "group by \"deptno\", \"empid\", \"name\", \"salary\", \"commission\"").returnsCount(
                4).planContains(".distinct(");
    }
    ...
    @Test public void testOrderSorted() throws IOException {
        // The ArrayTable "store" is sorted by "store_id".
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select \"store_id\"\n" + "from \"store\"\n" + "order by \"store_id\" limit 3").returns(
                "store_id=0\n" + "store_id=1\n" + "store_id=2\n");
    }
    ...
    @Test public void testCountStar() {
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            CalciteAssert.hr().query("select count(*) c from \"hr\".\"emps\", \"hr\".\"depts\"").convertContains(
    ...
    @Test public void testUnionAll() {
        CalciteAssert.hr().query("select \"empid\", \"name\" from \"hr\".\"emps\" where \"deptno\"=10\n" + "union all\n"
                                 + "select \"empid\", \"name\" from \"hr\".\"emps\" where \"empid\">=150").explainContains(
                "" + "PLAN=EnumerableUnion(all=[true])").returnsUnordered("empid=100; name=Bill",
    ...
    @Test public void testUnion() {
        ...
        CalciteAssert.hr().query(sql).explainContains("" + "PLAN=EnumerableUnion(all=[false])").returnsUnordered(
    ...
    @Test public void testIntersect() {
        final String sql = "" + "select \"empid\", \"name\" from \"hr\".\"emps\" where \"deptno\"=10\n" + "intersect\n"
                           + "select \"empid\", \"name\" from \"hr\".\"emps\" where \"empid\">=150";
        CalciteAssert.hr().query(sql).withHook(Hook.PLANNER, new Function<RelOptPlanner, Void>() {

            @Override public Void apply(RelOptPlanner planner) {
                planner.removeRule(IntersectToDistinctRule.INSTANCE);
                return null;
            }
        }).explainContains("" + "PLAN=EnumerableIntersect(all=[false])").returnsUnordered("empid=150; name=Sebastian");
    }

    @Test public void testExcept() {
        final String sql = "" + "select \"empid\", \"name\" from \"hr\".\"emps\" where \"deptno\"=10\n" + "except\n"
                           + "select \"empid\", \"name\" from \"hr\".\"emps\" where \"empid\">=150";
        CalciteAssert.hr().query(sql).explainContains("" + "PLAN=EnumerableMinus(all=[false])").returnsUnordered(
                "empid=100; name=Bill", "empid=110; name=Theodore");
    }
    ...
    @Test public void testAggregateEmpty() {
        CalciteAssert.hr().query(
                "select\n" + " count(*) as cs,\n" + " count(\"deptno\") as c,\n" + " sum(\"deptno\") as s,\n"
                + " avg(\"deptno\") as a\n" + "from \"hr\".\"emps\"\n" + "where \"deptno\" < 0").explainContains(""
    ...
    @Test public void testOrderByOnSortedTable() {
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).query(
                "select * from \"time_by_day\"\n" + "order by \"time_id\"").explainContains(
    ...
    @Test public void testWinAgg() {
        CalciteAssert.hr().query(
                ...
                + "from \"hr\".\"emps\"\n"
                + "window w as (partition by \"deptno\" order by \"empid\" rows 1 preceding)").typeIs(
                "[deptno INTEGER NOT NULL, empid INTEGER NOT NULL, S REAL, FIVE INTEGER NOT NULL, M REAL, C BIGINT NOT NULL]").explainContains(
                ""
                ...
                "deptno=20; empid=200; S=8200.0; FIVE=5; M=8000.0; C=1").planContains(
                CalcitePrepareImpl.DEBUG ? "_list.add(new Object[] {\n" + "        row[0],\n" // box-unbox is optimized
                                           + "        row[1],\n" + "        row[2],\n" + "        row[3],\n"
                                           + "        COUNTa0w0,\n" + "        $SUM0a1w0,\n" + "        MINa2w0,\n"
                                           + "        COUNTa3w0});" :
    ...
    @Test public void testWinAgg2() {
        CalciteAssert.hr().query(
                ...
                + " w11dept as (partition by \"deptno\" order by \"empid\" rows between 1 preceding and 1 following)").typeIs(
                "[deptno INTEGER NOT NULL, empid INTEGER NOT NULL, S REAL, FIVE INTEGER NOT NULL, M REAL, C BIGINT NOT NULL, C2 BIGINT NOT NULL, C11 BIGINT NOT NULL, C11DEPT BIGINT NOT NULL]")
                     ...
                     .planContains("tempList.size()").returnsUnordered(
    ...
    @Test public void testWinAggScalarNonNullPhysType() {
        ...
        CalciteAssert.hr().query("select min(\"salary\"+1) over w as m\n" + "from \"hr\".\"emps\"\n"
                ...
                "[M REAL]").planContains(planLine).returnsUnordered("M=7001.0", "M=7001.0", "M=8001.0", "M=10001.0");
    }
    ...
    @Test public void testWinAggScalarNonNullPhysTypePlusOne() {
        ...
        CalciteAssert.hr().query("select 1+min(\"salary\"+1) over w as m\n" + "from \"hr\".\"emps\"\n"
                                 + "window w as (order by \"salary\"+1 rows 1 preceding)\n").typeIs(
                "[M REAL]").planContains(planLine).returnsUnordered("M=7002.0", "M=7002.0", "M=8002.0", "M=10002.0");
    }
    ...
    @Test public void testWinAggRank() {
        CalciteAssert.hr().query("select  \"deptno\",\n" + " \"empid\",\n" + " \"commission\",\n"
                                 ...
                                 + "from \"hr\".\"emps\"").typeIs(
                "[deptno INTEGER NOT NULL, empid INTEGER NOT NULL, commission INTEGER, RCNF BIGINT NOT NULL, RCNL BIGINT NOT NULL, R BIGINT NOT NULL, RD BIGINT NOT NULL]").returnsUnordered(
    ...
    @Test public void testWinAggRankValues() {
        CalciteAssert.hr().query(
                ...
                "[deptno INTEGER NOT NULL, R BIGINT NOT NULL]").returnsUnordered("deptno=10; R=1", "deptno=10; R=1",
                                                                                 "deptno=10; R=1",
    ...
    @Test public void testWinAggRankValuesDesc() {
        CalciteAssert.hr().query("select  \"deptno\",\n" + " rank() over (order by \"deptno\" desc) as r\n"
                                 + "from \"hr\".\"emps\"").typeIs(
                "[deptno INTEGER NOT NULL, R BIGINT NOT NULL]").returnsUnordered("deptno=10; R=2", "deptno=10; R=2",
                                                                                 "deptno=10; R=2", "deptno=20; R=1");
    }
    ...
    @Test public void testWinIntervalFrame() {
        CalciteAssert.hr().query("select  \"deptno\",\n" + " \"empid\",\n" + " \"hire_date\",\n"
                                 ...
                                 + "  from \"hr\".\"emps\")").typeIs(
                "[deptno INTEGER NOT NULL, empid INTEGER NOT NULL, hire_date DATE NOT NULL, R BIGINT NOT NULL]").returnsUnordered(
                "deptno=10; empid=100; hire_date=2014-06-12; R=3", "deptno=10; empid=110; hire_date=2014-06-12; R=3",
                "deptno=10; empid=150; hire_date=2014-06-12; R=3", "deptno=20; empid=200; hire_date=2014-06-12; R=1");
    }
    ...
    private void startOfGroupStep1(String startOfGroup) {
        CalciteAssert.that().query(
                ...
                "[RN INTEGER NOT NULL, VAL INTEGER NOT NULL, EXPECTED INTEGER NOT NULL, START_OF_GROUP INTEGER NOT NULL]").returnsUnordered(
                "RN=1; VAL=0; EXPECTED=1; START_OF_GROUP=1", "RN=2; VAL=0; EXPECTED=1; START_OF_GROUP=0",
                "RN=3; VAL=1; EXPECTED=2; START_OF_GROUP=1", "RN=4; VAL=0; EXPECTED=3; START_OF_GROUP=1",
                "RN=5; VAL=0; EXPECTED=3; START_OF_GROUP=0", "RN=6; VAL=0; EXPECTED=3; START_OF_GROUP=0",
                "RN=7; VAL=1; EXPECTED=4; START_OF_GROUP=1", "RN=8; VAL=1; EXPECTED=4; START_OF_GROUP=0");
    }
    ...
    private void startOfGroupStep2(String startOfGroup) {
        CalciteAssert.that().query("select t.*\n"
                                   ...
                                   + ") t\n").typeIs(
                "[RN INTEGER NOT NULL, VAL INTEGER NOT NULL, EXPECTED INTEGER NOT NULL, START_OF_GROUP INTEGER NOT NULL, GROUP_ID INTEGER NOT NULL]").returnsUnordered(
    ...
    private void startOfGroupStep3(String startOfGroup) {
        CalciteAssert.that().query(
                "select group_id, min(rn) min_rn, max(rn) max_rn,\n" + "  count(rn) cnt_rn, avg(val) avg_val"
                + " from (\n" + "select t.*\n"
                // current row is assumed, group_id should be NOT NULL
                + "       ,sum(start_of_group) over (order by rn rows unbounded preceding) group_id\n" + "  from (\n"
                + "       select  t.*,\n" + "               case when " + startOfGroup
                + " then 0 else 1 end start_of_group\n" + "         from " + START_OF_GROUP_DATA + ") t\n"
                + ") group by group_id\n").typeIs(
                "[GROUP_ID INTEGER NOT NULL, MIN_RN INTEGER NOT NULL, MAX_RN INTEGER NOT NULL, CNT_RN BIGINT NOT NULL, AVG_VAL INTEGER NOT NULL]").returnsUnordered(
    ...
    @Test public void testLagDefaultValue() {
        CalciteAssert.that().query(
                "select t.*, lag(rn+expected,1,42) over (order by rn) l\n" + " from " + START_OF_GROUP_DATA).typeIs(
                "[RN INTEGER NOT NULL, VAL INTEGER NOT NULL, EXPECTED INTEGER NOT NULL, L INTEGER NOT NULL]").returnsUnordered(
                "RN=1; VAL=0; EXPECTED=1; L=42", "RN=2; VAL=0; EXPECTED=1; L=2", "RN=3; VAL=1; EXPECTED=2; L=3",
                "RN=4; VAL=0; EXPECTED=3; L=5", "RN=5; VAL=0; EXPECTED=3; L=7", "RN=6; VAL=0; EXPECTED=3; L=8",
                "RN=7; VAL=1; EXPECTED=4; L=9", "RN=8; VAL=1; EXPECTED=4; L=11");
    }
    ...
    @Test public void testLeadDefaultValue() {
        CalciteAssert.that().query(
                ...
                "[RN INTEGER NOT NULL, VAL INTEGER NOT NULL, EXPECTED INTEGER NOT NULL, L INTEGER NOT NULL]").returnsUnordered(
                "RN=1; VAL=0; EXPECTED=1; L=3", "RN=2; VAL=0; EXPECTED=1; L=5", "RN=3; VAL=1; EXPECTED=2; L=7",
                "RN=4; VAL=0; EXPECTED=3; L=8", "RN=5; VAL=0; EXPECTED=3; L=9", "RN=6; VAL=0; EXPECTED=3; L=11",
                "RN=7; VAL=1; EXPECTED=4; L=12", "RN=8; VAL=1; EXPECTED=4; L=42");
    }
    ...
    @Test public void testLagExpressionOffset() {
        CalciteAssert.that().query(
                ...
                "[RN INTEGER NOT NULL, VAL INTEGER NOT NULL, EXPECTED INTEGER NOT NULL, L INTEGER NOT NULL]").returnsUnordered(
                "RN=1; VAL=0; EXPECTED=1; L=42", "RN=2; VAL=0; EXPECTED=1; L=1", "RN=3; VAL=1; EXPECTED=2; L=1",
                "RN=4; VAL=0; EXPECTED=3; L=1", "RN=5; VAL=0; EXPECTED=3; L=2", "RN=6; VAL=0; EXPECTED=3; L=3",
                "RN=7; VAL=1; EXPECTED=4; L=3", "RN=8; VAL=1; EXPECTED=4; L=4");
    }
    ...
    @Test public void testNtile1() {
        CalciteAssert.that().query(
                ...
                "[RN INTEGER NOT NULL, L BIGINT NOT NULL]").returnsUnordered("RN=1; L=1", "RN=2; L=1", "RN=3; L=1",
                                                                             "RN=4; L=1", "RN=5; L=1", "RN=6; L=1",
                                                                             "RN=7; L=1", "RN=8; L=1");
    }
     ...
     * Tests expression in offset value of LAG function.
     */
    @Ignore("Have no idea how to validate that expression is constant") @Test public void testNtileConstantArgs() {
        CalciteAssert.that().query(
                "select rn, ntile(1+1) over (order by rn) l\n" + " from " + START_OF_GROUP_DATA).typeIs(
                "[RN INTEGER NOT NULL, VAL INTEGER NOT NULL, EXPECTED INTEGER NOT NULL, L INTEGER NOT NULL]").returnsUnordered(
                "RN=1; L=1", "RN=2; L=1", "RN=3; L=1", "RN=4; L=1", "RN=5; L=2", "RN=6; L=2", "RN=7; L=2", "RN=8; L=2");
    }
    ...
    @Test public void testNtileNegativeArg() {
        CalciteAssert.that().query(
                "select rn, ntile(-1) over (order by rn) l\n" + " from " + START_OF_GROUP_DATA).throws_(
    ...
    @Test public void testWinAggFirstValue() {
        CalciteAssert.hr().query("select  \"deptno\",\n" + " \"empid\",\n" + " \"commission\",\n"
                                 ...
                                 + "from \"hr\".\"emps\"").typeIs(
                "[deptno INTEGER NOT NULL, empid INTEGER NOT NULL, commission INTEGER, R INTEGER]").returnsUnordered(
                "deptno=10; empid=100; commission=1000; R=1000", "deptno=10; empid=110; commission=250; R=1000",
                "deptno=10; empid=150; commission=null; R=1000", "deptno=20; empid=200; commission=500; R=500");
    }
    ...
    @Test public void testWinAggFirstValueDesc() {
        CalciteAssert.hr().query("select  \"deptno\",\n" + " \"empid\",\n" + " \"commission\",\n"
                                 ...
                                 + "from \"hr\".\"emps\"").typeIs(
                "[deptno INTEGER NOT NULL, empid INTEGER NOT NULL, commission INTEGER, R INTEGER]").returnsUnordered(
                "deptno=10; empid=100; commission=1000; R=null", "deptno=10; empid=110; commission=250; R=null",
                "deptno=10; empid=150; commission=null; R=null", "deptno=20; empid=200; commission=500; R=500");
    }
    ...
    @Test public void testWinAggFirstValueEmptyWindow() {
        CalciteAssert.hr().query("select \"deptno\",\n" + " \"empid\",\n" + " \"commission\",\n"
                ...
                "[deptno INTEGER NOT NULL, empid INTEGER NOT NULL, commission INTEGER, R INTEGER]").returnsUnordered(
                "deptno=10; empid=100; commission=1000; R=null", "deptno=10; empid=110; commission=250; R=null",
                "deptno=10; empid=150; commission=null; R=null", "deptno=20; empid=200; commission=500; R=null");
    }
    ...
    @Test public void testWinRowNumber() {
        CalciteAssert.hr().query("select \"deptno\",\n" + " \"empid\",\n" + " \"commission\",\n"
                                 ...
                                 + "from \"hr\".\"emps\"").typeIs(
                "[deptno INTEGER NOT NULL, empid INTEGER NOT NULL, commission INTEGER, R BIGINT NOT NULL, RCNF BIGINT NOT NULL, RCNL BIGINT NOT NULL, R BIGINT NOT NULL, RD BIGINT NOT NULL]").returnsUnordered(
    ...
    @Test public void testOverUnboundedPreceding() {
        CalciteAssert.hr().query(
                ...
                "[empid INTEGER NOT NULL, commission INTEGER, M BIGINT NOT NULL]").returnsUnordered(
                "empid=100; commission=1000; M=4", "empid=200; commission=500; M=3", "empid=150; commission=null; M=1",
    ...
    @Test public void testSumOverUnboundedPreceding() {
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(
                ...
                + "    order by \"commission\" nulls first\n"
                + "    rows between UNBOUNDED PRECEDING and current row) as m\n" + "from \"hr\".\"emps\"").typeIs(
                "[empid INTEGER NOT NULL, commission INTEGER, M INTEGER NOT NULL]").returnsUnordered(
                "empid=100; commission=1000; M=560", "empid=110; commission=250; M=260",
                "empid=150; commission=null; M=150", "empid=200; commission=500; M=460");
    }
    ...
    @Test public void testSumOverPossiblyEmptyWindow() {
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(
                ...
                "[empid INTEGER NOT NULL, commission INTEGER, M INTEGER]").returnsUnordered(
                "empid=100; commission=1000; M=460", "empid=110; commission=250; M=150",
                "empid=150; commission=null; M=null", "empid=200; commission=500; M=260");
    }
    ...
    @Test public void testOverNoOrder() {
        ...
        final String sql = "select *,\n" + " count(*) over (partition by deptno) as m1,\n"
                           ...
                           + " count(*) over () as m3\n" + "from emp";
        withEmpDept(sql).returnsUnordered("ENAME=Adam ; DEPTNO=50; GENDER=M; M1=2; M2=1; M3=9",
    ...
    @Test public void testTrimFields() throws Exception {
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            CalciteAssert.hr().query("select \"name\", count(\"commission\") + 1\n" + "from \"hr\".\"emps\"\n"
                                     + "group by \"deptno\", \"name\"").convertContains(
                    "LogicalProject(name=[$1], EXPR$1=[+($2, 1)])\n"
    ...
    @Test public void testTrimFieldsOver() throws Exception {
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            ...
            CalciteAssert.hr().query(
                    ...
                    + "from \"hr\".\"emps\"\n" + "where \"empid\" > 10").convertContains(""
    ...
    @Test public void testWinAggConstantMultipleConstants() {
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(
                ...
                + "from \"hr\".\"emps\"").returnsUnordered("deptno=10; A=1; B=-1", "deptno=10; A=2; B=-2",
                                                           "deptno=10; A=3; B=-3", "deptno=20; A=1; B=-1");
    }
    ...
    @Test public void testWinAggPartitionByConstant() {
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(""
                                                                      ...
                                                                      + "  order by \"empid\") as m\n"
                                                                      + "from \"hr\".\"emps\"").returnsUnordered("M=0",
                                                                                                                 "M=0",
                                                                                                                 "M=0",
                                                                                                                 "M=0");
    }
     ...
     */
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(
                "with tst(c) as (values('a'),('b'),('c'),(cast(null as varchar)))"
                + " select u.c u, v.c v from tst u, tst v where ((u.c = 'a' and v.c = 'b') or (u.c = 'b' and v.c = 'c'))").returnsUnordered(
                "U=a; V=b", "U=b; V=c");
    }
    ...
    @Ignore("Fails with org.codehaus.commons.compiler.CompileException: Line 16, Column 112:"
            + " Cannot compare types \"int\" and \"java.lang.String\"\n") @Test public void testComparingIntAndString()
            throws Exception {
        // if (((...test.ReflectiveSchemaTest.IntAndString) inputEnumerator.current()).id == "T")

        CalciteAssert.that().withSchema("s", new ReflectiveSchema(new ReflectiveSchemaTest.CatchallSchema())).query(
                "select a.\"value\", b.\"value\"\n" + "  from \"bools\" a\n" + "     , \"bools\" b\n"
                + " where b.\"value\" = 'T'\n" + " order by 1, 2").returnsUnordered(
    ...
    @Test public void testTrivialSort() {
        final String sql =
                "select a.\"value\", b.\"value\"\n" + "  from \"bools\" a\n" + "     , \"bools\" b\n" + " offset 0";
        CalciteAssert.that().withSchema("s", new ReflectiveSchema(new ReflectiveSchemaTest.CatchallSchema())).query(
                sql).returnsUnordered("value=T; value=T", "value=T; value=F", "value=T; value=null", "value=F; value=T",
                                      "value=F; value=F", "value=F; value=null");
    }
    ...
    @Test public void testArrayIndexing() {
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(
                ...
                "deptno=10; E={100, 10, Bill, 10000.0, 1000}", "deptno=30; E=null",
                "deptno=40; E={200, 20, Eric, 8000.0, 500}");
    }

    @Test public void testVarcharEquals() {
        CalciteAssert.model(FOODMART_MODEL).query(
                "select \"lname\" from \"customer\" where \"lname\" = 'Nowmer'").returns("lname=Nowmer\n");
        ...
        CalciteAssert.model(FOODMART_MODEL).query("select count(*) as c from \"customer\" "
                                                  + "where \"lname\" = 'this string is longer than 30 characters'").returns(
                "C=0\n");

        CalciteAssert.model(FOODMART_MODEL).query("select count(*) as c from \"customer\" "
                                                  + "where cast(\"customer_id\" as char(20)) = 'this string is longer than 30 characters'").returns(
                "C=0\n");
    }
    ...
    @Test public void testJoinMismatchedVarchar() {
        ...
        CalciteAssert.model(FOODMART_MODEL).query(sql).returns("C=607\n");
    }
    ...
    @Test public void testIntersectMismatchedVarchar() {
        ...
        CalciteAssert.model(FOODMART_MODEL).query(sql).returns("C=12\n");
    }
    ...
    @Test public void testNotIn() {
        predicate("\"name\" not in ('a', 'b') or \"name\" is null").returns(
                "" + "empid=100; deptno=10; name=Bill; salary=10000.0; commission=1000\n"
                + "empid=200; deptno=20; name=Eric; salary=8000.0; commission=500\n"
                + "empid=150; deptno=10; name=Sebastian; salary=7000.0; commission=null\n"
                + "empid=110; deptno=10; name=Theodore; salary=11500.0; commission=250\n");
    ...
    @Test public void testNotInEmptyQuery() {
        ...
        withEmpDept(sql)
                ...
                .returnsUnordered("DEPTNO=null", "DEPTNO=10", "DEPTNO=10", "DEPTNO=20", "DEPTNO=30", "DEPTNO=30",
                                  "DEPTNO=50", "DEPTNO=50", "DEPTNO=60");
    }
    ...
    @Test public void testTrim() {
        CalciteAssert.model(FOODMART_MODEL).query(
                "select trim(\"lname\") as \"lname\" " + "from \"customer\" where \"lname\" = 'Nowmer'").returns(
                "lname=Nowmer\n");

        CalciteAssert.model(FOODMART_MODEL).query("select trim(leading 'N' from \"lname\") as \"lname\" "
                                                  + "from \"customer\" where \"lname\" = 'Nowmer'").returns(
                "lname=owmer\n");
    }

    private CalciteAssert.AssertQuery predicate(String foo) {
        return CalciteAssert.that().with(CalciteAssert.Config.REGULAR).query(
                "select * from \"hr\".\"emps\"\n" + "where " + foo).runs();
    }
    ...
    @Test public void testExistsCorrelated() {
        ...
        final String plan = "" + "LogicalProject(empid=[$0], deptno=[$1], name=[$2], salary=[$3], commission=[$4])\n"
                            ...
                            + "  EnumerableTableScan(table=[[hr, depts]])\n" + "})], variablesSet=[[$cor0]])\n"
                            + "    EnumerableTableScan(table=[[hr, emps]])\n";
        CalciteAssert.hr().query(sql).convertContains(plan).returnsUnordered(
                "empid=100; deptno=10; name=Bill; salary=10000.0; commission=1000",
                "empid=150; deptno=10; name=Sebastian; salary=7000.0; commission=null",
                "empid=110; deptno=10; name=Theodore; salary=11500.0; commission=250");
    }
    ...
    @Test public void testNotExistsCorrelated() {
        ...
        final String sql = "select * from \"hr\".\"emps\" where not exists (\n" + " select 1 from \"hr\".\"depts\"\n"
                           + " where \"emps\".\"deptno\"=\"depts\".\"deptno\")";
        CalciteAssert.hr().with("forceDecorrelate", false).query(sql).explainContains(plan).returnsUnordered(
                "empid=200; deptno=20; name=Eric; salary=8000.0; commission=500");
    }
    ...
    @Test public void testNotExistsCorrelated2() {
        ...
        final String explain = "" + "EnumerableCalc(expr#0..6=[{inputs}], proj#0..4=[{exprs}], I=[$t6])\n"
                               ...
                               + "      EnumerableAggregate(group=[{0}])\n"
                               + "        EnumerableTableScan(table=[[hr, depts]])";
        CalciteAssert.hr().query(sql).explainContains(explain).returnsUnordered(
    ...
    @Test public void testJoinInCorrelatedSubQuery() {
        CalciteAssert.hr().query(
                ...
                + "where d.\"deptno\" = d2.\"deptno\")").convertMatches(new Function<RelNode, Void>() {
    ...
    @Test public void testScalarSubQuery() {
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_EXPAND.push(true)) {
            CalciteAssert.hr().query("select \"empid\", \"deptno\",\n" + " (select \"name\" from \"hr\".\"depts\"\n"
                                     ...
                                     + "from \"hr\".\"emps\" as e").returnsUnordered(
                    "empid=100; deptno=10; DNAME=Sales", "empid=110; deptno=10; DNAME=Sales",
                    "empid=150; deptno=10; DNAME=Sales", "empid=200; deptno=20; DNAME=null");
        }
    }
    ...
    @Test public void testJoinCorrelatedScalarSubQuery() throws SQLException {
        final String sql = "select e.employee_id, d.department_id " + " from employee e, department d "
                           + " where e.department_id = d.department_id " + " and e.salary > (select avg(e2.salary) "
                           + "                 from employee e2 " + "                 where e2.store_id = e.store_id)";
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).with(Lex.JAVA).query(sql).returnsCount(599);
    }
     ...
     */
    @Ignore("[CALCITE-685]") @Test public void testCorrelatedScalarSubQuery() throws SQLException {
        ...
        final String explain = "EnumerableJoin(condition=[true], joinType=[left])\n"
                               ...
                               + "      EnumerableTableScan(table=[[foodmart2, employee]])\n";
        CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE).with(Lex.JAVA).query(sql).explainContains(
                explain).returnsCount(0);
    }

    @Test public void testLeftJoin() {
        CalciteAssert.hr().query("select e.\"deptno\", d.\"deptno\"\n" + "from \"hr\".\"emps\" as e\n"
                                 + "  left join \"hr\".\"depts\" as d using (\"deptno\")").returnsUnordered(
                "deptno=10; deptno=10", "deptno=10; deptno=10", "deptno=10; deptno=10", "deptno=20; deptno=null");
    }

    @Test public void testFullJoin() {
        CalciteAssert.hr().query("select e.\"deptno\", d.\"deptno\"\n" + "from \"hr\".\"emps\" as e\n"
                                 + "  full join \"hr\".\"depts\" as d using (\"deptno\")").returnsUnordered(
                "deptno=10; deptno=10", "deptno=10; deptno=10", "deptno=10; deptno=10", "deptno=20; deptno=null",
                "deptno=null; deptno=30", "deptno=null; deptno=40");
    }

    @Test public void testRightJoin() {
        CalciteAssert.hr().query("select e.\"deptno\", d.\"deptno\"\n" + "from \"hr\".\"emps\" as e\n"
                                 + "  right join \"hr\".\"depts\" as d using (\"deptno\")").returnsUnordered(
                "deptno=10; deptno=10", "deptno=10; deptno=10", "deptno=10; deptno=10", "deptno=null; deptno=30",
    ...
    @Test public void testVariousOuter() {
        ...
        withEmpDept(sql).returnsUnordered("ENAME=Alice; DEPTNO=30; GENDER=F; DEPTNO0=30; DNAME=Engineering",
    ...
    private CalciteAssert.AssertQuery withEmpDept(String sql) {
        ...
        return CalciteAssert.that().query(
    ...
    @Test public void testScalarSubQueryUncorrelated() {
        CalciteAssert.hr().query("select \"empid\", \"deptno\",\n" + " (select \"name\" from \"hr\".\"depts\"\n"
                                 ...
                                 + "from \"hr\".\"emps\" as e").returnsUnordered(
                "empid=100; deptno=10; DNAME=Marketing", "empid=110; deptno=10; DNAME=Marketing",
                "empid=150; deptno=10; DNAME=Marketing", "empid=200; deptno=20; DNAME=Marketing");
    }
    ...
    @Test public void testScalarSubQueryInCase() {
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_EXPAND.push(true)) {
            CalciteAssert.hr().query("select e.\"name\",\n" + " (CASE e.\"deptno\"\n"
                                     ...
                                     + "from \"hr\".\"emps\" e").returnsUnordered("name=Bill; DEPTNAME=Sales",
    ...
    @Test public void testMetaTables() {
        CalciteAssert.that().with(CalciteAssert.Config.REGULAR_PLUS_METADATA).query(
                "select * from \"metadata\".TABLES").returns(
                CalciteAssert.checkResultContains("tableSchem=metadata; tableName=COLUMNS; tableType=SYSTEM_TABLE; "));
        ...
        CalciteAssert.hr().doWithConnection(new Function<CalciteConnection, Object>() {
            ...
            public Object apply(CalciteConnection connection) {
                try {
                    ...
                    resultSet = preparedStatement.executeQuery();
                    assertEquals("deptno=10; name=Theodore\n", CalciteAssert.toString(resultSet));
    ...
    @Test public void testModel() {
        CalciteAssert.model(FOODMART_MODEL).query("select count(*) as c from \"foodmart\".\"time_by_day\"").returns(
                "C=730\n");
    }
     ...
     */
    @Ignore("until JdbcSchema can define materialized views") @Test public void testModelWithMaterializedView() {
        CalciteAssert.model(FOODMART_MODEL).enable(false).query(
                "select count(*) as c from \"foodmart\".\"sales_fact_1997\" join \"foodmart\".\"time_by_day\" using (\"time_id\")").returns(
                "C=86837\n");
        CalciteAssert.that().withMaterializations(FOODMART_MODEL, "agg_c_10_sales_fact_1997",
                                                  "select t.`month_of_year`, t.`quarter`, t.`the_year`, sum(s.`store_sales`) as `store_sales`, sum(s.`store_cost`), sum(s.`unit_sales`), count(distinct s.`customer_id`), count(*) as `fact_count` from `time_by_day` as t join `sales_fact_1997` as s using (`time_id`) group by t.`month_of_year`, t.`quarter`, t.`the_year`").query(
                "select t.\"month_of_year\", t.\"quarter\", t.\"the_year\", sum(s.\"store_sales\") as \"store_sales\", sum(s.\"store_cost\"), sum(s.\"unit_sales\"), count(distinct s.\"customer_id\"), count(*) as \"fact_count\" from \"time_by_day\" as t join \"sales_fact_1997\" as s using (\"time_id\") group by t.\"month_of_year\", t.\"quarter\", t.\"the_year\"").explainContains(
                "JdbcTableScan(table=[[foodmart, agg_c_10_sales_fact_1997]])").enableMaterializations(
                false).explainContains(
                "JdbcTableScan(table=[[foodmart, sales_fact_1997]])").sameResultWithMaterializationsDisabled();
    }
    ...
    @Test public void testModelCustomTable() {
        CalciteAssert.model("{\n" + "  version: '1.0',\n" + "   schemas: [\n" + "     {\n" + "       name: 'adhoc',\n"
                            ...
                            + "     }\n" + "   ]\n" + "}").query(
                "select * from \"adhoc\".EMPLOYEES where \"deptno\" = 10").returns(
                "" + "empid=100; deptno=10; name=Bill; salary=10000.0; commission=1000\n"
                + "empid=150; deptno=10; name=Sebastian; salary=7000.0; commission=null\n"
                + "empid=110; deptno=10; name=Theodore; salary=11500.0; commission=250\n");
    }
    ...
    private void testRangeTable(String elementType) {
        CalciteAssert.model("{\n" + "  version: '1.0',\n" + "   schemas: [\n" + "     {\n" + "       name: 'MATH',\n"
                            ...
                            + "           operand: {'column': 'N', 'start': 3, 'end': 7, " + " 'elementType': '"
                            + elementType + "'}\n" + "         }\n" + "       ]\n" + "     }\n" + "   ]\n" + "}").query(
                "select * from math.integers").returns("N=3\n" + "N=4\n" + "N=5\n" + "N=6\n");
    }
    ...
    @Test public void testModelCustomSchema() throws Exception {
        ...
        that.doWithConnection(new Function<CalciteConnection, Object>() {
        ...
        that.query("select * from \"adhoc\".ELVIS where \"deptno\" = 10").returns(
                "" + "empid=100; deptno=10; name=Bill; salary=10000.0; commission=1000\n"
                + "empid=150; deptno=10; name=Sebastian; salary=7000.0; commission=null\n"
                + "empid=110; deptno=10; name=Theodore; salary=11500.0; commission=250\n");
    ...
    private void checkCustomSchemaInFileInPwd(String fileName) throws SQLException {
        ...
        try (final PrintWriter pw = Util.printWriter(file)) {
            ...
            try (Connection c = DriverManager.getConnection(
                    url); Statement s = c.createStatement(); ResultSet r = s.executeQuery("values 1")) {
    ...
    @Test public void testCustomSchemaDirectConnection() throws Exception {
        final String url =
                "jdbc:calcite:" + "schemaFactory=" + MySchemaFactory.class.getName() + "; schema.tableName=ELVIS";
    ...
    private CalciteAssert.AssertThat modelWithView(String view, Boolean modifiable) {
        ...
        return CalciteAssert.model(
                ...
                + new JsonBuilder().toJsonString(view) + "\n" + "         }\n" + "       ]\n" + "     }\n" + "   ]\n"
                + "}");
    }
    ...
    @Test public void testModelView() throws Exception {
        ...
        with.query("select * from \"adhoc\".V order by \"name\" desc").returns(
                "" + "empid=110; deptno=10; name=Theodore; salary=11500.0; commission=250\n"
                + "empid=150; deptno=10; name=Sebastian; salary=7000.0; commission=null\n"
                + "empid=100; deptno=10; name=Bill; salary=10000.0; commission=1000\n");

        // Make sure that views appear in metadata.
        with.doWithConnection(new Function<CalciteConnection, Void>() {
            ...
            public Void apply(CalciteConnection a0) {
                try {
                    ...
                    assertEquals(
                            ...
                            CalciteAssert.toString(metaData.getTables(null, "adhoc", null, null)));

                    // views only
                    assertEquals(
                            "TABLE_CAT=null; TABLE_SCHEM=adhoc; TABLE_NAME=V; TABLE_TYPE=VIEW; REMARKS=null; TYPE_CAT=null; TYPE_SCHEM=null; TYPE_NAME=null; SELF_REFERENCING_COL_NAME=null; REF_GENERATION=null\n",
                            CalciteAssert.toString(metaData.getTables(null, "adhoc", null,
                    ...
                    assertEquals(
                            ...
                            CalciteAssert.toString(metaData.getColumns(null, "adhoc", "V", null)));

                    // catalog
                    assertEquals("TABLE_CAT=null\n", CalciteAssert.toString(metaData.getCatalogs()));

                    // schemas
                    assertEquals(
                            "TABLE_SCHEM=adhoc; TABLE_CATALOG=null\n" + "TABLE_SCHEM=metadata; TABLE_CATALOG=null\n",
                            CalciteAssert.toString(metaData.getSchemas()));

                    // schemas (qualified)
                    assertEquals("TABLE_SCHEM=adhoc; TABLE_CATALOG=null\n",
                                 CalciteAssert.toString(metaData.getSchemas(null, "adhoc")));

                    // table types
                    assertEquals("TABLE_TYPE=TABLE\n" + "TABLE_TYPE=VIEW\n",
                                 CalciteAssert.toString(metaData.getTableTypes()));
    ...
    @Test public void testOrderByView() throws Exception {
        final CalciteAssert.AssertThat with = modelWithView(
                "select * from \"EMPLOYEES\" where \"deptno\" = 10 " + "order by \"empid\" limit 2", null);
    ...
    @Test public void testSelfReferentialView() throws Exception {
        ...
        with.query("select \"name\" from \"adhoc\".V").throws_(
                "Cannot resolve 'adhoc.V'; it references view 'adhoc.V', " + "whose definition is cyclic");
    }

    @Test public void testSelfReferentialView2() throws Exception {
        final String model = "{\n" + "  version: '1.0',\n" + "  defaultSchema: 'adhoc',\n" + "  schemas: [ {\n"
                             + "    name: 'adhoc',\n" + "    tables: [ {\n" + "      name: 'A',\n"
                             + "      type: 'view',\n" + "      sql: " + new JsonBuilder().toJsonString(
                "select * from B") + "\n" + "    }, {\n" + "      name: 'B',\n" + "      type: 'view',\n"
                             + "      sql: " + new JsonBuilder().toJsonString("select * from C") + "\n" + "    }, {\n"
                             + "      name: 'C',\n" + "      type: 'view',\n" + "      sql: "
                             + new JsonBuilder().toJsonString("select * from D, B") + "\n" + "    }, {\n"
                             + "      name: 'D',\n" + "      type: 'view',\n" + "      sql: "
                             + new JsonBuilder().toJsonString("select * from (values (1, 'a')) as t(x, y)") + "\n"
                             + "    } ]\n" + "  } ]\n" + "}";
        ...
        // C is cyclic
        // D is not cyclic
        with.query("select x from \"adhoc\".a").throws_(
                "Cannot resolve 'adhoc.A'; it references view 'adhoc.B', " + "whose definition is cyclic");
        with.query("select x from \"adhoc\".b").throws_(
                "Cannot resolve 'adhoc.B'; it references view 'adhoc.B', " + "whose definition is cyclic");
        // as previous, but implicit schema
        with.query("select x from b").throws_(
                "Cannot resolve 'B'; it references view 'adhoc.B', " + "whose definition is cyclic");
        with.query("select x from \"adhoc\".c").throws_(
                "Cannot resolve 'adhoc.C'; it references view 'adhoc.C', " + "whose definition is cyclic");
        with.query("select x from \"adhoc\".d").returns("X=1\n");
        with.query("select x from \"adhoc\".d except select x from \"adhoc\".a").throws_(
                "Cannot resolve 'adhoc.A'; it references view 'adhoc.B', " + "whose definition is cyclic");
    }
    ...
    @Test public void testAutomaticTemporaryTable() throws Exception {
        ...
        CalciteAssert.that().with(new CalciteAssert.ConnectionFactory() {
            ...
            public CalciteConnection createConnection() throws SQLException {
                CalciteConnection connection = (CalciteConnection) new AutoTempDriver(objects).connect("jdbc:calcite:",
                                                                                                       new Properties());
    ...
    @Test public void testExplain() {
        final CalciteAssert.AssertThat with = CalciteAssert.that().with(CalciteAssert.Config.FOODMART_CLONE);
        ...
        final String expectedJson = "PLAN={\n" + "  \"rels\": [\n" + "    {\n" + "      \"id\": \"0\",\n"
                                    ...
                                    + "        ]\n" + "      ],\n" + "      \"inputs\": []\n" + "    }\n" + "  ]\n"
                                    + "}\n";
        with.query("explain plan as json for values (1, 'ab')").returns(expectedJson);
        with.query("explain plan with implementation for values (1, 'ab')").returns(
                "PLAN=EnumerableValues(tuples=[[{ 1, 'ab' }]])\n\n");
        with.query("explain plan without implementation for values (1, 'ab')").returns(
                "PLAN=LogicalValues(tuples=[[{ 1, 'ab' }]])\n\n");
    ...
    @Test public void testCurrentTimestamp() throws Exception {
        CalciteAssert.that().with("timezone", "GMT+1:00").doWithConnection(new Function<CalciteConnection, Void>() {
    ...
    @Test public void testGetTimestamp() throws Exception {
        CalciteAssert.that().with("timezone", "GMT+1:00")
                     // Workaround, until [CALCITE-1667] is fixed in Avatica
                     .with("TIME_ZONE", "GMT+1:00").doWithConnection(new Function<CalciteConnection, Void>() {
    ...
    @Test public void testGetDate() throws Exception {
        CalciteAssert.that().with(CalciteAssert.Config.JDBC_FOODMART).doWithConnection(
                new Function<CalciteConnection, Object>() {
                    ...
                    public Object apply(CalciteConnection conn) {
                        try {
                            ...
                            assertTrue(rs.next());
                            assertEquals(Date.valueOf("1997-01-01"), rs.getDate(1));
                            assertFalse(rs.next());
                            return null;
                        } catch (SQLException e) {
                            throw new RuntimeException(e);
    ...
    @Test public void testGetDateAsString() throws Exception {
        CalciteAssert.that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                "select min(\"date\") mindate from \"foodmart\".\"currency\"").returns2("MINDATE=1997-01-01\n");
    }
                ...
                new Function<CalciteConnection, Object>() {
                    ...
                    public Object apply(CalciteConnection conn) {
                        try {
                            ...
                            assertTrue(rs.next());
                            assertEquals(Timestamp.valueOf("1994-12-01 00:00:00"), rs.getTimestamp(1));
                            assertFalse(rs.next());
                            return null;
                        } catch (SQLException e) {
                            throw new RuntimeException(e);
    ...
    @Test public void testUnicode() throws Exception {
        ...
        with.query("select * from \"employee\" where \"full_name\" = '\u82f1\u56fd'").throws_(
                "Failed to encode '\u82f1\u56fd' in character set 'ISO-8859-1'");
        with.query("select * from \"employee\" where \"full_name\" = _UTF16'\u82f1\u56fd'").throws_(
        ...
        with.query("select * from \"employee\"\n"
                   + "where convert(\"full_name\" using UTF16) = _UTF16'\u82f1\u56fd'").throws_(
                "Column 'UTF16' not found in any table");
    }
    ...
    @Test public void testLexMySQL() throws Exception {
        CalciteAssert.that().with(Lex.MYSQL).doWithConnection(new Function<CalciteConnection, Void>() {
    ...
    @Test public void testLexMySQLANSI() throws Exception {
        CalciteAssert.that().with(Lex.MYSQL_ANSI).doWithConnection(new Function<CalciteConnection, Void>() {
    ...
    @Test public void testLexSqlServer() throws Exception {
        CalciteAssert.that().with(Lex.SQL_SERVER).doWithConnection(new Function<CalciteConnection, Void>() {
    ...
    @Test public void testLexOracle() throws Exception {
        CalciteAssert.that().with(Lex.ORACLE).doWithConnection(new Function<CalciteConnection, Void>() {
    ...
    @Test public void testLexJava() throws Exception {
        CalciteAssert.that().with(Lex.JAVA).doWithConnection(new Function<CalciteConnection, Void>() {
    ...
    @Test public void testLexOracleAsJava() throws Exception {
        CalciteAssert.that().with(Lex.ORACLE).with("quoting", "BACK_TICK").with("unquotedCasing", "UNCHANGED").with(
                "quotedCasing", "UNCHANGED").with("caseSensitive", "TRUE").doWithConnection(
    ...
    @Test public void testLexCaseInsensitive() {
        final CalciteAssert.AssertThat with = CalciteAssert.that().with(Lex.MYSQL);
        with.query("select COUNT(*) as c from metaData.tAbles").returns("c=2\n");
        with.query("select COUNT(*) as c from `metaData`.`tAbles`").returns("c=2\n");

        // case-sensitive gives error
        final CalciteAssert.AssertThat with2 = CalciteAssert.that().with(Lex.JAVA);
        with2.query("select COUNT(*) as c from `metaData`.`tAbles`").throws_(
                "Object 'metaData' not found; did you mean 'metadata'?");
        with2.query("select COUNT(*) as c from `metaData`.`TABLES`").throws_(
                "Object 'metaData' not found; did you mean 'metadata'?");
        with2.query("select COUNT(*) as c from `metaData`.`tables`").throws_(
                "Object 'metaData' not found; did you mean 'metadata'?");
        with2.query("select COUNT(*) as c from `metaData`.`nonExistent`").throws_(
                "Object 'metaData' not found; did you mean 'metadata'?");
        with2.query("select COUNT(*) as c from `metadata`.`tAbles`").throws_(
                "Object 'tAbles' not found within 'metadata'; did you mean 'TABLES'?");
    }
    ...
    @Test public void testLexCaseInsensitiveTableAlias() {
        CalciteAssert.that().with(Lex.MYSQL).query("select e.empno\n" + "from (values (1, 2)) as E (empno, deptno),\n"
                                                   + "  (values (3, 4)) as d (deptno, name)").returnsUnordered(
                "empno=1");
    }

    @Test public void testFunOracle() {
        CalciteAssert.that(CalciteAssert.Config.REGULAR).with("fun", "oracle").query(
                "select nvl(\"commission\", -99) as c from \"hr\".\"emps\"").returnsUnordered("C=-99", "C=1000",
                                                                                              "C=250", "C=500");

        // NVL is not present in the default operator table
        CalciteAssert.that(CalciteAssert.Config.REGULAR).query(
                "select nvl(\"commission\", -99) as c from \"hr\".\"emps\"").throws_(
                "No match found for function signature NVL(<NUMERIC>, <NUMERIC>)");
    }
    ...
    @Test public void testHook() {
        ...
        try (Hook.Closeable hook = Hook.PARSE_TREE.addThread(new Function<Object[], Object>() {
    ...
    @Test public void testDialect() {
        ...
        CalciteAssert.that().with(CalciteAssert.Config.JDBC_FOODMART).query(
                ...
                + "  and \"gender\" = 'F'").withHook(Hook.QUERY_PLAN, new Function<String, Void>() {
    ...
    @Test public void testSimpleCalciteSchemaWithView() throws Exception {
        ...
        final SchemaPlus aSchema = rootSchema.add("a", new AbstractSchema() {
            ...
            @Override protected Multimap<String, org.apache.calcite.schema.Function> getFunctionMultimap() {
                return functionMap;
            }
        });
        // add view definition
        final String viewName = "V";
        final org.apache.calcite.schema.Function view = ViewTable.viewMacro(rootSchema.getSubSchema("a"),
    ...
    @Test public void testCaseSensitiveSubQueryOracle() {
        final CalciteAssert.AssertThat with = CalciteAssert.that().with(Lex.ORACLE);
    ...
    @Test public void testUnquotedCaseSensitiveSubQuerySqlServer() {
        CalciteAssert.that().with(Lex.SQL_SERVER).query("select DID from (select deptid as did FROM\n"
                                                        + "     ( values (1), (2) ) as T1(deptid) ) ").returnsUnordered(
                "DID=1", "DID=2");
    }
    ...
    public static class HrSchema {
        ...
        public final Department[] depts      = {
                ...
                new Department(30, "Marketing", Collections.<Employee>emptyList(), new Location(0, 52)),
    ...
    public static class Department {
        ...
        @org.apache.calcite.adapter.java.Array(component = Employee.class) public final List<Employee> employees;
    ...
    public static class Dependent {
        ...
        @Override public boolean equals(Object obj) {
            return obj == this || obj instanceof Dependent && empid == ((Dependent) obj).empid && Objects.equals(name,
                                                                                                                 ((Dependent) obj).name);
        }
    }
    ...
    public static class AutoTempDriver extends org.apache.calcite.jdbc.Driver {
        ...
        @Override protected Handler createHandler() {
            return new HandlerImpl() {
                ...
                @Override public void onStatementExecute(AvaticaStatement statement, ResultSink resultSink) {
                ...
                @Override public CalcitePrepare apply() {
                    return new CalcitePrepareImpl() {
                        ...
                        @Override protected SqlParser.ConfigBuilder createParserConfig() {
                            return super.createParserConfig().setParserFactory(new SqlParserImplFactory() {
