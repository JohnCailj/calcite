...
public class LatticeTest {
    ...
    private CalciteAssert.AssertThat modelWithLattice(String name, String sql, String... extras) {
        final StringBuilder buf = new StringBuilder("{ name: '").append(name).append("', sql: ").append(
                TestUtil.escapeString(sql));
    ...
    @Test public void testLatticeWithSameNameAsTable() {
        modelWithLattice("EMPLOYEES", "select * from \"foodmart\".\"days\"").query(
                "select count(*) from EMPLOYEES").returnsValue("4");
    }
    ...
    @Test public void testTwoLatticesWithSameNameFails() {
        modelWithLattices("{name: 'Lattice1', sql: 'select * from \"foodmart\".\"days\"'}",
                          "{name: 'Lattice1', sql: 'select * from \"foodmart\".\"time_by_day\"'}").connectThrows(
                "Duplicate lattice 'Lattice1'");
    }
    ...
    @Test public void testLatticeInvalidSqlFails() {
        modelWithLattice("star", "select foo from nonexistent").connectThrows(
                "Error instantiating JsonLattice(name=star, ").connectThrows("Object 'NONEXISTENT' not found");
    }
    ...
    @Test public void testLatticeSqlWithGroupByFails() {
        modelWithLattice("star",
                         "select 1 from \"foodmart\".\"sales_fact_1997\" as s group by \"product_id\"").connectThrows(
                "Invalid node type LogicalAggregate in lattice query");
    }
    ...
    @Test public void testLatticeSqlWithOrderByFails() {
        modelWithLattice("star",
                         "select 1 from \"foodmart\".\"sales_fact_1997\" as s order by \"product_id\"").connectThrows(
                "Invalid node type LogicalSort in lattice query");
    }
    ...
    @Test public void testLatticeSqlWithUnionFails() {
        modelWithLattice("star", "select 1 from \"foodmart\".\"sales_fact_1997\" as s\n" + "union all\n"
                                 + "select 1 from \"foodmart\".\"sales_fact_1997\" as s").connectThrows(
                "Invalid node type LogicalUnion in lattice query");
    }
    ...
    @Test public void testLatticeSqlWithJoin() {
        foodmartModel().query("values 1").returnsValue("1");
    }
    ...
    @Test public void testLatticeInvalidSql() {
        modelWithLattice("star", "select 1 from \"foodmart\".\"sales_fact_1997\" as s\n"
                                 + "join \"foodmart\".\"product\" as p using (\"product_id\")\n"
                                 + "join \"foodmart\".\"time_by_day\" as t on s.\"product_id\" = 100").connectThrows(
                "only equi-join of columns allowed: 100");
    }
    ...
    @Test public void testLatticeInvalidSql2() {
        modelWithLattice("star", "select 1 from \"foodmart\".\"sales_fact_1997\" as s\n"
                ...
                "only inner join allowed, but got LEFT");
    }
    ...
    @Test public void testLatticeStarTable() {
        ...
        try {
            foodmartModel().query("select count(*) from \"adhoc\".\"star\"").convertMatches(CalciteAssert.checkRel(
                    "" + "LogicalAggregate(group=[{}], EXPR$0=[COUNT()])\n" + "  LogicalProject(DUMMY=[0])\n"
                    + "    StarTableScan(table=[[adhoc, star]])\n", counter));
        } catch (RuntimeException e) {
            assertThat(Throwables.getStackTraceAsString(e), containsString("CannotPlanException"));
        }
    ...
    @Test public void testLatticeRecognizeJoin() {
        ...
        foodmartModel().query(
                ...
                true).substitutionMatches(CalciteAssert.checkRel("LogicalProject(unit_sales=[$7], brand_name=[$10])\n"
    ...
    @Test public void testLatticeRecognizeGroupJoin() {
        final AtomicInteger counter = new AtomicInteger();
        CalciteAssert.AssertQuery that = foodmartModel().query(
                "select distinct p.\"brand_name\", s.\"customer_id\"\n" + "from \"foodmart\".\"sales_fact_1997\" as s\n"
                + "join \"foodmart\".\"product\" as p using (\"product_id\")\n").enableMaterializations(
                true).substitutionMatches(new Function<RelNode, Void>() {

            public Void apply(RelNode relNode) {
                counter.incrementAndGet();
                String s = Util.toLinux(RelOptUtil.toString(relNode));
                assertThat(s, anyOf(containsString(
        ...
        // materialization.
        that.withHook(Hook.CREATE_MATERIALIZATION, new Function<String, Void>() {
    ...
    @Test public void testLatticeWithPreDefinedTiles() {
        foodmartModel(
                ...
                + "join \"foodmart\".\"time_by_day\" as t using (\"time_id\")\n").enableMaterializations(
                true).explainContains("EnumerableTableScan(table=[[adhoc, m{27, 31}").returnsCount(4);
    }
    ...
    @Test public void testLatticeWithPreDefinedTilesFewerMeasures() {
        foodmartModelWithOneTile().query(
                "select t.\"the_year\", t.\"quarter\", count(*) as c\n" + "from \"foodmart\".\"sales_fact_1997\" as s\n"
                + "join \"foodmart\".\"time_by_day\" as t using (\"time_id\")\n"
                + "group by t.\"the_year\", t.\"quarter\"").enableMaterializations(true).explainContains(
                "" + "EnumerableCalc(expr#0..4=[{inputs}], proj#0..2=[{exprs}])\n"
                + "  EnumerableTableScan(table=[[adhoc, m{27, 31}").returnsUnordered(
                "the_year=1997; quarter=Q1; C=21588", "the_year=1997; quarter=Q2; C=20368",
                "the_year=1997; quarter=Q3; C=21453",
                "the_year=1997; quarter=Q4; C=23428").sameResultWithMaterializationsDisabled();
    }
    ...
    @Test public void testLatticeWithPreDefinedTilesRollUp() {
        foodmartModelWithOneTile().query(
                ...
                + "join \"foodmart\".\"time_by_day\" as t using (\"time_id\")\n"
                + "group by t.\"the_year\"").enableMaterializations(true).explainContains(""
                                                                                          ...
                                                                                          + "    EnumerableTableScan(table=[[adhoc, m{27, 31}").enable(
                CalciteAssert.DB != CalciteAssert.DatabaseInstance.ORACLE).returnsUnordered(
                "the_year=1997; C=86837; Q=Q1; US=2667730.0000").sameResultWithMaterializationsDisabled();
    }
    ...
    @Test public void testTileAlgorithm2() {
        ...
        // (27, 31).
        checkTileAlgorithm(Lattices.class.getCanonicalName() + "#CACHED_SQL", "EnumerableAggregate(group=[{0, 1}])\n"
    ...
    private void checkTileAlgorithm(String statisticProvider, String expectedExplain) {
        ...
        foodmartModel(" auto: false,\n" + "  algorithm: true,\n" + "  algorithmMaxMillis: -1,\n"
                ...
                "select distinct t.\"the_year\", t.\"quarter\"\n" + "from \"foodmart\".\"sales_fact_1997\" as s\n"
                + "join \"foodmart\".\"time_by_day\" as t using (\"time_id\")\n").enableMaterializations(
                true).explainContains(expectedExplain).returnsUnordered("the_year=1997; quarter=Q1",
                                                                        "the_year=1997; quarter=Q2",
                                                                        "the_year=1997; quarter=Q3",
                                                                        "the_year=1997; quarter=Q4").returnsCount(4);
    }
    ...
    @Test public void testJG() {
        ...
        CalciteAssert.that().with(CalciteAssert.Config.JDBC_FOODMART).query(sql).explainContains(explain);
    }
    ...
    @Test public void testGroupByEmpty() {
        foodmartModel().query("select count(*) as c from \"foodmart\".\"sales_fact_1997\"").enableMaterializations(
                true).returnsUnordered("C=86837");
    }
    ...
    @Test public void testGroupByEmpty2() {
        foodmartModel().query(
                "select sum(\"unit_sales\") as s\n" + "from \"foodmart\".\"sales_fact_1997\"").enableMaterializations(
                true).enable(CalciteAssert.DB != CalciteAssert.DatabaseInstance.ORACLE).returnsUnordered(
                "S=266773.0000");
    }
    ...
    @Test public void testGroupByEmpty3() {
        ...
        final Function<String, Void> handler = new Function<String, Void>() {
        ...
        that.query(
                "select sum(\"unit_sales\") as s, count(*) as c\n" + "from \"foodmart\".\"sales_fact_1997\"").withHook(
                Hook.CREATE_MATERIALIZATION, handler).enableMaterializations(true).explainContains(
                "EnumerableTableScan(table=[[adhoc, m{}]])").enable(
                CalciteAssert.DB != CalciteAssert.DatabaseInstance.ORACLE).returnsUnordered("S=266773.0000; C=86837");
        ...
        that.query("select sum(\"unit_sales\") as s\n" + "from \"foodmart\".\"sales_fact_1997\"").withHook(
                Hook.CREATE_MATERIALIZATION, handler).enableMaterializations(true).enable(
                CalciteAssert.DB != CalciteAssert.DatabaseInstance.ORACLE).returnsUnordered("S=266773.0000");
        assertThat(mats.toString(), mats.size(), equalTo(2));
    }
    ...
    @Test public void testSum() {
        foodmartModelWithOneTile().query("select sum(\"unit_sales\") as c\n" + "from \"foodmart\".\"sales_fact_1997\"\n"
                                         + "group by \"product_id\"\n"
                                         + "order by 1 desc limit 1").enableMaterializations(true).enable(
                CalciteAssert.DB != CalciteAssert.DatabaseInstance.ORACLE).returnsUnordered("C=267.0000");
    }
    ...
    @Test public void testDistinctCount() {
        foodmartModelWithOneTile().query(
                ...
                + "group by \"the_year\"").enableMaterializations(true).explainContains(
                "EnumerableCalc(expr#0..1=[{inputs}], C=[$t1])\n"
                + "  EnumerableAggregate(group=[{0}], C=[COUNT($1)])\n"
                + "    EnumerableTableScan(table=[[adhoc, m{27, 31}]])").returnsUnordered("C=4");
    }
    ...
    @Test public void testDistinctCount2() {
        foodmartModelWithOneTile().query(
                ...
                + "      EnumerableTableScan(table=[[adhoc, m{27, 31}]])").returnsUnordered("C=1");
    }
     ...
     */
    @Ignore @Test public void testAllFoodmartQueries() throws IOException {
        // Test ids that had bugs in them until recently. Useful for a sanity check.
        final List<Integer> fixed = ImmutableList.of(13, 24, 28, 30, 61, 76, 79, 81, 85, 98, 101, 107, 128, 129, 130,
                                                     131);
    ...
    private void check(int n) throws IOException {
        final FoodmartTest.FoodmartQuery query = FoodmartTest.FoodMartQuerySet.instance().queries.get(n);
        if (query == null) {
            return;
        }
        foodmartModelWithOneTile().withDefaultSchema("foodmart").query(
                query.sql).sameResultWithMaterializationsDisabled();
    }
    ...
    @Test public void testTwoLattices() {
        final AtomicInteger counter = new AtomicInteger();
        modelWithLattices(SALES_LATTICE, INVENTORY_LATTICE).query(
                "select s.\"unit_sales\", p.\"brand_name\"\n" + "from \"foodmart\".\"sales_fact_1997\" as s\n"
                + "join \"foodmart\".\"product\" as p using (\"product_id\")\n").enableMaterializations(
                true).substitutionMatches(CalciteAssert.checkRel("LogicalProject(unit_sales=[$7], brand_name=[$10])\n"
                                                                 + "  LogicalProject(product_id=[$0], time_id=[$1], customer_id=[$2], promotion_id=[$3], store_id=[$4], store_sales=[$5], store_cost=[$6], unit_sales=[$7], product_class_id=[$8], product_id0=[$9], brand_name=[$10], product_name=[$11], SKU=[$12], SRP=[$13], gross_weight=[$14], net_weight=[$15], recyclable_package=[$16], low_fat=[$17], units_per_case=[$18], cases_per_pallet=[$19], shelf_width=[$20], shelf_height=[$21], shelf_depth=[$22])\n"
    ...
    @Test public void testOneLatticeOneMV() {
        ...
        CalciteAssert.model(model).withDefaultSchema("foodmart").query(
                "select * from \"foodmart\".\"sales_fact_1997\" where \"product_id\" = 10").enableMaterializations(
                true).substitutionMatches(CalciteAssert.checkRel("EnumerableTableScan(table=[[mat, m0]])\n", counter));
        assertThat(counter.intValue(), equalTo(1));
    }
     ...
     */
    @Ignore @Test public void testLatticeWithBadRowCountEstimate() {
        final String lattice = INVENTORY_LATTICE.replace("rowCountEstimate: 4070,", "rowCountEstimate: 4074070,");
    ...
    private static void runJdbc() throws SQLException {
        ...
        final ResultSet resultSet = connection.createStatement().executeQuery("select * from \"adhoc\".\"m{27, 31}\"");
