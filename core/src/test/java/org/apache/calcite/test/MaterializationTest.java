...
public class MaterializationTest {
    ...
    private static final Function<ResultSet, Void> CONTAINS_M0 = CalciteAssert.checkResultContains(
            "EnumerableTableScan(table=[[hr, m0]])");

    private static final Function<ResultSet, Void> CONTAINS_LOCATIONS = CalciteAssert.checkResultContains(
            "EnumerableTableScan(table=[[hr, locations]])");

    private static final Ordering<Iterable<String>> CASE_INSENSITIVE_LIST_COMPARATOR = Ordering.from(
            String.CASE_INSENSITIVE_ORDER).lexicographical();
    ...
    private final RexSimplify         simplify    = new RexSimplify(rexBuilder, false, RexUtil.EXECUTOR);
    ...
    @Test public void testScan() {
        CalciteAssert.that().withMaterializations(
                ...
                + JdbcTest.SCOTT.url + "',\n" + "      jdbcSchema: 'SCOTT'\n" + "   } } ]\n" + "}", "m0",
                "select empno, deptno from emp order by deptno").query(
                "select empno, deptno from emp").enableMaterializations(true).explainContains(
                "EnumerableTableScan(table=[[SCOTT_CLONE, m0]])").sameResultWithMaterializationsDisabled();
    }

    @Test public void testFilter() {
        CalciteAssert.that().withMaterializations(HR_FKUK_MODEL, "m0",
                                                  "select * from \"emps\" where \"deptno\" = 10").query(
                "select \"empid\" + 1 from \"emps\" where \"deptno\" = 10").enableMaterializations(
                true).explainContains("EnumerableTableScan(table=[[hr, m0]])").sameResultWithMaterializationsDisabled();
    }
    ...
    @Test public void testFilterQueryOnProjectView() {
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            ...
            CalciteAssert.that().withMaterializations(HR_FKUK_MODEL, "m0",
                                                      "select \"deptno\", \"empid\" from \"emps\"").query(
                    "select \"empid\" + 1 as x from \"emps\" where \"deptno\" = 10").enableMaterializations(
                    true).explainContains(
                    "EnumerableTableScan(table=[[hr, m0]])").sameResultWithMaterializationsDisabled();
        }
    }
    ...
    private void checkMaterialize(String materialize, String query, String model,
        ...
        checkMaterialize(materialize, query, model, explainChecker, RuleSets.ofList(ImmutableList.<RelOptRule>of()));
    }
     ...
     * definition.
     */
    private CalciteAssert.AssertQuery checkThatMaterialize(String materialize, String query, String name,
                                                           boolean existing, String model,
                                                           Function<ResultSet, Void> explainChecker,
                                                           final RuleSet rules) {
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            MaterializationService.setThreadLocal();
            CalciteAssert.AssertQuery that = CalciteAssert.that().withMaterializations(model, existing, name,
                                                                                       materialize).query(
                    query).enableMaterializations(true);
    ...
    private void checkNoMaterialize(String materialize, String query, String model) {
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            MaterializationService.setThreadLocal();
            CalciteAssert.that().withMaterializations(model, "m0", materialize).query(query).enableMaterializations(
                    true).explainContains("EnumerableTableScan(table=[[hr, emps]])");
        }
    }
    ...
    @Test public void testFilterQueryOnProjectView4() {
        checkNoMaterialize("select \"deptno\" - 10 as \"x\", \"empid\" + 1, \"name\" from \"emps\"",
                           "select \"name\" from \"emps\" where \"deptno\" + 10 = 20", HR_FKUK_MODEL);
    }
    ...
    @Test public void testFilterQueryOnProjectView5() {
        checkMaterialize("select \"deptno\" - 10 as \"x\", \"empid\" + 1 as ee, \"name\"\n" + "from \"emps\"",
                         ...
                         HR_FKUK_MODEL, CalciteAssert.checkResultContains(
    ...
    @Test public void testFilterQueryOnProjectView6() {
        checkNoMaterialize("select \"deptno\" - 10 as \"x\", \"empid\"  from \"emps\"",
                           "select \"name\" from \"emps\" where \"deptno\" - 10 = 0", HR_FKUK_MODEL);
    }
    ...
    @Test public void testFilterQueryOnProjectView7() {
        checkNoMaterialize("select \"deptno\" - 10 as \"x\", \"empid\" + 1, \"name\" from \"emps\"",
                           "select \"name\", \"empid\" + 2 from \"emps\" where \"deptno\" - 10 = 0", HR_FKUK_MODEL);
    }
    ...
    @Test public void testFilterQueryOnProjectView8() {
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            ...
            final String model =
                    ...
                    + "      }\n" + "    }\n" + "  ]\n" + "}\n";
            CalciteAssert.that().withModel(model).query(q).enableMaterializations(true).explainMatches("",
                                                                                                       CONTAINS_M0).sameResultWithMaterializationsDisabled();
        }
    }
     ...
     * query.
     */
    @Ignore @Test public void testFilterQueryOnFilterView2() {
        checkMaterialize("select \"deptno\", \"empid\", \"name\" from \"emps\" where \"deptno\" = 10",
     ...
     * view.
     */
    @Ignore("not implemented") @Test public void testFilterQueryOnFilterView3() {
        checkMaterialize("select \"deptno\", \"empid\", \"name\" from \"emps\" "
                         + "where \"deptno\" = 10 or \"deptno\" = 20 or \"empid\" < 160",
                         "select \"empid\" + 1 as x, \"name\" from \"emps\" where \"deptno\" = 10", HR_FKUK_MODEL,
    ...
    @Test public void testFilterQueryOnFilterView7() {
        checkMaterialize("select * from \"emps\" where " + "((\"salary\" < 1111.9 and \"deptno\" > 10)"
    ...
    @Test public void testFilterQueryOnFilterView8() {
        checkNoMaterialize("select \"name\", \"deptno\" from \"emps\" where \"deptno\" > 10",
                           "select \"name\", \"empid\" from \"emps\" where \"deptno\" > 30", HR_FKUK_MODEL);
    }
    ...
    @Test public void testFilterQueryOnFilterView10() {
        checkNoMaterialize(
                ...
                "select \"name\", \"empid\" from \"emps\" where \"deptno\" > 30 " + "or \"empid\" > 10", HR_FKUK_MODEL);
    }
    ...
    @Test public void testFilterQueryOnFilterView11() {
        checkNoMaterialize(
                ...
                "select \"name\" from \"emps\" where \"deptno\" > 30 and \"salary\" > 3000", HR_FKUK_MODEL);
    }
    ...
    @Test public void testFilterQueryOnFilterView13() {
        checkNoMaterialize("select * from \"emps\" where " + "(\"salary\" < 1111.9 and \"deptno\" > 10)"
                           + "or (\"empid\" > 400 and \"salary\" > 5000)",
                           "select \"name\" from \"emps\" where \"salary\" > 1000 "
                           + "or (\"deptno\" > 30 and \"salary\" > 3000)", HR_FKUK_MODEL);
    }
    ...
    @Test public void testFilterQueryOnFilterView14() {
        ...
        String m = "select \"deptno\", \"empid\", \"name\", \"salary\", \"commission\" " + "from \"emps\" as em where "
                   + "((\"salary\" < 1111.9 and \"deptno\" > 10)" + "or (\"empid\" > 400 and \"salary\" > 5000) "
                   + "or \"salary\" > 500)";
    ...
    @Test public void testAlias() {
        checkMaterialize("select * from \"emps\" as em where " + "(em.\"salary\" < 1111.9 and em.\"deptno\" > 10)"
    ...
    @Test public void testAggregateRollUp() {
        checkMaterialize("select \"empid\", \"deptno\", count(*) as c, sum(\"empid\") as s from \"emps\" "
                         + "group by \"empid\", \"deptno\"",
                         "select count(*) + 1 as c, \"deptno\" from \"emps\" group by \"deptno\"", HR_FKUK_MODEL,
     ...
     */
    @Ignore("work in progress") @Test public void testAggregateProject() {
    ...
    @Ignore @Test public void testSwapJoin() {
    ...
    @Ignore @Test public void testOrderByQueryOnProjectView() {
        checkMaterialize("select \"deptno\", \"empid\" from \"emps\"",
                         "select \"empid\" from \"emps\" order by \"deptno\"");
    }

    @Ignore @Test public void testOrderByQueryOnOrderByView() {
        checkMaterialize("select \"deptno\", \"empid\" from \"emps\" order by \"deptno\"",
                         "select \"empid\" from \"emps\" order by \"deptno\"");
    }

    @Ignore @Test public void testDifferentColumnNames() {
    }

    @Ignore @Test public void testDifferentType() {
    }

    @Ignore @Test public void testPartialUnion() {
    }

    @Ignore @Test public void testNonDisjointUnion() {
    }

    @Ignore @Test public void testMaterializationReferencesTableInOtherSchema() {
    }
    ...
    @Test public void testSatisfiable() {
        ...
        final RexNode e4 = rexBuilder.makeCall(SqlStdOperatorTable.AND, i0_eq_0,
                                               rexBuilder.makeCall(SqlStdOperatorTable.AND, i1_eq_1,
        ...
        final RexNode e6 = rexBuilder.makeCall(SqlStdOperatorTable.AND, i0_eq_0,
                                               rexBuilder.makeCall(SqlStdOperatorTable.NOT,
                                                                   rexBuilder.makeCall(SqlStdOperatorTable.AND, i0_eq_0,
                                                                                       i1_eq_1)));
        checkSatisfiable(e6, "AND(=($0, 0), OR(<>($0, 0), <>($1, 1)))");

        // "$0 = 0 AND ($1 = 1 AND NOT ($0 = 0))" is not satisfiable.
        final RexNode e7 = rexBuilder.makeCall(SqlStdOperatorTable.AND, i0_eq_0,
                                               rexBuilder.makeCall(SqlStdOperatorTable.AND, i1_eq_1,
                                                                   rexBuilder.makeCall(SqlStdOperatorTable.NOT,
                                                                                       i0_eq_0)));
    ...
    @Test public void testSplitFilter() {
        ...
        final RexNode x_eq_2 = rexBuilder.makeCall(SqlStdOperatorTable.EQUALS, x, i2); // $0 = 2
        ...
        // yields
        //   residue:   x = 1
        newFilter = SubstitutionVisitor.splitFilter(simplify, x_eq_1,
     ...
     * </ol>
     */
    @Ignore @Test public void testFilterGroupQueryOnStar() {
     ...
     */
    @Ignore @Test public void testQueryOnStar() {
        String q = "select *\n" + "from \"foodmart\".\"sales_fact_1997\" as f\n"
    ...
    @Test public void testUnionAll() {
        ...
        checkMaterialize(m, q, HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains("EnumerableTableScan(table=[[hr, m0]])", 1));
    }

    @Test public void testAggregateMaterializationNoAggregateFuncs1() {
        checkMaterialize("select \"empid\", \"deptno\" from \"emps\" group by \"empid\", \"deptno\"",
                         "select \"empid\", \"deptno\" from \"emps\" group by \"empid\", \"deptno\"", HR_FKUK_MODEL,
    ...
    @Test public void testAggregateMaterializationNoAggregateFuncs2() {
        checkMaterialize("select \"empid\", \"deptno\" from \"emps\" group by \"empid\", \"deptno\"",
                         "select \"deptno\" from \"emps\" group by \"deptno\"", HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains(
                                 "EnumerableAggregate(group=[{1}])\n" + "  EnumerableTableScan(table=[[hr, m0]])"));
    }

    @Test public void testAggregateMaterializationNoAggregateFuncs3() {
        checkNoMaterialize("select \"deptno\" from \"emps\" group by \"deptno\"",
                           "select \"empid\", \"deptno\" from \"emps\" group by \"empid\", \"deptno\"", HR_FKUK_MODEL);
    }
    ...
    @Test public void testAggregateMaterializationNoAggregateFuncs4() {
        checkMaterialize(
                ...
                "select \"deptno\" from \"emps\" where \"deptno\" = 10 group by \"deptno\"", HR_FKUK_MODEL,
                CalciteAssert.checkResultContains(
                        "EnumerableAggregate(group=[{1}])\n" + "  EnumerableTableScan(table=[[hr, m0]])"));
    }
    ...
    @Test public void testAggregateMaterializationNoAggregateFuncs6() {
        checkMaterialize(
                "select \"empid\", \"deptno\" from \"emps\" where \"deptno\" > 5 group by \"empid\", \"deptno\"",
                "select \"deptno\" from \"emps\" where \"deptno\" > 10 group by \"deptno\"", HR_FKUK_MODEL,
                CalciteAssert.checkResultContains("EnumerableAggregate(group=[{1}])\n"
    ...
    @Test public void testAggregateMaterializationNoAggregateFuncs8() {
        checkNoMaterialize("select \"empid\" from \"emps\" group by \"empid\", \"deptno\"",
                           "select \"deptno\" from \"emps\" group by \"deptno\"", HR_FKUK_MODEL);
    }
    ...
    @Test public void testAggregateMaterializationAggregateFuncs2() {
        checkMaterialize("select \"empid\", \"deptno\", count(*) as c, sum(\"empid\") as s\n"
                         ...
                         + "from \"emps\" group by \"deptno\"", HR_FKUK_MODEL, CalciteAssert.checkResultContains(
    ...
    @Test public void testAggregateMaterializationAggregateFuncs3() {
        checkMaterialize("select \"empid\", \"deptno\", count(*) as c, sum(\"empid\") as s\n"
                         ...
                         + "from \"emps\" group by \"empid\", \"deptno\"", HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains(
    ...
    @Test public void testAggregateMaterializationAggregateFuncs4() {
        checkMaterialize("select \"empid\", \"deptno\", count(*) as c, sum(\"empid\") as s\n"
                         ...
                         CalciteAssert.checkResultContains("EnumerableAggregate(group=[{1}], S=[$SUM0($3)])\n"
    ...
    @Ignore @Test public void testAggregateMaterializationAggregateFuncs8() {
    ...
    @Test public void testJoinAggregateMaterializationNoAggregateFuncs5() {
        checkMaterialize("select \"depts\".\"deptno\", \"emps\".\"empid\" from \"depts\"\n"
                         ...
                         + "group by \"depts\".\"deptno\", \"emps\".\"empid\"", HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains(
    ...
    @Test public void testJoinAggregateMaterializationNoAggregateFuncs6() {
        checkMaterialize("select \"depts\".\"deptno\", \"emps\".\"empid\" from \"depts\"\n"
                         ...
                         "select \"depts\".\"deptno\" from \"depts\"\n"
                         + "join \"emps\" using (\"deptno\") where \"emps\".\"empid\" > 15\n"
                         + "group by \"depts\".\"deptno\"", HR_FKUK_MODEL, CalciteAssert.checkResultContains(
    ...
    @Test public void testJoinAggregateMaterializationNoAggregateFuncs7() {
        checkMaterialize("select \"depts\".\"deptno\", \"dependents\".\"empid\"\n" + "from \"depts\"\n"
                         ...
                         CalciteAssert.checkResultContains("EnumerableAggregate(group=[{0}])",
    ...
    @Test public void testJoinAggregateMaterializationNoAggregateFuncs10() {
        checkMaterialize("select \"depts\".\"name\", \"dependents\".\"name\" as \"name2\", "
                         + "\"emps\".\"deptno\", \"depts\".\"deptno\" as \"deptno2\", " + "\"dependents\".\"empid\"\n"
                         + "from \"depts\", \"dependents\", \"emps\"\n" + "where \"depts\".\"deptno\" > 10\n"
                         + "group by \"depts\".\"name\", \"dependents\".\"name\", "
                         + "\"emps\".\"deptno\", \"depts\".\"deptno\", " + "\"dependents\".\"empid\"",
                         "select \"dependents\".\"empid\"\n" + "from \"depts\"\n"
                         + "join \"dependents\" on (\"depts\".\"name\" = \"dependents\".\"name\")\n"
                         + "join \"emps\" on (\"emps\".\"deptno\" = \"depts\".\"deptno\")\n"
                         + "where \"depts\".\"deptno\" > 10\n" + "group by \"dependents\".\"empid\"", HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains("EnumerableAggregate(group=[{4}])\n"
    ...
    @Test public void testJoinAggregateMaterializationAggregateFuncs4() {
        checkMaterialize("select \"empid\", \"emps\".\"deptno\", count(*) as c, sum(\"empid\") as s\n"
                         ...
                         + "from \"emps\" join \"depts\" using (\"deptno\")\n"
                         + "where \"emps\".\"deptno\" > 10 group by \"depts\".\"deptno\"", HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains("EnumerableAggregate(group=[{1}], S=[$SUM0($3)])\n"
                                                           + "  EnumerableCalc(expr#0..3=[{inputs}], expr#4=[10], expr#5=[>($t1, $t4)], "
                                                           + "proj#0..3=[{exprs}], $condition=[$t5])\n"
                                                           + "    EnumerableTableScan(table=[[hr, m0]])"));
    ...
    @Test public void testJoinAggregateMaterializationAggregateFuncs5() {
        checkMaterialize("select \"empid\", \"depts\".\"deptno\", count(*) + 1 as c, sum(\"empid\") as s\n"
                         ...
                         + "from \"emps\" join \"depts\" using (\"deptno\")\n"
                         + "where \"depts\".\"deptno\" > 10 group by \"depts\".\"deptno\"", HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains(
                                 "EnumerableCalc(expr#0..1=[{inputs}], expr#2=[1], expr#3=[+($t1, $t2)], "
                                 + "deptno=[$t0], S=[$t3])\n"
                                 + "  EnumerableAggregate(group=[{1}], agg#0=[$SUM0($3)])\n"
    ...
    @Ignore @Test public void testJoinAggregateMaterializationAggregateFuncs6() {
    ...
    @Test public void testJoinAggregateMaterializationAggregateFuncs7() {
        checkMaterialize(
                ...
                + "join \"depts\" on (\"emps\".\"deptno\" = \"depts\".\"deptno\")\n"
                + "join \"dependents\" on (\"emps\".\"empid\" = \"dependents\".\"empid\")\n"
                + "group by \"dependents\".\"empid\"", HR_FKUK_MODEL, CalciteAssert.checkResultContains(
    ...
    @Test public void testJoinAggregateMaterializationAggregateFuncs8() {
        checkMaterialize(
                ...
                + "join \"depts\" on (\"emps\".\"deptno\" = \"depts\".\"deptno\")\n"
                + "join \"dependents\" on (\"emps\".\"empid\" = \"dependents\".\"empid\")\n"
                + "group by \"depts\".\"name\"", HR_FKUK_MODEL, CalciteAssert.checkResultContains(
    ...
    @Test public void testJoinAggregateMaterializationAggregateFuncs11() {
        checkMaterialize("select \"depts\".\"deptno\", \"dependents\".\"empid\", count(\"emps\".\"salary\") as s\n"
                        ...
                        "EnumerableUnion(all=[true])", "EnumerableAggregate(group=[{2}], agg#0=[COUNT()])",
                        "EnumerableAggregate(group=[{1}], agg#0=[$SUM0($2)])", "EnumerableTableScan(table=[[hr, m0]])",
                        "expr#13=[OR($t10, $t12)], expr#14=[AND($t6, $t8, $t13)]"));
    }
    ...
    @Test public void testJoinMaterialization4() {
        checkMaterialize("select \"empid\" \"deptno\" from \"emps\"\n" + "join \"depts\" using (\"deptno\")",
                         "select \"empid\" \"deptno\" from \"emps\"\n"
                         + "join \"depts\" using (\"deptno\") where \"empid\" = 1", HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains(
    ...
    @Test public void testJoinMaterialization7() {
        checkMaterialize("select \"depts\".\"name\"\n" + "from \"emps\"\n"
                         ...
                         "select \"dependents\".\"empid\"\n" + "from \"emps\"\n"
                         + "join \"depts\" on (\"emps\".\"deptno\" = \"depts\".\"deptno\")\n"
                         + "join \"dependents\" on (\"depts\".\"name\" = \"dependents\".\"name\")", HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains("EnumerableCalc(expr#0..2=[{inputs}], empid0=[$t1])\n"
    ...
    @Test public void testJoinMaterialization8() {
        checkMaterialize("select \"depts\".\"name\"\n" + "from \"emps\"\n"
                         ...
                         "select \"dependents\".\"empid\"\n" + "from \"depts\"\n"
                         + "join \"dependents\" on (\"depts\".\"name\" = \"dependents\".\"name\")\n"
                         + "join \"emps\" on (\"emps\".\"deptno\" = \"depts\".\"deptno\")", HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains("EnumerableCalc(expr#0..4=[{inputs}], empid=[$t2])\n"
    ...
    @Test public void testJoinMaterialization10() {
        checkMaterialize("select \"depts\".\"deptno\", \"dependents\".\"empid\"\n" + "from \"depts\"\n"
                         ...
                         + "where \"depts\".\"deptno\" > 30", "select \"dependents\".\"empid\"\n" + "from \"depts\"\n"
                                                              + "join \"dependents\" on (\"depts\".\"name\" = \"dependents\".\"name\")\n"
                                                              + "join \"emps\" on (\"emps\".\"deptno\" = \"depts\".\"deptno\")\n"
                                                              + "where \"depts\".\"deptno\" > 10", HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains("EnumerableUnion(all=[true])",
                                                           "EnumerableTableScan(table=[[hr, m0]])",
    ...
    @Test public void testJoinMaterializationUKFK1() {
        checkMaterialize(
                ...
                "select \"a\".\"empid\" from \n" + "(select * from \"emps\" where \"empid\" = 1) \"a\"\n"
                + "join \"dependents\" using (\"empid\")\n", HR_FKUK_MODEL,
                CalciteAssert.checkResultContains("PLAN=EnumerableTableScan(table=[[hr, m0]])"));
    }
    ...
    @Test public void testJoinMaterializationUKFK5() {
        checkMaterialize(
                ...
                + "join \"dependents\" using (\"empid\")" + "where \"emps\".\"empid\" = 1",
                "select \"emps\".\"empid\" from \"emps\"\n" + "join \"dependents\" using (\"empid\")\n"
                + "where \"emps\".\"empid\" = 1", HR_FKUK_MODEL, CalciteAssert.checkResultContains(
                        "EnumerableCalc(expr#0..1=[{inputs}], empid=[$t0])\n"
                        + "  EnumerableTableScan(table=[[hr, m0]])"));
    }
    ...
    @Test public void testViewMaterialization() {
        checkThatMaterialize("select \"depts\".\"name\"\n" + "from \"emps\"\n"
                             + "join \"depts\" on (\"emps\".\"deptno\" = \"depts\".\"deptno\")",
                             "select \"depts\".\"name\"\n" + "from \"depts\"\n"
                             + "join \"emps\" on (\"emps\".\"deptno\" = \"depts\".\"deptno\")", "matview", true,
                             HR_FKUK_MODEL,
                             CalciteAssert.checkResultContains("EnumerableValues(tuples=[[{ 'noname' }]])"),
                             RuleSets.ofList(ImmutableList.<RelOptRule>of())).returnsValue("noname");
    }
    ...
    @Test public void testTableModify() {
        ...
        final String q = "upsert into \"dependents\"" + "select \"empid\" + 1 as x, \"name\""
                         + "from \"emps\" where \"deptno\" = 10";

        final List<List<List<String>>> substitutedNames = new ArrayList<>();
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            MaterializationService.setThreadLocal();
            CalciteAssert.that().withMaterializations(HR_FKUK_MODEL, "m0", m).query(q).withHook(Hook.SUB,
                                                                                                new Function<RelNode, Void>() {

                                                                                                    public Void apply(
                                                                                                            RelNode input) {
                                                                                                        substitutedNames.add(
                                                                                                                new TableNameVisitor().run(
                                                                                                                        input));
                                                                                                        return null;
                                                                                                    }
                                                                                                }).enableMaterializations(
                    true).explainContains("hr, m0");
        } catch (Exception e) {
    ...
    @Test public void testPrePopulated() {
        ...
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            MaterializationService.setThreadLocal();
            CalciteAssert.that().withMaterializations(HR_FKUK_MODEL, new Function<JsonBuilder, List<Object>>() {
            ...
            }).query(q).enableMaterializations(true).explainMatches("",
                                                                    CONTAINS_LOCATIONS).sameResultWithMaterializationsDisabled();
        }
    }
    ...
    @Test public void testSingleMaterializationMultiUsage() {
        ...
        String m = "select * from \"emps\" where \"empid\" < 500";
        checkMaterialize(m, q, HR_FKUK_MODEL,
                         CalciteAssert.checkResultContains("EnumerableTableScan(table=[[hr, m0]])", 2));
    }
    ...
    @Test public void testMultiMaterializationMultiUsage() {
        ...
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            ...
            CalciteAssert.that().withMaterializations(HR_FKUK_MODEL, "m0",
                                                      ...
                                                      "m1", "select * from \"emps\" where \"empid\" < 500").query(
                    q).enableMaterializations(true).explainContains(
                    "EnumerableTableScan(table=[[hr, m0]])").explainContains(
                    "EnumerableTableScan(table=[[hr, m1]])").sameResultWithMaterializationsDisabled();
        }
    }
    ...
    @Test public void testMaterializationOnJoinQuery() {
        ...
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            MaterializationService.setThreadLocal();
            CalciteAssert.that().withMaterializations(HR_FKUK_MODEL, "m0",
                                                      "select * from \"emps\" where \"empid\" < 500").query(
                    q).enableMaterializations(true).explainContains(
                    "EnumerableTableScan(table=[[hr, m0]])").sameResultWithMaterializationsDisabled();
        }
    }

    @Ignore("Creating mv for depts considering all its column throws exception") @Test public void testMultiMaterializationOnJoinQuery() {
        final String q = "select *\n" + "from \"emps\"\n" + "join \"depts\" using (\"deptno\") where \"empid\" < 300 "
                         + "and \"depts\".\"deptno\" > 200";
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            MaterializationService.setThreadLocal();
            CalciteAssert.that().withMaterializations(HR_FKUK_MODEL, "m0",
                                                      "select * from \"emps\" where \"empid\" < 500", "m1",
                                                      "select * from \"depts\" where \"deptno\" > 100").query(
                    q).enableMaterializations(true).explainContains(
                    "EnumerableTableScan(table=[[hr, m0]])").explainContains(
                    "EnumerableTableScan(table=[[hr, m1]])").sameResultWithMaterializationsDisabled();
        }
    ...
    @Test public void testMaterializationSubstitution() {
        ...
        final String[][][] expectedNames = { { { "hr", "emps" }, { "hr", "m0" } }, { { "hr", "emps" }, { "hr", "m1" } },
                                             { { "hr", "m0" }, { "hr", "emps" } }, { { "hr", "m0" }, { "hr", "m0" } },
                                             { { "hr", "m0" }, { "hr", "m1" } }, { { "hr", "m1" }, { "hr", "emps" } },
                                             { { "hr", "m1" }, { "hr", "m0" } }, { { "hr", "m1" }, { "hr", "m1" } } };

        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            MaterializationService.setThreadLocal();
            final List<List<List<String>>> substitutedNames = new ArrayList<>();
            CalciteAssert.that().withMaterializations(HR_FKUK_MODEL, "m0",
                                                      "select * from \"emps\" where \"empid\" < 300", "m1",
                                                      "select * from \"emps\" where \"empid\" < 600").query(q).withHook(
                    Hook.SUB, new Function<RelNode, Void>() {
    ...
    @Test public void testMaterializationSubstitution2() {
        ...
        final String[][][] expectedNames = { { { "hr", "emps" }, { "hr", "m0" } }, { { "hr", "emps" }, { "hr", "m1" } },
                                             { { "hr", "emps" }, { "hr", "m2" } }, { { "hr", "m0" }, { "hr", "emps" } },
                                             { { "hr", "m0" }, { "hr", "m0" } }, { { "hr", "m0" }, { "hr", "m1" } },
                                             { { "hr", "m0" }, { "hr", "m2" } }, { { "hr", "m1" }, { "hr", "emps" } },
                                             { { "hr", "m1" }, { "hr", "m0" } }, { { "hr", "m1" }, { "hr", "m1" } },
                                             { { "hr", "m1" }, { "hr", "m2" } }, { { "hr", "m2" }, { "hr", "emps" } },
                                             { { "hr", "m2" }, { "hr", "m0" } }, { { "hr", "m2" }, { "hr", "m1" } },
                                             { { "hr", "m2" }, { "hr", "m2" } } };
        ...
        try (final TryThreadLocal.Memo ignored = Prepare.THREAD_TRIM.push(true)) {
            ...
            CalciteAssert.that().withMaterializations(HR_FKUK_MODEL, "m0",
                                                      "select * from \"emps\" where \"empid\" < 300", "m1",
                                                      "select * from \"emps\" where \"empid\" < 600", "m2",
                                                      "select * from \"m1\"").query(q).withHook(Hook.SUB,
                                                                                                new Function<RelNode, Void>() {
                    ...
                    true).sameResultWithMaterializationsDisabled();
            Collections.sort(substitutedNames, CASE_INSENSITIVE_LIST_LIST_COMPARATOR);
            assertThat(substitutedNames, is(list3(expectedNames)));
        }
    }
    ...
    public static class HrFKUKSchema {
        ...
        public final Department[] depts      = {
                new Department(10, "Sales", Arrays.asList(emps[0], emps[2], emps[3]), new Location(-122, 38)),
        ...
        public final RelReferentialConstraint rcs0 = RelReferentialConstraintImpl.of(ImmutableList.of("hr", "emps"),
                                                                                     ImmutableList.of("hr", "depts"),
                                                                                     ImmutableList.of(
                                                                                             IntPair.of(1, 0)));
