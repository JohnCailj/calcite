...
import org.apache.calcite.model.*;
...
public class ModelTest {
    ...
    @Test public void testModelImmutableSchemaCannotContainMaterialization() throws Exception {
        CalciteAssert.model("{\n" + "  version: '1.0',\n" + "  defaultSchema: 'adhoc',\n" + "  schemas: [\n" + "    {\n"
                            ...
                            + "        {\n" + "          table: 'v',\n" + "          sql: 'values (1)'\n"
                            + "        }\n" + "      ]\n" + "    }\n" + "  ]\n" + "}").connectThrows(
                "Cannot define materialization; parent schema 'adhoc' " + "is not a SemiMutableSchema");
    }
    ...
    @Test public void testSchemaWithoutName() throws Exception {
        final String model =
                "{\n" + "  version: '1.0',\n" + "  defaultSchema: 'adhoc',\n" + "  schemas: [ {\n" + "  } ]\n" + "}";
        CalciteAssert.model(model).connectThrows("Field 'name' is required in JsonMapSchema");
    }

    @Test public void testCustomSchemaWithoutFactory() throws Exception {
        final String model = "{\n" + "  version: '1.0',\n" + "  defaultSchema: 'adhoc',\n" + "  schemas: [ {\n"
                             + "    type: 'custom',\n" + "    name: 'my_custom_schema'\n" + "  } ]\n" + "}";
        CalciteAssert.model(model).connectThrows("Field 'factory' is required in JsonCustomSchema");
    }
    ...
    @Test public void testReadBadMultiLineSql() throws IOException {
        ...
        try {
            ...
            assertThat(e.getMessage(), equalTo("each element of a string list must be a string; found: 2"));
        }
    }
}
