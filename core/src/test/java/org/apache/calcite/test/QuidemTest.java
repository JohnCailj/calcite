 ...
 */
@RunWith(Parameterized.class) public class QuidemTest {
    ...
    private Method findMethod(String path) {
        ...
        String methodName = AvaticaUtils.toCamelCase("test_" + path.replace('/', '_').replaceAll("\\.iq$", ""));
        Method m;
        try {
     ...
     */
    @Parameterized.Parameters(name = "{index}: quidem({0})") public static Collection<Object[]> data() {
        ...
        final String base = File.separatorChar == '\\' ? x.substring(1, x.length() - first.length()).replace('/',
                                                                                                             File.separatorChar) : x.substring(
                0, x.length() - first.length());
    ...
    private void checkRun(String path) throws Exception {
        ...
        try (final Reader reader = Util.reader(inFile); final Writer writer = Util.printWriter(
                outFile); final Closer closer = new Closer()) {
            new Quidem(reader, writer, env(), new QuidemConnectionFactory()).withPropertyHandler(
     ...
     */
    private static class QuidemConnectionFactory implements Quidem.ConnectionFactory {
        ...
        public Connection connect(String name, boolean reference) throws Exception {
            if (reference) {
                if (name.equals("foodmart")) {
                    final ConnectionSpec db = CalciteAssert.DatabaseInstance.HSQLDB.foodmart;
            ...
            switch (name) {
                ...
                case "post":
                    return CalciteAssert.that().with(CalciteAssert.Config.REGULAR).with(
                            CalciteAssert.SchemaSpec.POST).withDefaultSchema("POST").connect();
                case "catchall":
                    return CalciteAssert.that().withSchema("s", new ReflectiveSchema(
                            new ReflectiveSchemaTest.CatchallSchema())).connect();
                case "orinoco":
                    return CalciteAssert.that().with(CalciteAssert.SchemaSpec.ORINOCO).withDefaultSchema(
                            "ORINOCO").connect();
                case "blank":
                    return CalciteAssert.that().with("parserFactory",
                                                     "org.apache.calcite.sql.parser.parserextensiontesting"
                                                     + ".ExtensionSqlParserImpl#FACTORY").with(
                            CalciteAssert.SchemaSpec.BLANK).withDefaultSchema("BLANK").connect();
                case "seq":
                    final Connection connection = CalciteAssert.that().withSchema("s", new AbstractSchema()).connect();
                    connection.unwrap(CalciteConnection.class).getRootSchema().getSubSchema("s").add("my_seq",
                                                                                                     new AbstractTable() {
