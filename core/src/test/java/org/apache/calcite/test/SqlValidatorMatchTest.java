...
public class SqlValidatorMatchTest extends SqlValidatorTestCase {
    ...
    @Test public void testMatchRecognizeInternals() throws Exception {
        ...
        sql("values ^\"RUNNING\"(1, 2)^").fails(
                "No match found for function signature RUNNING\\(<NUMERIC>, <NUMERIC>\\)");
    ...
    @Test public void testMatchRecognizeDefines4() throws Exception {
        final String sql =
                ...
                + "  ) mr";
        sql(sql).fails("Cannot nest PREV/NEXT under LAST/FIRST 'PREV\\(`UP`\\.`SAL`, 1\\)'");
    }
    ...
    @Test public void testMatchRecognizeDefines5() throws Exception {
        final String sql =
                ...
                + "  ) mr";
        sql(sql).fails("Cannot nest PREV/NEXT under LAST/FIRST 'FIRST\\(`UP`\\.`SAL`, 0\\)'");
    }
    ...
    @Test public void testMatchRecognizeDefines6() throws Exception {
        final String sql =
                ...
                + "  ) mr";
        sql(sql).fails("Invalid number of parameters to COUNT method");
    }

    @Test public void testMatchRecognizeMeasures1() throws Exception {
        final String sql = "select *\n" + "  from emp match_recognize (\n" + "    measures STRT.sal as start_sal,"
                           + "      ^LAST(null)^ as bottom_sal," + "      LAST(up.ts) as end_sal"
                           + "    pattern (strt down+ up+)\n" + "    define\n"
                           + "      down as down.sal < PREV(down.sal),\n" + "      up as up.sal > prev(up.sal)\n"
                           + "  ) mr";
        sql(sql).fails("Null parameters in 'LAST\\(NULL, 0\\)'");
    }
    ...
    @Test public void testMatchRecognizeSkipTo1() throws Exception {
        final String sql =
                ...
                + "    define\n" + "      down as down.sal < PREV(down.sal),\n" + "      up as up.sal > prev(up.sal)\n"
                + "  ) mr";
        sql(sql).fails("(?s).*Encountered \"to null\" at .*");
    }

    @Test public void testMatchRecognizeSkipTo2() throws Exception {
        final String sql = "select *\n" + "  from emp match_recognize (\n" + "    after match skip to ^no_exists^\n"
                           + "    measures\n" + "      STRT.sal as start_sal," + "      LAST(up.ts) as end_sal"
                           + "    pattern (strt down+ up+)\n" + "    define\n"
                           + "      down as down.sal < PREV(down.sal),\n" + "      up as up.sal > prev(up.sal)\n"
                           + "  ) mr";
        sql(sql).fails("(?s).*Encountered \"measures\" at .*");
    }
    ...
    @Test public void testMatchRecognizeSkipTo3() throws Exception {
        final String sql =
                ...
                + "    pattern (strt down+ up+)\n" + "    define\n" + "      down as down.sal < PREV(down.sal),\n"
                + "      up as up.sal > prev(up.sal)\n" + "  ) mr";
        sql(sql).fails("Unknown pattern 'NO_EXISTS'");
    }
    ...
    @Test public void testMatchRecognizeSkipToCaseInsensitive() throws Exception {
        final String sql =
                ...
                + "    pattern (strt down+ up+)\n" + "    define\n" + "      down as down.sal < PREV(down.sal),\n"
                + "      up as up.sal > prev(up.sal)\n" + "  ) mr";
        sql(sql).fails("Unknown pattern 'strt'");
        sql(sql).tester(tester.withCaseSensitive(false)).sansCarets().ok();
    }
    ...
    @Test public void testMatchRecognizeSubset() throws Exception {
        final String sql = "select *\n" + "from emp match_recognize (\n" + "    pattern (strt down+ up+)\n"
                           ...
                           + "      down as down.sal < PREV(down.sal),\n" + "      up as up.sal > prev(up.sal)\n"
                           + "  ) mr";
        sql(sql).fails("Unknown pattern 'STRT1'");
    }
    ...
    @Test public void testMatchRecognizeSubset2() throws Exception {
        final String sql = "select *\n" + "from emp match_recognize (\n" + "    pattern (strt down+ up+)\n"
                           ...
                           + "      down as down.sal < PREV(down.sal),\n" + "      up as up.sal > prev(up.sal)\n"
                           + "  ) mr";
        sql(sql).fails("Pattern variable 'STRT' has already been defined");
    }
    ...
    @Test public void testMatchRecognizeWithin() throws Exception {
        final String sql = "select *\n" + "from emp match_recognize (\n"
                           ...
                           + "      down as down.sal < PREV(down.sal),\n" + "      up as up.sal > prev(up.sal)\n"
                           + "  ) mr";
        sql(sql).fails("Must contain an ORDER BY clause when WITHIN is used");
    }
    ...
    @Test public void testMatchRecognizeWithin2() throws Exception {
        final String sql = "select *\n" + "from emp match_recognize (\n" + "    order by sal\n"
                           ...
                           + "      down as down.sal < PREV(down.sal),\n" + "      up as up.sal > prev(up.sal)\n"
                           + "  ) mr";
        sql(sql).fails("First column of ORDER BY must be of type TIMESTAMP");
    }
}
