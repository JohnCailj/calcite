...
public class SqlValidatorTestCase {
    ...
    private static final Pattern LINE_COL_TWICE_PATTERN = Pattern.compile(
            "(?s)From line ([0-9]+), column ([0-9]+) to line ([0-9]+), column ([0-9]+): (.*)");

    private static final SqlTestFactory EXTENDED_TEST_FACTORY = new DelegatingSqlTestFactory(
            DefaultSqlTestFactory.INSTANCE) {

        @Override public MockCatalogReader createCatalogReader(SqlTestFactory factory, JavaTypeFactory typeFactory) {
            return super.createCatalogReader(this, typeFactory).init2();
        }
    };

    static final SqlTesterImpl EXTENDED_CATALOG_TESTER = new SqlTesterImpl(EXTENDED_TEST_FACTORY);

    static final SqlTesterImpl EXTENDED_CATALOG_TESTER_2003 = new SqlTesterImpl(EXTENDED_TEST_FACTORY).withConformance(
            SqlConformanceEnum.PRAGMATIC_2003);

    static final SqlTesterImpl EXTENDED_CATALOG_TESTER_LENIENT = new SqlTesterImpl(
            EXTENDED_TEST_FACTORY).withConformance(SqlConformanceEnum.LENIENT);
    ...
    public void checkExp(String sql) {
        tester.assertExceptionIsThrown(SqlTesterImpl.buildQuery(sql), null);
    }
    ...
    public final void checkExpFails(String sql, String expected) {
        tester.assertExceptionIsThrown(SqlTesterImpl.buildQuery(sql), expected);
    }
    ...
    public final void checkExpType(String sql, String expected) {
        checkColumnType(SqlTesterImpl.buildQuery(sql), expected);
    }
    ...
    public final void checkIntervalConv(String sql, String expected) {
        tester.checkIntervalConv(SqlTesterImpl.buildQuery(sql), expected);
    }
    ...
    public static void checkEx(Throwable ex, String expectedMsgPattern, SqlParserUtil.StringAndPos sap) {
        if (null == ex) {
            if (expectedMsgPattern == null) {
                ...
                throw new AssertionError(
                        "Expected query to throw exception, " + "but it did not; query [" + sap.sql + "]; expected ["
                        + expectedMsgPattern + "]");
            }
        }
        ...
        if (null == expectedMsgPattern) {
            ...
            fail("Validator threw unexpected exception" + "; query [" + sap.sql + "]; exception [" + actualMessage
                 + "]; class [" + actualException.getClass() + "]; pos [line " + actualLine + " col " + actualColumn
                 + " thru line " + actualLine + " col " + actualColumn + "]");
        }
        ...
        if (actualColumn <= 0 || actualLine <= 0 || actualEndColumn <= 0 || actualEndLine <= 0) {
            if (sap.pos != null) {
                AssertionError e = new AssertionError(
                        "Expected error to have position," + " but actual error did not: " + " actual pos [line "
                        + actualLine + " col " + actualColumn + " thru line " + actualEndLine + " col "
                        + actualEndColumn + "]");
            ...
            sqlWithCarets = SqlParserUtil.addCarets(sap.sql, actualLine, actualColumn, actualEndLine,
                                                    actualEndColumn + 1);
            if (sap.pos == null) {
        ...
        if (actualMessage == null || !actualMessage.matches(expectedMsgPattern)) {
            actualException.printStackTrace();
            final String actualJavaRegexp = (actualMessage == null) ? "null" : TestUtil.quoteForJava(
            ...
            fail("Validator threw different " + "exception than expected; query [" + sap.sql + "];\n"
                 ...
                 + " actual as java regexp [" + actualJavaRegexp + "]; pos [" + actualLine + " col " + actualColumn
                 + " thru line " + actualEndLine + " col " + actualEndColumn + "]; sql [" + sqlWithCarets + "]");
        } else if (sap.pos != null && (actualLine != sap.pos.getLineNum() || actualColumn != sap.pos.getColumnNum()
            ...
            fail("Validator threw expected " + "exception [" + actualMessage + "];\nbut at pos [line " + actualLine
                 + " col " + actualColumn + " thru line " + actualEndLine + " col " + actualEndColumn + "];\nsql ["
                 + sqlWithCarets + "]");
        }
    }
    ...
    static class Sql {
        ...
        public Sql bindType(final String bindType) {
            tester.check(sql, null, new SqlTester.ParameterChecker() {
