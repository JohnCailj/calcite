...
public class StreamTest {
    ...
    private static final String STREAM_JOINS_MODEL =
            ...
            + "         type: 'custom',\n" + "         name: 'PRODUCTS',\n" + "         factory: '"
            + ProductsTableFactory.class.getName() + "'\n" + "       }]\n" + "     }]}";

    public static final String STREAM_MODEL =
            "{\n" + "  version: '1.0',\n" + "  defaultSchema: 'foodmart',\n" + "   schemas: [\n" + schemaFor(
                    STREAM_SCHEMA_NAME, OrdersStreamTableFactory.class) + ",\n" + schemaFor(INFINITE_STREAM_SCHEMA_NAME,
                                                                                            InfiniteOrdersStreamTableFactory.class)
            + "\n" + "   ]\n" + "}";

    @Test public void testStream() {
        CalciteAssert.model(STREAM_MODEL).withDefaultSchema("STREAMS").query(
                "select stream * from orders").convertContains(
                "LogicalDelta\n" + "  LogicalProject(ROWTIME=[$0], ID=[$1], PRODUCT=[$2], UNITS=[$3])\n"
                + "    LogicalTableScan(table=[[STREAMS, ORDERS]])\n").explainContains(
                "EnumerableInterpreter\n" + "  BindableTableScan(table=[[STREAMS, ORDERS, (STREAM)]])").returns(
                startsWith("ROWTIME=2015-02-15 10:15:00; ID=1; PRODUCT=paint; UNITS=10",
                           "ROWTIME=2015-02-15 10:24:15; ID=2; PRODUCT=paper; UNITS=5"));
    }

    @Test public void testStreamFilterProject() {
        CalciteAssert.model(STREAM_MODEL).withDefaultSchema("STREAMS").query(
                "select stream product from orders where units > 6").convertContains(
    ...
    @Test public void testStreamOrderBy() {
        CalciteAssert.model(STREAM_MODEL).withDefaultSchema("STREAMS").query(
                "select stream floor(rowtime to hour) as rowtime,\n" + "  product, units\n" + "from orders\n"
                + "order by floor(orders.rowtime to hour), product desc").convertContains(
    ...
    @Ignore @Test public void testStreamUnionAllOrderBy() {
        CalciteAssert.model(STREAM_MODEL).withDefaultSchema("STREAMS").query(
                ...
                "EnumerableSort(sort0=[$0], sort1=[$1], dir0=[ASC], dir1=[DESC])\n"
                + "  EnumerableCalc(expr#0..3=[{inputs}], expr#4=[FLAG(HOUR)], expr#5=[FLOOR($t0, $t4)], ROWTIME=[$t5], PRODUCT=[$t2], UNITS=[$t3])\n"
                + "    EnumerableInterpreter\n" + "      BindableTableScan(table=[[]])").returns(
                startsWith("ROWTIME=2015-02-15 10:00:00; PRODUCT=paper; UNITS=5",
                           "ROWTIME=2015-02-15 10:00:00; PRODUCT=paint; UNITS=10",
                           "ROWTIME=2015-02-15 10:00:00; PRODUCT=paint; UNITS=3"));
    }
    ...
    @Test public void testInfiniteStreamsDoNotBufferInMemory() {
        CalciteAssert.model(STREAM_MODEL).withDefaultSchema(INFINITE_STREAM_SCHEMA_NAME).query(
                "select stream * from orders").limit(100).explainContains("EnumerableInterpreter\n"
                                                                          + "  BindableTableScan(table=[[INFINITE_STREAMS, ORDERS, (STREAM)]])").returnsCount(
                100);
    }
    ...
    @Test(timeout = 10000) public void testStreamCancel() {
        ...
        CalciteAssert.model(STREAM_MODEL).withDefaultSchema(INFINITE_STREAM_SCHEMA_NAME).query(
                "select stream * from orders").explainContains(explain).returns(new Function<ResultSet, Void>() {
    ...
    @Test public void testStreamToRelationJoin() {
        CalciteAssert.model(STREAM_JOINS_MODEL).withDefaultSchema(STREAM_JOINS_SCHEMA_NAME).query(
                "select stream " + "orders.rowtime as rowtime, orders.id as orderId, products.supplier as supplierId "
                + "from orders join products on orders.product = products.id").convertContains(
    ...
    @Ignore @Test public void testTumbleViaOver() {
        ...
        CalciteAssert.model(STREAM_JOINS_MODEL).query(sql);
    }
    ...
    private Function<ResultSet, Void> startsWith(String... rows) {
        ...
        return new Function<ResultSet, Void>() {
            ...
            public Void apply(ResultSet input) {
                try {
                    final CalciteAssert.ResultSetFormatter formatter = new CalciteAssert.ResultSetFormatter();
    ...
    private abstract static class BaseOrderStreamTable implements ScannableTable {
        ...
        protected final RelProtoDataType protoRowType = new RelProtoDataType() {
            ...
            public RelDataType apply(RelDataTypeFactory a0) {
                return a0.builder().add("ROWTIME", SqlTypeName.TIMESTAMP).add("ID", SqlTypeName.INTEGER).add("PRODUCT",
                                                                                                             SqlTypeName.VARCHAR,
                                                                                                             10).add(
                        "UNITS", SqlTypeName.INTEGER).build();
            }
        };
     ...
     */
    @SuppressWarnings("UnusedDeclaration") public static class OrdersStreamTableFactory implements TableFactory<Table> {
        ...
        public static ImmutableList<Object[]> getRowList() {
            final Object[][] rows = { { ts(10, 15, 0), 1, "paint", 10 }, { ts(10, 24, 15), 2, "paper", 5 },
                                      { ts(10, 24, 45), 3, "brush", 12 }, { ts(10, 58, 0), 4, "paint", 3 },
     ...
     * Mock table that returns a stream of orders from a fixed array.
     */
    @SuppressWarnings("UnusedDeclaration") public static class InfiniteOrdersStreamTableFactory
    ...
    public static final Function0<Object[]> ROW_GENERATOR = new Function0<Object[]>() {
    ...
    public static class ProductsTableFactory implements TableFactory<Table> {
        ...
        public Table create(SchemaPlus schema, String name, Map<String, Object> operand, RelDataType rowType) {
            final Object[][] rows = { { "paint", 1 }, { "paper", 0 }, { "brush", 1 } };
    ...
    public static class ProductsTable implements ScannableTable {
        ...
        private final RelProtoDataType protoRowType = new RelProtoDataType() {
            ...
            public RelDataType apply(RelDataTypeFactory a0) {
                return a0.builder().add("ID", SqlTypeName.VARCHAR, 32).add("SUPPLIER", SqlTypeName.INTEGER).build();
            }
        };
