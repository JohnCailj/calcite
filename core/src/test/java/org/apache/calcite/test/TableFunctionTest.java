...
public class TableFunctionTest {
     ...
     * a single column.
     */
    @Test public void testScannableTableFunction() throws SQLException, ClassNotFoundException {
        Connection connection = DriverManager.getConnection("jdbc:calcite:");
        CalciteConnection calciteConnection = connection.unwrap(CalciteConnection.class);
        SchemaPlus rootSchema = calciteConnection.getRootSchema();
        SchemaPlus schema = rootSchema.add("s", new AbstractSchema());
     ...
     */
    @Test public void testScannableTableFunctionWithNamedParameters() throws SQLException, ClassNotFoundException {
        Connection connection = DriverManager.getConnection("jdbc:calcite:");
        CalciteConnection calciteConnection = connection.unwrap(CalciteConnection.class);
        SchemaPlus rootSchema = calciteConnection.getRootSchema();
        SchemaPlus schema = rootSchema.add("s", new AbstractSchema());
    ...
    @Test public void testMultipleScannableTableFunctionWithNamedParameters()
        ...
        try (Connection connection = DriverManager.getConnection(
                "jdbc:calcite:"); Statement statement = connection.createStatement()) {
            CalciteConnection calciteConnection = connection.unwrap(CalciteConnection.class);
            SchemaPlus rootSchema = calciteConnection.getRootSchema();
            SchemaPlus schema = rootSchema.add("s", new AbstractSchema());
     ...
     */
    @Test public void testTableFunctionDynamicStructure() throws SQLException, ClassNotFoundException {
        Connection connection = getConnectionWithMultiplyFunction();
        final PreparedStatement ps = connection.prepareStatement(
                "select *\n" + "from table(\"s\".\"multiplication\"(4, 3, ?))\n");
        ps.setInt(1, 100);
        ResultSet resultSet = ps.executeQuery();
        assertThat(CalciteAssert.toString(resultSet), equalTo("row_name=row 0; c1=101; c2=102; c3=103; c4=104\n"
     ...
     * as literals.
     */
    @Ignore("SQLException does not include message from nested exception") @Test public void testTableFunctionNonNullableMustBeLiterals()
            throws SQLException, ClassNotFoundException {
        Connection connection = getConnectionWithMultiplyFunction();
        try {
            final PreparedStatement ps = connection.prepareStatement(
            ...
            assertThat(e.getMessage(), containsString(
                    "Wrong arguments for table function 'public static " + "org.apache.calcite.schema.QueryableTable "
                    + "org.apache.calcite.test.JdbcTest" + ".multiplicationTable(int,int,java.lang.Integer)'"
    ...
    @Ignore("CannotPlanException: Node [rel#18:Subset#4.ENUMERABLE.[]] "
            + "could not be implemented") @Test public void testTableFunctionCursorInputs()
            throws SQLException, ClassNotFoundException {
    ...
    @Ignore("CannotPlanException: Node [rel#24:Subset#6.ENUMERABLE.[]] "
            + "could not be implemented") @Test public void testTableFunctionCursorsInputs()
            throws SQLException, ClassNotFoundException {
    ...
    @Test public void testUserDefinedTableFunction() {
        ...
        with().query(q).returnsUnordered("row_name=row 0; c1=101; c2=102", "row_name=row 1; c1=102; c2=104",
    ...
    @Test public void testUserDefinedTableFunction6() {
        ...
        with().query(q).returns(new Function<ResultSet, Void>() {
