...
public class UdfTest {
     ...
     * non-static methods.
     */
    @Ignore("[CALCITE-1561] Intermittent test failures") @Test public void testUserDefinedFunction() throws Exception {
        ...
        withUdf().query(sql).returnsUnordered("P=110", "P=120", "P=110", "P=110");
    ...
    @Test public void testUserDefinedFunctionInView() throws Exception {
        ...
        final String viewSql =
                ...
                + "from \"hr\".\"emps\"";
        post.add("V_EMP",
                 ViewTable.viewMacro(post, viewSql, ImmutableList.<String>of(), ImmutableList.of("POST", "V_EMP"),
                                     null));
    ...
    @Test public void testNotNullImplementor() {
        ...
        with.query("select upper(\"adhoc\".my_str(\"name\")) as p from \"adhoc\".EMPLOYEES").returns(
                "P=<BILL>\n" + "P=<ERIC>\n" + "P=<SEBASTIAN>\n" + "P=<THEODORE>\n");
        with.query("select \"name\" as p from \"adhoc\".EMPLOYEES\n"
                   + "where \"adhoc\".my_str(\"name\") is not null").returns(
                "P=Bill\n" + "P=Eric\n" + "P=Sebastian\n" + "P=Theodore\n");
        with.query("select \"name\" as p from \"adhoc\".EMPLOYEES\n"
                   + "where \"adhoc\".my_str(upper(\"name\")) is not null").returns(
                "P=Bill\n" + "P=Eric\n" + "P=Sebastian\n" + "P=Theodore\n");
        with.query("select \"name\" as p from \"adhoc\".EMPLOYEES\n"
                   + "where upper(\"adhoc\".my_str(\"name\")) is not null").returns(
                "P=Bill\n" + "P=Eric\n" + "P=Sebastian\n" + "P=Theodore\n");
        with.query(
    ...
    @Test public void testSemiStrict() {
        ...
        with.query(sql).returnsUnordered("P=null", "P=null", "P=SEBASTIAN", "P=THEODORE");
        // my_str is non-strict; it must be called when args are null
        final String sql2 = "select\n" + "  \"adhoc\".my_str(upper(\"adhoc\".null4(\"name\"))) as p\n"
                            + " from \"adhoc\".EMPLOYEES";
        with.query(sql2).returnsUnordered("P=<null>", "P=<null>", "P=<SEBASTIAN>", "P=<THEODORE>");
        ...
        with.query(sql3).returnsUnordered("P=null", "P=null", "P=Sebastian", "P=null");
    }
    ...
    @Test public void testUdfOverloaded() {
        ...
        with.query("select max(\"adhoc\".count_args()) as p0,\n" + " min(\"adhoc\".count_args(0)) as p1,\n"
                   + " max(\"adhoc\".count_args(0, 0)) as p2\n" + "from \"adhoc\".EMPLOYEES limit 1").returns(
                "P0=0; P1=1; P2=2\n");
    }
    ...
    @Test public void testUdfArgumentName() {
        ...
        with.query("values (\"adhoc\".my_left(\"n\" => 3, \"n\" => 2, \"s\" => 'hello'))").throws_(
                "Duplicate argument name 'n'");
        // invalid argument names
        with.query("values (\"adhoc\".my_left(\"n\" => 3, \"m\" => 2, \"s\" => 'h'))").throws_(
                "No match found for function signature " + "MY_LEFT(n => <NUMERIC>, m => <NUMERIC>, s => <CHARACTER>)");
        // missing arguments
        with.query("values (\"adhoc\".my_left(\"n\" => 3))").throws_(
                "No match found for function signature MY_LEFT(n => <NUMERIC>)");
        with.query("values (\"adhoc\".my_left(\"s\" => 'hello'))").throws_(
                "No match found for function signature MY_LEFT(s => <CHARACTER>)");
        // arguments of wrong type
        with.query("values (\"adhoc\".my_left(\"n\" => 'hello', \"s\" => 'x'))").throws_(
                "No match found for function signature " + "MY_LEFT(n => <CHARACTER>, s => <CHARACTER>)");
        with.query("values (\"adhoc\".my_left(\"n\" => 1, \"s\" => 0))").throws_(
                "No match found for function signature " + "MY_LEFT(n => <NUMERIC>, s => <NUMERIC>)");
    }
    ...
    @Test public void testUdfArgumentOptional() {
        final CalciteAssert.AssertThat with = withUdf();
        with.query("values (\"adhoc\".abcde(a=>1,b=>2,c=>3,d=>4,e=>5))").returns(
                "EXPR$0={a: 1, b: 2, c: 3, d: 4, e: 5}\n");
        with.query("values (\"adhoc\".abcde(1,2,3,4,CAST(NULL AS INTEGER)))").returns(
                "EXPR$0={a: 1, b: 2, c: 3, d: 4, e: null}\n");
        with.query("values (\"adhoc\".abcde(a=>1,b=>2,c=>3,d=>4))").returns(
                "EXPR$0={a: 1, b: 2, c: 3, d: 4, e: null}\n");
        ...
        with.query("values (\"adhoc\".abcde(1,2))").throws_(
                "No match found for function signature ABCDE(<NUMERIC>, <NUMERIC>)");
        with.query("values (\"adhoc\".abcde(1,DEFAULT,3))").returns("EXPR$0={a: 1, b: null, c: 3, d: null, e: null}\n");
        with.query("values (\"adhoc\".abcde(1,DEFAULT,'abcde'))").throws_(
                "No match found for function signature ABCDE(<NUMERIC>, <ANY>, <CHARACTER>)");
        with.query("values (\"adhoc\".abcde(true))").throws_("No match found for function signature ABCDE(<BOOLEAN>)");
        with.query("values (\"adhoc\".abcde(true,DEFAULT))").throws_(
                "No match found for function signature ABCDE(<BOOLEAN>, <ANY>)");
        with.query("values (\"adhoc\".abcde(1,DEFAULT,3,DEFAULT))").returns(
                "EXPR$0={a: 1, b: null, c: 3, d: null, e: null}\n");
        with.query("values (\"adhoc\".abcde(1,2,DEFAULT))").throws_("DEFAULT is only allowed for optional parameters");
        with.query("values (\"adhoc\".abcde(a=>1,b=>2,c=>DEFAULT))").throws_(
                "DEFAULT is only allowed for optional parameters");
        with.query("values (\"adhoc\".abcde(a=>1,b=>DEFAULT,c=>3))").returns(
                "EXPR$0={a: 1, b: null, c: 3, d: null, e: null}\n");
    }
    ...
    @Test public void testUserDefinedFunction2() throws Exception {
        withBadUdf(Smalls.AwkwardFunction.class).connectThrows(
    ...
    @Test public void testUserDefinedFunctionWithMethodName() throws Exception {
        ...
        with.query("values \"adhoc\".\"nonStatic\"(2)").throws_(
                "No match found for function signature nonStatic(<NUMERIC>)");
    }
    ...
    @Test public void testUserDefinedAggregateFunction() throws Exception {
        ...
        with.withDefaultSchema(null).query(
        ...
        with.query("select my_sum(\"name\") as p from EMPLOYEES\n").throws_(
                "No match found for function signature MY_SUM(<CHARACTER>)");
        ...
        with.query(
                "select \"deptno\", my_sum(\"deptno\") as p from EMPLOYEES\n" + "group by \"deptno\"").returnsUnordered(
                "deptno=20; P=20", "deptno=10; P=30");
    ...
    @Test public void testUserDefinedAggregateFunctionWithMultipleParameters() throws Exception {
        ...
        with.query("select \"name\",\"adhoc\".my_sum3(\"empid\",\"deptno\",\"commission\") as p "
                   + "from \"adhoc\".EMPLOYEES\n" + "group by \"name\"").returnsUnordered("name=Theodore; P=0",
                                                                                          "name=Eric; P=220",
                                                                                          "name=Bill; P=110",
                                                                                          "name=Sebastian; P=0");
        with.query("select \"adhoc\".my_sum3(\"empid\",\"deptno\",\"salary\") as p "
                   + "from \"adhoc\".EMPLOYEES\n").throws_(
                "No match found for function signature MY_SUM3(<NUMERIC>, " + "<NUMERIC>, <APPROXIMATE_NUMERIC>)");
        ...
        with.query("select \"adhoc\".my_sum2(\"name\",250) as p from \"adhoc\".EMPLOYEES\n").throws_(
                "No match found for function signature MY_SUM2(<CHARACTER>, <NUMERIC>)");
        with.query("select \"adhoc\".my_sum2(\"empid\",0.0) as p from \"adhoc\".EMPLOYEES\n").throws_(
                "No match found for function signature MY_SUM2(<NUMERIC>, <NUMERIC>)");
    }
    ...
    @Test public void testUserDefinedAggregateFunctionImplementsInterface() {
        ...
        final String model = "{\n" + "  version: '1.0',\n" + "   schemas: [\n" + "     {\n" + "       name: 'adhoc',\n"
                             ...
                             + "           className: '" + mySum3 + "'\n" + "         }\n" + "       ]\n" + "     }\n"
                             + "   ]\n" + "}";
        final CalciteAssert.AssertThat with = CalciteAssert.model(model).withDefaultSchema("adhoc");

        with.query("select my_sum3(\"deptno\") as p from EMPLOYEES\n").returns("P=50\n");
        with.withDefaultSchema(null).query(
                "select \"adhoc\".my_sum3(\"deptno\") as p\n" + "from \"adhoc\".EMPLOYEES\n").returns("P=50\n");
        with.query("select my_sum3(\"empid\"), \"deptno\" as p from EMPLOYEES\n").throws_(
                "Expression 'deptno' is not being grouped");
        with.query("select my_sum3(\"deptno\") as p from EMPLOYEES\n").returns("P=50\n");
        with.query("select my_sum3(\"name\") as p from EMPLOYEES\n").throws_(
                "No match found for function signature MY_SUM3(<CHARACTER>)");
    ...
    @Test public void testArrayUserDefinedFunction() throws Exception {
        try (Connection connection = DriverManager.getConnection("jdbc:calcite:")) {
            ...
            final String result = "" + "EMPLOYEE_ID=100; EMPLOYEE_NAME=Bill Bill;"
        ...
        @Override public CallImplementor getImplementor() {
            return new CallImplementor() {
                ...
                public Expression implement(RexToLixTranslator translator, RexCall call, NullAs nullAs) {
                    ...
                    return Expressions.call(lookupMethod, translator.translateList(call.getOperands(), nullAs));
                }
            };
        }
    }
    ...
    private class ArrayAppendIntegerFunction extends ArrayAppendScalarFunction {
        ...
        @Override public List<RelProtoDataType> getParams() {
            return ImmutableList.of(new RelProtoDataType() {
