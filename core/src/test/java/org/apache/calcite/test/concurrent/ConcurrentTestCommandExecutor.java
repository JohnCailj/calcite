...
class ConcurrentTestCommandExecutor extends Thread {
     ...
     */
    ConcurrentTestCommandExecutor(int threadId, String threadName, String jdbcURL, Properties jdbcProps,
                                  Iterable<ConcurrentTestCommand> commands, Sync synchronizer,
    ...
    public void run() {
        ...
        for (ConcurrentTestCommand command : commands) {
            ...
            boolean isSync = command instanceof ConcurrentTestCommandGenerator.SynchronizationCommand;

            if (isSync || ((connection != null) && (command != null) && (error == null))) {
                try {
    ...
    private void handleError(Throwable error, String when, ConcurrentTestCommand command) {
        ...
        if (debugPrintStream != null) {
            debugPrintStream.println(Thread.currentThread().getName() + ": " + when);
