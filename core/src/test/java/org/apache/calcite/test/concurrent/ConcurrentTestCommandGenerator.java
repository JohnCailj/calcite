...
public class ConcurrentTestCommandGenerator {
    ...
    public ConcurrentTestCommand addFetchAndCompareCommand(int threadId, int order, int timeout, String expected) {
        ConcurrentTestCommand command = new FetchAndCompareCommand(timeout, expected);
    ...
    protected ConcurrentTestCommand addCommand(int threadId, int order, ConcurrentTestCommand command) {
        ...
        TreeMap<Integer, ConcurrentTestCommand> commandMap = threadMap.get(threadId);
        if (commandMap == null) {
    ...
    protected ConcurrentTestCommandExecutor[] innerExecute() throws Exception {
        ...
        for (int threadId : threadIds) {
            ...
            if (debug) {
                debugStream.println("Thread ID: " + threadId + " (" + getThreadName(threadId) + ")");
                printCommands(debugStream, threadId);
            }

            threads[threadIndex++] = new ConcurrentTestCommandExecutor(threadId, getThreadName(threadId), this.jdbcURL,
                                                                       this.jdbcProps, commands, sync,
    ...
    protected void postExecute(ConcurrentTestCommandExecutor[] threads) throws Exception {
        ...
        if (requiresCustomErrorHandling()) {
            ...
            for (ConcurrentTestCommandExecutor thread : threads) {
                ...
                if (cause != null) {
                    failedThreads.add(new FailedThread(thread.getName(), thread.getFailureLocation(), cause));
                }
            }
        }
    }
    ...
    public void synchronizeCommandSets() {
        ...
        for (TreeMap<Integer, ConcurrentTestCommand> map : threadMap.values()) {
            ...
            maxCommands = Math.max(maxCommands, map.size());
        }
     ...
     */
    protected abstract static class AbstractCommand implements ConcurrentTestCommand {
        ...
        protected abstract void doExecute(ConcurrentTestCommandExecutor exec) throws Exception;
        ...
        public void execute(ConcurrentTestCommandExecutor exec) throws Exception {
            try {
                doExecute(exec);
                if (shouldFail) {
                    throw new ConcurrentTestCommand.ShouldHaveFailedException(failComment);
                }
    ...
    private static class FetchAndCompareCommand extends CommandWithTimeout {
        ...
        private void parseExpected(String expected) {
            ...
            for (int i = 0; i < expected.length(); i++) {
                ...
                char nextCh = ((i + 1) < expected.length()) ? expected.charAt(i + 1) : 0;
                switch (state) {
                    ...
                    case STATE_OTHER_VALUE: // handle other values (numeric, null)
                        ...
                        if (stringValue.matches("^-?[0-9]+$")) {
                            ...
                            throw new IllegalStateException(
                                    "unknown value type '" + stringValue + "' for FetchAndCompare command");
                        }
                    ...
                    case STATE_VALUE_END: // find comma or end of row
                        if (ch == COMMA) {
                            ...
                            throw new IllegalStateException(
                                    "unexpected character '" + ch + "' at position " + i + " of expected values");
                        }
                        break;
                }
            }
        ...
        private void testValues() {
            if (expected.size() != result.size()) {
                dumpData("Expected " + expected.size() + " rows, got " + result.size());
            }
        ...
        private void testValues(List<Object> expectedRow, List<Object> resultRow, int rowNum) {
            if (expectedRow.size() != resultRow.size()) {
                dumpData("Row " + rowNum + " Expected " + expected.size() + " columns, got " + result.size());
            }
            ...
            while (expectedIter.hasNext() && resultIter.hasNext()) {
                ...
                if ((expectedValue == null) || (expectedValue instanceof String)
                    ...
                    if (expectedInt.bitLength() <= 31) {
                        test(expectedInt.intValue(), ((Number) resultValue).intValue(), rowNum, colNum);
                    } else if (expectedInt.bitLength() <= 63) {
                        test(expectedInt.longValue(), ((Number) resultValue).longValue(), rowNum, colNum);
                    } else {
                    ...
                    if ((asFloat != Float.POSITIVE_INFINITY) && (asFloat != Float.NEGATIVE_INFINITY)) {
                        test(asFloat, ((Number) resultValue).floatValue(), rowNum, colNum);
                    } else if ((asDouble != Double.POSITIVE_INFINITY) && (asDouble != Double.NEGATIVE_INFINITY)) {
                        test(asDouble, ((Number) resultValue).doubleValue(), rowNum, colNum);
                    } else {
                    ...
                    throw new IllegalStateException(
                            "unknown type of expected value: " + expectedValue.getClass().getName());
                }
        ...
        private void test(Object expected, Object got, int rowNum, int colNum) {
            ...
            if ((expected == null) || !expected.equals(got)) {
                reportError(String.valueOf(expected), String.valueOf(got), rowNum, colNum);
            }
        }
        ...
        private void reportError(String expected, String got, int rowNum, int colNum) {
            dumpData("Row " + rowNum + ", column " + colNum + ": expected <" + expected + ">, got <" + got + ">");
        }
        ...
        private void dumpData(String message) {
            ...
            while (expectedIter.hasNext() || resultIter.hasNext()) {
                ...
                while (((expectedRowIter != null) && expectedRowIter.hasNext()) || ((resultRowIter != null)
                                                                                    && resultRowIter.hasNext())) {
