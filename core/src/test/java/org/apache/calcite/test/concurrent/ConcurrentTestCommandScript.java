...
public class ConcurrentTestCommandScript extends ConcurrentTestCommandGenerator {
    ...
    private static final StateAction[] STATE_TABLE = { new StateAction(PRE_SETUP_STATE, new StateDatum[] {
            new StateDatum(VAR, PRE_SETUP_STATE), new StateDatum(LOCKSTEP, PRE_SETUP_STATE),
            new StateDatum(NOLOCKSTEP, PRE_SETUP_STATE), new StateDatum(ENABLED, PRE_SETUP_STATE),
            new StateDatum(DISABLED, PRE_SETUP_STATE), new StateDatum(PLUGIN, PRE_SETUP_STATE),
            new StateDatum(SETUP, SETUP_STATE), new StateDatum(CLEANUP, CLEANUP_STATE),
    ...
    private final Map<String, ConcurrentTestPlugin> pluginForCommand         = new HashMap<>();
    private final Map<String, ConcurrentTestPlugin> preSetupPluginForCommand = new HashMap<>();
    private final List<String>                      setupCommands            = new ArrayList<>();
    private final List<String>                      cleanupCommands          = new ArrayList<>();

    private final Map<Integer, BufferedWriter> threadBufferedWriters = new HashMap<>();
    private final Map<Integer, StringWriter>   threadStringWriters   = new HashMap<>();
    private final Map<Integer, ResultsReader>  threadResultsReaders  = new HashMap<>();
    ...
    static int runAppProcess(ProcessBuilder pb, Logger logger, Reader appInput, Writer appOutput)
        ...
        if (appInput != null) {
            OutputStream out = new BufferedOutputStream(p.getOutputStream(), 100 * 1024);
            int c;
        ...
        try {
            for (String command : commands) {
                ...
                if (sql.startsWith("!set")) {
                    ...
                    if ((tokens.length > 2) && tokens[1].equalsIgnoreCase("force")) {
    ...
    private boolean isSelect(String sql) {
        ...
        try {
            ...
            while ((line = rdr.readLine()) != null) {
                ...
                return line.startsWith("select") || line.startsWith("values") || line.startsWith("explain");
            }
        } catch (IOException e) {
    ...
    public void printResults(PrintWriter out) throws IOException {
        ...
        if (verbose) {
            out.write(
                    String.format(Locale.ROOT, "script execution started at %tc (%d)%n", new Timestamp(scriptStartTime),
                                  scriptStartTime));
        }
    ...
    private void printThreadResults(PrintWriter out, String[] threadResult) throws IOException {
    ...
    private class CommandParser {
            ...
            try (BufferedReader in = Util.reader(scriptFile)) {
                ...
                while ((line = in.readLine()) != null) {
                    ...
                    if (!commandStateMap.containsKey(command)) {
                        throw new IllegalStateException(command + " not allowed in state " + state);
                    }
                ...
                if (currentDirectory.size() == 1) {
                    ...
                    if (!lookupState(state).containsKey(EOF)) {
                        throw new IllegalStateException("Premature end of file in '" + state + "' state");
                    }
                }
            }
        }
        ...
        private boolean loadCommand(String command, String line, BufferedReader in) throws IOException {
            if (VAR.equals(command)) {
                ...
                assert lockstep == null : LOCKSTEP + " and " + NOLOCKSTEP + " may only appear once";
                ...
                assert disabled == null : DISABLED + " and " + ENABLED + " may only appear once";
                ...
                String includedFile = vars.expand(line.substring(INCLUDE_LEN).trim());
                ...
                while (threadNamesTok.hasMoreTokens()) {
                    setThreadName(nextThreadId++, threadNamesTok.nextToken());
                }
                    ...
                    if (repeatCount > 0) {
                        try {
                            ...
                            throw new IllegalStateException(
                                    "Unable to reset reader -- repeat " + "contents must be less than "
                ...
                trace("@" + command, cmd);
                for (int i = threadId; i < nextThreadId; i++) {
                    addCommand(i, order, new PluginCommand(command, cmd));
                }
                order++;
                ...
                cmd = readLine(cmd, in);
                trace("@" + command, cmd);
                ConcurrentTestPlugin plugin = preSetupPluginForCommand.get(command);
                ...
                for (int i = threadId; i < nextThreadId; i++) {
                    addCommand(i, order, new FetchAndPrintCommand(millis));
                }
                order++;
        ...
        private void plugin(String pluginName) throws IOException {
            try {
                @SuppressWarnings("unchecked") Class<ConcurrentTestPlugin> pluginClass = (Class<ConcurrentTestPlugin>) Class.forName(
                        pluginName);
                final Constructor<ConcurrentTestPlugin> constructor = pluginClass.getConstructor();
        ...
        private void addExtraCommands(Iterable<String> commands, String state) {
            ...
            for (int i = 0, n = STATE_TABLE.length; i < n; i++) {
                if (state.equals(STATE_TABLE[i].state)) {
                    ...
                    final List<StateDatum> stateDataList = new ArrayList<>(Arrays.asList(stateData));
     ...
     */
    private abstract static class CommandWithTimeoutAndRowLimit extends CommandWithTimeout {
        ...
        protected void doExecute(ConcurrentTestCommandExecutor executor) throws SQLException {
            ...
            try {
                ...
                if (haveResults) {
                    ...
                    storeMessage(executor.getThreadId(), "0 rows affected.");
    ...
    private class ResultsReader {
        ...
        void read(ResultSet rset, long timeout) throws SQLException {
            ...
            try {
                ...
                if (verbose) {
                    out.printf(Locale.ROOT, "fetch started at %tc %d, %s at %tc %d%n", startTime, startTime,
                               timedOut ? "timeout" : "eos", endTime, endTime);
                }
                if (totaled) {
                    ...
                    assert dt >= 0;
                    out.printf(Locale.ROOT, "fetched %d rows in %d msecs %s%n", rowCount, dt,
                               timedOut ? "(timeout)" : "(end)");
                }
            }
        }
