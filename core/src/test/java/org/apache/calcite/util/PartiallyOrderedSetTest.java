...
public class PartiallyOrderedSetTest {
    ...
    @Test public void testPoset() {
        String empty = "''";
        String abcd = "'abcd'";
        PartiallyOrderedSet<String> poset = new PartiallyOrderedSet<String>(STRING_SUBSET_ORDERING);
        ...
        assertEqualsList("['ab', 'abcd', 'b', 'bcd']", poset.getAncestors(empty));
    ...
    @Test public void testPosetBits() {
        final PartiallyOrderedSet<Integer> poset = new PartiallyOrderedSet<Integer>(IS_BIT_SUPERSET);
    ...
    @Test public void testDivisorPoset() {
        ...
        PartiallyOrderedSet<Integer> integers = new PartiallyOrderedSet<Integer>(IS_DIVISOR, range(1, 1000));
        assertEquals("[1, 2, 3, 4, 5, 6, 8, 10, 12, 15, 20, 24, 30, 40, 60]",
                     new TreeSet<Integer>(integers.getDescendants(120)).toString());
        assertEquals("[240, 360, 480, 600, 720, 840, 960]",
                     new TreeSet<Integer>(integers.getAncestors(120)).toString());
        assertTrue(integers.getDescendants(1).isEmpty());
        assertEquals(998, integers.getAncestors(1).size());
    ...
    @Test public void testDivisorRandom() {
        boolean ok = false;
        try {
            checkPoset(IS_DIVISOR, DEBUG, random(random, SCALE, SCALE * 3), false);
            ok = true;
        } finally {
            if (!ok) {
    ...
    @Test public void testDivisorRandomWithRemoval() {
        boolean ok = false;
        try {
            checkPoset(IS_DIVISOR, DEBUG, random(random, SCALE, SCALE * 3), true);
            ok = true;
        } finally {
            if (!ok) {
                System.out.println("Random seed: " + seed);
    ...
    @Test public void testDivisorInverseRandom() {
        boolean ok = false;
        try {
            checkPoset(IS_DIVISOR_INVERSE, DEBUG, random(random, SCALE, SCALE * 3), false);
            ok = true;
        } finally {
            if (!ok) {
                System.out.println("Random seed: " + seed);
    ...
    @Test public void testDivisorInverseRandomWithRemoval() {
        boolean ok = false;
        try {
            checkPoset(IS_DIVISOR_INVERSE, DEBUG, random(random, SCALE, SCALE * 3), true);
            ok = true;
        } finally {
            if (!ok) {
                System.out.println("Random seed: " + seed);
    ...
    @Test public void testSubsetRandom() {
        boolean ok = false;
        try {
            checkPoset(IS_BIT_SUBSET, DEBUG, random(random, SCALE / 4, SCALE), false);
            ok = true;
        } finally {
            if (!ok) {
                System.out.println("Random seed: " + seed);
    ...
    public void checkPoset(PartiallyOrderedSet.Ordering<Integer> ordering, boolean debug, Iterable<Integer> generator,
                           boolean remove) {
        final PartiallyOrderedSet<Integer> poset = new PartiallyOrderedSet<Integer>(ordering);
        int n = 0;
        int z = 0;
        if (debug) {
            dump(poset);
    ...
    private static Iterable<Integer> random(Random random, final int size, final int max) {
    ...
    private static void assertEqualsList(String expected, List<String> ss) {
        assertEquals(expected, new TreeSet<String>(ss).toString());
    }
}
